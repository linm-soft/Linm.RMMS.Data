# Ùn tắc / ngập úng — Feature Context

> **Slug:** `rpt-un-tac` · **Module:** `Report` · **Phase:** P2 · **Wave:** C  
> **Status:** Context  
> **Kind:** **E** (report) — `/erp-report-context` · AnalyticsReportShell  
> **Sources:** GOVOne › Ùn tắc giao thông / ngập úng · hub [reports.md](reports.md) · [15-SCREEN-AI-MAP.md](../15-SCREEN-AI-MAP.md)  
> **Parent / list pack:** `incident` — **cấm** copy CRUD vào trang này  
> **MFE:** `Linm.Web.RMMS.Report` · route `/bao-cao/un-tac` · mfeStdUrl `http://localhost:9311/bao-cao/un-tac`  
...
| Align MFE | http://localhost:9311/bao-cao/un-tac |
...
**Filter:** tuyến · từ/đến · loại (ùn tắc|ngập)  
...
**Mock:** 8–15 dòng mẫu từ domain Incident type=congestion|flood.

P2 = lưới kỳ. ITS/TOC overlay DEFER P3.
...
| Method | Path | Mô tả |
|--------|------|-------|
| GET | /api/v1/reports/congestion?from=&to=&routeId=&q= | Xem báo cáo (paged) |
| GET | /api/v1/reports/congestion/export | Excel |

Query đọc Incident type=congestion|flood — không bảng báo cáo riêng bắt buộc P1 (read-model P2 nếu nặng).

Auth: JWT · tenant · perm `report.un-tac.read`.

## 4. Database

Read-model / view trên entity nguồn (Incident type=congestion|flood). Index theo filter UI (tuyến · ngày · tenant).
...
| ID | Default |
|----|---------|
| GAP-F-RPT-LEAF-01 Tách khỏi hub `reports` | Leaf slug + route /bao-cao/un-tac |
