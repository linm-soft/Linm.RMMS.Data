# Tai nạn giao thông — Feature Context

> **Slug:** `rpt-tngt` · **Module:** `Report` · **Phase:** P2 · **Wave:** C  
> **Status:** Context  
> **Kind:** **E** (report) — `/erp-report-context` · AnalyticsReportShell  
> **Sources:** GOVOne › Tai nạn giao thông · hub [reports.md](reports.md) · [15-SCREEN-AI-MAP.md](../15-SCREEN-AI-MAP.md)  
> **Parent / list pack:** `incident` — **cấm** copy CRUD vào trang này  
> **MFE:** `Linm.Web.RMMS.Report` · route `/bao-cao/tngt` · mfeStdUrl `http://localhost:9311/bao-cao/tngt`  
...
| Align MFE | http://localhost:9311/bao-cao/tngt |
...
**Filter:** tuyến · kỳ · mức · loại thống kê  
...
**Mock:** 8–15 dòng mẫu từ domain Incident type=tngt.

Một slug · 6 tab. Cấm 6 feature trùng filter.
...
| Method | Path | Mô tả |
|--------|------|-------|
| GET | /api/v1/report/traffic-accidents?tab=&from=&to=&routeId=&severity=&search= | Xem báo cáo (paged) |
| GET | /api/v1/report/traffic-accidents/export | Excel |

Query đọc Incident type=tngt — không bảng báo cáo riêng bắt buộc P1 (read-model P2 nếu nặng).

Auth: JWT · tenant · perm `report.tngt.read`.

## 4. Database

Read-model / view trên entity nguồn (Incident type=tngt). Index theo filter UI (tuyến · ngày · tenant).
...
| ID | Default |
|----|---------|
| GAP-F-RPT-LEAF-01 Tách khỏi hub `reports` | Leaf slug + route /bao-cao/tngt |
