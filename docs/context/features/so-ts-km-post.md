...
> **Slug:** `so-ts-km-post` · **type:** `KM_POST` · **cluster:** `atgt_point` · ô `t09`  
> **Parent SSOT:** [`so-ts-type-grid.md`](so-ts-type-grid.md) · fields [`import-gov-asset-fields.md`](import-gov-asset-fields.md)  
> **Mẫu:** `docs/img/gov-mau-tai-san/3-tbl_km_post-list.png` · `3-tbl_km_post-detail.png`  
> **Route:** `/so-ts?type=KM_POST` · form `/so-ts/tao-moi` · `/so-ts/sua?id=`  
