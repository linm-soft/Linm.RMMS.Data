# Sổ TS — Đường tràn (so-ts-spillway) — filter bar context

**Page:** so-ts-spillway list · **kind:** catalog (Kind B) · **typeCode:** `SPILLWAY`  
**Route live:** `/so-ts?type=SPILLWAY` · alias board `/so-ts-spillway` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-spillway-list`  
...
**SA query keys:** `specs/so-ts-spillway/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-spillway/ui/design.md` §3.1 Zone B
...
|---|----------|---------|------|-------------|
| 1 | Tìm kiếm | `SearchTextInput` (không nút Tìm riêng) | `leading` | `search` — mã · tên công trình · sông · tuyến · QR |
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `SPILLWAY` deep-link · **ẩn** khi type cố định |
...
- Title trái («Danh sách đường tràn») · mọi input + 🔍 cụm phải · wrap `flex-end` (`filter-bar-layout-hard` V1–V5).
- Actions CRUD = toolbar Zone B (`catalogToolbar`) — **không** trên filter bar.
- Deep-link `type=SPILLWAY` giữ khi clear filters (type lock).
      ...
      <LinErpListFilterBar
        ...
        leading={(
          <>
            ...
            {/* type SearchInput ẩn khi deep-link SPILLWAY */}
...
Empty search/route/km/org = all `SPILLWAY` rows for company · type luôn `SPILLWAY`.
