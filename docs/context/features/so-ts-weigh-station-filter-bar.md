# Sổ TS — Trạm kiểm soát tải (so-ts-weigh-station) — filter bar context

**Page:** so-ts-weigh-station list · **kind:** catalog (Kind B) · **typeCode:** `WEIGH_STATION`  
**Route live:** `/so-ts?type=WEIGH_STATION` · alias board `/so-ts-weigh-station` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-weigh-station-list`  
...
**SA query keys:** `specs/so-ts-weigh-station/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-weigh-station/ui/design.md` § Zone B
...
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `WEIGH_STATION` deep-link · **ẩn** khi type cố định |
...
| Rule | Value |
|------|-------|
| Title | trái — «Sổ TS — Trạm kiểm soát tải» / listTitle «Danh sách trạm kiểm soát tải» |
...
Smoke trên `http://localhost:9301/so-ts?type=WEIGH_STATION` (QA/Dev — **cấm** TL start:std).

| ID | Check |
|----|-------|
| V1 | Title trái · filter inputs cụm phải |
| V2 | 🔍 trên bar · **không** nút Tìm riêng |
| V3 | type lock WEIGH_STATION · deep-link |
| V4 | SearchInput route/org · **cấm** free-text |
| V5 | 0× ErpListHeaderFilters / LinListFilterField / export trên bar |

## 4. Cấm

Demo/localStorage SSOT · ERP.* · invent date filter · fork filter shell · lẫn type khác trong WEIGH list
