# Sổ TS — Kè / tường chắn (so-ts-retaining) — filter bar context

**Page:** so-ts-retaining list · **kind:** catalog (Kind B) · **typeCode:** `RETAINING`  
**Route live:** `/so-ts?type=RETAINING` · alias board `/so-ts-retaining` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-retaining-list`  
...
**SA query keys:** `specs/so-ts-retaining/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-retaining/ui/design.md` § Zone B
...
|---|----------|---------|------|-------------|
| 1 | Tìm kiếm | `SearchTextInput` (không nút Tìm riêng) | `leading` | `search` — mã · loại tường · tuyến · QR · địa danh |
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `RETAINING` deep-link · **ẩn** khi type cố định |
...
**List GET:** `?type=RETAINING&search=&route=&kmFrom=&kmTo=&orgUnit=&page=&pageSize=`
...
| Rule | Value |
|------|-------|
| Title | trái — «Sổ TS — Kè / tường chắn» / listTitle «Danh sách kè / tường chắn» |
...
Smoke trên `http://localhost:9301/so-ts?type=RETAINING` (QA/Dev — **cấm** TL start:std).

| ID | Check |
|----|-------|
| V1 | Title trái · filter inputs cụm phải |
| V2 | 🔍 trên bar · **không** nút Tìm riêng |
| V3 | type lock RETAINING · deep-link |
| V4 | SearchInput route/org · **cấm** free-text |
| V5 | 0× ErpListHeaderFilters / LinListFilterField / export trên bar |

## 4. Cấm

Demo/localStorage SSOT · ERP.* · invent date filter · fork filter shell · lẫn type khác / gộp `SLOPE_PROTECT` trong RETAINING list
