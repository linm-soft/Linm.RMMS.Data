# Sổ TS — Cọc tiêu / cọc H

> **Slug:** `so-ts-delineator` · **type:** `DELINEATOR` · **cluster:** `atgt_point` · ô `t14`  
> **Parent SSOT:** [`so-ts-type-grid.md`](so-ts-type-grid.md) · fields [`import-gov-asset-fields.md`](import-gov-asset-fields.md)  
> **Mẫu:** `docs/img/gov-mau-tai-san/14-moc_dbvn.tbl_guide_post-list.png` · `14-moc_dbvn.tbl_guide_post-detail.png`  
> **Route:** `/so-ts?type=DELINEATOR` · form `/so-ts/tao-moi` · `/so-ts/sua?id=`  
> **Cấm** tab legacy · **cấm** fork form — reuse section S-META / S-ROUTE / S-LOC-* / S-NAME / S-ATTR / S-GPS

## DoD

1. Grid cột đúng loại · ẩn cột không thông tin (`GAP-SOTS-COL-01`)
2. Form field = tab Thông tin chung trên mẫu · control Linm
3. Import section từ parent — không copy-paste `AssetFormPage`

## Implement tracking

| lane | phase | status | updatedAt |
|------|-------|--------|-----------|
| web | `done` | `done` | `2026-09-01T15:19:58.503Z` |
| mobile | — | — | — |

## Filter bar

- [`so-ts-delineator-filter-bar.md`](so-ts-delineator-filter-bar.md) · live `/so-ts?type=DELINEATOR`
