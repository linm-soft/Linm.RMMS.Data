# Sổ TS — Công trình cứu hộ (so-ts-rescue-station) — filter bar context

**Page:** so-ts-rescue-station list · **kind:** catalog (Kind B) · **typeCode:** `RESCUE_STATION`  
**Route live:** `/so-ts?type=RESCUE_STATION` · alias board `/so-ts-rescue-station` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-rescue-station-list`  
...
**SA query keys:** `specs/so-ts-rescue-station/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-rescue-station/ui/design.md` § Zone B
...
|---|----------|---------|------|-------------|
| 1 | Tìm kiếm | `SearchTextInput` (không nút Tìm riêng) | `leading` | `search` — mã · tên kho bãi · tuyến · QR · dumpSpecs |
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `RESCUE_STATION` deep-link · **ẩn** khi type cố định |
...
| Rule | Value |
|------|-------|
| Title | trái — «Sổ TS — Công trình cứu hộ» / listTitle «Danh sách công trình cứu hộ» |
...
Smoke trên `http://localhost:9301/so-ts?type=RESCUE_STATION` (QA/Dev — **cấm** TL start:std).

| ID | Check |
|----|-------|
| V1 | Title trái · filter inputs cụm phải |
| V2 | 🔍 trên bar · **không** nút Tìm riêng |
| V3 | type lock RESCUE_STATION · deep-link |
