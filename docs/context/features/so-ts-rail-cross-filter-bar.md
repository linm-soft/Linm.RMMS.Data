# Sổ TS — Giao cắt đường sắt (so-ts-rail-cross) — filter bar context

**Page:** so-ts-rail-cross list · **kind:** catalog (Kind B) · **typeCode:** `RAIL_CROSS`  
**Route live:** `/so-ts?type=RAIL_CROSS` · alias board `/so-ts-rail-cross` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-rail-cross-list`  
...
**SA query keys:** `specs/so-ts-rail-cross/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-rail-cross/ui/design.md` §3.1 Zone B
...
|---|----------|---------|------|-------------|
| 1 | Tìm kiếm | `SearchTextInput` (không nút Tìm riêng) | `leading` | `search` — mã · tên giao cắt · tuyến · QR |
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `RAIL_CROSS` deep-link · **ẩn** khi type cố định |
...
**List GET:** `?type=RAIL_CROSS&search=&route=&kmFrom=&kmTo=&orgUnit=&page=&pageSize=`

---

## 2. Layout

- Title trái («Danh sách giao cắt đường sắt») · mọi input + 🔍 cụm phải · wrap `flex-end` (`filter-bar-layout-hard` V1–V5).
- Actions CRUD = toolbar Zone B (`catalogToolbar`) — **không** trên filter bar.
- Deep-link `type=RAIL_CROSS` giữ khi clear filters (type lock).
      ...
      <LinErpListFilterBar
        ...
        leading={(
          <>
            ...
            {/* type SearchInput ẩn khi deep-link RAIL_CROSS */}
...
Empty search/route/km/org = all `RAIL_CROSS` rows for company · type luôn `RAIL_CROSS`.
