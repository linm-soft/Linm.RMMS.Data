# Sổ TS — Bãi đỗ xe (so-ts-parking) — filter bar context

**Page:** so-ts-parking list · **kind:** catalog (Kind B) · **typeCode:** `PARKING`  
**Route live:** `/so-ts?type=PARKING` · alias board `/so-ts-parking` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-parking-list`  
...
**SA query keys:** `specs/so-ts-parking/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-parking/ui/design.md` § Zone B
...
|---|----------|---------|------|-------------|
| 1 | Tìm kiếm | `SearchTextInput` (không nút Tìm riêng) | `leading` | `search` — mã · tên bãi · loại · chủ SH · tuyến · QR · dumpSpecs |
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `PARKING` deep-link · **ẩn** khi type cố định |
...
| Rule | Value |
|------|-------|
| Title | trái — «Sổ TS — Bãi đỗ xe» / listTitle «Danh sách bãi đỗ xe» |
...
Smoke trên `http://localhost:9301/so-ts?type=PARKING` (QA/Dev — **cấm** TL start:std).

| ID | Check |
|----|-------|
| V1 | Title trái · filter inputs cụm phải |
| V2 | 🔍 trên bar · **không** nút Tìm riêng |
| V3 | type lock PARKING · deep-link |
| V4 | SearchInput route/org · **cấm** free-text |
| V5 | 0× ErpListHeaderFilters / LinListFilterField / export trên bar |

## 4. Cấm

Demo/localStorage SSOT · ERP.* · invent date filter · fork filter shell · lẫn REST_AREA trong PARKING list
