# Sổ TS — Hộ lan / tôn sóng

> **Slug:** `so-ts-guardrail` · **type:** `GUARDRAIL` · **cluster:** `linear_protect` · ô `t17`  
> **Parent SSOT:** [`so-ts-type-grid.md`](so-ts-type-grid.md) · fields [`import-gov-asset-fields.md`](import-gov-asset-fields.md)  
> **Mẫu:** `docs/img/gov-mau-tai-san/5-tbl_guardrail-list.png` · `5-tbl_guardrail-detail.png`  
> **Route:** `/so-ts?type=GUARDRAIL` · form `/so-ts/tao-moi` · `/so-ts/sua?id=`  
