# Vi phạm HLATĐB — Feature Context

> **Slug:** `rpt-vi-pham-hlatdb` · **Module:** `Report` · **Phase:** P2 · **Wave:** C  
> **Status:** Context  
...
**Mock:** 8–15 dòng mẫu từ domain RowViolation.

Nhập liệu ở catalog. Trang này chỉ tổng hợp.
...
| Method | Path | Mô tả |
|--------|------|-------|
| GET | /api/v1/reports/row-violations?from=&to=&routeId=&q= | Xem báo cáo (paged) |
| GET | /api/v1/reports/row-violations/export | Excel |

Query đọc RowViolation — không bảng báo cáo riêng bắt buộc P1 (read-model P2 nếu nặng).

Auth: JWT · tenant · perm `report.vi-pham-hlatdb.read`.

## 4. Database

Read-model / view trên entity nguồn (RowViolation). Index theo filter UI (tuyến · ngày · tenant).
