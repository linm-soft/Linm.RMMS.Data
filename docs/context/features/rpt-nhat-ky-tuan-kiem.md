# Nhật ký tuần kiểm — Feature Context

> **Slug:** `rpt-nhat-ky-tuan-kiem` · **Module:** `Report` · **Phase:** P2 · **Wave:** C  
> **Status:** Context  
> **Kind:** **E** (report) — `/erp-report-context` · AnalyticsReportShell  
> **Sources:** GOVOne › Nhật ký tuần kiểm · csdl sổ 8 · hub [reports.md](reports.md) · [15-SCREEN-AI-MAP.md](../15-SCREEN-AI-MAP.md)  
> **Parent / list pack:** `csdl-so-sach` — **cấm** copy CRUD vào trang này  
> **MFE:** `Linm.Web.RMMS.Report` · route `/bao-cao/nhat-ky-tuan-kiem` · mfeStdUrl `http://localhost:9311/bao-cao/nhat-ky-tuan-kiem`  
...
| Align MFE | http://localhost:9311/bao-cao/nhat-ky-tuan-kiem |
...
**Mock:** 8–15 dòng mẫu từ domain InspectionLogBook entries.

Read-model sổ kiểm. Nhập liệu ở `csdl-so-sach` / patrol.
...
| Method | Path | Mô tả |
|--------|------|-------|
| GET | /api/v1/reports/patrol-log-inspect?from=&to=&routeId=&q= | Xem báo cáo (paged) |
| GET | /api/v1/reports/patrol-log-inspect/export | Excel |

Query đọc InspectionLogBook entries — không bảng báo cáo riêng bắt buộc P1 (read-model P2 nếu nặng).

Auth: JWT · tenant · perm `report.nhat-ky-tuan-kiem.read`.

## 4. Database

Read-model / view trên entity nguồn (InspectionLogBook entries). Index theo filter UI (tuyến · ngày · tenant).
...
| ID | Default |
|----|---------|
| GAP-F-RPT-LEAF-01 Tách khỏi hub `reports` | Leaf slug + route /bao-cao/nhat-ky-tuan-kiem |
