# Sổ TS — Trạm đếm (so-ts-count-station) — filter bar context

**Page:** so-ts-count-station list · **kind:** catalog (Kind B) · **typeCode:** `COUNT_STATION`  
**Route live:** `/so-ts?type=COUNT_STATION` · alias board `/so-ts-count-station` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-count-station-list`  
...
**SA query keys:** `specs/so-ts-count-station/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-count-station/ui/design.md` § Zone B
...
|---|----------|---------|------|-------------|
| 1 | Tìm kiếm | `SearchTextInput` (không nút Tìm riêng) | `leading` | `search` — mã · tên VI · tuyến · QR · dumpSpecs |
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `COUNT_STATION` deep-link · **ẩn** khi type cố định |
...
| Rule | Value |
|------|-------|
| Title | trái — «Sổ TS — Trạm đếm» / listTitle «Danh sách trạm đếm» |
...
Smoke trên `http://localhost:9301/so-ts?type=COUNT_STATION` (QA/Dev — **cấm** TL start:std).

| ID | Check |
|----|-------|
| V1 | Title trái · filter inputs cụm phải |
| V2 | 🔍 trên bar · **không** nút Tìm riêng |
| V3 | type lock COUNT_STATION · deep-link |
| V4 | SearchInput route/org · **cấm** free-text |
| V5 | 0× ErpListHeaderFilters / LinListFilterField / export trên bar |

## 4. Cấm

Demo/localStorage SSOT · ERP.* · invent date filter · fork filter shell · lẫn type khác trong COUNT list
