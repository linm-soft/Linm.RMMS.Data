# Khối lượng thiệt hại — Feature Context

> **Slug:** `rpt-thiet-hai` · **Module:** `Report` · **Phase:** P2 · **Wave:** C  
> **Status:** Context  
> **Kind:** **E** (report) — `/erp-report-context` · AnalyticsReportShell  
> **Sources:** GOVOne › Khối lượng thiệt hại · hub [reports.md](reports.md) · [15-SCREEN-AI-MAP.md](../15-SCREEN-AI-MAP.md)  
> **Parent / list pack:** `incident` — **cấm** copy CRUD vào trang này  
> **MFE:** `Linm.Web.RMMS.Report` · route `/bao-cao/thiet-hai` · mfeStdUrl `http://localhost:9311/bao-cao/thiet-hai`  
...
| Align MFE | http://localhost:9311/bao-cao/thiet-hai |
...
**Mock:** 8–15 dòng mẫu từ domain Incident damage lines.

Bảng KL — không phải form ghi nhận hiện trường.
...
| Method | Path | Mô tả |
|--------|------|-------|
| GET | /api/v1/reports/damage-qty?from=&to=&routeId=&q= | Xem báo cáo (paged) |
| GET | /api/v1/reports/damage-qty/export | Excel |

Query đọc Incident damage lines — không bảng báo cáo riêng bắt buộc P1 (read-model P2 nếu nặng).

Auth: JWT · tenant · perm `report.thiet-hai.read`.

## 4. Database

Read-model / view trên entity nguồn (Incident damage lines). Index theo filter UI (tuyến · ngày · tenant).
...
| ID | Default |
|----|---------|
| GAP-F-RPT-LEAF-01 Tách khỏi hub `reports` | Leaf slug + route /bao-cao/thiet-hai |
