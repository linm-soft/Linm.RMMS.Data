# Sổ TS — Hệ thống ITS (so-ts-its-camera) — filter bar context

**Page:** so-ts-its-camera list · **kind:** catalog (Kind B) · **typeCode:** `ITS_CAMERA`  
**Route live:** `/so-ts?type=ITS_CAMERA` · alias board `/so-ts-its-camera` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-its-camera-list`  
...
**SA query keys:** `specs/so-ts-its-camera/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-its-camera/ui/design.md` § Zone B
...
|---|----------|---------|------|-------------|
| 1 | Tìm kiếm | `SearchTextInput` (không nút Tìm riêng) | `leading` | `search` — mã · tên phòng ITS · TTĐH · tuyến · QR |
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `ITS_CAMERA` deep-link · **ẩn** khi type cố định |
...
**List GET:** `?type=ITS_CAMERA&search=&route=&kmFrom=&kmTo=&orgUnit=&page=&pageSize=`
...
| Rule | Value |
|------|-------|
| Title | trái — «Sổ TS — Hệ thống ITS» / listTitle «Danh sách hệ thống ITS» |
...
Smoke trên `http://localhost:9301/so-ts?type=ITS_CAMERA` (QA/Dev — **cấm** TL start:std).

| ID | Check |
|----|-------|
| V1 | Title trái · filter inputs cụm phải |
| V2 | 🔍 trên bar · **không** nút Tìm riêng |
| V3 | type lock ITS_CAMERA · deep-link |
| V4 | SearchInput route/org · **cấm** free-text |
| V5 | 0× ErpListHeaderFilters / LinListFilterField / export trên bar |

## 4. Cấm

Demo/localStorage SSOT · ERP.* · invent date filter · fork filter shell · lẫn type khác trong ITS_CAMERA list · camera-connect merge
