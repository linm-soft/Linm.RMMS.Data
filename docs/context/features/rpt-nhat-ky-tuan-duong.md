# Nhật ký tuần đường — Feature Context
...
**Mock:** 8–15 dòng mẫu từ domain PatrolLogBook entries.

Read-model sổ BDTX mẫu 1. Không form nhập trên trang report.
...
| Method | Path | Mô tả |
|--------|------|-------|
| GET | /api/v1/reports/patrol-log-road?from=&to=&routeId=&q= | Xem báo cáo (paged) |
| GET | /api/v1/reports/patrol-log-road/export | Excel |

Query đọc PatrolLogBook entries — không bảng báo cáo riêng bắt buộc P1 (read-model P2 nếu nặng).

Auth: JWT · tenant · perm `report.nhat-ky-tuan-duong.read`.

## 4. Database

Read-model / view trên entity nguồn (PatrolLogBook entries). Index theo filter UI (tuyến · ngày · tenant).
