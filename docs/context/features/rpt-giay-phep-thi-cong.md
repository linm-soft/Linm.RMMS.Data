# Giấy phép thi công — Feature Context

> **Slug:** `rpt-giay-phep-thi-cong` · **Module:** `Report` · **Phase:** P3 · **Wave:** D  
> **Status:** Context  
> **Kind:** **E** (report) — `/erp-report-context` · AnalyticsReportShell  
> **Sources:** GOVOne Tài liệu › Giấy phép thi công · csdl sổ 6 · hub [reports.md](reports.md) · [15-SCREEN-AI-MAP.md](../15-SCREEN-AI-MAP.md)  
> **Parent / list pack:** `csdl-so-sach` — **cấm** copy CRUD vào trang này  
> **MFE:** `Linm.Web.RMMS.Report` · route `/bao-cao/giay-phep/thi-cong` · mfeStdUrl `http://localhost:9311/bao-cao/giay-phep/thi-cong`  
...
| Align MFE | http://localhost:9311/bao-cao/giay-phep/thi-cong |
...
**Mock:** 8–15 dòng mẫu từ domain ConstructionPermit.

P3. Không form cấp phép trên report.
...
| Method | Path | Mô tả |
|--------|------|-------|
| GET | /api/v1/reports/construction-permits?from=&to=&routeId=&q= | Xem báo cáo (paged) |
| GET | /api/v1/reports/construction-permits/export | Excel |

Query đọc ConstructionPermit — không bảng báo cáo riêng bắt buộc P1 (read-model P2 nếu nặng).

Auth: JWT · tenant · perm `report.giay-phep-thi-cong.read`.

## 4. Database

Read-model / view trên entity nguồn (ConstructionPermit). Index theo filter UI (tuyến · ngày · tenant).
...
| ID | Default |
|----|---------|
| GAP-F-RPT-LEAF-01 Tách khỏi hub `reports` | Leaf slug + route /bao-cao/giay-phep/thi-cong |
