...
> **Slug:** `so-ts-ditch` · **type:** `DITCH` · **cluster:** `linear_protect` · ô `t10`  
> **Parent SSOT:** [`so-ts-type-grid.md`](so-ts-type-grid.md) · fields [`import-gov-asset-fields.md`](import-gov-asset-fields.md)  
> **Mẫu:** `docs/img/gov-mau-tai-san/13-moc_dbvn.tbl_longitudinal-list.png` · `13-moc_dbvn.tbl_longitudinal-detail.png`  
> **Route:** `/so-ts?type=DITCH` · form `/so-ts/tao-moi` · `/so-ts/sua?id=`  
