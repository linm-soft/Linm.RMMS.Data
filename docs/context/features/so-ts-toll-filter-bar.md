# Sổ TS — Trạm thu phí (so-ts-toll) — filter bar context

**Page:** so-ts-toll list · **kind:** catalog (Kind B) · **typeCode:** `TOLL`  
**Route live:** `/so-ts?type=TOLL` · alias board `/so-ts-toll` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-toll-list`  
...
**SA query keys:** `specs/so-ts-toll/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-toll/ui/design.md` § Zone B
...
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `TOLL` deep-link · **ẩn** khi type cố định |
...
| Rule | Value |
|------|-------|
| Title | trái — «Sổ TS — Trạm thu phí» / listTitle «Danh sách trạm thu phí» |
...
Smoke trên `http://localhost:9301/so-ts?type=TOLL` (QA/Dev — **cấm** TL start:std).

| ID | Check |
|----|-------|
| V1 | Title trái · filter inputs cụm phải |
| V2 | 🔍 trên bar · **không** nút Tìm riêng |
| V3 | type lock TOLL · deep-link |
| V4 | SearchInput route/org · **cấm** free-text |
| V5 | 0× ErpListHeaderFilters / LinListFilterField / export trên bar |

## 4. Cấm

Demo/localStorage SSOT · ERP.* · invent date filter · fork filter shell · lẫn type khác trong TOLL list
