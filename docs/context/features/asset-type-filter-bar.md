# Loại tài sản (asset-type) — filter bar context

**Page:** asset-type list · **kind:** catalog (Kind B flat)  
**Route live:** `/mas/loai-ts` · **testIdPrefix:** `rmms-asset-type-list`  
...
**SA query keys:** `specs/master/be/solution-discovery.md` §2d · AT-01
...
| 2 | Nhóm | `Select` / Dropdown · options `init-data.groupCodes` + «Tất cả» | `leading` | `groupCode` · THOAT_NUOC · AN_TOAN · … |
...
**Dropdown:** chỉ `GET …/integration/asset-types/init-data` — **cấm** hardcode FE.
...
- Title trái («Danh sách loại tài sản») · input + 🔍 cụm phải · wrap `flex-end`.
- **Không** date range P1.
      ...
      <LinErpListFilterBar
        ...
        leading={(
          <>
            ...
            <div data-testid={`${TEST_ID}-field-groupCode`}>…Select groupCode…</div>
          </>
        )}
      />
    )}
