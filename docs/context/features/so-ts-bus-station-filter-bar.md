# Sổ TS — Bến xe (so-ts-bus-station) — filter bar context

**Page:** so-ts-bus-station list · **kind:** catalog (Kind B) · **typeCode:** `BUS_STATION`  
**Route live:** `/so-ts?type=BUS_STATION` · alias board `/so-ts-bus-station` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-bus-station-list`  
...
**SA query keys:** `specs/so-ts-bus-station/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-bus-station/ui/design.md` § Zone B
...
|---|----------|---------|------|-------------|
| 1 | Tìm kiếm | `SearchTextInput` (không nút Tìm riêng) | `leading` | `search` — mã · tên bến · loại · chủ SH · tuyến · QR · dumpSpecs |
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `BUS_STATION` deep-link · **ẩn** khi type cố định |
...
| Rule | Value |
|------|-------|
| Title | trái — «Sổ TS — Bến xe» / listTitle «Danh sách bến xe» |
...
Smoke trên `http://localhost:9301/so-ts?type=BUS_STATION` (QA/Dev — **cấm** TL start:std).

| ID | Check |
|----|-------|
| V1 | Title trái · filter inputs cụm phải |
| V2 | 🔍 trên bar · **không** nút Tìm riêng |
| V3 | type lock BUS_STATION · deep-link |
