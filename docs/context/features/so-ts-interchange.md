...
> **Slug:** `so-ts-interchange` · **type:** `INTERCHANGE` · **cluster:** `crossing` · ô `t23`  
> **Parent SSOT:** [`so-ts-type-grid.md`](so-ts-type-grid.md) · fields [`import-gov-asset-fields.md`](import-gov-asset-fields.md)  
> **Mẫu:** `docs/img/gov-mau-tai-san/26-moc_dbvn.tbl_intersection-list.png` · `26-moc_dbvn.tbl_intersection-detail.png`  
> **Route:** `/so-ts?type=INTERCHANGE` · form `/so-ts/tao-moi` · `/so-ts/sua?id=`  
