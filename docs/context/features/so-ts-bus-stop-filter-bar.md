# Sổ TS — Điểm dừng xe buýt (so-ts-bus-stop) — filter bar context

**Page:** so-ts-bus-stop list · **kind:** catalog (Kind B) · **typeCode:** `BUS_STOP`  
**Route live:** `/so-ts?type=BUS_STOP` · alias board `/so-ts-bus-stop` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-bus-stop-list`  
...
**SA query keys:** `specs/so-ts-bus-stop/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-bus-stop/ui/design.md` § Zone B
...
|---|----------|---------|------|-------------|
| 1 | Tìm kiếm | `SearchTextInput` (không nút Tìm riêng) | `leading` | `search` — mã · tên điểm · loại · ĐV QL · tuyến · QR |
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `BUS_STOP` deep-link · **ẩn** khi type cố định |
...
| Rule | Value |
|------|-------|
| Title | trái — «Sổ TS — Điểm dừng xe buýt» / listTitle «Danh sách điểm dừng xe buýt» |
...
Smoke trên `http://localhost:9301/so-ts?type=BUS_STOP` (QA/Dev — **cấm** TL start:std).

| ID | Check |
|----|-------|
| V1 | Title trái · filter inputs cụm phải |
| V2 | 🔍 trên bar · **không** nút Tìm riêng |
| V3 | type lock BUS_STOP · deep-link |
| V4 | SearchInput route/org · **cấm** free-text |
| V5 | 0× ErpListHeaderFilters / LinListFilterField / export trên bar |

## 4. Cấm

Demo/localStorage SSOT · ERP.* · invent date filter · fork filter shell · lẫn type khác trong BUS_STOP list
