...
> **Slug:** `so-ts-its-camera` · **type:** `ITS_CAMERA` · **cluster:** `ops` · ô `t19`  
> **Parent SSOT:** [`so-ts-type-grid.md`](so-ts-type-grid.md) · fields [`import-gov-asset-fields.md`](import-gov-asset-fields.md)  
> **Mẫu:** `docs/img/gov-mau-tai-san/25-moc_dbvn.tbl_its-list.png` · `25-moc_dbvn.tbl_its-detail.png`  
> **Route:** `/so-ts?type=ITS_CAMERA` · form `/so-ts/tao-moi` · `/so-ts/sua?id=`  
