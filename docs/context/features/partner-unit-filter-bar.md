# Đơn vị đối tác (partner-unit) — filter bar context

**Page:** partner-unit list · **kind:** catalog (Kind B flat)  
**Route live:** `/mas/doi-tac` · **testIdPrefix:** `rmms-partner-unit-list`  
...
**SA query keys:** `specs/master/be/solution-discovery.md` §2e · PU-01
...
|---|----------|---------|------|-------------|
| 1 | Tìm kiếm | `SearchTextInput` (không nút Tìm riêng) | `leading` | `search` — mã · tên · folder |
| 2 | Loại ĐV | `Select` / Dropdown · options `init-data.partnerKinds` + «Tất cả» | `leading` | `partnerKind` · SO_GTVT · BOT · DOANH_NGHIEP |
...
**Dropdown:** chỉ `GET …/integration/partner-units/init-data` — **cấm** hardcode FE.

**Code scheme (GAP-PARTNER-01):** `SO-*` · `BOT-*` · `DN-*` — validate trên form (T-UI-FORM), không trên filter.
...
- Title trái («Danh sách đơn vị đối tác») · input + 🔍 cụm phải · wrap `flex-end`.
- **Không** date range P1.
      ...
      <LinErpListFilterBar
        ...
        leading={(
          <>
            ...
            <div data-testid={`${TEST_ID}-field-partnerKind`}>…Select partnerKind…</div>
          </>
        )}
      />
    )}
...
| GAP-PARTNER-01 | Code scheme trên form — không filter |
