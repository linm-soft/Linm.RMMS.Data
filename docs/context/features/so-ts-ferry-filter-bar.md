# Sổ TS — Bến phà (so-ts-ferry) — filter bar context

**Page:** so-ts-ferry list · **kind:** catalog (Kind B) · **typeCode:** `FERRY`  
**Route live:** `/so-ts?type=FERRY` · alias board `/so-ts-ferry` (optional redirect)  
**testIdPrefix:** `rmms-so-ts-ferry-list`  
...
**SA query keys:** `specs/so-ts-ferry/be/solution-discovery.md` § FormType pack · API-01  
**Design:** `specs/so-ts-ferry/ui/design.md` § Zone B
...
|---|----------|---------|------|-------------|
| 1 | Tìm kiếm | `SearchTextInput` (không nút Tìm riêng) | `leading` | `search` — mã · tên bến phà · tuyến · QR |
| 2 | Loại tài sản | `SearchInput` `asset-type` | `leading` | `type` — **required** `FERRY` deep-link · **ẩn** khi type cố định |
...
| Rule | Value |
|------|-------|
| Title | trái — «Sổ TS — Bến phà» / listTitle «Danh sách bến phà» |
...
Smoke trên `http://localhost:9301/so-ts?type=FERRY` (QA/Dev — **cấm** TL start:std).

| ID | Check |
|----|-------|
| V1 | Title trái · filter inputs cụm phải |
| V2 | 🔍 trên bar · **không** nút Tìm riêng |
| V3 | type lock FERRY · deep-link |
