# Sổ TS — Hầm chui dân sinh

> **Slug:** `so-ts-underpass` · **type:** `UNDERPASS` · **cluster:** `crossing` · ô `t06`  
> **Parent SSOT:** [`so-ts-type-grid.md`](so-ts-type-grid.md) · fields [`import-gov-asset-fields.md`](import-gov-asset-fields.md)  
> **Mẫu:** `docs/img/gov-mau-tai-san/12-tbl_underpass_box-list.png` · `12-tbl_underpass_box-detail.png`  
> **Route:** `/so-ts?type=UNDERPASS` · form `/so-ts/tao-moi` · `/so-ts/sua?id=`  
