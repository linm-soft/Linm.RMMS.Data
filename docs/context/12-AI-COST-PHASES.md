...
> **GPU burst:** [Vast.ai](https://cloud.vast.ai/) — giá marketplace động  
...
3. **Train theo đợt** (Vast RTX 4090 **hoặc** lab vật lý) — tắt máy sau export ONNX.  
4. **Infer prod** trên host ổn định (L4 / box VN / cloud VN SLA) — **không** 24/7 Vast.  
...
| **1 Gate** | Giữ 0 | Giữ 0 | — |
| **2 Train** | DMS | GPT (so sánh) | Vast **hoặc** lab vật lý |
| **3 Hybrid** | DMS + queue | **GPU worker cố định** + GPT fallback | Retrain burst |
| **4 Prod** | DMS | L4 vật lý / cloud VN SLA | Vast/lab khi cần |

→ Chi tiết bảng + checklist: **`13-AI-SERVER-BY-PHASE.md`**.
...
| 50k + triage | ~**$175** |
| Train Vast 4090 · 40–80h | ~**$16–45** / chu kỳ (@ ~$0.25–0.55/h) |
| 4090 always-on 1 tháng | ~**$220–400** — **không khuyến nghị** prod |
...
| **2** | Train Vast (burst) | $16–45/đợt + đội gán nhãn | Model ONNX/engine |
| **3** | Hybrid infer | Token giảm + GPU worker | Bill ổn khi volume tăng |
| **4** | GPU prod cố định | Thuê L4-class / máy VN | SLA · compliance |
...
- [ ] Vast: on-demand · RAM≥64 · tắt sau export  
- [ ] Prod: queue + worker tách DMS · GPT fallback  
- [ ] Không A100 từ đầu (YOLO mặt đường không cần)
