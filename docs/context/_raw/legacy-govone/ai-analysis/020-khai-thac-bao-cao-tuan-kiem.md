...
| **screenshot** | `screenshots/020-khai-thac-bao-cao-tuan-kiem.png` |
...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `020-khai-thac-bao-cao-tuan-kiem.png`  
> **Capture note:** PNG byte-identical với twin `023-khai-thac-bao-cao-tuan-kiem.png` (SHA256 `72798A43…`) — ảnh pixel gần trắng (header + divider + body trống); OCR/vision thấy tiêu đề modal **Chấm công** (cũng xuất hiện cuối `bodySample`). Mục tiêu packet = **KHAI THÁC BÁO CÁO › Tuần kiểm**; reconstruct UI từ `capture/reports/tuan-kiem/view/inventory.json` `bodySample` + sibling dashboard `019`/`016` + control-map `patrol`.
...
Capture path: `capture/reports/tuan-kiem/view/` (master/page/action) · pageTitle **Tuần kiểm** · twin `023`.  
Mở từ sidebar / tile **Tuần kiểm** trong module khai thác báo cáo (khác catalog **Báo cáo tổng hợp** `018` · khác Bảng tổng hợp nhanh root `019` nhưng **cùng shell dashboard**).
...
| **Z3 Panel — CÔNG TÁC TUẦN KIỂM (focus)** | Cột giữa: header tím + collapse · list tuyến / trạng thái | Header button **CÔNG TÁC TUẦN KIỂM** · thu gọn `−` · **QL.48C** — **Chưa thực hiện checkin** · **QL.7** — **Chưa thực hiện checkin** · (sibling `019` lúc load: overlay **Đang xử lý** / alert empty) |
...
**Primary — KHAI THÁC BÁO CÁO › Tuần kiểm (`020` / twin `023`)**
...
Sibling `016` (SỬA CHỮA ĐỊNH KỲ › Tuần kiểm) · `019` (Bảng THN) · twin `023` — giữ trong `patrol-actions.md` / control-map. **Không** bịa button không có trên ảnh/DOM.

### Map → step context

- Feature: `docs/context/features/patrol.md`
- Section: `## Legacy GOVOne (auto-capture)` → capture id `020-khai-thac-bao-cao-tuan-kiem`
...
- **Capture path:** `capture/reports/tuan-kiem/view/` (master/page/action)
- Sibling: twin `023` · `016` (SỬA CHỮA › Tuần kiểm) · `019`/`009`/`022` (Bảng THN) · `021`/`024` (Tuần đường)
