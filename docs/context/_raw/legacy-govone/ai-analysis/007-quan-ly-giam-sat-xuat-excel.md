...
| **screenshot** | `screenshots/007-quan-ly-giam-sat-xuat-excel.png` |
...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `007-quan-ly-giam-sat-xuat-excel.png`
...
Capture path: `capture/patrol/xuat-excel/view/` · page JSON id `007-quan-ly-giam-sat-xuat-excel` · `via: deep` · trigger toolbar **Xuất excel**.  
Sibling gần trùng DOM: `008-quan-ly-giam-sat-xuat-excel` (cùng path view / cùng labels+fields) · drill form-sample `009-quan-ly-giam-sat-xuat-excel` (`› +`).  
...
Upstream: `005` root / `006` user menu · Downstream: file download Excel · đóng modal · sibling `008` (cùng modal) · `009` form-sample `+` · reload `008/010/011-tai-lai`.  
...
**Primary — Modal Xuất dữ liệu Excel (`007`)**
...
- Upstream: `005`/`006` shell · Downstream: file Excel · sibling `008` (cùng modal) · `009-xuat-excel` (`› +`) · reload variants
...
`008-xuat-excel` (cùng modal) · `009-xuat-excel` (+ drill) · `008/010/011-tai-lai` · `009-oi-mat-khau` — giữ trong `patrol-actions.md` / control-map.

### Map → step context

- Feature: `docs/context/features/patrol.md`
- Section: `## Legacy GOVOne (auto-capture)` → capture id `007-quan-ly-giam-sat-xuat-excel`
...
- **Capture path:** `capture/patrol/xuat-excel/view/` (master/page/action) · page id `007`
- Sibling: `008-quan-ly-giam-sat-xuat-excel` · `009-quan-ly-giam-sat-xuat-excel` · root `005` · user `006`
