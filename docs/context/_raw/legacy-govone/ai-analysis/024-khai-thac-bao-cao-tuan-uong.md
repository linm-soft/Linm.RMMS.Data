...
| **screenshot** | `screenshots/024-khai-thac-bao-cao-tuan-uong.png` |
...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `024-khai-thac-bao-cao-tuan-uong.png`  
> **Capture note:** PNG byte-identical với `020`/`023` (SHA256 `72798A43FA92042B…`) — ảnh pixel gần trắng (header + divider + body trống); OCR/vision thấy tiêu đề modal **Chấm công** (cũng xuất hiện cuối `bodySample`). Twin `021` (SHA256 `69B4A847…`) = cùng menu path nhưng viewport loading (**Đang xử lý**). Inventory primary id = `024` · path `capture/reports/tuan-uong/view/`. Mục tiêu packet = **KHAI THÁC BÁO CÁO › Tuần đường**; reconstruct UI từ `inventory.json` `bodySample` + sibling dashboard `019`/`022`/`021` + control-map `patrol`.
...
Capture path: `capture/reports/tuan-uong/view/` (master/page/action) · pageTitle **Tuần đường** · inventory id **`024`** · twin screenshot `021`.  
Mở từ sidebar / tile **Tuần đường** trong module khai thác báo cáo (khác catalog **Báo cáo tổng hợp** `018` · khác Bảng tổng hợp nhanh root `019`/`022` nhưng **cùng shell dashboard** · khác focus **Tuần kiểm** `020`/`023`).
...
Ảnh PNG (miscapture): modal/shell tiêu đề **Chấm công** · body trắng · maximize + × / **Đóng** — không lộ KPI/panels; dùng evidence modal chrome. Twin `021` = cùng shell + overlay xám + purple badge **Đang xử lý**.  
...
| Empty / loading (panel / modal) | EmptyState / Spinner | — | Twin `021`: **Đang xử lý** · sibling: **Chưa có dữ liệu được ghi nhận!** |
...
**Primary — KHAI THÁC BÁO CÁO › Tuần đường (`024` / twin `021`)**
...
- Loading / empty state khi panel chưa có bản ghi ngày (twin `021`: **Đang xử lý**)
...
Sibling `017` (SỬA CHỮA ĐỊNH KỲ › Tuần đường) · `019`/`022` (Bảng THN) · twin `021` · `020`/`023` (Tuần kiểm) — giữ trong `patrol-actions.md` / control-map. **Không** bịa button không có trên ảnh/DOM.

### Map → step context

- Feature: `docs/context/features/patrol.md`
- Section: `## Legacy GOVOne (auto-capture)` → capture id `024-khai-thac-bao-cao-tuan-uong`
...
- **Capture path:** `capture/reports/tuan-uong/view/` (master/page/action) · inventory id `024`
- Sibling: twin `021` · `017` (SỬA CHỮA › Tuần đường) · `019`/`009`/`022` (Bảng THN) · `020`/`023` (Tuần kiểm)
