...
| **screenshot** | `screenshots/013-quan-ly-van-e-ban-tk-nguyen-anh-phuc.png` |
...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `013-quan-ly-van-e-ban-tk-nguyen-anh-phuc.png`
...
Điểm khác root `010`/`012-quan-ly-van-e`: drill **user shell** — DOM có **Hồ sơ của tôi** · **Đổi mật khẩu** · **Đăng xuất** (+ trigger **Ban.TK.Nguyễn Anh Phúc**); ảnh viewport: user menu **đóng** (chỉ thấy avatar + label header).  
**Mục đích:** (1) cùng shell list+map quản lý **vấn đề / sự cố** · (2) ghi nhận context **user Ban.TK** trên panel Vấn đề · (3) filter đa tiêu chí (DOM) · (4) CRUD / giao việc (**Thêm** · **Thêm CV** · **Xem CV** · **Xem** · **Xóa** · **Phản hồi**) · (5) **Xuất dữ liệu** · **BC sở** · (6) map đối chiếu vị trí · (7) shell auth menu (parity).  

Ảnh: shell **list trái + map phải** · list **empty** («Không có bản ghi nào để hiển thị») · footer **Trang … của 0** + 3 ô đếm màu (xanh / nâu / đỏ = `0`) · toolbar **Thêm · Thêm CV · Xem CV · Xem · Xuất dữ liệu** (+ icon refresh · icon overflow/tools) · map Nghệ An–Thanh Hóa (nhãn `QL.217` · Quan Sơn · Quan Hóa · Ngọc Lặc · Con Cuông · …) · scale **20 km / 10 mi**.  
...
Sibling: root `005`/`010`/`012` · Ban.TK view twin `012-ban-tk` · create `011`/`013-them`/`014`/`016`/`017` · Thêm CV `014`/`015` · user drills password sibling.  
...
| **Z1b User dropdown (DOM)** | Menu user — không mở trên ảnh | **Hồ sơ của tôi** · **Đổi mật khẩu** · **Đăng xuất** (DOM actions; twin `012-ban-tk`) |
...
| **Z3 Toolbar list** | Hàng action trên panel list | Icon **refresh** (tải lại list+map) · **Thêm** (primary) · **Thêm CV** · **Xem CV** · **Xem** · **Xuất dữ liệu** (dropdown) · icon overflow/tools (→ **Xóa** / **Phản hồi** / **BC sở**) |
...
| **Z5 Grid / list content** | Danh sách vấn đề — empty | Empty **Không có bản ghi nào để hiển thị** · không lộ cột (`tableHeaders: []`) |
| **Z6 List footer** | Status chips + pager | 3 ô đếm màu **0** (xanh / nâu / đỏ — filter nhanh theo nhóm trạng thái/mức độ) · pager `<<` `<` **Trang … của 0** `>` `>>` · `inputItem` = trang hiện tại |
| **Z7 Splitter** | Thanh dọc list ↔ map | Grip thu/mở panel list |
| **Z8 Map canvas** | Bản đồ mạng tuyến / vị trí VĐ | Polyline đường · nhãn `QL.217` · địa danh Quan Sơn / Quan Hóa / Ngọc Lặc / Con Cuông · vùng Nghệ An–Thanh Hóa |
...
| Công ty | Combo / Select | — (filter) | DOM `congty` |
| Tuần đường | Combo / Select | — (filter) | DOM `tuanduong` |
| Trạng thái (xử lý) | Combo / Select | — (filter) | DOM `trangthaixuly` |
...
| Hướng xử lý | Combo / Select | — (filter) | DOM `huongxuly` |
| Tài sản | Combo / Select | — (filter) | DOM `loaiTaiSan` |
| TT báo cáo | Combo / Select | — (filter) | DOM `trangthaibaocao` |
...
| Trang (pager) | Number | — | DOM `inputItem` · ảnh **Trang … của 0** |
| Status chips 0/0/0 | Metric / filter chips | — | Footer Z6 — 3 màu xanh/nâu/đỏ |
| Empty state | EmptyState text | — | **Không có bản ghi nào để hiển thị** |
...
**Primary — Ban.TK view `013` (user shell trên QUẢN LÝ VẤN ĐỀ)**
...
- List empty + status chips 0/0/0 + pager **Trang của 0**
...
- ≠ GIS editor geometry (`geditor`) · ≠ dashboard KPI `004` · ≠ patrol-only monitor
...
Twin Ban.TK `012-ban-tk` · root `005`/`010`/`012` · Create/Thêm `011`/`013-them`/`014`/`016`/`017` · Thêm CV `014`/`015` · export menu trong `incident-actions.md` — giữ parity demo.

### Map → step context

- Feature: `docs/context/features/incident.md`
- Section: `## Legacy GOVOne (auto-capture)` → capture id `013-quan-ly-van-e-ban-tk-nguyen-anh-phuc`
...
- Sibling: `012-ban-tk` (twin) · root `005`/`010`/`012` · `013-them` create · `014`/`015` Thêm CV · password drills
