# AI Vision — KHAI THÁC BÁO CÁO › Báo cáo tổng hợp
...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `008-khai-thac-bao-cao-bao-cao-tong-hop.png`  
> bodySample: `Báo cáo tổng hợp · Bảng tổng hợp nhanh · QUẢN LÝ BẢO TRÌ · QUẢN LÝ BDTX · BẢO DƯỠNG THƯỜNG XUYÊN · TÀI SẢN ĐƯỜNG BỘ · Nhật ký tuần đường · 01/08/2026 - 31/08/2026 · Chọn tuyến đường · Mới nhất · Tổng hợp · Hãy chọn các điều kiện lọc dữ liệu, tổng hợp và tải tệp về máy tính.`  
> Capture: `capture/reports/root/view/` · menu path **KHAI THÁC BÁO CÁO › Báo cáo tổng hợp** · twin UI `007` / `018` / `021` (cùng URL)

### Màn hình / mục đích

Màn **KHAI THÁC BÁO CÁO › Báo cáo tổng hợp** (`/DuongBo/baocaotonghop/nhatkytuanduong`) trên GOVOne PMDB.  
Mở từ sidebar / drill **Báo cáo tổng hợp** trong module khai thác báo cáo (khác tile root `007-khai-thac-bao-cao` nhưng **cùng shell URL** · cùng empty-state trước tổng hợp).  
pageTitle **GOVONE - Báo cáo tổng hợp** · headings **Báo cáo tổng hợp** · **LIÊN KẾT TRUY CẬP NHANH** · **Nhật ký tuần đường**.  
Viewport: sidebar item **Báo cáo tổng hợp** active · catalog trái nhóm **BẢO DƯỠNG THƯỜNG XUYÊN** + **TÀI SẢN ĐƯỜNG BỘ** · content đang mở báo cáo con **Nhật ký tuần đường**.  
Twin / sibling gần trùng: `007` / `018` / `021` (root KHAI THÁC BÁO CÁO) · catalog drill `009`/`019`/`022` (Bảng tổng hợp nhanh) · `010` (TNGT) · `011`/`022`/`025` (BDTX) · `020`/`023` (Tuần kiểm) · `021`/`024` (Tuần đường dashboard).

**Mục đích:** (1) điều hướng shell **Báo cáo tổng hợp** (sidebar active) · (2) chọn loại BC từ catalog trái (BDTX / Tài sản đường bộ + cây báo cáo con) · (3) lọc **khoảng ngày** + **tuyến đường** + **thứ tự** · (4) bấm **Tổng hợp** → tổng hợp + **tải tệp** về máy · (5) theme sáng/tối + user menu + footer quick nav (parity).

Ảnh viewport: layout **sidebar tối + catalog trắng + report filter** · kết quả **chưa có grid** (vùng trắng dưới alert vàng) · Kind shell: **E (AnalyticsReportShell / erp-report-context)** — demo MFE theo `/erp-form-context` Kind E — **không** clone skin GOVOne.  
...
|------|-------|------------------------|
| **Z1 Header / app bar** | Thanh trên: brand · title module · utilities | Logo **govOne** · title **Báo cáo tổng hợp** · app-switcher (grid) · theme sáng/tối (sun) · notification · avatar user · (DOM) govone.vn · youtube · facebook · user menu **Ban.TK.…** / Thông tin của tôi / Đổi MK / Đăng xuất |
| **Z2 Sidebar nav** | Menu trái tối — **Báo cáo tổng hợp** active | **Bảng tổng hợp nhanh** · **Báo cáo tổng hợp** (active / highlight) · nhóm **QUẢN LÝ BẢO TRÌ** (Tổng hợp bảo trì · Dự án bảo trì) · **QUẢN LÝ BDTX** (Tuần kiểm · Tuần đường · Số liệu · Tài liệu — chevron) · **QUẢN TRỊ ỨNG DỤNG** → **Phân quyền** |
| **Z3 Report catalog (content left)** | Panel chọn nhóm / loại BC | **BẢO DƯỠNG THƯỜNG XUYÊN** (icon wrench · expanded/active) · **TÀI SẢN ĐƯỜNG BỘ** (icon road) · (DOM tree) Nhật ký · Nhật ký tuần đường · Nhật ký tuần kiểm · Nhật ký công việc · Thiên tai… · Hạng mục hư hỏng · Kiểm tra cầu · TNGT · Vi phạm · Đếm xe · Tài liệu · Tổng hợp chung / theo tuyến / hạng mục… |
| **Z4 Report header** | Tiêu đề báo cáo đang mở + back | Mũi tên **←** back · **Nhật ký tuần đường** |
| **Z5 Filter / toolbar** | Hàng lọc trước khi tổng hợp | Date range **01/08/2026 - 31/08/2026** (+ clear × + calendar) · Select placeholder **Chọn tuyến đường** · icon sort ↑↓ · Select **Mới nhất** · kebab ⋮ (more) · primary xanh **Tổng hợp** · (DOM) unnamed `input` zone toolbar |
...
| **Z7 Result / grid** | Kết quả sau Tổng hợp | **Không lộ cột** trên ảnh 008 — empty trước chạy · parity demo sau mock: STT + cột báo cáo check-in/NKTĐ theo Kind E |
...
**Grid columns (ảnh 008):** không có — empty state.  
...
**Primary — KHAI THÁC BÁO CÁO › Báo cáo tổng hợp (`008`)**
- Mở / active sidebar **Báo cáo tổng hợp** → `/baocaotonghop/nhatkytuanduong`
...
- ≠ Bảng tổng hợp nhanh (sibling `009`/`019`/`022`) · ≠ Tuần đường dashboard (`021`/`024`) · ≠ GIS editor · ≠ dashboard KPI thuần
- Twin root tile `007` / `018` / `021` — cùng UI shell; packet `008` = menu path **› Báo cáo tổng hợp**
...
- Bảng tổng hợp nhanh
- **Báo cáo tổng hợp** (active — focus packet `008`)
...
- Section: `## Legacy GOVOne (auto-capture)` → capture id `008-khai-thac-bao-cao-bao-cao-tong-hop`
...
- Sibling: twin root `007`/`018`/`021` · `009`–`011` · `019`–`025` (catalog / BDTX / tuần kiểm-đường)
