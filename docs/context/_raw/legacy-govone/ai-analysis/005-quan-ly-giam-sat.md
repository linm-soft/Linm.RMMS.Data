...
| **screenshot** | `screenshots/005-quan-ly-giam-sat.png` |
...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `005-quan-ly-giam-sat.png`

### Màn hình / mục đích

Màn **QUẢN LÝ GIÁM SÁT** — panel **Vị trí hiện thời** (`dbv3giamsat.aspx#pnlViTriHienThoi`) trên app GOVOne gServer 2.1.  
Mở từ tile **QUẢN LÝ GIÁM SÁT** (app-tile / popup) · capture path `capture/patrol/root/view/`.  
...
Layout chính: **split pane** — list/filter trái + map phải (≠ dashboard KPI `004-dashboad` · ≠ GIS editor full `geditor` / `025-ve-tinh`).  
...
Ảnh capture: list **empty** («Không có bản ghi nào») · map đã load mạng đường Nghệ An / Thanh Hóa (nhãn `QL.16` · `DT.543B` · `QL.48` · …) · scale **20 km / 10 mi**.  
Sibling cùng URL/slug: `004-quan-ly-giam-sat` (capture song song) · drill user `006` · export modal `007/008/009-xuat-excel` · reload `008/010/011-tai-lai`.  
...
|---------|---------------|-----------|---------|
| Công ty / Nhân viên | TreePicker / Lookup | Có (filter) | Ảnh hiển thị giá trị **Tất cả** · DOM `treepickerex-1025-inputEl` · placeholder **Chọn Công ty/Nhân viên** · zone sidebar |
...
**Primary — Vị trí hiện thời `005` (root view)**
...
- Nav sidebar: **Giám sát nhân viên** · **Giám sát tuyến đường** · **Lịch sử checkin** · **Tổng hợp**
...
- ≠ form CRUD catalog thuần · ≠ GIS editor biên tập geometry (`geditor`) · ≠ dashboard KPI ngày (`004-dashboad`)
...
User menu (`006`) · Xuất Excel dialog (`007`–`009`) · Tải lại variants (`008`/`010`/`011`) · Đổi mật khẩu (`009`) · dashboard tuần kiểm/tuần đường cross-ref — giữ trong `patrol-actions.md` / control-map.

### Map → step context

- Feature: `docs/context/features/patrol.md`
- Section: `## Legacy GOVOne (auto-capture)` → capture id `005-quan-ly-giam-sat`
...
- **Capture path:** `capture/patrol/root/view/` (master/page/action)
- Sibling: `004-quan-ly-giam-sat` (cùng slug/URL) · `006`–`011` (user / export / reload drills)
