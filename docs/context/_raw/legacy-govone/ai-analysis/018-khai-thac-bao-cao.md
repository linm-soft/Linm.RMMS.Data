...
| **screenshot** | `screenshots/018-khai-thac-bao-cao.png` |
| **DOM fields** | 1 |
| **DOM labels** | 0 |
| **DOM buttons** | 98 |
...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `018-khai-thac-bao-cao.png`  
...
> Capture: `capture/reports/root/view/` · twin `007` / `021` (cùng URL/slug root)

### Màn hình / mục đích

Màn **KHAI THÁC BÁO CÁO** — shell **Báo cáo tổng hợp** (`/DuongBo/baocaotonghop/nhatkytuanduong`) trên GOVOne PMDB.  
Mở từ tile / menu **KHAI THÁC BÁO CÁO** · capture path `capture/reports/root/view/` (master/page/action) · pageTitle **_root**.  
Viewport đang ở báo cáo con **Nhật ký tuần đường** (heading content) trong nhóm **BẢO DƯỠNG THƯỜNG XUYÊN**.  
Twin capture gần trùng: `007-khai-thac-bao-cao` · `021-khai-thac-bao-cao` (cùng URL · cùng empty-state trước khi tổng hợp).  
Sibling drill catalog: `008` (Báo cáo tổng hợp), `009`/`019`/`022` (Bảng tổng hợp nhanh), `010` (TNGT), `011`/`022`/`025` (BDTX), `020`/`023` (Tuần kiểm), `021`/`024` (Tuần đường).
...
Ảnh viewport: layout **catalog + report filter** · kết quả **chưa có grid** (empty trắng dưới alert vàng) · Kind shell: **E (AnalyticsReportShell / erp-report-context)** — demo MFE theo `/erp-form-context` Kind E — **không** clone skin GOVOne.  
...
| **Z2 Sidebar nav** | Menu trái tối — module app | **Bảng tổng hợp nhanh** · **Báo cáo tổng hợp** (active) · nhóm **QUẢN LÝ BẢO TRÌ** (Tổng hợp bảo trì · Dự án bảo trì) · **QUẢN LÝ BDTX** (Tuần kiểm · Tuần đường · Số liệu · Tài liệu) · **QUẢN TRỊ ỨNG DỤNG** → **Phân quyền** |
| **Z3 Report catalog (content left)** | Panel chọn nhóm / loại BC | **BẢO DƯỠNG THƯỜNG XUYÊN** (active / expanded — icon wrench) · **TÀI SẢN ĐƯỜNG BỘ** (icon road/asset) · (DOM tree) Nhật ký · Nhật ký tuần đường · Nhật ký tuần kiểm · Nhật ký công việc · Thiên tai… · Hạng mục hư hỏng · Kiểm tra cầu · TNGT · Vi phạm · Đếm xe · Tài liệu · Trực GT · Tổng hợp chung / theo tuyến / hạng mục… |
| **Z4 Report header** | Tiêu đề báo cáo đang mở + back | Mũi tên back · **Nhật ký tuần đường** |
| **Z5 Filter / toolbar** | Hàng lọc trước khi tổng hợp | Date range **01/08/2026 - 31/08/2026** (+ clear ×) · Select placeholder **Chọn tuyến đường** · icon sort · Select **Mới nhất** · kebab ⋮ (more) · primary **Tổng hợp** · (DOM) `dropdown trigger` toolbar + unnamed `input` zone toolbar |
| **Z6 Guidance / empty** | Hướng dẫn khi chưa tổng hợp | Alert vàng: **Hãy chọn các điều kiện lọc dữ liệu, tổng hợp và tải tệp về máy tính.** · vùng kết quả trắng (chưa grid) |
| **Z7 Result / grid** | Kết quả sau Tổng hợp | **Không lộ cột** trên ảnh 018 — empty trước chạy · parity demo sau mock: STT + cột báo cáo check-in/NKTĐ theo Kind E |
...
|---------|---------------|-----------|---------|
| Khoảng thời gian | DateRangePicker | Có (theo alert) | Giá trị ảnh `01/08/2026 - 31/08/2026` · clear × · bodySample |
...
| (unnamed input) | Text / hidden filter | — | DOM `input` zone toolbar — capture không có name/id |
| Nhật ký tuần đường | Report title / breadcrumb | — | Heading content đang active |
| BẢO DƯỠNG THƯỜNG XUYÊN | Catalog group | — | Nhóm trái active |
...
**Grid columns (ảnh 018):** không có — empty state.  
...
**Primary — Báo cáo tổng hợp › Nhật ký tuần đường (`018` root)**
- Mở module **Báo cáo tổng hợp** / tile KHAI THÁC BÁO CÁO → `/baocaotonghop/nhatkytuanduong`
- Catalog trái: chọn **BẢO DƯỠNG THƯỜNG XUYÊN** / **TÀI SẢN ĐƯỜNG BỘ** và báo cáo con (Nhật ký tuần đường active)
...
- ≠ Bảng tổng hợp nhanh (sibling `009`/`019`) · ≠ GIS editor · ≠ dashboard KPI thuần

**Header / shell (ảnh + DOM)**
- Theme **Giao diện sáng** / **Giao diện tối**
- App switcher · avatar / **Ban.TK.Nguyễn Anh Phúc…**
...
- Date range · Chọn tuyến đường · Sort · Mới nhất · dropdown trigger · **Tổng hợp**

**Footer quick nav (DOM)**
- Báo cáo tổng hợp · Phân quyền · Bản đồ · Vấn đề · Giám sát · Hồ sơ

**Actions bổ sung từ sibling (map demo parity)**  
Bảng tổng hợp nhanh popup (`009`/`019`/`022`: CÔNG TÁC TUẦN ĐƯỜNG / TUẦN KIỂM / CÔNG VIỆC · Đóng · Thiết lập cỡ chữ) · catalog drill khác — giữ trong `reports-actions.md` (172). **Không** bịa button không có trên ảnh/DOM.

### Map → step context

- Feature: `docs/context/features/reports.md`
- Section: `## Legacy GOVOne (auto-capture)` → capture id `018-khai-thac-bao-cao`
...
- Sibling: `007`/`021` (twin root) · `008`–`011` · `019`–`025` (catalog / BDTX / tuần kiểm-đường)
