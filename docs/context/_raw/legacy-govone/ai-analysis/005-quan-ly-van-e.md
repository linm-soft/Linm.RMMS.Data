...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `005-quan-ly-van-e.png`

### Màn hình / mục đích

Màn **QUẢN LÝ VẤN ĐỀ** — panel **Vấn đề** (`dbv3baotri.aspx#panelVanDe`) trên app GOVOne gServer 2.1.  
Mở từ tile **QUẢN LÝ VẤN ĐỀ** (app-tile / popup) · inventory id root song song deep `010`/`012` (`capture/incident/root/view/`).  
...
Ảnh capture: shell **list trái + map phải** · list **empty** («Không có bản ghi» / «Không có bản ghi nào để hiển thị») · footer **Trang 0 của 0** + 3 ô đếm màu (xanh / nâu / đỏ = `0`) · toolbar **Thêm · Thêm CV · Xem CV · Xem · Xuất dữ liệu** (+ icon refresh · icon overflow/download) · map vùng Nghệ An (nhãn **Vinh** · **Cửa Lò** · mạng polyline xanh) · scale **20 km / 10 mi**.  
...
Sibling cùng URL/slug: deep root `010`/`012` · user `012-ban-tk`/`013` · create `011-root-them`/`013–017-them*` · Thêm CV drills `014`/`015`.  
...
| **Z8 Map canvas** | Bản đồ mạng tuyến / vị trí VĐ | Polyline đường · nhãn địa danh **Vinh** · **Cửa Lò** · vùng Nghệ An–Thanh Hóa |
...
**Primary — QUẢN LÝ VẤN ĐỀ root view `005` (app-tile)**
...
Deep root `010`/`012` · User `012-ban-tk`/`013` · Create/Thêm `011`/`013`/`014`/`016`/`017` · Thêm CV `014`/`015` · export menu variants trong `incident-actions.md` — giữ parity demo.

### Map → step context

- Feature: `docs/context/features/incident.md`
- Section: `## Legacy GOVOne (auto-capture)` → capture id `005-quan-ly-van-e`
...
- **Capture path:** app-tile / popup · sibling deep `010`/`012` → `capture/incident/root/view/`
- Sibling: `010`/`012` (cùng root) · `011-root-them` · `012-ban-tk` · `013`–`017` create/Thêm CV/tab
