...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `012-quan-ly-van-e.png`

### Màn hình / mục đích

Màn **QUẢN LÝ VẤN ĐỀ** — panel **Vấn đề** (`dbv3baotri.aspx#panelVanDe`) trên app GOVOne gServer 2.1.  
Mở từ tile **QUẢN LÝ VẤN ĐỀ** (app-tile / popup) · capture path `capture/incident/root/view/` (master/page/action · inventory id song song `010-quan-ly-van-e`).  
...
Ảnh capture: shell **list trái + map phải** · list **empty** («Không có bản ghi nào để hiển thị» / bodySample «Không có bản ghi nào») · footer **Trang … của 0** + 3 ô đếm màu (xanh / nâu / đỏ = `0`) · toolbar **Thêm · Thêm CV · Xem CV · Xem · Xuất dữ liệu** (+ icon refresh · icon overflow) · map Nghệ An / Thanh Hóa (nhãn `QL.217` · Quan Sơn · Quan Hóa · Yên Thủy · …) · scale **20 km / 10 mi**.  
...
Sibling cùng URL/slug: root `005`/`010` · user `012-ban-tk`/`013` · create `011-root-them`/`013–017-them*` · Thêm CV drills `014`/`015`.  
...
| **Z5 Grid / list content** | Danh sách vấn đề — empty | Empty **Không có bản ghi nào để hiển thị** · không lộ cột (inventory `tableHeaders: []`) |
...
| **Z8 Map canvas** | Bản đồ mạng tuyến / vị trí VĐ | Polyline đường · nhãn `QL.217` · địa danh Quan Sơn / Quan Hóa / Yên Thủy · vùng Nghệ An–Thanh Hóa |
...
| **Z10 User menu (DOM sibling)** | Dropdown user — không mở trên ảnh | **Hồ sơ của tôi** · **Đổi mật khẩu** · **Đăng xuất** (sibling `012-ban-tk` / drills) |
...
**Primary — QUẢN LÝ VẤN ĐỀ root view `012`**
- Mở từ tile **QUẢN LÝ VẤN ĐỀ** → `#panelVanDe`
- Toolbar: **refresh** · **Thêm** (→ create sibling `011`/`016`/`017`) · **Thêm CV** / **Xem CV** (→ assign/WO · sibling `014`/`015` · disabled khi chưa chọn) · **Xem** (chi tiết · disabled) · **Xuất dữ liệu** (dropdown export NKTĐ / sổ NK…) · overflow → **Xóa** · **Phản hồi** · **BC sở**
...
- User **Ban.TK.…** → menu (sibling `012-ban-tk`)
...
User `012-ban-tk`/`013` · Create/Thêm `011`/`013`/`014`/`016`/`017` · Thêm CV `014`/`015` · export menu variants trong `incident-actions.md` — giữ parity demo.

### Map → step context

- Feature: `docs/context/features/incident.md`
- Section: `## Legacy GOVOne (auto-capture)` → capture id `012-quan-ly-van-e`
...
- Sibling: `005`/`010` (cùng root) · `011-root-them` · `012-ban-tk` · `013`–`017` create/Thêm CV/tab
