...
| **screenshot** | `screenshots/010-quan-ly-giam-sat-tai-lai.png` |
...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `010-quan-ly-giam-sat-tai-lai.png`

### Màn hình / mục đích

Màn **QUẢN LÝ GIÁM SÁT** — panel **Vị trí hiện thời** (`dbv3giamsat.aspx#pnlViTriHienThoi`) sau drill action **Tải lại**.  
Capture path: `capture/patrol/tai-lai/view/` · kind left-rail / deep · pageTitle **Tải lại**.  
Ảnh: shell split list+map **không modal** — list empty «Không có bản ghi nào» · filter **Tất cả** / **Đoạn đường** · footer **Xuất excel** + **Tải lại** (primary drill) · **Tổng số : 0** · map Nghệ An (nhãn `QL.48` · `ĐT.532` · `QL.7` · Quỳ Châu · Anh Sơn · Tân Kỳ) · scale **10 km / 5 mi**.  
...
Upstream: root `005` · sibling reload `008` (cùng title) · Downstream: `011-tai-lai › +` (zoom miscapture) · export `007/008/009-xuat-excel` · user `006`.  
...
| **Z7 Map canvas** | Bản đồ vị trí / mạng tuyến | Polyline đường xanh · nhãn `QL.48` · `ĐT.532` · `QL.7` · địa danh Quỳ Châu / Anh Sơn / Tân Kỳ |
| **Z8 Map chrome** | Điều khiển map | Zoom **+** / **−** · **Bản đồ nền** (mở: **Vệ tinh** · **Google** · **Giao thông** · **Hành chính** · **Không nền**) · scale **10 km / 5 mi** |
...
**Primary — Tải lại `010` (reload drill)**
...
- Ảnh sau reload: list vẫn empty · **Tổng số : 0** · map giữ mạng tuyến · **không** modal / **không** Create
- Remap đúng: **Tải lại** = toolbar **reload/refetch** (không MemoryRouter navigate)
- Downstream: sibling `008` (cùng title) · `011 › +` (zoom) · **Xuất excel** modal
...
Root `005` · user `006` · Xuất Excel `007`–`009` · Tải lại `008`/`011` · Đổi mật khẩu `009` — giữ trong `patrol-actions.md` / control-map.

### Map → step context

- Feature: `docs/context/features/patrol.md`
- Section: `## Legacy GOVOne (auto-capture)` → capture id `010-quan-ly-giam-sat-tai-lai`
...
- **Capture path:** `capture/patrol/tai-lai/view/` (master/page/action)
- Sibling: `008-quan-ly-giam-sat-tai-lai` (cùng title) · `011-quan-ly-giam-sat-tai-lai` (`› +`) · root `005` · export `007`–`009`
