...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `003-dashboad.png`

### Màn hình / mục đích

Màn **DASHBOAD** (home tổng hợp BDTX) trên app GOVOne Đường bộ (`/DuongBo/dashboard`) — entry từ tile app-tile / popup menu **DASHBOAD**.  
...
Sibling `004-dashboad` (cùng URL/slug — capture sâu `capture/dashboard/root/view/`; **003** = vision packet song song từ menu/app-tile) · khác `014` (Bảng tổng hợp nhanh) · khác `016`/`017` (focus một KPI/panel).  
Capture DOM = 0 field (SPA); field/actions bổ sung từ ảnh + inventory DOM buttons.  
...
| **Z3 Panel — CÔNG TÁC TUẦN ĐƯỜNG** | Cột trái: header xanh + minimize · tree đơn vị/tuyến · chip Km | Header **CÔNG TÁC TUẦN ĐƯỜNG** · nút thu gọn `−` · node **Công ty Cổ phần 495** (expand) · nhóm **QL.48C** badge `8` · chips `Km 122 + 393` · `Km 35 + 999` · `Km 47 + 880` · `Km 70 + 000` · `Km 88 + 900` · `Km 91 + 750` · `Km 97 + 250` · `Km 114 + 000` · nhóm **QL.7** badge `20` · chips `Km 83 + 980` · `Km 9 + 113` · `Km 138 + 264` · `Km 182 + 791` · … `Km 206 + 873` |
...
| Tuyến (QL) | Tree group + badge count | — | **QL.48C** (`8`) · **QL.7** (`20`) — panel Tuần đường |
...
- Section: `## Legacy GOVOne (auto-capture)` → capture id `003-dashboad`
...
- **Capture path:** menu/app-tile vision (sibling deep capture `004` → `capture/dashboard/root/view/`)
- Sibling: `004-dashboad` (cùng slug) · `014` (Bảng tổng hợp nhanh) · `016`/`017` (focus panel)
