...
| **screenshot** | `screenshots/016-quan-tri-phan-quyen.png` |
| **DOM fields** | 1 |
| **DOM labels** | 1 |
| **DOM buttons** | 2 |
...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `016-quan-tri-phan-quyen.png`
...
Mở từ tile **QUẢN TRỊ PHÂN QUYỀN** (app-tile / popup) · capture path `capture/users/root/view/` (master/page/action) · pageTitle **_root**.  
Twin capture gần trùng: `006-quan-tri-phan-quyen` · `018-quan-tri-phan-quyen` (cùng URL/slug · user menu đóng · 2 actions).  
...
**Primary — Danh mục đoạn đường `016` (root view)**
...
- Bell thông báo
- **Ban.TK.Nguyễn Anh Phúc** (user menu trigger — đóng trên `016`)
...
User menu mở (`017`/`019`: Hồ sơ của tôi · Đổi mật khẩu · Đăng xuất) · Đổi mật khẩu form (`020`: MK cũ · MK mới · Xác nhận · submit · VỀ TRANG CHỦ) · twin root `006`/`018` — giữ trong `users-actions.md` / control-map. **Không** bịa thêm button không có trên ảnh/DOM.

### Map → step context

- Feature: `docs/context/features/users.md`
- Section: `## Legacy GOVOne (auto-capture)` → capture id `016-quan-tri-phan-quyen`
...
- **Capture path:** `capture/users/root/view/` (master/page/action)
- Sibling: `006`/`018` (cùng slug/URL root) · `017`/`019` (user menu) · `020` (đổi mật khẩu)
