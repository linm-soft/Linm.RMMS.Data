...
| **screenshot** | `screenshots/007-khai-thac-bao-cao.png` |
| **DOM fields** | 1 |
| **DOM labels** | 0 |
| **DOM buttons** | 98 |
...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `007-khai-thac-bao-cao.png`  
...
> Capture: `capture/reports/root/view/` · twin `018` / `021` (cùng URL/slug root)
...
Twin capture gần trùng: `018-khai-thac-bao-cao` · `021-khai-thac-bao-cao` (cùng URL · cùng empty-state trước khi tổng hợp).  
...
Ảnh viewport: layout **sidebar + catalog + report filter** · kết quả **chưa có grid** (empty trắng dưới alert vàng) · Kind shell: **E (AnalyticsReportShell / erp-report-context)** — demo MFE theo `/erp-form-context` Kind E — **không** clone skin GOVOne.  
...
|------|-------|------------------------|
| **Z1 Header / app bar** | Thanh trên: brand · title module · utilities | Logo **govOne** · title **Báo cáo tổng hợp** · app-switcher (grid) · theme sáng/tối (sun) · notification · avatar user (placeholder) · (DOM) govone.vn · youtube · facebook · user menu **Ban.TK.…** / Thông tin của tôi / Đổi MK / Đăng xuất |
...
| **Z5 Filter / toolbar** | Hàng lọc trước khi tổng hợp | Date range **01/08/2026 - 31/08/2026** (+ clear × + calendar) · Select placeholder **Chọn tuyến đường** · icon sort ↑↓ · Select **Mới nhất** · kebab ⋮ (more) · primary **Tổng hợp** · (DOM) `dropdown trigger` toolbar + unnamed `input` zone toolbar |
...
| **Z7 Result / grid** | Kết quả sau Tổng hợp | **Không lộ cột** trên ảnh 007 — empty trước chạy · parity demo sau mock: STT + cột báo cáo check-in/NKTĐ theo Kind E |
...
**Grid columns (ảnh 007):** không có — empty state.  
...
**Primary — Báo cáo tổng hợp › Nhật ký tuần đường (`007` root)**
...
- Section: `## Legacy GOVOne (auto-capture)` → capture id `007-khai-thac-bao-cao`
...
- Sibling: `018`/`021` (twin root) · `008`–`011` · `019`–`025` (catalog / BDTX / tuần kiểm-đường)
