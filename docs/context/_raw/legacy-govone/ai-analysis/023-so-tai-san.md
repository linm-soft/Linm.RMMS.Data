...
> Vision reviewed: 2026-08-02 · ai-autocode autopilot · screenshot `023-so-tai-san.png`
...
Ảnh capture: map Việt Nam đã load; sidebar tree «Công ty Cổ phần 495» → `QL.48C` · `QL.7` (mỗi node có icon mắt); lưới dưới **trống** (chưa có bản ghi · pager `0 / 0`).  
...
Sibling cùng shell: `008-so-tai-san` · `012-so-tai-san` · drill `024`/`025` (thông báo / tiện ích).
...
| **Z2 Nav rail** | Cột icon hẹp bên trái | Home/monitor · **Map** (active, xanh) · History · Settings |
| **Z3 Sidebar filter** | Panel tuyến / lớp dữ liệu | Tabs **Tuyến đường** (active) · **Lớp dữ liệu** · search placeholder **Nhập và Enter để lọc dữ liệu** (`textfield-1033`) · tree: **Công ty Cổ phần 495** (− expand) · **QL.48C** · **QL.7** (mỗi node icon mắt hiện/ẩn lớp) |
| **Z4 Filter toolbar** | Thanh lọc lý trình trên map | **+** (thêm / expand filter — góc trái) · **Lý trình từ:** (`textfield-1079`, placeholder `vd: km1+100`) · **Lý trình đến:** (`textfield-1080`, `vd: km5+100`) · nút xanh **Lấy dữ liệu** (kính lúp) · DOM thêm **Xóa điều kiện** · **Tiện ích** (có thể menu/ẩn trên ảnh) |
| **Z5 Map canvas** | Bản đồ trung tâm | Basemap VN / ĐNA · controls góc phải: call · layer-stack · basemap thumb · **+** · **−** · fullscreen · measure · DOM: **Vị trí của tôi** · **Lớp nền** · **Lớp chuyên đề** · **⇧** |
| **Z6 Result grid** | Panel dưới map — kết quả tài sản | Toolbar: kính lúp (xem/lọc) · edit (bút) · **delete** (thùng rác đỏ) · Excel export · refresh · pager `<<` `<` `0` `/ 0` `>` `>>` · header bar xám · body trống (chưa cột label trên ảnh) · `inputItem` = ô page hiện tại |
...
**Primary — Sổ tài sản (focus vision `023`)**
...
- **+** filter/sidebar — thêm mục (tuyến/lớp — theo ngữ cảnh tab)
...
- Tìm/lọc trên lưới (kính lúp) · Sửa · Xóa bản ghi · Xuất Excel · Làm mới
...
- **Remap note:** control-map map `+`/`−` → zoom GIS; sidebar/filter `+` = thêm · map `+`/`−` = zoom; badge `24` = thông báo (nav), không phải business field
