...
| versionName / versionCode | `1.0.0` / `2` | AAB (1) đã upload · Play đòi API **≥ 36** |
| compileSdk / targetSdk | **36** | AGP **8.9.2** (Gradle wrapper 8.11.1) |
...
| 2 | Chờ Gradle xong (lần đầu AGP/SDK 36 có thể hỏi upgrade — chấp nhận **8.9.2**) |
...
Kiểm tra AAB trước upload: `applicationId` `com.drvn.rmms.store` · `versionCode` **3** · `targetSdk` **36** · Release `BFF_BASE` HTTPS prod · `STORE_W1=false`.

---

## 6. Capture + pack theo version (`/store-image-capture`)

Folder: `Linm.RMMS.Data/docs/plan/release-mobile/store/out/android/1.0.0-3/` (`{VersionFolder}` = `{versionName}-{versionCode}`). Phone listing 01–04 vẫn `1.0.0-2/` (UI không đổi). **Cấm** copy icon/feature vào folder mới trước `released_success`.

AskQuestion **trước** copy AAB: `local_build_confirm` · `capture_account` · `move_aab`.

| Phải | Cấm |
|------|-----|
| Run / cài **`w3Release`** cùng `1.0.0 (3)` | `w3Debug` (map ON) |
...
1. Kéo AAB `store/out/android/1.0.0-3/rmms-w3-release-1.0.0-3.aab` vào release (Internal / Closed / Production). **Cấm** upload lại `1.0.0-2` (versionCode 2 đã khóa).  
...
| Pack | `store/out/android/1.0.0-3/` |
...
1. `versionCode` **+1** mỗi lần **upload Play** (đã dùng 1 và 2 → hiện **3**; lần sau **4**). `versionName` chỉ đổi khi marketing mới. Local assemble/capture **không** bắt buộc tăng.  
