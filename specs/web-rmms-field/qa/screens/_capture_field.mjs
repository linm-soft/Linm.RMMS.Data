...
async function fatalUi(p) {
  ...
  if (await overlay.count()) {
    ...
    const featureOk = await p
      .locator(
        "[data-feature='web-rmms-field'], #doorPatrol, #guestLogin, #loginUser",
      )
      .count();
...
async function dumpZones() {
  return page.evaluate(() => {
    const root = document.querySelector(
      '[data-feature="web-rmms-field"], [data-feature="web-rmms-home"], [data-feature="web-rmms-shell"]',
    );
    ...
    const guestGate = /Đăng nhập để dùng Field/i.test(text);
    return {
...
try {
  // S0 — guest Field hub gate
  pageErrors.length = 0;
  await clearSession();
  await page.goto(base + "/web-rmms-field", {
    waitUntil: "domcontentloaded",
    timeout: 60000,
  });
  await captureCurrent(
    "S0",
    "[data-feature='web-rmms-field']",
  );
  ...
  // S1 — staff Field hub after LoginSheet
  pageErrors.length = 0;
  await loginViaSheet();
  await page.goto(base + "/web-rmms-field", {
    waitUntil: "domcontentloaded",
  ...
  await captureCurrent(
    "S1",
    "#doorPatrol, #doorInspect, [data-feature='web-rmms-field'] [data-zone='FL-01']",
  );
} finally {
  await browser.close();
}
...
writeFileSync(join(outDir, "_capture_field.result.json"), JSON.stringify(summary, null, 2));
