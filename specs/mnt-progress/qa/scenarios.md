...
| e2e result | **ok:true** · `2026-09-01T05:07:41.912Z` |
| updatedAt | `2026-09-01T05:10:00.000Z` |

**Scope:** slug `mnt-progress` · mnt-list `#i-sync` / `btn-mnt-sync-{liveId}` → `#sc-mnt-progress` · POST progress (+ complete @100). Live WO `11111111-1111-1111-1111-111111111101` («Vá mặt đường»). **Cấm** AC sibling chat/estimate/log. **Cấm** mfeStdUrl / start:std.

## VERIFY GATE

| Gate | Result |
|------|--------|
| iOS `xcodegen` + build dest **iPhone 17 Pro Max** | **PASS** |
| Android `./gradlew :app:assembleDebug` | **PASS** |
| Mobile.Bff `dotnet build` | **PASS** (0 warning · 0 error) |
| API docker + Mobile.Bff `:5202` | **PASS** (`--skip-start` reuse) |
| Maestro iOS + Android | **PASS** · `#sc-mnt-progress` · live IDs |
...
| 4 | Entry list → progress | tab-work → `#sc-mnt-list` → sync live WO → `#sc-mnt-progress` | **PASS** | Maestro · A3 / P6 |
| 5 | TopBar | title **Cập nhật trạng thái** · iOS back **Công việc** · Android icon-only | **PASS** | A3 / P6 |
| 6 | WO header | title · code · status readonly (live DB) | **PASS** | A3 / P6 |
...
| 15 | Live-only entry | **cấm** `demo-wo-1` · sync `btn-mnt-sync-11111111-…101` | **PASS** | post cleanup_mock |
