...
await p.waitForSelector('[data-testid="rmms-csdl-bieu-10-list-page"]', {
  timeout: 45000,
});
...
const hasExportBtn =
  (await p
    .locator('[data-testid="rmms-csdl-bieu-10-list-export-excel-btn"]')
    .count()) > 0;
const hasImportBtn =
  (await p
    .locator('[data-testid="rmms-csdl-bieu-10-list-import-excel-btn"]')
    .count()) > 0;
const peerSots =
  (await p.locator('[data-testid="rmms-csdl-bieu-10-list-peer-sots"]').count()) >
  0;

/** filter bar must NOT host Xuất (GAP-FILTER-BAR-08) */
const filterBarExport = await p.evaluate(() => {
  const filter = document.querySelector(
    '[data-testid="rmms-csdl-bieu-10-list-filters"]',
  );
...
try {
  ...
  const okName = /^Bieu10_KeTuongChan_\d{8}\.xls$/i.test(fileName);
...
const assert = {
  url: "http://localhost:9301/csdl-bieu-10",
  changeScope: "edit_page",
  hasTitle: /Biểu\s*10|Kè|tường chắn/i.test(body),
  hasFilter: body.includes("Tìm") || /Tỉnh|Loại tường|wallKind/i.test(body),
...
writeFileSync(
  "D:/AI-QLBD/Linm.RMMS.Data/specs/csdl-bieu-10/qa/screens/live-assert.json",
