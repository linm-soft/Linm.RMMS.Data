/**
 * Live DOM assert + DTM filter-bar overflow for so-ts-rest-area.
 */
...
const { chromium } = pw;
const outDir =
  "D:\\AI-QLBD\\Linm.RMMS.Data\\specs\\so-ts-rest-area\\qa\\screens";
const listUrl = "http://localhost:9301/so-ts?type=REST_AREA";
const formUrl = "http://localhost:9301/so-ts/tao-moi?type=REST_AREA";
const TEST_ID = "rmms-so-ts-rest-area-list";
...
const list = await page.evaluate((tid) => {
  ...
  return {
    titleRest: /Danh sách trạm dừng nghỉ/i.test(body),
    headerRest: /Sổ TS — Trạm dừng nghỉ/i.test(body),
...
await page.waitForSelector('[data-testid="asset-rest-area-attr"]', {
  timeout: 15000,
});
await new Promise((r) => setTimeout(r, 1200));
...
const form = await page.evaluate(() => {
  ...
  return {
    cols5: cols,
    restAttr: !!document.querySelector('[data-testid="asset-rest-area-attr"]'),
