# SA — Solution discovery — so-ts-rest-area (Sổ TS — Trạm dừng nghỉ)

> Status: **confirmed** (`solution_confirm=approve` · autoApprove=ON · `task_4d1221b7`)  
...
| solution_confirm | **approve** (autoApprove=ON · `task_4d1221b7`) |
| domain_map | **Asset** (`so-ts-rest-area` → inherit parent `asset` · prefix `api/v1/asset`) |
| sa_tz_gate | **`tz_na`** |
...
| domain | **Asset** |
| controlHint | `specs/_data-analy/features/so-ts-rest-area-control-hint.md` |
| realData | `specs/_data-analy/features/so-ts-rest-area-real-data.md` |
| design | `specs/so-ts-rest-area/ui/design.md` (confirmed) |
...
| priorTask | `task_a11a8936` (design completed) |
...
| DOMAIN-MAP | slug `asset` → Asset · `so-ts-rest-area` chưa liệt kê | Cite **Asset** · docs delta optional row `so-ts-rest-area`→Asset | cite + optional docs |
| Entity | `RoadAssetEntity` · `rmms_road_assets` · `DumpSpecs` text | **giữ** dumpSpecs P1 cho toàn bộ attr REST_AREA §4 · mirror `name_work` | **no Schema_*** flatten |
| Import name | rebuild `FindOfficialName` gồm `name_work` · sample «Trạm dừng chân Mê Kông - Hải Vân» | `name` ← `name_work` · trống OK · **cấm** IsWeak → đoạn tuyến — **GAP-RA-NAME-01** | guard rebuild + form |
| dumpSpecs attrs | FE `dumpSpecLabels` partial (thiếu categorized/service/parking/svc/boolean keys) | Label VN đủ header dump · form Input/Select merge keys — **GAP-RA-SPEC-01** | FE labels + form write |
| LOOKUP type_work / categorized / owner / grade / location | text trong dumpSpecs | Dropdown LOOKUP_STATIC · init-data delta arrays (dump distinct / seed) — **GAP-RA-LOOKUP-01** | delta init-data |
...
| Grid profile | 1 schema (+ profile peer types) | Hide `type`/`kmTo`/qty/unit/parking default · show REST_AREA attrs · hide-empty ON: chiều dài/DT/cứu hộ/cấp cứu | FE type-profile |
| Point `kmTo` | form hiện / bắt buộc với type ≠ KM_POST | **Ẩn** + không required khi `type=REST_AREA` · `kmFrom` **không** required · S-LOC-POINT only | FE validation |
| REST vs PARKING | cùng dump `tbl_rest_stops` | Filter `?type=REST_AREA` · import `RefineImportedType` tách — **GAP-RA-SPLIT-01** | list + import guard |
...
**Không đổi:** Kind B shell `/so-ts` · BFF proxy · CRUD methods · SearchInput asset-type / road-route / org-unit · catalogKind UI `road-assets` · **cấm ERP.*** · map canvas OUT · parking_lot **OFF** default (defer peer `so-ts-parking`).
...
| Import | `RoadAssetCatalogHandler` · catalogKey `road_assets` · type seed `REST_AREA` · `RefineImportedType` tách PARKING |
| Rebuild CSV | `Linm.RMMS.Data/local-script/RebuildGovVn.cs` · `FindOfficialName` incl. `name_work` · Prefix `DN-` · `IsWeakAssetName` |
| GIS | `GisInventoryMapper.cs` · `tram-dung-nghi` ↔ `REST_AREA` · IdCode live `TDN` / import `DN-` |
...
| Docs | `docs/context/features/so-ts-rest-area.md` · parent `so-ts-type-grid.md` · `import-gov-asset-fields.md` |
...
| Surface | Path |
|---------|------|
| UI list | `/so-ts?type=REST_AREA` · alias board `/so-ts-rest-area` (optional redirect) |
...
|------|----------|----------------------|-------|------|
| TZ | **`tz_na`** | Design Zone B **không** `fromDate`/`toDate` · form **không** business date (chỉ `updatedAt` readonly) | `/review-timezone-implement` | Parent API vẫn có optional `fromDate`/`toDate` + `CatalogUpdatedAtRange` — **không** mount trên pack REST_AREA |
...
AskQuestion (autoApprove=ON · agent confirm): `sa_tz_gate=tz_na` · `sa_xco_gate=xco_get_only` · `sa_shared_table=share_tenant` · `2026-09-01T04:30:00.000Z`
...
|---------|---------|----------|-----|
| S-LIST | Kind B A–D+F+H | list | API-01 list `?type=REST_AREA` |
| S-FORM-CREATE | Kind B full-page 5 cột | create | API-03 POST (`type=REST_AREA`) |
...
| S-ALIAS | navigate optional | — | `/so-ts-rest-area` → `/so-ts?type=REST_AREA` |
...
| type_work_id | Dropdown LOOKUP_STATIC | form S-ATTR · grid label | API-06 init-data `restAreaWorkTypes` |
| categorized_id | Dropdown LOOKUP_STATIC | form S-ATTR · grid | API-06 init-data `restAreaCategories` |
| owner_id | Dropdown LOOKUP_STATIC | form S-ATTR · grid | API-06 init-data `restAreaOwners` |
| office_building_grade_id | Dropdown LOOKUP_STATIC | form S-ATTR | API-06 init-data `officeBuildingGrades` |
| auxiliary_works_grade_id | Dropdown LOOKUP_STATIC | form S-ATTR | API-06 init-data `auxiliaryWorksGrades` |
...
|----------|------|---------------|-------|
| list | GET list `?type=REST_AREA` + filters | — | page=1 on filter change · profile cột REST_AREA · **cấm** lẫn PARKING |
| create | empty · type lock `REST_AREA` · init-data | POST body + dumpSpecs merge | IdCode BE prefix `DN-` · **không** required `kmTo`/`kmFrom` |
| edit | GET `/{id}` | PUT `/{id}` · merge dumpSpecs | leave-confirm dirty |
| view | GET `/{id}` | — | readOnly · **không** disabled xám |
| copy | GET `/{id}` → clear id/code | POST | keep `REST_AREA` · new IdCode |
...
|-----------|------------|--------|
| `type` | SearchInput asset-type (prefill/ẩn) | **required** `REST_AREA` deep-link |
...
| type | SearchInput `asset-type` | `?type=` / body `type` · lock `REST_AREA` |
...
| kmTo | Text filter only | `?kmTo=` filter · **ẩn** form/grid REST_AREA |
| orgUnit | SearchInput tree | `?orgUnit=` |
| code | Text readonly | BE IdCode · prefix `DN-` |
...
| type_work_id | Dropdown LOOKUP_STATIC | **dumpSpecs** key · options init-data `restAreaWorkTypes` · **required** form |
| categorized_id | Dropdown LOOKUP_STATIC | **dumpSpecs** key · options init-data `restAreaCategories` |
| owner_id | Dropdown LOOKUP_STATIC | **dumpSpecs** key · options init-data `restAreaOwners` |
| actual_length | Number | **dumpSpecs** key · grid ON hide-empty |
| site_area_using_land | Number | **dumpSpecs** key · grid ON hide-empty |
| office_building_grade_id | Dropdown LOOKUP_STATIC | **dumpSpecs** key · options init-data `officeBuildingGrades` |
| total_area_floors | Number | **dumpSpecs** key · form S-ATTR |
| service_area | Number | **dumpSpecs** key · form S-ATTR |
| total_area_office_building | Number | **dumpSpecs** key · form S-ATTR |
| auxiliary_works_grade_id | Dropdown LOOKUP_STATIC | **dumpSpecs** key · options init-data `auxiliaryWorksGrades` |
| total_area_auxiliary_works | Number | **dumpSpecs** key · form S-ATTR |
| parking_lot / total_parking_lot | Select boolean / Number | **dumpSpecs** · **OFF** default REST_AREA · defer peer PARKING |
...
| quantity / unitCode | — | **ẩn** REST_AREA · giữ DTO parent |
...
| parking_lot | via DumpSpecs | `dumpSpecs.parking_lot` · optional |
| total_parking_lot | via DumpSpecs | `dumpSpecs.total_parking_lot` · optional |
...
| Flatten REST_AREA attrs | **DEFER P2** — **không** Schema_* this turn (Design/PO OUT · dumpSpecs P1 · **GAP-RA-FLAT-01**) |
| dumpSpecs write | Form S-ATTR (+ name_work mirror) merge keys vào JSON string trên POST/PUT · list grid parse cùng keys |
| New Schema_* | **none** |
| Seed / init | delta `restAreaWorkTypes[]` · `restAreaCategories[]` · `restAreaOwners[]` · `officeBuildingGrades[]` · `auxiliaryWorksGrades[]` · `buildLocations[]` trên init-data · UI schema `road-assets` type-profile hide-empty |
| data-import | `RoadAssetCatalogHandler` · rebuild CSV **name**=`name_work` · giữ toàn bộ attr REST_AREA trong dumpSpecs · `RefineImportedType` tách PARKING · km trống khi null |
| Default đơn vị | II.1/QL.1 khi reopen import (parent) |

### Import IsWeak / name (GAP-RA-NAME-01)

| Cite | Decision |
|------|----------|
| `RebuildGovVn.cs` `FindOfficialName` | gồm `name_work` — **MUST** ưu tiên khi non-junk cho REST_AREA |
| `ResolveTypeAssetName` / `ResolveAssetName` | Dev verify CSV `name` = `name_work` (cite sample `DN-rest_stops_761412` → «Trạm dừng chân Mê Kông - Hải Vân») |
| `IsWeakAssetName` | chỉ QL./CT. — **cấm** classify tên trạm weak · **cấm** fallback `name_of_route_asset` khi `name_work` hợp lệ (trống OK — **không** ép đoạn) |
| `RoadAssetCatalogHandler` | đọc CSV `name` đã đúng · dumpSpecs giữ `name_work` + attrs §4 · **cấm** gộp PARKING |
| Form | S-NAME bind `name` ← `name_work` · sync mirror dumpSpecs |

### REST vs PARKING split (GAP-RA-SPLIT-01)

| Cite | Decision |
|------|----------|
| List filter | **MUST** `?type=REST_AREA` · deep-link tile `t26` · **cấm** hiện row `PARKING` |
| Import | `RefineImportedType` trong `RoadAssetCatalogHandler` tách theo tên/dump · peer `so-ts-parking` riêng pack |
| Grid/form | parking_lot/total_parking_lot **OFF** default REST_AREA · peer profile riêng |
...
| | |
|--|--|
| Purpose | Paged list · filter type REST_AREA |
...
| Context | `docs/context/features/so-ts-rest-area.md` |
...
BFF: `GET web-bff/api/v1/asset/road-assets?type=REST_AREA&…`
...
| | |
|--|--|
| Purpose | Create · type `REST_AREA` · IdCode BE `DN-` |
| Permission | `asset.road-assets.create` |
| Tenant | current company |
| Request | `CreateRoadAssetRequest` · `DumpSpecs` JSON string incl. name_work · REST_AREA attrs §4 |
| Response | `RoadAssetDto` |
...
| | |
|--|--|
| Purpose | LOOKUP_STATIC statuses · sources · units · materials (peer) · **restAreaWorkTypes** · **restAreaCategories** · **restAreaOwners** · **officeBuildingGrades** · **auxiliaryWorksGrades** · **buildLocations** (delta) |
| Permission | read |
| Request | — |
| Response | `RoadAssetInitDataDto` + delta arrays `RoadAssetLookupOption[]` |
| Form surfaces | S-META · S-ATTR · S-LOC |
| Decision | **GAP-RA-LOOKUP-01** P1 — options = dump distinct / seed labels (value=label dump string · cite dump `tbl_rest_stops`) · **cấm** invent master SearchInput · **cấm** hardcode FE không cite dump |
...
|-------------|-----|-------------|
| asset-type | LKP | SearchInput · lock `REST_AREA` |
| road-route | LKP | SearchInput |
| org-unit | LKP | SearchInput tree |
| LOOKUP_STATIC status/source/units | API-06 | keep init-data |
| LOOKUP_STATIC type_work | API-06 `restAreaWorkTypes` | **delta** · dump distinct |
| LOOKUP_STATIC categorized | API-06 `restAreaCategories` | **delta** · dump distinct |
| LOOKUP_STATIC owner | API-06 `restAreaOwners` | **delta** · dump distinct |
...
| LOOKUP_STATIC boolean svc | True/False | dump / init cite |
...
|----|-------------|-------|
| GAP-SOTS-COL-01 | Type-profile REST_AREA hide type/kmTo/qty/unit/parking default · show dump attrs · hide-empty ON: chiều dài/DT/cứu hộ/cấp cứu | Dev UI |
...
| GAP-RA-NAME-01 | `name`←`name_work` · trống OK · guard IsWeak + rebuild | Dev BE import + rebuild + form |
| GAP-RA-SPEC-01 | Label VN đủ key REST_AREA · giữ attrs trong dumpSpecs · km trống khi null | Dev FE labels + form + import |
| GAP-RA-POINT-01 | Ẩn + không required `kmTo` · `kmFrom` không required · không ép `"0"` · không mount S-LOC-RANGE | Dev UI |
| GAP-RA-LOOKUP-01 | Dropdown LOOKUP_STATIC · init-data delta arrays · dumpSpecs write | Dev BE init + UI |
| GAP-RA-SPLIT-01 | Filter/import tách REST_AREA vs PARKING · **cấm** gộp list | Dev BE import + UI filter |
| GAP-RA-LEAVE-01 | LeaveConfirmModal + useAlert/Modal | Dev UI |
| GAP-RA-ROUTE-01 | Live URL `?type=REST_AREA` · alias board optional redirect | Dev UI optional |
| GAP-SOTS-API-DOC | Cite `api/v1/asset/road-assets` only · DOMAIN-MAP Asset | docs |
| GAP-SOTS-TAB-01 | **Cấm** tab legacy DRVN | Dev UI |
| GAP-RA-FLAT-01 | Flatten DB columns **DEFER P2** | — |
| GAP-RA-AUTH-01 | RequirePermission attr TODO align Auth codes | BE later |
...
`solution_confirm` = **approve** — autoApprove **ON** · agent tự confirm gates TZ/XCO/SHARE (`task_4d1221b7`).  
...
| Field | Value |
|-------|-------|
| Kind / pattern | B A–D+F+H + Full page `data-form-cols="5"` · type `REST_AREA` · cluster `station` · tile `t26` |
| FormMode↔API | § FormType pack |
| Filter keys | type · search · route · kmFrom · kmTo · orgUnit · page · pageSize |
| API list | API-01…07 · LKP Integration · init restAreaWorkTypes + restAreaCategories + restAreaOwners + officeBuildingGrades + auxiliaryWorksGrades + buildLocations delta |
| Entity | `RoadAssetEntity` flat + DumpSpecs · **cấm** parent line JSON · **no** flatten P1 |
| Gates | tz_na · xco_get_only · share_tenant |
| Delta Dev | GAP-SOTS-* · GAP-RA-* (name/spec/point/lookup/leave/route/split) · init LOOKUP |
...
---
<!-- Version meta: skillId=agent-sa skillVersion=2026.08.24.01 schemaVersion=1 workflowVersion=2026.08.30.01 rulesVersion=2026.08.31.2 versionGate=rechecked taskId=task_4d1221b7 contentHashPriorDataAnaly=sha256:b92dcb2dca6f96078721289e687ddbcaa3339828968fe1d11ce202c4e8569bc -->
