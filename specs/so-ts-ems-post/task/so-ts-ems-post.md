# Team lead — tasks — so-ts-ems-post
...
| formType | `list` |
| typeCode | `EMS_POST` |
| cluster | `station` · tile `t29` |
| dump | `tbl_first_aid_station` |
| prefix | `CCU-` |
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-EMS-NAME/SPEC/POINT/LOOKUP/LEAVE/ROUTE/TILE · flatten DEFER P2 |
| solution_confirm | **approve** (`task_fa58e8b4`) |
| design_confirm | **approve** (`task_1d2490b1`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=EMS_POST`** · alias board `/so-ts-ems-post` optional redirect · alt B primary-alias-only rejected |
| productRoot | `D:/AI-QLBD/Linm.RMMS.Data` |
| reviewUrl | `file:///D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-ems-post/ui/prototype/so-ts-ems-post-list-prototype.html` |
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:07fe22b464638b45f6be1286d9b99d3a7551dd5ef62be076013bbedd692885c8` |
...
| saTaskId | `task_fa58e8b4` |
| updatedAt | `2026-09-01T05:28:00.000Z` |
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=EMS_POST` | **giữ** · alias `/so-ts-ems-post` optional | **route_confirm=route_a** · **GAP-EMS-ROUTE-01** |
| Grid profile | generic / thiếu EMS_POST | Hide `type`/`kmTo`/qty/unit/DT/cấp · **ON mẫu**: tên trạm · tuyến · lý trình · chủ SH · loại trạm · khoảng cách | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
| Form S-ATTR | `<dl>` readonly dumpSpecs | Editable Dropdown/Number đủ dump §4 · merge dumpSpecs | **T-UI-FORM-01** · **GAP-SOTS-FORM-01** |
| Point `kmTo` | form hiện / bắt buộc | **Ẩn** + không required khi `EMS_POST` · kmFrom **không** required · **cấm** ép `"0"` | **T-UI-FORM-01** · **GAP-EMS-POINT-01** |
| Name | IsWeak / đoạn tuyến risk | `name` ← `name_station` · label «Tên trạm» · trống OK · **cấm** IsWeak→đoạn | **T-BE-CRUD-01** + form · **GAP-EMS-NAME-01** |
| LOOKUP | text dumpSpecs | LOOKUP_STATIC init `ownerOptions[]` · `stationTypeOptions[]` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-EMS-LOOKUP-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** · **GAP-EMS-LEAVE-01** |
| Filter context | thiếu file | `so-ts-ems-post-filter-bar.md` | **T-UI-FILTER-01** · **T-CTX-01** |
...
| dumpSpecLabels | thiếu attr keys | đủ dump §4: name_station · owner_id · station_type_id · distance_nearest_major_road | **T-UI-FORM-01** · **GAP-EMS-SPEC-01** |
| KCHT tile | generic | tile `t29` drill OK · count import **240** | **GAP-EMS-TILE-01** |
...
| `source.routes` | **`/so-ts?type=EMS_POST`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=EMS_POST` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=EMS_POST` (Dev điền/verify) · alias `http://localhost:9301/so-ts-ems-post` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=EMS_POST` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `EMS_POST`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-ems-post-filter-bar.md` (create T-CTX-01) |
...
|--------|------|----------|
| A | `/so-ts?type=EMS_POST` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-ems-post` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Trạm trực cấp cứu» khi `type=EMS_POST` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-ems-post-filter-bar.md` |
| DES-GRID-C0 | listTitle «Danh sách trạm trực cấp cứu» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · EMS_POST column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=EMS_POST` + filters |
...
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · **`ownerOptions[]`** · **`stationTypeOptions[]`** delta |
| API-07 | GET | `/api/v1/asset/road-assets/summary-by-type` | peer tile t29 · count **240** |
...
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent parent line JSON / EmsPostJson |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-EMS-NAME/SPEC** import + rebuild |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-EMS-LOOKUP-01** ownerOptions + stationTypeOptions |
...
| T-UI-LEAVE-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | LeaveConfirmModal · **GAP-EMS-LEAVE-01** |
...
**new_page.ssot_rereview:** pending → pass|fail
  ...
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-EMS-NAME/SPEC/POINT/LOOKUP/LEAVE
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-ems-post.md` + parent `so-ts-type-grid.md` · tạo `so-ts-ems-post-filter-bar.md` · lane web → `dev` · **cấm** re-scan demo
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B A–D+F+H · `shared_grid_example=v1` · type profile `EMS_POST`  
...
**ssot.reuse:**
  ...
  ui_filter: LinErpListFilterBar · **T-UI-FILTER-01** · `so-ts-ems-post-filter-bar.md` — cấm ErpListHeaderFilters / LinListFilterField / nút Tìm
  ui_toolbar: catalogToolbar FULL (refresh·history·editConfig·view·edit·delete·create) · fa-cog
  ui_grid: LinCatalogDataGrid · kéo cột default ON · **EMS_POST profile** hide type/kmTo/qty/unit · show name · route · kmFrom · owner_id · station_type_id · distance_nearest_major_road (**GAP-SOTS-COL-01**)
...
**implement.page_shell:**
  ...
  header: title «Sổ TS — Trạm trực cấp cứu» khi type=EMS_POST · titleIconClass + pageId
  filterBarLayout: erp-filter-bar · **cấm** filterMaxWidthPx hack
  listTitle: «Danh sách trạm trực cấp cứu»
...
**implement.toolbar:** (tl-grid-full-flow §1)
  ...
  form_pair: onAdd→`/so-ts/tao-moi` · onEdit/onView→`/so-ts/sua?id=` · type lock EMS_POST
  cấm: thiếu Create/Refresh/Config · History invent API · icon lệch erp-control-icon-map

**implement.grid:**
  component: LinCatalogDataGrid
  columns: buildDynamicGridColumns(schema) + type-profile EMS_POST
  visible ON: name · route · routeNamed · routeSegment · kmFrom · owner_id · station_type_id · distance_nearest_major_road · (status/gps optional)
  visible OFF default: type · kmTo · quantity · unitCode
  dumpSpecs: parse tbl_first_aid_station §4 keys · labels via dumpSpecLabels (**GAP-EMS-SPEC-01**)
...
**implement.wire:**
  ...
  alias: optional `/so-ts-ems-post` → `/so-ts?type=EMS_POST` (**GAP-EMS-ROUTE-01**)

**implement.test:**
  listFilters · type lock EMS_POST · pageSize · selection → toolbar · column profile EMS_POST

**DoD:**
- [ ] Kind B 1 shell · grid visible khi API có data · profile EMS_POST (**GAP-SOTS-COL-01**)
...
**skills (REQUIRED load trước Write):**
  ...
  - context: `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-ems-post-filter-bar.md`
...
**implement.filter:**
  ...
  type lock: luôn `EMS_POST` trên deep-link
...
**ssot.reuse:**
  ...
  controls: SearchInput type/route · Dropdown status/source/ownerOptions/stationTypeOptions · Number distance_nearest_major_road · Text name_station («Tên trạm»)
  http: API-02/03/04 · IdCode readonly prefix `CCU-` · init-data API-06

**implement.form:**
  modes: create · edit · view(readOnly **không** disabled xám) · copy(clear id/code → POST · keep EMS_POST)
  sections: S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR · S-GPS
  required: type · status · route — name trống OK · kmFrom **không** required · **không** required `kmTo` (**GAP-EMS-POINT-01**)
  S-ATTR: owner_id Dropdown · station_type_id Dropdown · distance_nearest_major_road Number · merge dumpSpecs (**GAP-SOTS-FORM-01** · **GAP-EMS-LOOKUP-01** · **GAP-EMS-SPEC-01**)
  S-NAME: `name` ← `name_station` · label «Tên trạm» · trống OK · mirror dumpSpecs (**GAP-EMS-NAME-01**)
  kmFrom: **cấm** ép `"0"` khi dump null (**GAP-EMS-SPEC-01** · **GAP-EMS-POINT-01**)
...
**implement.init_data:**
  options: GET `/asset/road-assets/init-data` only — statuses · sources · **ownerOptions** · **stationTypeOptions** · **cấm** KIND_LABEL hardcode (**GAP-DEV-DROPDOWN-HARDCODE-01**)
...
- [ ] kmTo ẩn · EMS LOOKUP Dropdown từ init-data
- [ ] reuse AssetFormPage S-* — **cấm** fork

**Fail:** GAP-SOTS-FORM-01 · GAP-SOTS-REUSE-01 · GAP-EMS-POINT-01 · GAP-EMS-LOOKUP-01 · GAP-P2-FORM-GRID-05 · GAP-LIST-FIELD-01
...
**DoD:** dirty → `LeaveConfirmModal` · full `useFormLeaveGuard` · **cấm** `window.confirm`/`alert` · **GAP-EMS-LEAVE-01** / **GAP-P2-36** / **GAP-DEV-LEAVE-01** / **GAP-TL-LEAVE-01**
...
| +Tạo mới | toolbar | navigate `/so-ts/tao-moi` type=EMS_POST | POST API-03 |
...
| Alias board | `/so-ts-ems-post` | optional redirect → `?type=EMS_POST` | — |
...
**status:** pending / verify  
**devSlash:** `/agent-dev`  
**DoD:** filter+form type/route/org = SearchInput Integration · owner/stationType = LOOKUP_STATIC init-data (**không** SearchInput master P1) · **GAP-LIST-LKP-01** / **GAP-EMS-LOOKUP-01**
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** controlHint Design/DA 1:1 · SA field map · dumpSpecs keys `name_station` · `owner_id` · `station_type_id` · `distance_nearest_major_road` · **GAP-LIST-FIELD-01** · **GAP-EMS-SPEC-01**
...
**ssot.reuse:**
  ...
  import: RoadAssetCatalogHandler · RebuildGovVn `FindOfficialName incl. name_station` / `IsWeakAssetName`
  gates: tz_na · xco_get_only · share_tenant

**implement.wire:**
  API-01…05: verify live · **cấm** invent `api/v1/so-ts/*` · ERP.*
  GAP-EMS-NAME-01: rebuild CSV `name`←`name_station` · **cấm** IsWeak reject «Tên trạm»/station label · **cấm** fallback `name_of_route_asset` khi name_station hợp lệ
  GAP-EMS-SPEC-01: giữ tbl_first_aid_station §4 attrs trong dumpSpecs · km trống khi dump null · FE dumpSpecLabels đủ key (name_station · owner_id · station_type_id · distance_nearest_major_road)
  XCO: GET by id AllowedCompanyIds
  POST/PUT: accept dumpSpecs merge · **không** required kmTo for EMS_POST

**DoD:**
- [ ] API-01…05 verify
- [ ] name/import fix (**GAP-EMS-NAME-01** · **GAP-EMS-SPEC-01**)
- [ ] no-parent-json · migration **none**

---

### T-BE-INIT-01 — init-data EMS_POST LOOKUP

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**from_solution:** API-06  
**DoD:** `RoadAssetInitDataDto.ownerOptions` + `stationTypeOptions: RoadAssetLookupOption[]` · options = dump LOOKUP_STATIC / seed · **cấm** invent station master SearchInput P1 · **cấm** FE hardcode · **GAP-EMS-LOOKUP-01**
...
| T-QA-FORM-01 | từng field required · UI value = body · dumpSpecs EMS attrs · kmTo ẩn · name «Tên trạm» |
| T-QA-FILTER-01 | V1–V5 + fields 1:1 `so-ts-ems-post-filter-bar.md` · 🔍 work · fail ErpListHeaderFilters / export trên bar |
...
| focus | COL profile EMS_POST · S-ATTR editable · kmTo ẩn · ownerOptions/stationTypeOptions init · name←name_station («Tên trạm») · LeaveConfirm · filter-bar · dumpSpecs P1 · prefix CCU- · tile t29 |
| mfeStdRoute | `/so-ts?type=EMS_POST` |
...
| init-data ownerOptions/stationTypeOptions · cấm hardcode dropdown | ✅ |
...
---
<!-- Version meta: skillId=agent-team-lead skillVersion=2026.08.19.04 schemaVersion=2 workflowVersion=2026.08.30.01 rulesVersion=2026.08.31.2 versionGate=rechecked taskId=task_7f7674e5 contentHashPriorDataAnaly=sha256:07fe22b464638b45f6be1286d9b99d3a7551dd5ef62be076013bbedd692885c8 route_confirm=route_a -->
