# Team lead — tasks — so-ts-rescue-station
...
| cluster | `station` · ô KCHT `—` (list only · **cấm** invent tile · **GAP-RS-TILE-01**) |
| dump | `tbl_disaster_res_facility` |
| prefix | `CN-` |
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-RS-NAME/SPEC/POINT/LOOKUP/LEAVE/ROUTE/TILE · flatten DEFER P2 |
| solution_confirm | **approve** (`task_784c6afd`) |
| design_confirm | **approve** (`task_64104e4c`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=RESCUE_STATION`** · alias board `/so-ts-rescue-station` optional redirect · alt B primary-alias-only rejected |
...
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:5e5eb3da5e2e10fae059fd2082ba0ed1c1f0c1337d89e41940099e39d1320e47` |
...
**Cấm:** implement product code (trừ template task) · e2e · `yarn build` / `start:std` · Step 4b/migration · ERP.* · invent `api/v1/so-ts/*` · invent KCHT tile · nhầm `RESCUE_VEHICLE` · start role khác (**GAP-PKT-ROLE-01**).
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=RESCUE_STATION` | **giữ** · alias `/so-ts-rescue-station` optional | **route_confirm=route_a** · **GAP-RS-ROUTE-01** |
| Grid profile | generic / thiếu RESCUE_STATION | Hide `type`/`kmTo`/qty/unit · **ON mẫu**: vật tư · DT khuôn viên · cấp+DT nhà/CT phụ/nhà kho | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
| Form S-ATTR | `<dl>` readonly dumpSpecs | Editable Dropdown/Number/TextArea đủ dump §4 · merge dumpSpecs | **T-UI-FORM-01** · **GAP-SOTS-FORM-01** |
| Point `kmTo` | form hiện / bắt buộc | **Ẩn** + không required khi `RESCUE_STATION` · kmFrom **không** required · **cấm** ép `"0"` | **T-UI-FORM-01** · **GAP-RS-POINT-01** |
| Name | IsWeak / đoạn tuyến risk | `name` ← `name_building` · label «Tên kho bãi» · trống OK · **cấm** IsWeak→đoạn | **T-BE-CRUD-01** + form · **GAP-RS-NAME-01** |
| LOOKUP cấp/vitri | text dumpSpecs | LOOKUP_STATIC init `officeBuildingGrades[]` · `auxiliaryWorksGrades[]` · `storedBuildingGrades[]` · `vitriOptions[]` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-RS-LOOKUP-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** · **GAP-RS-LEAVE-01** |
| Filter context | thiếu file | `so-ts-rescue-station-filter-bar.md` | **T-UI-FILTER-01** · **T-CTX-01** |
...
| dumpSpecLabels | thiếu attr keys | đủ dump §4 + materials/stored/vitri | **T-UI-FORM-01** · **GAP-RS-SPEC-01** |
| KCHT tile | ô `—` | **giữ** `—` · **cấm** invent facility tile | out of write · **GAP-RS-TILE-01** |
...
| `source.routes` | **`/so-ts?type=RESCUE_STATION`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=RESCUE_STATION` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=RESCUE_STATION` (Dev điền/verify) · alias `http://localhost:9301/so-ts-rescue-station` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=RESCUE_STATION` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `RESCUE_STATION`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-rescue-station-filter-bar.md` (create T-CTX-01) |
...
|--------|------|----------|
| A | `/so-ts?type=RESCUE_STATION` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-rescue-station` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Công trình cứu hộ» khi `type=RESCUE_STATION` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-rescue-station-filter-bar.md` |
| DES-GRID-C0 | listTitle «Danh sách công trình cứu hộ» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · RESCUE_STATION column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=RESCUE_STATION` + filters |
...
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · units · **`officeBuildingGrades[]`** · **`auxiliaryWorksGrades[]`** · **`storedBuildingGrades[]`** · **`vitriOptions[]`** delta |
| API-07 | GET | `/api/v1/asset/road-assets/summary-by-type` | peer · **no** facility tile write (**GAP-RS-TILE-01**) |
...
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent parent line JSON / RescueStationJson |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-RS-NAME/SPEC** import + rebuild |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-RS-LOOKUP-01** office/aux/stored grades + vitriOptions |
...
| T-UI-LEAVE-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | LeaveConfirmModal · **GAP-RS-LEAVE-01** |
...
**new_page.ssot_rereview:** pending → pass|fail
  ...
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-RS-NAME/SPEC/POINT/LOOKUP/LEAVE
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-rescue-station.md` + parent `so-ts-type-grid.md` · tạo `so-ts-rescue-station-filter-bar.md` · lane web → `dev` · **cấm** re-scan demo
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B A–D+F+H · `shared_grid_example=v1` · type profile `RESCUE_STATION`  
...
**ssot.reuse:**
  ...
  ui_filter: LinErpListFilterBar · **T-UI-FILTER-01** · `so-ts-rescue-station-filter-bar.md` — cấm ErpListHeaderFilters / LinListFilterField / nút Tìm
  ui_toolbar: catalogToolbar FULL (refresh·history·editConfig·view·edit·delete·create) · fa-cog
  ui_grid: LinCatalogDataGrid · kéo cột default ON · **RESCUE_STATION profile** hide type/kmTo/qty/unit · show name · route · kmFrom · materials_in_store · site_area_using_land · office/aux/stored grade+area (**GAP-SOTS-COL-01**)
...
**implement.page_shell:**
  ...
  header: title «Sổ TS — Công trình cứu hộ» khi type=RESCUE_STATION · titleIconClass + pageId
  filterBarLayout: erp-filter-bar · **cấm** filterMaxWidthPx hack
  listTitle: «Danh sách công trình cứu hộ»
...
**implement.toolbar:** (tl-grid-full-flow §1)
  ...
  form_pair: onAdd→`/so-ts/tao-moi` · onEdit/onView→`/so-ts/sua?id=` · type lock RESCUE_STATION
  cấm: thiếu Create/Refresh/Config · History invent API · icon lệch erp-control-icon-map

**implement.grid:**
  component: LinCatalogDataGrid
  columns: buildDynamicGridColumns(schema) + type-profile RESCUE_STATION
  visible ON: name · route · routeNamed · routeSegment · kmFrom · materials_in_store · site_area_using_land · office_building_grade_id · total_area_office_building · auxiliary_works_grade_id · total_area_auxiliary_works · stored_building_grade_id · total_area_stored_building · (status/gps optional)
  visible OFF default: type · kmTo · quantity · unitCode
  dumpSpecs: parse facility dump §4 keys · labels via dumpSpecLabels (**GAP-RS-SPEC-01**)
...
**implement.wire:**
  list: GET `?type=RESCUE_STATION&search=&route=&kmFrom=&kmTo=&orgUnit=&page=&pageSize=`
  alias: optional `/so-ts-rescue-station` → `/so-ts?type=RESCUE_STATION` (**GAP-RS-ROUTE-01**)

**implement.test:**
  listFilters · type lock · pageSize · selection → toolbar · column profile RESCUE_STATION

**DoD:**
- [ ] Kind B 1 shell · grid visible khi API có data · profile RESCUE_STATION (**GAP-SOTS-COL-01**)
...
**skills (REQUIRED load trước Write):**
  ...
  - context: `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-rescue-station-filter-bar.md`
...
**implement.filter:**
  ...
  type lock: luôn `RESCUE_STATION` trên deep-link
...
**ssot.reuse:**
  ...
  controls: SearchInput type/route · Dropdown status/source/officeBuildingGrades/auxiliaryWorksGrades/storedBuildingGrades/vitriOptions · Text/Number/TextArea dump §4 · Text name_building («Tên kho bãi»)
  http: API-02/03/04 · IdCode readonly prefix `CN-` · init-data API-06

**implement.form:**
  modes: create · edit · view(readOnly **không** disabled xám) · copy(clear id/code → POST · keep RESCUE_STATION)
  sections: S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR · S-GPS
  required: type · status · route — name trống OK · kmFrom **không** required · **không** required `kmTo` (**GAP-RS-POINT-01**)
  S-ATTR: materials_in_store TextArea · site_area_using_land Number · office_building_grade_id Dropdown · total_area_office_building Number · auxiliary_works_grade_id Dropdown · total_area_auxiliary_works Number · stored_building_grade_id Dropdown · total_area_stored_building Number · vitri Text/Dropdown · merge dumpSpecs (**GAP-SOTS-FORM-01** · **GAP-RS-LOOKUP-01** · **GAP-RS-SPEC-01**)
  S-NAME: `name` ← `name_building` · label «Tên kho bãi» · trống OK · mirror dumpSpecs (**GAP-RS-NAME-01**)
  kmFrom: **cấm** ép `"0"` khi dump null (**GAP-RS-SPEC-01** · **GAP-RS-POINT-01**)
  **Không mount:** kmTo · quantity/unit · S-LOC-RANGE · tab Chi tiết/Bảo trì/Tệp/Lịch sử legacy
  cấm: invent History path · flatten Schema_* · ERP.* · Modal form · nhầm RESCUE_VEHICLE fields

**implement.init_data:**
  options: GET `/asset/road-assets/init-data` only — statuses · sources · **officeBuildingGrades** · **auxiliaryWorksGrades** · **storedBuildingGrades** · **vitriOptions** · **cấm** KIND_LABEL hardcode (**GAP-DEV-DROPDOWN-HARDCODE-01**)
...
- [ ] kmTo ẩn · facility LOOKUP Dropdown từ init-data
- [ ] reuse AssetFormPage S-* — **cấm** fork

**Fail:** GAP-SOTS-FORM-01 · GAP-SOTS-REUSE-01 · GAP-RS-POINT-01 · GAP-RS-LOOKUP-01 · GAP-P2-FORM-GRID-05 · GAP-LIST-FIELD-01
...
**DoD:** dirty → `LeaveConfirmModal` · full `useFormLeaveGuard` · **cấm** `window.confirm`/`alert` · **GAP-RS-LEAVE-01** / **GAP-P2-36** / **GAP-DEV-LEAVE-01** / **GAP-TL-LEAVE-01**
...
| +Tạo mới | toolbar | navigate `/so-ts/tao-moi` type=RESCUE_STATION | POST API-03 |
...
| Alias board | `/so-ts-rescue-station` | optional redirect → `?type=RESCUE_STATION` | — |
...
**status:** pending / verify  
**devSlash:** `/agent-dev`  
**DoD:** filter+form type/route/org = SearchInput Integration · office/aux/stored grade + vitri = LOOKUP_STATIC init-data (**không** SearchInput master P1) · **GAP-LIST-LKP-01** / **GAP-RS-LOOKUP-01**
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** controlHint Design/DA 1:1 · SA field map · dumpSpecs keys `name_building` · `materials_in_store` · `site_area_using_land` · `office_building_grade_id` · `total_area_office_building` · `auxiliary_works_grade_id` · `total_area_auxiliary_works` · `stored_building_grade_id` · `total_area_stored_building` · `vitri` · **GAP-LIST-FIELD-01** · **GAP-RS-SPEC-01**
...
**ssot.reuse:**
  ...
  import: RoadAssetCatalogHandler · RebuildGovVn `FindOfficialName incl. name_building` / `IsWeakAssetName`
  gates: tz_na · xco_get_only · share_tenant

**implement.wire:**
  API-01…05: verify live · **cấm** invent `api/v1/so-ts/*` · ERP.*
  GAP-RS-NAME-01: rebuild CSV `name`←`name_building` · **cấm** IsWeak reject «Tên kho bãi»/facility label · **cấm** fallback `name_of_route_asset` khi name_building hợp lệ
  GAP-RS-SPEC-01: giữ facility dump §4 attrs trong dumpSpecs · km trống khi dump null · FE dumpSpecLabels đủ key (materials_in_store · stored_building_* · vitri)
  XCO: GET by id AllowedCompanyIds
  POST/PUT: accept dumpSpecs merge · **không** required kmTo for RESCUE_STATION

**DoD:**
- [ ] API-01…05 verify
- [ ] name/import fix (**GAP-RS-NAME-01** · **GAP-RS-SPEC-01**)
- [ ] no-parent-json · migration **none**

---

### T-BE-INIT-01 — init-data RESCUE_STATION LOOKUP

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**from_solution:** API-06  
**DoD:** `RoadAssetInitDataDto.officeBuildingGrades` + `auxiliaryWorksGrades` + `storedBuildingGrades` + `vitriOptions: RoadAssetLookupOption[]` · options = dump LOOKUP_STATIC / seed · **cấm** invent facility master SearchInput P1 · **cấm** FE hardcode · **GAP-RS-LOOKUP-01**
...
| T-QA-FORM-01 | từng field required · UI value = body · dumpSpecs facility attrs · kmTo ẩn · name «Tên kho bãi» |
| T-QA-FILTER-01 | V1–V5 + fields 1:1 `so-ts-rescue-station-filter-bar.md` · 🔍 work · fail ErpListHeaderFilters / export trên bar |
...
| Auth NuGet | DEFER |
| Facility LOOKUP master SearchInput | P2 |
| Excel import wizard UI | OUT |
| Kind F map canvas | **cấm** |
| invent History API | **cấm** |
| invent `api/v1/so-ts/*` | **cấm** |
| invent KCHT facility tile | **cấm** · **GAP-RS-TILE-01** |
| nhầm RESCUE_VEHICLE | **cấm** |
...
| focus | COL profile · S-ATTR editable · kmTo ẩn · office/aux/stored grades + vitriOptions init · name←name_building («Tên kho bãi») · LeaveConfirm · filter-bar · dumpSpecs P1 · **cấm** invent tile |
| mfeStdRoute | `/so-ts?type=RESCUE_STATION` |
...
| init-data office/aux/stored grades + vitriOptions · cấm hardcode dropdown | ✅ |
...
---
<!-- Version meta: skillId=agent-team-lead skillVersion=2026.08.19.04 schemaVersion=2 workflowVersion=2026.08.30.01 rulesVersion=2026.08.31.2 versionGate=rechecked taskId=task_1b274dc7 contentHashPriorDataAnaly=sha256:5e5eb3da5e2e10fae059fd2082ba0ed1c1f0c1337d89e41940099e39d1320e47 route_confirm=route_a -->
