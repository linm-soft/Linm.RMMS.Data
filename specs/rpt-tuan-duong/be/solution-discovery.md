# SA — solution-discovery — rpt-tuan-duong (report pack · Kind E)
...
| solution_confirm | **approve** (`autoApprove=ON` packet `task_87bb6006` — agent tự confirm · **không** chờ board) |
| changeScope | `edit_page` |
| packKind | **`report`** (Kind **E** AnalyticsReportShell) — packet board `list` **bỏ** (GAP-PO-BTD-02) |
| Feature Kind | **E** · leaf `/bao-cao/tuan-duong` · **không** CRUD form |
| domain | **Report** (DOMAIN-MAP slug `rpt-tuan-duong` → kebab `report`) |
| BackendRoot | `D:/AI-QLBD/Linm.RMMS.WebService` |
| MFE | `D:/AI-QLBD/MFE-Source/Linm.Web.RMMS.Report` · `/bao-cao/tuan-duong` |
| mfeStdUrl | `http://localhost:9311/bao-cao/tuan-duong` |
| prefix | **`api/v1/report`** · BFF **`web-bff/api/v1/report`** |
| prior · design | **confirmed** (`task_7d300c6b`) · `ui/design.md` + prototype + reviewUrl |
| prior · po | **confirmed** · `po/requirement.md` · `task_0366c0ab` |
| prior · data_analy | **confirmed** · `specs/_data-analy/features/rpt-tuan-duong-control-hint.md` · hash `sha256:rpt-tuan-duong-context-20260816` · handoff path `specs/rpt-tuan-duong/specs/_data-analy/clusters/rpt-tuan-duong.md` **không tồn tại** — dùng artifact thật |
...
| autoApprove | **ON** |
| chain | **ON** |
| taskId | `task_87bb6006` |
| confirmedBy | agent autoApprove · `task_87bb6006` |
...
> **Cấm** reuse `GET api/v1/report/patrol-log-road` (`rpt-nhat-ky-tuan-duong` sổ) · **cấm** gộp `PatrolType=inspect` / tuần kiểm vào lưới.

**Cấm** `GET /api/v1/reports/patrol-road` (context cũ — GAP-PO-BTD-01 **đóng**).
...
| sa_shared_table | **share_na** — không schema warehouse / EF `PatrolSession` bắt buộc P1 (GAP-PO-BTD-07 = **P2**) |
| lookup_share | road-route = **share_a** Integration Type A — read-only SearchInput |
| parent_json | **cấm** |
| design_confirm | **approve** (Design `task_7d300c6b`) |
...
| Models | `api/domains/report/LINM.RMMS.Report.Models/DTOs/ReportQueryDtos.cs` · `ReportPatrolRoadRowDto` |
...
| FE BASE | `/report` (relative `VITE_API_URL`) · `reportEndpoint.getPatrolRoad` / `exportPatrolRoad` |
...
Query + export + BFF proxy + MFE Kind E **đã có** (prior chain `task_9d72ff3d`). Pack **không** rewrite domain. SA chốt contract + GAP còn lại cho TL/Dev:

| ID | Live today | Required this pack | Layer |
|----|------------|--------------------|-------|
| GAP-SA-BTD-PREFIX | `GET api/v1/report/patrol-road` + `/patrol-road/export` | **Giữ** — đóng GAP-PO-BTD-01 · **cấm** `api/v1/reports` · **cấm** reuse `patrol-log-road` | document |
| GAP-SA-BTD-ENVELOPE | `ApiResponse<ReportPagedResult<ReportPatrolRoadRowDto>>` `{ success, message, data }` · `data.items` + paging | **Giữ** · BFF proxy raw · **không** envelope mới · **không** KPI block trên DTO | document |
| GAP-SA-BTD-ROW | `id` `sessionId` `code` `day` `startedAt` `route` `staffId` `userName` `patrolType` `patrolTypeLabel` `checkInCount` `coveragePercent` `status` `statusLabel` `offlineQueued` `note` | **Giữ P1 trên BE** — map `PatrolSessionEntity` scalars | keep |
| GAP-SA-BTD-GRID | Live MFE: day · code · route · userName · patrolTypeLabel · checkInCount · coveragePercent · statusLabel · offlineQueued · note · drill | **Khớp** PO §5 + Design — **không** thiếu cột P1 · **cấm** Company/QL/Km | keep |
| GAP-SA-BTD-TYPE | Query `type` **bị ignore** (`_ = type`) · lưới **hard-filter** `PatrolType=road` | **Giữ ignore `type`** · **cấm** dùng `type` để gộp inspect · seed `TK-*` (`pr14`) **excluded** | keep |
| GAP-SA-BTD-STAFF | query **`staffId`** exact `StaffId` · empty/`all` = all · seed `nva` `ttb` `lvc` `pmd` | **Giữ** · NV = **enum seed FE P1** — **cấm** native Select | keep |
| GAP-SA-BTD-STATUS | query **`status`** exact `Status` · empty/`all` = all · `in_progress`/`done`/`missed`/`offline` | **Giữ** · SearchInput enum FE | keep |
| GAP-SA-BTD-ROUTE | `FilterRoute` **exact** (không prefix) · empty/`all` = all · seed CUC2 (`QL.1` `QL.15` `QL.217` `HCM` `CT.001` `QL.7` `QL.10` `QL.8` `QL.9` `QL.46`) | **Giữ** GAP-PO-BTD-04 · lookup UI = 38 CUC2 + trống · **cấm** invent `QL.22` | BE keep · FE lookup |
| GAP-SA-BTD-SEARCH | Contains `Code`/`Route`/`UserName`/`Note` · query **`q`** canonical · coalesce `q ?? search` | **Giữ** · FE gửi `q` · alias `search` OK | keep |
| GAP-SA-BTD-PAGE | allow `{50,100,200,500}` · invalid → 50 | **Khớp** Design Zone D | keep |
| GAP-SA-BTD-EXPORT | CSV UTF-8 BOM · filename **`patrol-road.csv`** · header live: `day,code,route,userName,patrolType,checkInCount,coveragePercent,status,offlineQueued,note,sessionId` · **không** page | **IN P1** · FE `subsetCsv` theo cột đang hiện | keep |
| GAP-SA-BTD-DRILL | Live MFE `drillSource(sessionId)` → `/patrol?id={sessionId}` · seed P1 `SessionId = Code` (`TD-*`) | **LOCK live P1** — **không** API Report GetById · P2 map `PatrolSession.Id` thật khi EF | FE keep |
| GAP-SA-BTD-FOOTER | Live `LinCatalogListPagination` luôn | Zone D **luôn** hiện — **FE only** | FE keep |
| GAP-SA-BTD-PERM | Không `[RequirePermission]` | **P1 stub** `report.tuan-duong.read` (+ export cùng read) — gắn khi CommonLib ≥1.4.0 · **không** block | document |
| GAP-SA-BTD-RM | In-memory **15** seed (14 `road` + 1 `inspect` excluded) · filter `PatrolType=road` | **P1 giữ** · EF `rmms_patrol_sessions` **P2** | document |
| GAP-SA-BTD-KPI-MAP | Không KPI dashboard / map trên DTO | **Giữ** — Chart SoCai **client** từ `items` (số phiên theo tuyến · theo trạng thái) · **cấm** gộp hub `reports` / `rpt-nhat-ky-tuan-duong` / tuần kiểm | document |
| GAP-SA-BTD-CONFIG | Live `ReportDisplayConfigModal` FULL | **Giữ** — **cấm** `LinListTableConfigModal` / `configHint` / bắt `LinCatalogUiSchemaEditorModal` | FE keep |
...
|----|---------|---------|-------|----------|---------|
| S-BTD | Report A–D + SoCai chart | Full page `LinPageLayout` kind=`report` | `/bao-cao/tuan-duong` | report | filter draft → **Xem** load · Làm mới · Excel · chart SoCai · In · Config FULL |
...
| Action | Endpoint |
|--------|----------|
| Xem | **API-01** GET `report/patrol-road` |
| Xuất Excel | **API-02** GET `report/patrol-road/export` |
...
### API-01 — Patrol road sessions (Xem)

`GET /api/v1/report/patrol-road?staffId=&routeId=&status=&from=&to=&q=&search=&page=&pageSize=`  
BFF `GET web-bff/api/v1/report/patrol-road` + cùng query.
...
| `type` | **ignored** P1 (compat param) — **cấm** dùng để gộp inspect |
...
Server **luôn** `Where PatrolType == road`. Grid P1 bắt buộc gồm `patrolTypeLabel` + drill `sessionId`.

### API-02 — Export patrol road

`GET /api/v1/report/patrol-road/export` — cùng filter API-01 **trừ** `page`/`pageSize`.  
`text/csv; charset=utf-8` · UTF-8 **BOM** · filename `patrol-road.csv`.
...
- Làm mới: chỉ re-fetch applied; nếu `!viewed` → toast, **không** load (GAP-DS-BTD-10).
- Drill: FE `/patrol?id={sessionId}` — **không** API Report GetById.
- Chart P1: **client** từ `items` trang hiện tại — **không** API chart riêng P1.
- Lưới **chỉ** phiên `road` — seed inspect **không** vào `items`.
...
| Code | Surfaces |
|------|----------|
| `report.tuan-duong.read` | API-00 · API-01 · API-02 |
...
- DoD: 1× LinPageLayout kind=`report` · grid kéo cột · **luôn** LinCatalogListPagination · SearchInput tuyến/trạng thái/NV · Xem mới load · Excel UTF-8 BOM `patrol-road.csv` · Config FULL report modal · SoCai · drill `/patrol?id=` · cột Loại tuần · chỉ `PatrolType=road`.
- autoApprove **ON** → SA **confirmed** (agent) · enqueue TL.
...
- This task = `roleOnly=sa` · **không** chạy TL trong `task_87bb6006`.

## Build (this role)

SA **không** sửa MFE/BE runtime. Verify = live contract audit (`ReportQueryController` · `ReportService.FilterPatrolRoad` · `ReportPatrolRoadRowDto` · BFF `patrol-road` · MFE `PatrolRoadReportPage` + `reportEndpoint.getPatrolRoad`). DOMAIN-MAP slug row. **Không** `yarn build` / `dotnet build` trong role này.
