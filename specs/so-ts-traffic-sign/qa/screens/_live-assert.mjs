/**
 * Live DOM assert + DTM filter-bar overflow for so-ts-traffic-sign.
 * POINT: kmTo ẩn trên form · filter kmTo ± · type filter hidden · BB- · traffic-sign attrs.
 */
...
const list = await page.evaluate((tid) => {
  ...
  return {
    titleList: /Danh sách biển báo|Biển báo/i.test(body),
    headerTrafficSign: /Sổ TS — Biển báo/i.test(body),
    ...
    colContent: headers.some((h) =>
      /Nội dung|road_sign_content|Số hiệu/i.test(h),
    ),
    colMaterial: headers.some((h) => /Vật liệu|material_sign/i.test(h)),
    colShape: headers.some((h) => /Hình dạng|shape_sign/i.test(h)),
...
await page.waitForSelector('[data-testid="asset-traffic-sign-attr"]', {
  timeout: 15000,
});
await new Promise((r) => setTimeout(r, 1200));
...
const form = await page.evaluate(() => {
  ...
  const codeHint = /BB-/i.test(body) || /prefix|Mã/i.test(body);
  return {
    cols5: cols,
    trafficSignAttr: !!document.querySelector(
      '[data-testid="asset-traffic-sign-attr"]',
    ),
    noiDung: /Nội dung|road_sign_content/i.test(body),
    soHieu: /Số hiệu QCVN|sign_code/i.test(body),
    vatLieu: /Vật liệu biển|material_sign/i.test(body),
    hinhDang: /Hình dạng biển|shape_sign/i.test(body),
    ngayLap: /Ngày lắp đặt|ngaylapdat/i.test(body),
    kmToVisible,
...
const fail =
  !list.titleList ||
  !list.headerTrafficSign ||
  !list.search ||
  !list.route ||
  !list.kmToFilter ||
  !form.cols5 ||
  !form.trafficSignAttr ||
  !form.kmToHiddenOk ||
  !form.noiDung ||
  !form.vatLieu ||
  !form.hinhDang ||
