# Team lead — tasks — so-ts-traffic-sign
...
| dump | `tbl_road_sign` |
| prefix | `BB-` |
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-SIGN-NAME/SPEC/POINT/MAT/SHAPE/LEAVE/ROUTE · GAP-SIGN-FLAT-01 DEFER · Auth DEFER |
| solution_confirm | **approve** (`task_49a79beb`) |
| design_confirm | **approve** (`task_6bf578d2`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=TRAFFIC_SIGN`** · alias board `/so-ts-traffic-sign` optional redirect · alt B primary-alias-only rejected |
...
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:36d61492d82e2fbb37adf4b9935116f9ce71e357e336150de46e95049566de88` |
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=TRAFFIC_SIGN` | **giữ** · alias `/so-ts-traffic-sign` optional | **route_confirm=route_a** · **GAP-SIGN-ROUTE-01** |
| Grid profile | generic / thiếu TRAFFIC_SIGN | Hide `type`/`kmTo`/qty/unit · show `sign_code_number`/`road_sign_content`/width/height/area/`material_sign_id`/`shape_sign_id`/`location_id`/`ngaylapdat` | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
| Form S-ATTR | `<dl>` readonly dumpSpecs | Editable Number + Dropdown + Text · merge dumpSpecs | **T-UI-FORM-01** · **GAP-SOTS-FORM-01** · **GAP-SIGN-SPEC-01** |
| Point `kmTo` | form hiện / bắt buộc risk | **Ẩn** + không required khi `TRAFFIC_SIGN` | **T-UI-FORM-01** · **GAP-SIGN-POINT-01** |
| Name | IsWeak / đoạn tuyến risk | `name` ← `sign_code_number` · SearchInput traffic-sign-types · content=`road_sign_content` | **T-BE-CRUD-01** + form · **GAP-SIGN-NAME-01** |
| Prefix IdCode | live DefaultCodePrefix `TS-` | type override **`BB-`** | **T-BE-CRUD-01** · GIS bien-bao |
| LOOKUP material/shape | text dumpSpecs | LOOKUP_STATIC init `materialsSign[]` · `shapesSign[]` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-SIGN-MAT/SHAPE-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** · **GAP-SIGN-LEAVE-01** |
| Filter context | thiếu file | `so-ts-traffic-sign-filter-bar.md` | **T-UI-FILTER-01** · **T-CTX-01** |
| Form surface | full-page 5 cột | **giữ** `data-form-cols="5"` + header chrome | **T-UI-FORM-01** · **cấm** Slideout/footer Lưu · **cấm** PoleCount |
| Flatten DB | — | **DEFER P2** | dumpSpecs P1 · migration **none** · **GAP-SIGN-FLAT-01** |
| dumpSpecLabels | thiếu keys biển | đủ dump biển · primary label «Số hiệu QCVN» · content «Nội dung» | **T-UI-FORM-01** · **GAP-SIGN-SPEC-01** |
...
| `source.routes` | **`/so-ts?type=TRAFFIC_SIGN`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=TRAFFIC_SIGN` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=TRAFFIC_SIGN` (Dev điền/verify) · alias `http://localhost:9301/so-ts-traffic-sign` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=TRAFFIC_SIGN` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `TRAFFIC_SIGN`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-traffic-sign-filter-bar.md` (create T-CTX-01) |
...
|--------|------|----------|
| A | `/so-ts?type=TRAFFIC_SIGN` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-traffic-sign` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Biển báo» khi `type=TRAFFIC_SIGN` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-traffic-sign-filter-bar.md` |
| DES-GRID-C0 | listTitle «Danh sách biển báo» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · TRAFFIC_SIGN column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=TRAFFIC_SIGN` + filters |
| API-02 | GET | `/api/v1/asset/road-assets/{id}` | detail · XCO get_only |
| API-03 | POST | `/api/v1/asset/road-assets` | create · dumpSpecs merge · prefix **`BB-`** · **không** required `kmTo` |
| API-04 | PUT | `/api/v1/asset/road-assets/{id}` | update · dumpSpecs merge |
| API-05 | DELETE | `/api/v1/asset/road-assets/{id}` | soft |
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · units · **`materialsSign[]`** · **`shapesSign[]`** delta |
| API-07 | GET | `/api/v1/asset/road-assets/summary-by-type` | peer tile t32 · out of write pack |
...
| API-LKP-04 | GET | Integration `traffic-sign-types/search` | S-NAME `sign_code_number` SearchInput |
...
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent parent line JSON / RoadSignJson |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-SIGN-NAME/SPEC** + prefix **BB-** |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-SIGN-MAT/SHAPE-01** materialsSign + shapesSign |
...
| T-UI-FORM-01 | Dev | pending (delta) | T-UI-LIST-01 · T-BE-INIT-01 | `/agent-dev` | full-page 5 cột · S-ATTR · **GAP-SOTS-FORM/POINT/REUSE** · **GAP-SIGN-*** |
| T-UI-LEAVE-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | LeaveConfirmModal · **GAP-SIGN-LEAVE-01** |
| T-UI-ACT-01 | Dev | pending | T-UI-LIST-01 | `/agent-dev` | inventory below |
| T-UI-LKP-01 | Dev | pending / verify | API-LKP-* | `/agent-dev` | asset-type · road-route · org-unit · traffic-sign-types |
...
**new_page.ssot_rereview:** pending → pass|fail
  ...
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-SIGN-NAME/SPEC/POINT/MAT/SHAPE/LEAVE/ROUTE
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-traffic-sign.md` + parent `so-ts-type-grid.md` · **create** `so-ts-traffic-sign-filter-bar.md` · lane web → `dev` · **cấm** re-scan demo
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B A–D+F+H · `shared_grid_example=v1` · type profile `TRAFFIC_SIGN`  
...
**ssot.reuse:**
  ...
  ui_filter: LinErpListFilterBar · **T-UI-FILTER-01** · `so-ts-traffic-sign-filter-bar.md` — cấm ErpListHeaderFilters / LinListFilterField / nút Tìm
  ui_toolbar: catalogToolbar FULL (refresh·history·editConfig·view·edit·delete·create) · fa-cog
  ui_grid: LinCatalogDataGrid · kéo cột default ON · **TRAFFIC_SIGN profile** hide type/kmTo/qty/unit · show sign_code_number · road_sign_content · width · height · area · material_sign_id · shape_sign_id · location_id · ngaylapdat (**GAP-SOTS-COL-01**)
...
**implement.page_shell:**
  ...
  header: title «Sổ TS — Biển báo» khi type=TRAFFIC_SIGN · titleIconClass + pageId
  filterBarLayout: erp-filter-bar · **cấm** filterMaxWidthPx hack
  listTitle: «Danh sách biển báo»
...
**implement.toolbar:** (tl-grid-full-flow §1)
  ...
  form_pair: onAdd→`/so-ts/tao-moi` · onEdit/onView→`/so-ts/sua?id=` · type lock TRAFFIC_SIGN
  cấm: thiếu Create/Refresh/Config · History invent API · icon lệch erp-control-icon-map

**implement.grid:**
  component: LinCatalogDataGrid
  columns: buildDynamicGridColumns(schema) + type-profile TRAFFIC_SIGN
  visible ON: name (sign_code_number) · route · routeNamed · routeSegment · kmFrom · road_sign_content · width · height · area · material_sign_id · shape_sign_id · location_id · ngaylapdat · (status/gps optional)
  visible OFF default: type · kmTo · quantity · unitCode
  dumpSpecs: parse tbl_road_sign keys · labels via dumpSpecLabels (**GAP-SIGN-SPEC-01**)
...
**implement.wire:**
  ...
  alias: optional `/so-ts-traffic-sign` → `/so-ts?type=TRAFFIC_SIGN` (**GAP-SIGN-ROUTE-01**)

**implement.test:**
  listFilters · type lock · pageSize · selection → toolbar · column profile TRAFFIC_SIGN

**DoD:**
- [ ] Kind B 1 shell · grid visible khi API có data · profile TRAFFIC_SIGN (**GAP-SOTS-COL-01**)
...
**skills (REQUIRED load trước Write):**
  ...
  - context: `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-traffic-sign-filter-bar.md`
...
**implement.filter:**
  ...
  type lock: luôn `TRAFFIC_SIGN` trên deep-link
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B · `formSurface: full` · `data-form-cols="5"` · header chrome · **cấm** Slideout/Modal hồ sơ · **cấm** PoleCount  
...
**ssot.reuse:**
  ...
  controls: SearchInput type/route · SearchInput traffic-sign-types (sign_code_number) · Dropdown materialsSign/shapesSign/status/source · Text road_sign_content · Number width/height/area · Dropdown/Text location_id · Date ngaylapdat · Number lat/lng
  http: API-02/03/04 · IdCode readonly prefix `BB-` · init-data API-06

**implement.form:**
  modes: create · edit · view(readOnly **không** disabled xám) · copy(clear id/code → POST · keep TRAFFIC_SIGN)
  sections: S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR · S-GPS
  required: type · status · route — name=`sign_code_number` via SearchInput · **không** required `kmTo` (**GAP-SIGN-POINT-01**)
  S-NAME: `name` ← `sign_code_number` · SearchInput `traffic-sign-types/search` · mirror dumpSpecs · label «Số hiệu QCVN» (**GAP-SIGN-NAME-01**)
  S-ATTR: road_sign_content Text · width/height/area Number · material_sign_id Dropdown * · shape_sign_id Dropdown * · location_id Dropdown/Text · ngaylapdat Date · merge dumpSpecs (**GAP-SOTS-FORM-01** · **GAP-SIGN-SPEC-01** · **GAP-SIGN-MAT/SHAPE-01**)
  IdCode: create prefix **`BB-`**
  **Không mount:** kmTo · quantity/unit · S-LOC-RANGE · PoleCount · tab Chi tiết/Bảo trì/Tệp/Lịch sử legacy
  cấm: invent History path · flatten Schema_* · ERP.* · Modal form

**implement.init_data:**
  options: GET `/asset/road-assets/init-data` only — statuses · sources · **materialsSign** · **shapesSign** · **cấm** KIND_LABEL hardcode (**GAP-DEV-DROPDOWN-HARDCODE-01**)
...
- [ ] kmTo ẩn · materialsSign/shapesSign Dropdown từ init-data · name SearchInput traffic-sign-types · prefix BB-
- [ ] reuse AssetFormPage S-* — **cấm** fork · **cấm** PoleCount

**Fail:** GAP-SOTS-FORM-01 · GAP-SOTS-REUSE-01 · GAP-SIGN-POINT-01 · GAP-SIGN-MAT-01 · GAP-SIGN-SHAPE-01 · GAP-SIGN-NAME-01 · GAP-P2-FORM-GRID-05 · GAP-LIST-FIELD-01
...
**DoD:** dirty → `LeaveConfirmModal` · full `useFormLeaveGuard` · **cấm** `window.confirm`/`alert` · **GAP-SIGN-LEAVE-01** / **GAP-P2-36** / **GAP-DEV-LEAVE-01** / **GAP-TL-LEAVE-01**
...
| +Tạo mới | toolbar | navigate `/so-ts/tao-moi` type=TRAFFIC_SIGN | POST API-03 |
...
| Alias board | `/so-ts-traffic-sign` | optional redirect → `?type=TRAFFIC_SIGN` | — |
...
**status:** pending / verify  
**devSlash:** `/agent-dev`  
**DoD:** filter+form type/route/org = SearchInput Integration · S-NAME sign_code_number = SearchInput `traffic-sign-types/search` · material_sign_id/shape_sign_id = LOOKUP_STATIC init-data (**không** SearchInput master P1) · **GAP-LIST-LKP-01** / **GAP-SIGN-MAT/SHAPE-01** / **GAP-SIGN-NAME-01**
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** controlHint Design/DA 1:1 · SA field map · dumpSpecs keys `sign_code_number` · `road_sign_content` · `width` · `height` · `area` · `material_sign_id` · `shape_sign_id` · `location_id` · `ngaylapdat` · **GAP-LIST-FIELD-01**
...
**ssot.reuse:**
  ...
  import: RoadAssetCatalogHandler · RebuildGovVn `FindOfficialName incl. sign_code_number` / `IsWeakAssetName` · GIS prefix `BB` / bien-bao
  gates: tz_na · xco_get_only · share_tenant

**implement.wire:**
  API-01…05: verify live · **cấm** invent `api/v1/so-ts/*` · ERP.*
  GAP-SIGN-NAME-01: rebuild CSV `name`←`sign_code_number` · mirror dumpSpecs · **cấm** IsWeak reject khi sign_code hợp lệ · **cấm** fallback đoạn tuyến khi name hợp lệ
  GAP-SIGN-SPEC-01: giữ road_sign dumpSpecs attrs · FE dumpSpecLabels đủ key biển
  Prefix: create/import IdCode prefix **`BB-`** (override DefaultCodePrefix `TS-` khi type=TRAFFIC_SIGN)
  XCO: GET by id AllowedCompanyIds
  POST/PUT: accept dumpSpecs merge · **không** required kmTo for TRAFFIC_SIGN

**DoD:**
- [ ] API-01…05 verify
- [ ] name/import/prefix fix (**GAP-SIGN-NAME-01** · **GAP-SIGN-SPEC-01** · BB-)
- [ ] no-parent-json · migration **none**

---

### T-BE-INIT-01 — init-data TRAFFIC_SIGN LOOKUP

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**from_solution:** API-06  
**DoD:** `RoadAssetInitDataDto.materialsSign` + `shapesSign: RoadAssetLookupOption[]` · options = dump LOOKUP_STATIC / seed · **cấm** invent material/shape master SearchInput P1 · **cấm** FE hardcode · **GAP-SIGN-MAT-01** · **GAP-SIGN-SHAPE-01**
...
| T-QA-FORM-01 | từng field required · UI value = body · dumpSpecs biển · kmTo ẩn · prefix BB- · materialsSign/shapesSign Dropdown |
| T-QA-FILTER-01 | V1–V5 + fields 1:1 `so-ts-traffic-sign-filter-bar.md` · 🔍 work · fail ErpListHeaderFilters / export trên bar |
...
| ID | Note |
|----|------|
| Flatten Schema_* | P2 · dumpSpecs P1 · **GAP-SIGN-FLAT-01** |
| Auth NuGet | DEFER |
| Material/shape LOOKUP master SearchInput | P2 |
...
| focus | COL profile · S-ATTR editable · kmTo ẩn · materialsSign/shapesSign LOOKUP · name←sign_code_number · traffic-sign-types SearchInput · prefix BB- · LeaveConfirm · filter-bar · dumpSpecs P1 |
| mfeStdRoute | `/so-ts?type=TRAFFIC_SIGN` |
...
| init-data materialsSign/shapesSign · cấm hardcode dropdown | ✅ |
| Source assignment · API cite SA · **cấm** invent | ✅ |
| prefix BB- · GAP-SIGN-NAME/MAT/SHAPE | ✅ |
| handoff compact ≤5KB | ✅ |
