# Data-analy — controlHint — so-ts-rest-area (Kind B list + full-page form · type `REST_AREA`)
...
| uiRepo | `D:/AI-QLBD/MFE-Source/Linm.Web.RMMS.Asset` · live list `/so-ts?type=REST_AREA` · form `/so-ts/tao-moi` · `/so-ts/sua?id=` |
| mfeStdUrl | `http://localhost:9301/so-ts-rest-area` (STATUS alias) · live filter `http://localhost:9301/so-ts?type=REST_AREA` |
| typeCode | `REST_AREA` |
| dump | `tbl_rest_stops` · mẫu `docs/img/gov-mau-tai-san/17-moc_dbvn.tbl_rest_stops-list.png` · `17-moc_dbvn.tbl_rest_stops-detail.png` · CSV gov-vn **106** row `REST_AREA` · prefix `DN-` |
| clusterUi | `station` · ô KCHT `t26` |
| parent | `so-ts-type-grid` · fields `import-gov-asset-fields` |
| runMode | `new_page` · greenfield type-profile trên shell Kind B đã có |
| peer | `so-ts-parking` (cùng dump · **tách** type `PARKING` · **GOV-IMP-02**) |
...
> Mẫu list: tên trạm · 3 tầng tuyến · loại · lý trình · xếp loại · chủ SH · chiều dài · DT · cứu hộ · cấp cứu (**GAP-SOTS-COL-01** hide-empty khi fill 0).  
> **REST_AREA** ≠ **PARKING** — cùng dump `tbl_rest_stops` · import `RefineImportedType` tách theo tên · filter `?type=REST_AREA` **cấm** lẫn bãi đỗ.

## Sources

| Source | Path | sha256 / note |
|--------|------|---------------|
| Context | `docs/context/features/so-ts-rest-area.md` | `b92dcb2dca6f96078721289e687ddbcaa3339828968fe1d11ce202c4e8569bc` |
| Parent shell | `docs/context/features/so-ts-type-grid.md` | section reuse · cluster `station` · ô `t26` |
...
| Mẫu detail | `docs/img/gov-mau-tai-san/17-moc_dbvn.tbl_rest_stops-detail.png` | tab Thông tin chung · vị trí điểm · attr trạm dừng nghỉ |
| Import set | `…/gov/sets/gov-vn/road_assets*.csv` | **106** `REST_AREA` · prefix `DN-` |
...
| Import handler | `…/Import/RoadAssetCatalogHandler.cs` | type seed `REST_AREA` · «Trạm dừng nghỉ» · unit `TRAM` · `RefineImportedType` tách PARKING |
| GIS map | `GisInventoryMapper.cs` | `tram-dung-nghi` ↔ `REST_AREA` · IdCode prefix live `TDN` / import `DN-` |
| MFE list | `AssetListPage.tsx` | Kind B · `?type=` · **thiếu** profile `REST_AREA` |
| MFE form | `AssetFormPage.tsx` | Kind B full-page · `data-form-cols="5"` · S-ATTR REST_AREA **chưa** editable đủ |
| FE labels | `services/asset/dumpSpecLabels.ts` | có `name_work` · `type_work_id` · `owner_id` · `actual_length` · `site_area_using_land` · `office_building_grade_id` · `total_area_floors` · `auxiliary_works_grade_id` · `total_area_auxiliary_works` · `build_location` — **thiếu** `categorized_id` · `service_area` · `total_area_office_building` · `parking_lot` · `total_parking_lot` · `traffic_emergency_service` · `first_aid_service` · `build_location_id` |
| FE endpoint | `services/asset/endpoint.ts` | `BASE=/asset/road-assets` |
| KCHT tile | `kchtTileConfig.ts` | `t26` · drill `REST_AREA` · «Trạm dừng nghỉ» · icon `BX` |
| DOMAIN-MAP | `Linm.RMMS.WebService/docs/DOMAIN-MAP.md` | Asset · `api/v1/asset` · **cấm ERP.*** · slug peer `so-ts-parking` / `so-ts-bus-station` |
| Shared catalogs | asset-type · road-route · org-unit | APPROVED A (parent asset) |

Normalized header (REST_AREA dump attr + 3 tầng tuyến + điểm):
...
Giữ shell `/so-ts` live (list+form CRUD BFF). Delta **bắt buộc** = type-profile `REST_AREA` (station) + fill L3 analy stubs (không xóa parent / peer parking artifacts).
...
| GAP-SOTS-COL-01 | Grid schema chung (+ profile peer station) | Profile `REST_AREA`: tên trạm · 3 tầng tuyến · lý trình · loại · xếp loại · chủ SH · chiều dài · DT · cứu hộ · cấp cứu · **ẩn** `type` khi `?type=` · **ẩn** `kmTo` · **ẩn** SL/ĐVT generic · hide-empty cột fill 0 | list |
| GAP-SOTS-FORM-01 | S-ATTR = readonly `<dl>` `dumpSpecs` (trừ type đã editable) | Field editable đủ dump §4 REST_AREA = mẫu Thông tin chung | form |
| GAP-SOTS-REUSE-01 | Form flat + dump dl | Mount S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR · S-GPS — **cấm** fork file form | form |
| GAP-RA-NAME-01 | Rebuild: `name` có thể lệch dump | `name` ← `name_work` · **cấm** IsWeak → đoạn tuyến · trống OK | import + form |
| GAP-RA-SPEC-01 | FE labels thiếu nhiều cột §4 REST_AREA | Label VN khớp dump/mẫu · form Input/Select/Checkbox đủ cột §4 | FE / form |
| GAP-RA-POINT-01 | Form hiện `kmTo` với type chưa profile | Station point: **không** bắt buộc `kmTo` · **ẩn** khi `type=REST_AREA` · **cấm** ép `"0"` | form |
| GAP-RA-ROUTE-01 | STATUS `mfeStdRoute=/so-ts-rest-area` · index chưa Navigate | Live `/so-ts?type=REST_AREA` · alias route **DEFER** Design (tile `t26` deep-link OK) | shell |
| GAP-RA-LEAVE-01 | `window.confirm` dirty / delete (nếu còn) | `LeaveConfirmModal` + `useAlert` / Modal stacked — **cấm** native dialog | form |
| GAP-RA-LOOKUP-01 | `type_work_id` · `categorized_id` · `owner_id` · `office_building_grade_id` · `auxiliary_works_grade_id` · `build_location_id` = text dump | controlHint **Dropdown** LOOKUP_STATIC dump **hoặc** SearchInput nếu SA seed — PO chốt | form |
| GAP-RA-SPLIT-01 | Cùng dump với PARKING | Filter/import **tách** `REST_AREA` vs `PARKING` · **cấm** gộp list | list + import |
| GAP-SOTS-API-DOC | Parent CTX có thể ghi `api/v1/so-ts/…` | Cite live **`api/v1/asset/road-assets`** | docs |

**Không** đổi: Kind B A–D list · full-page form · `LinPageLayout` · `LinCatalogDataGrid` · `LinCatalogListPagination` · `LinCatalogUiSchemaEditorModal` · API prefix `api/v1/asset/road-assets` · BFF `web-bff/api/v1/asset/road-assets` · entity `rmms_road_assets` · SearchInput asset-type / road-route · **cấm ERP.*** · **cấm** invent API mới · map canvas **out of scope** list pack (GIS deep-link `tram-dung-nghi` optional).

## Kind / zones (handoff Design)

| Zone / surface | Pattern | DoD |
|----------------|---------|-----|
| List A | Header | title «Sổ TS — Trạm dừng nghỉ» khi `type=REST_AREA` · **cấm** Thêm mới trên A |
| List B | Toolbar + filter | `LinErpListFilterBar` · SearchTextInput · type SearchInput (prefill `REST_AREA` / ẩn) · route SearchInput · kmFrom/kmTo Text · org tree · Tạo mới · Refresh · SchemaConfig · History · **search must work** · **cấm** nút Tìm riêng (**GAP-FILTER-BAR-01**) |
| List C | `LinCatalogDataGrid` | kéo cột default ON · row menu Xem/Sửa/Copy/Lịch sử · STT · **profile cột REST_AREA** |
...
| Map | none (list pack) | deep-link gis `tram-dung-nghi` optional · **cấm** invent map canvas |
...
## Control hint — grid columns (`type=REST_AREA`)
...
| parking_lot / total_parking_lot | Bãi đỗ | Number | optional / OFF default | thuộc profile PARKING peer · **ẩn** REST_AREA default |
...
| build_location_id | Vị trí mặt cắt | `Dropdown` | | dump `build_location_id` · Bên trái/phải/giữa · **GAP-RA-LOOKUP-01** |
...
| parking_lot | Bãi đỗ xe | `Select` boolean | | dump · optional REST_AREA · profile PARKING peer |
| total_parking_lot | Tổng DT bãi đỗ (m²) | `Number` | | dump · optional |
...
2. Alias route `/so-ts-rest-area` → Navigate `?type=REST_AREA` (như bus-station/spillway) — Design chốt?
3. Grid: cột cứu hộ/cấp cứu/DT — luôn ON theo mẫu list, hay hide-empty theo parent GAP-SOTS-COL-01?
4. Cột `parking_lot` trên form REST_AREA — hiện optional hay defer sang peer `so-ts-parking`?
