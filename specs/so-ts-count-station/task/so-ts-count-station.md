# Team lead — tasks — so-ts-count-station
...
| prefix | `THC-` (keep live GIS · rebuild TX debt → **T-BE-CRUD-01** / T-COUNT-06) |
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-COUNT-NAME/SPEC/POINT/ROUTE/LOOKUP/COORD/LEAVE/GIS/LABEL/TILE · flatten DEFER P2 |
| solution_confirm | **approve** (`task_c54bef0c`) |
| design_confirm | **approve** (`task_0e1b860d`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=COUNT_STATION`** · alias board `/so-ts-count-station` optional redirect · alt B primary-alias-only rejected |
...
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:dbbe8d52c360d78919c4a7bb313973ee20cd193b6b5c1cee4a0da0e7559be87a` |
...
| SA hint map | T-COUNT-01→T-UI-LIST-01 · T-COUNT-02→T-UI-FORM-01/FIELD · T-COUNT-03→T-UI-FORM-01 · T-COUNT-04→T-UI-FORM/LEAVE · T-COUNT-05→T-BE-INIT-01 · T-COUNT-06→T-BE-CRUD-01 · T-COUNT-07→T-UI-LIST/LKP · T-COUNT-08→T-CTX/DOMAIN-MAP |

**Cấm:** implement product code (trừ template task) · e2e · `yarn build` / `start:std` · Step 4b/migration · ERP.* · invent `api/v1/so-ts/*` · fork AssetFormPage · tab legacy · invent GIS slug/prefix FE · start role khác (**GAP-PKT-ROLE-01**).
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=COUNT_STATION` | **giữ** · alias `/so-ts-count-station` optional | **route_confirm=route_a** · **GAP-COUNT-ROUTE-01** |
| Grid profile | generic / thiếu COUNT | Hide `type`/`kmTo`/qty/unit · **ON+hide-empty**: ĐVQL · tên EN · số làn · tốc độ · 3 tầng tuyến tách | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
| Form S-ATTR | `<dl>` readonly dumpSpecs | Editable Dropdown/Number/Text đủ dump §4 · merge dumpSpecs · coord parse → lat/lng | **T-UI-FORM-01** · **GAP-SOTS-FORM-01** · **GAP-COUNT-COORD-01** |
| Point `kmTo` | form hiện / bắt buộc | **Ẩn** + không required khi `COUNT_STATION` · kmFrom **không** required · **cấm** ép `"0"` | **T-UI-FORM-01** · **GAP-COUNT-POINT-01** |
| Name | IsWeak / đoạn tuyến risk | `name` ← `name_vi` · trống OK · **cấm** IsWeak→đoạn | **T-BE-CRUD-01** + form · **GAP-COUNT-NAME-01** |
| LOOKUP | text dumpSpecs | LOOKUP_STATIC init `countAgencies[]` · Dropdown `agency_id` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-COUNT-LOOKUP-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** · **GAP-COUNT-LEAVE-01** |
| Filter context | thiếu file | `so-ts-count-station-filter-bar.md` | **T-UI-FILTER-01** · **T-CTX-01** |
| Prefix IdCode | `THC-` live (GIS) | **giữ `THC-`** · rebuild TX debt align · **cấm** invent FE | **T-BE-CRUD-01** · **GAP-COUNT-GIS-01** |
| Label | lookups thiếu COUNT | UI «Trạm đếm» · BE seed «Trạm đếm xe» giữ · lookups.ts | **T-UI-LKP-01** · **GAP-COUNT-LABEL-01** |
| Form surface | full-page 5 cột | **giữ** `data-form-cols="5"` + header chrome | **T-UI-FORM-01** · **cấm** Slideout/footer Lưu |
| Flatten DB | — | **DEFER P2** | dumpSpecs P1 · migration **none** (**GAP-COUNT-FLAT-01**) |
| dumpSpecLabels | thiếu attr keys | đủ dump §4 COUNT keys | **T-UI-FORM-01** · **GAP-COUNT-SPEC-01** |
| Tile t30 | count cite 377 | list profile sync · summary-by-type | **T-UI-LIST-01** · **GAP-COUNT-TILE-01** |
| DOMAIN-MAP | optional row | `so-ts-count-station` → Asset | **T-CTX-01** · T-COUNT-08 |

**Không đổi:** API prefix `api/v1/asset/road-assets` · BFF proxy · entity `RoadAssetEntity` + DumpSpecs · catalogKind `road-assets` · SearchInput asset-type / road-route / org-unit · pageSize 50/100/200/500 · Kind B shell `/so-ts` · **cấm ERP.*** · map canvas OUT · GIS slug DEFER.
...
| `source.routes` | **`/so-ts?type=COUNT_STATION`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=COUNT_STATION` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=COUNT_STATION` (Dev điền/verify) · alias `http://localhost:9301/so-ts-count-station` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=COUNT_STATION` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `COUNT_STATION`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-count-station-filter-bar.md` (create T-CTX-01) |
...
|--------|------|----------|
| A | `/so-ts?type=COUNT_STATION` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-count-station` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Trạm đếm» khi `type=COUNT_STATION` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-count-station-filter-bar.md` |
| DES-GRID-C0 | listTitle «Danh sách trạm đếm» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · COUNT_STATION column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=COUNT_STATION` + filters |
...
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · **`countAgencies[]`** delta (**GAP-COUNT-LOOKUP-01**) |
| API-07 | GET | `/api/v1/asset/road-assets/summary-by-type` | peer tile t30 · out of write pack · cite **377** |
...
**Attr dump §4:** `agency_id` · `name_vi` · `name_en` · `from_coordinate` · `to_coordinate` · `no_of_lane` · `speed`.
...
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent parent line JSON / CountJson |
...
|---------|------|--------|------|------------|-------|
| T-CTX-01 | Dev | pending | — | `/agent-dev` | sync context + filter-bar.md · DOMAIN-MAP optional row · parent so-ts-type-grid |
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-COUNT-NAME/SPEC/GIS** import + rebuild THC vs TX |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-COUNT-LOOKUP-01** `countAgencies[]` |
...
| T-UI-LIST-01 | Dev | pending | T-BFF-01 | `/agent-dev` | full `tl-grid-task-template` · **GAP-SOTS-COL-01** · LAYOUT-06 · T-COUNT-01/07 |
...
| T-UI-FORM-01 | Dev | pending (delta) | T-UI-LIST-01 · T-BE-INIT-01 | `/agent-dev` | full-page 5 cột · S-ATTR · coord parse · **GAP-SOTS-FORM/POINT/REUSE** · T-COUNT-02/03/04 |
| T-UI-LEAVE-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | LeaveConfirmModal · **GAP-COUNT-LEAVE-01** |
| T-UI-ACT-01 | Dev | pending | T-UI-LIST-01 | `/agent-dev` | inventory below |
| T-UI-LKP-01 | Dev | pending / verify | API-LKP-* | `/agent-dev` | asset-type · road-route · org-unit · lookups COUNT label |
| T-UI-FIELD-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | controlHint 1:1 · dumpSpecs keys §4 |
...
| T-UI-MAP-FORM | — | **n/a** | — | — | packKind=list · map=none · GIS slug DEFER |
...
**new_page.ssot_rereview:** pending → pass|fail
  checklist: tl-grid-ssot · list_parity · filter-bar V1–V5 · form full 5col · LAYOUT-06 · leave Modal · S-ATTR name_vi/coord/agency
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-COUNT-NAME/SPEC/POINT/LOOKUP/COORD/LEAVE/LABEL
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-count-station.md` + parent `so-ts-type-grid.md` · tạo `so-ts-count-station-filter-bar.md` · optional DOMAIN-MAP row `so-ts-count-station`→Asset (T-COUNT-08) · lane web → `dev` · **cấm** re-scan demo
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B A–D+F+H · `shared_grid_example=v1` · type profile `COUNT_STATION`  
**from_solution:** API-01 · FormMode list · T-COUNT-01 · T-COUNT-07  
...
**ssot.reuse:**
  ...
  ui_filter: LinErpListFilterBar · **T-UI-FILTER-01** · `so-ts-count-station-filter-bar.md` — cấm ErpListHeaderFilters / LinListFilterField / nút Tìm
  ui_toolbar: catalogToolbar FULL (refresh·history·editConfig·view·edit·delete·create) · fa-cog
  ui_grid: LinCatalogDataGrid · kéo cột default ON · **COUNT_STATION profile** hide type/kmTo/qty/unit · show agency_id · name_en · no_of_lane · speed · 3 tầng tuyến tách (**GAP-SOTS-COL-01**)
...
**implement.page_shell:**
  ...
  header: title «Sổ TS — Trạm đếm» khi type=COUNT_STATION · titleIconClass + pageId
  filterBarLayout: erp-filter-bar · **cấm** filterMaxWidthPx hack
  listTitle: «Danh sách trạm đếm»
...
**implement.toolbar:** (tl-grid-full-flow §1)
  ...
  form_pair: onAdd→`/so-ts/tao-moi` · onEdit/onView→`/so-ts/sua?id=` · type lock COUNT_STATION
  cấm: thiếu Create/Refresh/Config · History invent API · icon lệch erp-control-icon-map

**implement.grid:**
  component: LinCatalogDataGrid
  columns: buildDynamicGridColumns(schema) + type-profile COUNT_STATION
  visible ON: name · route · routeNamed · routeSegment · kmFrom · agency_id · name_en · no_of_lane · speed · (status/gps optional)
  visible OFF default: type · kmTo · quantity · unitCode
  hide-empty ON: agency_id · name_en · no_of_lane · speed
  dumpSpecs: parse mst_counting_station §4 keys · labels via dumpSpecLabels (**GAP-COUNT-SPEC-01**)
...
**implement.wire:**
  list: GET `?type=COUNT_STATION&search=&route=&kmFrom=&kmTo=&orgUnit=&page=&pageSize=`
  alias: optional `/so-ts-count-station` → `/so-ts?type=COUNT_STATION` (**GAP-COUNT-ROUTE-01**)

**implement.test:**
  listFilters · type lock COUNT_STATION · pageSize · selection → toolbar · column profile COUNT_STATION

**DoD:**
- [ ] Kind B 1 shell · grid visible khi API có data · profile COUNT_STATION (**GAP-SOTS-COL-01**)
- [ ] Toolbar FULL · Grid menu · Config full cột · Grid flow
- [ ] Footer LinCatalogListPagination 50/100/200/500
- [ ] LAYOUT-06 live PASS
- [ ] tile t30 / count cite sync (**GAP-COUNT-TILE-01**)
- [ ] `rg` anti-clone PASS · **yarn build chỉ Dev**

**Fail:** GAP-TL-GRID-TASK-01 · GAP-TL-GRID-FLOW-01 · GAP-P2-LAYOUT-06 · GAP-SOTS-COL-01 · GAP-DEV-DUP-GRID · GAP-COUNT-TILE-01
...
**skills (REQUIRED load trước Write):**
  ...
  - context: `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-count-station-filter-bar.md`
...
**implement.filter:**
  ...
  type lock: luôn `COUNT_STATION` trên deep-link
...
**from_solution:** T-COUNT-02 · T-COUNT-03 · T-COUNT-04  
...
**ssot.reuse:**
  ...
  controls: SearchInput type/route · Dropdown status/source/countAgencies · Number/Text dump §4 · Text name_vi
  http: API-02/03/04 · IdCode readonly prefix `THC-` · init-data API-06

**implement.form:**
  modes: create · edit · view(readOnly **không** disabled xám) · copy(clear id/code → POST · keep COUNT_STATION)
  sections: S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR · S-GPS
  required: type · status · route — name trống OK · kmFrom **không** required · **không** required `kmTo` (**GAP-COUNT-POINT-01**)
  S-ATTR editable dump §4: agency_id Dropdown LOOKUP · name_vi · name_en · no_of_lane · speed · from_coordinate/to_coordinate (derived → lat/lng · dumpSpecs giữ · to_coordinate optional hide) · merge dumpSpecs (**GAP-SOTS-FORM-01** · **GAP-COUNT-LOOKUP-01** · **GAP-COUNT-SPEC-01** · **GAP-COUNT-COORD-01**)
  S-NAME: `name` ← `name_vi` · trống OK · mirror dumpSpecs (**GAP-COUNT-NAME-01**)
  S-GPS: lat/lng từ parse coord ưu tiên · **cấm** drop dumpSpecs keys
  kmFrom: **cấm** ép `"0"` khi dump null (**GAP-COUNT-SPEC-01** · **GAP-COUNT-POINT-01**)
  IdCode: prefix **`THC-`** keep · **cấm** invent FE / GIS slug (**GAP-COUNT-GIS-01**)
  **Không mount:** kmTo · quantity/unit · S-LOC-RANGE · tab Chi tiết/Bảo trì/Tệp/Lịch sử/Lưu lượng xe legacy
  cấm: invent History path · flatten Schema_* · ERP.* · Modal form

**implement.init_data:**
  options: GET `/asset/road-assets/init-data` only — statuses · sources · **countAgencies** · **cấm** KIND_LABEL hardcode (**GAP-DEV-DROPDOWN-HARDCODE-01**)
...
- [ ] S-ATTR editable · coord parse · dumpSpecs merge
- [ ] kmTo ẩn · agency Dropdown từ countAgencies
- [ ] reuse AssetFormPage S-* — **cấm** fork

**Fail:** GAP-SOTS-FORM-01 · GAP-SOTS-REUSE-01 · GAP-COUNT-POINT-01 · GAP-COUNT-LOOKUP-01 · GAP-COUNT-COORD-01 · GAP-COUNT-GIS-01 · GAP-P2-FORM-GRID-05 · GAP-LIST-FIELD-01
...
**DoD:** dirty → `LeaveConfirmModal` · full `useFormLeaveGuard` · **cấm** `window.confirm`/`alert` · **GAP-COUNT-LEAVE-01** / **GAP-P2-36** / **GAP-DEV-LEAVE-01** / **GAP-TL-LEAVE-01**
...
| +Tạo mới | toolbar | navigate `/so-ts/tao-moi` type=COUNT_STATION | POST API-03 |
...
| Alias board | `/so-ts-count-station` | optional redirect → `?type=COUNT_STATION` | — |
...
**status:** pending / verify  
**devSlash:** `/agent-dev`  
**DoD:** filter+form type/route/org = SearchInput Integration · agency_id = LOOKUP_STATIC init-data countAgencies (**không** SearchInput master P1) · lookups.ts `COUNT_STATION` label UI «Trạm đếm» · **GAP-LIST-LKP-01** / **GAP-COUNT-LOOKUP-01** / **GAP-COUNT-LABEL-01**
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** controlHint Design/DA 1:1 · SA field map · dumpSpecs keys `agency_id` · `name_vi` · `name_en` · `from_coordinate` · `to_coordinate` · `no_of_lane` · `speed` · **GAP-LIST-FIELD-01** · **GAP-COUNT-SPEC-01**
...
**layer:** api  
**devSlash:** `/agent-dev`  
**from_solution:** API-01…05 · T-COUNT-06 · **không** migration mới · dumpSpecs P1  
...
**ssot.reuse:**
  ...
  import: RoadAssetCatalogHandler · RebuildGovVn `FindOfficialName incl. name_vi` / `IsWeakAssetName` · IdCode prefix **THC** align (vs TX debt)
  gates: tz_na · xco_get_only · share_tenant

**implement.wire:**
  API-01…05: verify live · **cấm** invent `api/v1/so-ts/*` · ERP.*
  GAP-COUNT-NAME-01: rebuild CSV `name`←`name_vi` · **cấm** IsWeak reject «Trạm đếm» · **cấm** fallback đoạn tuyến khi name_vi hợp lệ
  GAP-COUNT-SPEC-01: giữ mst_counting_station §4 attrs trong dumpSpecs · km trống khi dump null · FE dumpSpecLabels đủ key
  GAP-COUNT-GIS-01: prefix **giữ `THC-`** · rebuild TX→THC debt · GIS slug **DEFER** · **cấm** invent FE
  XCO: GET by id AllowedCompanyIds
  POST/PUT: accept dumpSpecs merge · **không** required kmTo for COUNT_STATION

**DoD:**
- [ ] API-01…05 verify
- [ ] name/import + prefix THC fix (**GAP-COUNT-NAME-01** · **GAP-COUNT-SPEC-01** · **GAP-COUNT-GIS-01**)
- [ ] no-parent-json · migration **none**

---

### T-BE-INIT-01 — init-data COUNT LOOKUP

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**from_solution:** API-06 · T-COUNT-05  
**DoD:** `RoadAssetInitDataDto.countAgencies: RoadAssetLookupOption[]` · options = dump LOOKUP_STATIC / seed agency_id · **cấm** invent agency master SearchInput P1 · **cấm** FE hardcode · **GAP-COUNT-LOOKUP-01**
...
| T-QA-FORM-01 | từng field required · UI value = body · dumpSpecs COUNT attrs · kmTo ẩn · coord→lat/lng · agency Dropdown |
| T-QA-FILTER-01 | V1–V5 + fields 1:1 `so-ts-count-station-filter-bar.md` · 🔍 work · fail ErpListHeaderFilters / export trên bar |
...
| ID | Note |
|----|------|
| Flatten Schema_* | P2 · dumpSpecs P1 (**GAP-COUNT-FLAT-01**) |
| Auth NuGet | DEFER |
| GIS slug layer | DEFER (**GAP-COUNT-GIS-01**) · keep prefix `THC-` |
| agency LOOKUP master SearchInput | P2 |
| Excel import wizard UI | OUT |
| Kind F map canvas | **cấm** |
| invent History API | **cấm** |
| invent `api/v1/so-ts/*` | **cấm** |
| invent prefix FE | **cấm** · keep `THC-` |
| tab Lưu lượng xe / legacy | **cấm** |
...
| focus | COL profile COUNT · S-ATTR editable · name←name_vi · kmTo ẩn · countAgencies LOOKUP · coord parse · LeaveConfirm · filter-bar · dumpSpecs P1 · prefix THC · lookups label |
| mfeStdRoute | `/so-ts?type=COUNT_STATION` |
...
| T-UI-FORM-01 full-page 5 cột + S-ATTR name_vi/coord/agency (không Slideout) | ✅ |
| T-UI-LEAVE-01 | ✅ |
| tl-list-shell-height DoD | ✅ |
| init-data countAgencies · cấm hardcode dropdown | ✅ |
| Source assignment · API cite SA · **cấm** invent | ✅ |
| SA T-COUNT-01…08 mapped → T-* matrix | ✅ |
...
---
<!-- Version meta: skillId=agent-team-lead skillVersion=2026.08.19.04 schemaVersion=2 workflowVersion=2026.09.01.02 rulesVersion=2026.09.01.1 versionGate=rechecked taskId=task_11f55e31 contentHashPriorDataAnaly=sha256:dbbe8d52c360d78919c4a7bb313973ee20cd193b6b5c1cee4a0da0e7559be87a route_confirm=route_a -->
