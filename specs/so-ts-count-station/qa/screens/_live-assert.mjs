/**
 * Live DOM assert + DTM filter-bar overflow for so-ts-count-station.
...
const outDir =
  "D:\\AI-QLBD\\Linm.RMMS.Data\\specs\\so-ts-count-station\\qa\\screens";
mkdirSync(outDir, { recursive: true });
const listUrl = "http://localhost:9301/so-ts?type=COUNT_STATION";
const formUrl = "http://localhost:9301/so-ts/tao-moi?type=COUNT_STATION";
const TEST_ID = "rmms-so-ts-count-station-list";
...
const list = await page.evaluate((tid) => {
  ...
  const headers = Array.from(document.querySelectorAll("th, [role='columnheader']"))
    ...
    .filter(Boolean);
  return {
    titleCount: /Danh sách trạm đếm/i.test(body),
    headerCount: /Sổ TS — Trạm đếm/i.test(body),
...
await page.waitForSelector('[data-testid="asset-count-station-attr"]', {
  timeout: 15000,
});
await new Promise((r) => setTimeout(r, 1200));
...
const form = await page.evaluate(() => {
  ...
  return {
    cols5: cols,
    countAttr: !!document.querySelector('[data-testid="asset-count-station-attr"]'),
    agency: !!document.querySelector('[data-testid="asset-field-agency_id"]'),
    nameEn: !!document.querySelector('[data-testid="asset-field-name_en"]'),
    noOfLane: !!document.querySelector('[data-testid="asset-field-no_of_lane"]'),
    speed: !!document.querySelector('[data-testid="asset-field-speed"]') || /Tốc độ|speed/i.test(body),
    stationNameLabel: /Tên trạm|Tên \(VI\)/i.test(body),
    kmToVisible,
