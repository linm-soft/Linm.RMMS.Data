...
async function fatalUi(p) {
  ...
  if (await overlay.count()) {
    ...
    const featureOk = await p
      .locator(
        `[data-feature='${feature}'], [data-zone='WORK-L'], #loginUser`,
      )
      .count();
...
async function dumpZones() {
  return page.evaluate(() => {
    const root = document.querySelector(
      '[data-feature="web-rmms-work"], [data-feature="web-rmms-home"], [data-feature="web-rmms-shell"]',
    );
    ...
    const ids = ["loginUser", "loginPass", "loginSubmit", "guestLogin", "WORK-ROOT"].filter(
      (id) => document.getElementById(id),
    );
    const text = (document.body?.innerText || "").slice(0, 1600).replace(/\s+/g, " ");
    const hasFab = !!document.querySelector('[data-field="fab"], [data-control="FAB"]');
    const cardCount = document.querySelectorAll('[data-field="list.card"]').length;
    const chipCount = document.querySelectorAll('[data-control="Chip"]').length;
    return {
...
try {
  // S0 — guest WORK-L shell (no FAB · search/chips/hub present · live or empty/error OK)
  pageErrors.length = 0;
  await clearSession();
  await page.goto(base + "/web-rmms-work", {
    waitUntil: "domcontentloaded",
    timeout: 60000,
  });
  await captureCurrent(
    "S0",
    "[data-feature='web-rmms-work'] [data-field='search'], [data-feature='web-rmms-work']#WORK-ROOT, #WORK-ROOT [data-field='search']",
  );
  ...
  // S1 — staff WORK-L Live after LoginSheet · chip filter (FILTER-P1 · distinct vs S0)
  pageErrors.length = 0;
  await loginViaSheet();
  await page.goto(base + "/web-rmms-work", {
    waitUntil: "domcontentloaded",
  ...
  await page.waitForSelector(
    "[data-feature='web-rmms-work'] [data-field='search'], [data-field='hub.estimate'], [data-zone='WORK-L']",
    { timeout: 25000 },
  );
  await page.waitForTimeout(1200);
  await dismissOverlay(page);
  const statusChip = page
    .locator('[data-field="filter.status"] [data-control="Chip"]')
    .filter({ hasText: /Đang thực hiện/i })
    .first();
...
writeFileSync(join(outDir, "_capture_work.result.json"), JSON.stringify(summary, null, 2));
