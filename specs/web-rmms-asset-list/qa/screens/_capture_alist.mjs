...
const featureUrl = base + "/web-rmms-asset-list";
...
async function dumpZones() {
  return page.evaluate(() => {
    const root = document.querySelector(
      '[data-feature="web-rmms-asset-list"], [data-feature="web-rmms-asset-hub"]',
    );
    ...
    const rowCount = document.querySelectorAll('[data-zone="AL-06"]').length;
...
try {
  ...
  // S0 — Asset list AL-* Live list/empty
  {
    ...
    await captureCurrent(
      "S0",
      '[data-feature="web-rmms-asset-list"] #pageTitle, [data-feature="web-rmms-asset-list"] #alGuestGate',
    );
  }

  // S1 — peer Hub entry (#tileList)
  {
    ...
    await captureCurrent(
      "S1",
      '[data-feature="web-rmms-asset-hub"] #tileList, [data-feature="web-rmms-asset-hub"] #walletTitle',
    );
  }

  // QA-20 — Hub tileList → list → first row detail (JWT kept)
  {
    const entry = page.locator('#tileList, [data-route="/web-rmms-asset-list"]').first();
    if (await entry.count()) {
      await entry.click();
      await page.waitForSelector(
        '[data-feature="web-rmms-asset-list"] #pageTitle, [data-feature="web-rmms-asset-list"] #AL-04, [data-feature="web-rmms-asset-list"] #AL-05',
        { timeout: 25000 },
      );
      await page.waitForTimeout(800);
      const row = page.locator('[data-zone="AL-06"]').first();
      if (await row.count()) {
        await row.click();
        await captureCurrent(
          "QA-20",
          '[data-feature="web-rmms-asset-list"] #AL-11, [data-feature="web-rmms-asset-list"] #AL-12, [data-feature="web-rmms-asset-list"] #AL-13',
        );
      } else {
        await captureCurrent(
          "QA-20",
          '[data-feature="web-rmms-asset-list"] #pageTitle, [data-feature="web-rmms-asset-list"] #AL-05',
        );
      }
    } else {
      ...
      await captureCurrent(
        "QA-20",
        '[data-feature="web-rmms-asset-list"] #pageTitle',
      );
    }
  }
} catch (err) {
  ...
  JSON.stringify(
    {
      ...
      method:
        "capture_alist · MFE /login · phone 430 · S0 list · S1 Hub #tileList · QA-20 click→list/detail · no kill worker",
      steps: results,
      ok,
    },
    null,
