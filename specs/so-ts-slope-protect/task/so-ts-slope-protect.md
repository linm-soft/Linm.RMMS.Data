# Team lead — tasks — so-ts-slope-protect
...
| cluster | `linear_protect` · tile `t12` |
| dump | `tbl_slope` |
| prefix | **`MD-`** (GIS short **`MD`**) |
| unit | `KET_CAU` · CSV `10547` |
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-SLOPE-LOOKUP/NAME/PREFIX/RANGE/ROUTE/PEER/SPEC/FLAT · flatten DEFER P2 |
| solution_confirm | **approve** (`task_6fb60810`) |
| design_confirm | **approve** (`task_ddb66403`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=SLOPE_PROTECT`** · alias board `/so-ts-slope-protect` optional redirect · alt B primary-alias-only rejected |
...
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:52501076e559261162c4741e46e0826cab2059143221abe3e32b1ca279253294` |
...
| saTaskId | `task_6fb60810` |
| updatedAt | `2026-09-02T01:42:00.000Z` |
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=SLOPE_PROTECT` | **giữ** · alias `/so-ts-slope-protect` optional | **route_confirm=route_a** · **GAP-SLOPE-ROUTE-01** |
| Grid profile | generic / thiếu SLOPE_PROTECT | **ON**: kiểu BV · 3 tầng · kmFrom · kmTo · phân loại · dài BV · cao TB · **hide-empty** vị trí/địa danh · **ẩn** type/ảnh · primary=`protection_type_id` | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
| Form S-ATTR | `<dl>` readonly dumpSpecs | Editable Dropdown/Number/Text đủ dump · merge dumpSpecs | **T-UI-FORM-01** · **GAP-SOTS-FORM-01** |
| Range | risk POINT / ép `"0"` | **S-LOC-RANGE** km* + **4 XY dumpSpecs** · **cấm** ép `"0"` · **không** S-LOC-POINT | **T-UI-FORM-01** · **GAP-SLOPE-RANGE-01** |
| Name | IsWeak / đoạn risk | `name` **optional** · list primary = `protection_type_id` · **cấm** IsWeak→đoạn | **T-BE-CRUD-01** + form · **GAP-SLOPE-NAME-01** |
| Prefix | generic/`TS-` risk | IdCode create/import **`MD-`** · GIS short **`MD`** | **T-BE-CRUD-01** · **GAP-SLOPE-PREFIX-01** |
| LOOKUP | text dumpSpecs | LOOKUP_STATIC init `protectionTypes[]` · `slopeClassifications[]` · `locationOptions[]` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-SLOPE-LOOKUP-01** |
| dumpSpecLabels | thiếu 5 key SLOPE_PROTECT | Label VN đủ 5 key + 4 XY · địa danh hide-empty | **T-UI-FORM-01** · **GAP-SLOPE-SPEC-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** |
| Filter context | thiếu file | `so-ts-slope-protect-filter-bar.md` | **T-UI-FILTER-01** · **T-CTX-01** |
| Form surface | full-page 5 cột | **giữ** `data-form-cols="5"` + header chrome | **T-UI-FORM-01** · **cấm** Slideout/footer Lưu |
| Flatten DB | — | **DEFER P2** (GAP-SLOPE-FLAT-01) | dumpSpecs P1 · migration **none** |
| Peer scope | GIS `ta-luy` gộp RETAINING | page **SLOPE_PROTECT only** · RETAINING riêng · layer `mai-doc` | **GAP-SLOPE-PEER-01** |
...
| `source.routes` | **`/so-ts?type=SLOPE_PROTECT`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=SLOPE_PROTECT` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=SLOPE_PROTECT` (Dev điền/verify) · alias `http://localhost:9301/so-ts-slope-protect` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=SLOPE_PROTECT` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `SLOPE_PROTECT`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-slope-protect-filter-bar.md` (create T-CTX-01) |
...
|--------|------|----------|
| A | `/so-ts?type=SLOPE_PROTECT` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-slope-protect` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Bảo vệ mái dốc» khi `type=SLOPE_PROTECT` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-slope-protect-filter-bar.md` · **cấm** nút Tìm riêng |
| DES-GRID-C0 | listTitle «Danh sách bảo vệ mái dốc» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · SLOPE_PROTECT column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=SLOPE_PROTECT` + filters |
| API-02 | GET | `/api/v1/asset/road-assets/{id}` | detail · XCO get_only |
| API-03 | POST | `/api/v1/asset/road-assets` | create · dumpSpecs merge · prefix **`MD-`** |
| API-04 | PUT | `/api/v1/asset/road-assets/{id}` | update · dumpSpecs merge |
| API-05 | DELETE | `/api/v1/asset/road-assets/{id}` | soft |
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · **`protectionTypes[]`** · **`slopeClassifications[]`** · **`locationOptions[]`** |
| API-07 | GET | `/api/v1/asset/road-assets/summary-by-type` | peer tile t12 · out of write pack |
| API-LKP-01 | GET | Integration `asset-types` | filter type · form lock SLOPE_PROTECT |
...
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent SlopeJson / Schema_* P1 |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-SLOPE-NAME/PREFIX** · import + rebuild · dumpSpecs P1 |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-SLOPE-LOOKUP-01** protectionTypes/slopeClassifications/locationOptions |
...
| T-UI-FORM-01 | Dev | pending (delta) | T-UI-LIST-01 · T-BE-INIT-01 | `/agent-dev` | full-page 5 cột · S-ATTR · **S-LOC-RANGE** · **GAP-SOTS-FORM/REUSE** · **GAP-SLOPE-RANGE/SPEC** |
...
|-------|-----------|-------|
| T-MD-01 | T-UI-LIST-01 | SLOPE_PROTECT grid profile · hide-empty · primary `protection_type_id` |
| T-MD-02 | T-UI-FORM-01 | S-ATTR editable · dumpSpecLabels 5 key · merge dumpSpecs |
| T-MD-03 | T-UI-FORM-01 | S-LOC-RANGE · 4 XY dumpSpecs · cấm ép `"0"` |
| T-MD-04 | T-BE-CRUD-01 + form | name optional · list primary kiểu BV · cấm IsWeak |
| T-MD-05 | T-BE-CRUD-01 | DefaultCodePrefix **`MD-`** · GIS MD · unit KET_CAU |
| T-MD-06 | T-BE-INIT-01 | LOOKUP_STATIC protectionTypes/slopeClassifications/locationOptions |
| T-MD-07 | T-UI-LEAVE-01 | LeaveConfirmModal |
| T-MD-08 | T-CTX-01 / route | alias board-only optional |
| T-MD-09 | T-CTX-01 | DOMAIN-MAP optional row · context phase |
| T-MD-10 | this file | pack + gates · tile t12 · CSV 10547 · dumpSpecLabels |
...
**new_page.ssot_rereview:** pending → pass|fail
  ...
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-SLOPE-LOOKUP/NAME/PREFIX/RANGE/PEER/SPEC
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-slope-protect.md` + parent `so-ts-type-grid.md` · tạo `so-ts-slope-protect-filter-bar.md` · lane web → `dev` · **cấm** re-scan demo

---

### T-BE-CRUD-01 — Road-assets CRUD delta (SLOPE_PROTECT)

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**DoD:**
- API-01…05 verify · type filter `SLOPE_PROTECT`
- Persist scalars + **dumpSpecs P1** · **cấm** Schema_* / flatten P1
- `name` optional · **cấm** IsWeak→đoạn (**GAP-SLOPE-NAME-01**)
- IdCode create/import prefix **`MD-`** · GIS short **`MD`** · unit `KET_CAU` · CSV 10547 (**GAP-SLOPE-PREFIX-01**)
- import `RoadAssetCatalogHandler` / GIS `mai-doc` align
...
**deps:** T-BE-CRUD-01  
**devSlash:** `/agent-dev`  
**DoD:** init-data trả `protectionTypes[]` · `slopeClassifications[]` · `locationOptions[]` · FE Dropdown LOOKUP_STATIC bind protection_type_id / slope_classification_id / location_id (**GAP-SLOPE-LOOKUP-01**)
...
### T-UI-LIST-01 — Grid SLOPE_PROTECT
...
- Column profile SLOPE_PROTECT: **ON** kiểu BV (`protection_type_id` primary) · 3 tầng tuyến · kmFrom · kmTo · phân loại · dài BV · cao TB
- **Ẩn** type · ảnh
- **hide-empty** vị trí cắt ngang · xã/commune*
- Header title khi `type=SLOPE_PROTECT`
...
**devSlash:** `/agent-dev`  
**DoD:** load `so-ts-slope-protect-filter-bar.md` trước Write · `LinErpListFilterBar` · fields: search · type (prefill/ẩn SLOPE_PROTECT) · route 3 tầng · kmFrom/kmTo filter · orgTree · **cấm** nút Tìm riêng · V1–V5
...
- **S-LOC-RANGE:** kmFrom/kmTo + **4 XY** (`lat*`/`lng*` đầu/cuối) dumpSpecs · **cấm** ép `"0"` · **không** S-LOC-POINT (**GAP-SLOPE-RANGE-01**)
- S-NAME: `name` optional · trống OK · **cấm** IsWeak
- S-ATTR editable: `protection_type_id` · `slope_classification_id` · `actual_protected` · `average_height` · `location_id` · `province*` · `commune*` · dumpSpecLabels VN đủ 5 key (**GAP-SLOPE-SPEC-01**)
- Dropdown LOOKUP_STATIC protectionTypes/slopeClassifications/locationOptions · Number/Text đúng controlHint
...
| type | Loại TS | SearchInput asset-type | scalar lock SLOPE_PROTECT |
...
| handoff compact | `specs/so-ts-slope-protect/handoff/team_lead-compact.md` |
| mfe cwd | `D:/AI-QLBD/MFE-Source/Linm.Web.RMMS.Asset` |
| be | `D:/AI-QLBD/Linm.RMMS.WebService` |
| primary URL | `http://localhost:9301/so-ts?type=SLOPE_PROTECT` |
...
- task: `D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-slope-protect/task/so-ts-slope-protect.md`
- compact: `D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-slope-protect/handoff/team_lead-compact.md`
