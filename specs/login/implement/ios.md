...
| xcodegen | PASS |
| Kit | `LinmTextField` + `LinmPersonGlyph` · `LinmSecureTextField` · `LinmPrimaryButton` · `LinmToast`/`LinmToastHost` · `LinmNetSignalMark` display |
...
- Cold start không access token → `LoginView` ngoài `TabView`. Có token → Home placeholder.
