...
const SEED = () => ([
  {id:"AC-101",assetClass:"Hộ lan",score:0.92,status:"Draft",lat:20.12381,lng:105.85042,routeId:"QL1",routeLabel:"QL.1 · Km287",sectionId:"SEC-QL1-286-316",patrolTripId:"PTR-QL1-SIM-01",bbox:"[120,80,220,180]",modelVersion:"gpt-4o-vision",engine:"P1",nearbyRisk:false,nearbyOf:null,note:"Seed QL.1",assetCode:null,detectedAt:"2026-08-12T07:00:00",imageFileId:"a1111111-1111-4111-8111-111111111101",missFlag:false,expectedAssetId:null,missWindowMin:null,incidentDraftId:null,x:22,y:28},
  {id:"AC-102",assetClass:"Đèn chiếu sáng",score:0.87,status:"Draft",lat:20.07715,lng:105.8552,routeId:"QL1",routeLabel:"QL.1 · Km292",sectionId:"SEC-QL1-286-316",patrolTripId:"PTR-QL1-SIM-01",bbox:"[90,100,300,160]",modelVersion:"gpt-4o-vision",engine:"P1",nearbyRisk:false,nearbyOf:null,note:"Seed QL.1",assetCode:null,detectedAt:"2026-08-12T08:00:00",imageFileId:"a1111111-1111-4111-8111-111111111102",missFlag:false,expectedAssetId:null,missWindowMin:null,incidentDraftId:null,x:32,y:40},
  {id:"AC-103",assetClass:"Cột Km",score:0.81,status:"Draft",lat:19.98937,lng:105.84515,routeId:"QL1",routeLabel:"QL.1 · Km302",sectionId:"SEC-QL1-286-316",patrolTripId:"PTR-QL1-SIM-02",bbox:"[150,70,190,200]",modelVersion:"gpt-4o-vision",engine:"P1",nearbyRisk:false,nearbyOf:null,note:"Seed QL.1",assetCode:null,detectedAt:"2026-08-12T09:00:00",imageFileId:"a1111111-1111-4111-8111-111111111103",missFlag:false,expectedAssetId:null,missWindowMin:null,incidentDraftId:null,x:42,y:48},
  {id:"AC-104",assetClass:"Hộ lan",score:0.78,status:"Draft",lat:20.12389,lng:105.8505,routeId:"QL1",routeLabel:"QL.1 · Km287",sectionId:"SEC-QL1-286-316",patrolTripId:"PTR-QL1-SIM-02",bbox:"[118,82,218,178]",modelVersion:"gpt-4o-vision",engine:"P1",nearbyRisk:true,nearbyOf:"AC-101",note:"Cùng class <25m vs AC-101",assetCode:null,detectedAt:"2026-08-12T10:00:00",imageFileId:"a1111111-1111-4111-8111-111111111104",missFlag:false,expectedAssetId:null,missWindowMin:null,incidentDraftId:null,x:24,y:30},
  {id:"AC-MISS-01",assetClass:"Hộ lan",score:0,status:"Draft",lat:20.12857,lng:105.85542,routeId:"QL1",routeLabel:"QL.1 · Km286",sectionId:"SEC-QL1-286-316",patrolTripId:"PTR-QL1-SIM-03",bbox:"",modelVersion:"gpt-4o-vision",engine:"P1",nearbyRisk:false,nearbyOf:null,note:"Miss reconcile · GPS có TS · frame trống",assetCode:null,detectedAt:"2026-09-06T10:00:00",imageFileId:null,missFlag:true,expectedAssetId:"TS-QL1-HL-001",missWindowMin:15,incidentDraftId:null,x:18,y:24}
]);
let rows = SEED();
let selected = null;
let formMode = "view";
...
function filtered(){
  ...
  const missOnly=document.getElementById("fMissOnly").checked;
  return rows.filter(r=>{
    if(filterChecked.size && !filterChecked.has(r.id)) return false;
    if(missOnly && !r.missFlag) return false;
    ...
    if(!q) return true;
    const bag=[r.id,r.assetClass,r.routeLabel,r.patrolTripId,r.sectionId,r.assetCode,r.note,r.modelVersion,r.engine,r.expectedAssetId,r.imageFileId].join(" ").toLowerCase();
    return bag.includes(q);
  });
}
function pinClass(r){ if(r.missFlag) return "miss"; if(r.status==="Confirmed") return "ok"; return "ai"; }
function renderMap(list){
  ...
  canvas.innerHTML=EXISTING.map(e=>'<button type="button" class="pin exist" style="left:'+e.x+'%;top:'+e.y+'%" title="'+e.id+'">'+e.id.replace("TS-QL1-","")+"</button>").join("")
    +list.map(r=>'<button type="button" class="pin '+pinClass(r)+'" data-pin="'+r.id+'" style="left:'+r.x+'%;top:'+r.y+'%">'+r.id+"</button>").join("");
}
function render(){
  const list=filtered();
  tbody.innerHTML=list.map((r,i)=>`<tr data-code="${r.id}" class="${selected===r.id?"sel":""}">
    ...
    <td>${r.missFlag?'<span class="miss">mất</span>':"—"}</td>
    <td>${r.detectedAt.replace("T"," ").slice(0,16)}</td><td>${r.assetCode||"—"}</td>
    <td>${r.imageFileId?(r.imageFileId.slice(0,8)+"…"):"—"}</td>
    <td class="num" data-des-id="DES-GRID-C3"><details class="menu"><summary>⋮</summary><div class="menu-panel">
      ...
      <button type="button" data-menu="miss" class="miss">Mất?</button>
      <button type="button" data-menu="history">Lịch sử</button>
    </div></details></td></tr>`).join("")||'<tr><td colspan="15" style="text-align:center;color:#94a3b8;padding:24px">Không có dữ liệu</td></tr>';
...
function setFormRo(ro){
  ["fAssetClass","fScore","fEngine","fLat","fLng","fRouteId","fRouteLabel","fSection","fTrip","fBbox","fNote","fDetected","fExpectedAsset","fMissWindow","fImageFile"].forEach(id=>{
    const el=document.getElementById(id);if(!el)return;
    if(el.type==="file"){ el.disabled=!!ro; return; }
...
function fillForm(row){
  ...
  document.getElementById("fExpectedAsset").value=row.expectedAssetId||"";
  document.getElementById("fMissWindow").value=row.missWindowMin||"";
  document.getElementById("fIncidentId").value=row.incidentDraftId||"";
  document.getElementById("nearWarn").hidden=!row.nearbyRisk;
  const fm=document.getElementById("frameMock");
  if(row.missFlag || !row.bbox){ fm.classList.add("empty"); document.getElementById("frameLabel").textContent="Frame trống / 0 detect (miss)"; }
  else { fm.classList.remove("empty"); document.getElementById("frameLabel").textContent="Frame preview resign · "+(row.imageFileId||"no-file"); }
}
function renderFooter(){
  const foot=document.getElementById("formFooter");
  if(formMode==="view"){
    foot.innerHTML='<button type="button" class="tb" data-slide="close">Đóng</button><button type="button" class="tb" data-slide="edit"><i class="fas fa-pen"></i> Sửa</button><button type="button" class="tb" data-slide="copy"><i class="fas fa-copy"></i> Sao chép</button><button type="button" class="tb primary" data-slide="confirm">Confirm</button><button type="button" class="tb" data-slide="dismiss">Dismiss</button><button type="button" class="tb miss" data-slide="miss">Mất?</button>';
  } else {
...
function openForm(mode,row){
  ...
  document.getElementById("formHint").textContent=mode==="view"?"View = readOnly · footer only · LeaveConfirmModal":"Edit/Create/Copy · LeaveConfirmModal khi dirty · footer Hủy/Lưu only";
  fillForm(row);setFormRo(mode==="view");
  if(mode==="create"||mode==="copy"){document.getElementById("fId").value="AC-NEW";document.getElementById("fCandStatus").value="Draft";document.getElementById("fAssetCode").value="";document.getElementById("fImageFileId").value="";document.getElementById("fIncidentId").value="";}
  renderFooter();document.getElementById("slide-backdrop").classList.add("on");document.getElementById("slideout").classList.add("on");
}
function askLeave(){
  return new Promise((resolve)=>{ leaveResolve=resolve; openModal("modal-leave"); });
}
...
function openConfirm(id){
  const r=rowById(id);if(!r||r.status!=="Draft"||r.missFlag){toast(r&&r.missFlag?"Miss → dùng Mất? / Reconcile":"Chỉ Confirm Draft");return;}
...
function openMiss(id){
  const r=id?rowById(id):null;
  pendingMissId=id||null;
  document.getElementById("missText").textContent=r
    ? ("Reconcile mất · "+r.id+" · GPS kỳ vọng vs frame trống · cấm YOLO «mất»")
    : "Reconcile mất TS · chọn expected Asset · cửa sổ N phút · gim Incident draft";
  document.getElementById("missExpected").value=(r&&r.expectedAssetId)||"TS-QL1-HL-001";
  document.getElementById("missWindow").value=(r&&r.missWindowMin)||15;
  document.getElementById("missNote").value="";
  openModal("modal-miss");
}
tbody.addEventListener("click",(e)=>{
  ...
  if(e.target.closest("[data-menu]")){
    ...
    else if(act==="miss")openMiss(code);
...
["fRoute","fClass","fStatus","fFrom","fTo","fMissOnly"].forEach(id=>document.getElementById(id).addEventListener("change",render));
...
document.querySelector('[data-action="reset-seed"]').addEventListener("click",()=>{rows=SEED();filterChecked=new Set(filterVals());selected=null;render();toast("Reset seed 5 candidates (+miss)");});
document.querySelector('[data-action="sim-frame"]').addEventListener("click",()=>{
  const id="AC-"+(200+rows.length);
  const fid="b"+String(200+rows.length).padStart(7,"0")+"-1111-4111-8111-1111111111ff";
  rows.unshift({id,assetClass:"Camera ITS",score:0.86,status:"Draft",lat:19.85,lng:105.81,routeId:"QL1",routeLabel:"QL.1 · sim",sectionId:"SEC-QL1-286-316",patrolTripId:"PTR-SIM",bbox:"[100,100,200,200]",modelVersion:"gpt-4o-vision",engine:"P1",nearbyRisk:false,nearbyOf:null,note:"Giả lập frame",assetCode:null,detectedAt:new Date().toISOString().slice(0,16),imageFileId:fid,missFlag:false,expectedAssetId:null,missWindowMin:null,incidentDraftId:null,x:60,y:55});
  filterChecked.add(id);selected=id;render();toast("Detect stub → "+id+" · fileId="+fid.slice(0,8));
});
...
document.querySelector('[data-action="reconcile-miss"]').addEventListener("click",()=>openMiss(selected));
document.querySelector('[data-action="bar-search"]').addEventListener("click",()=>{render();toast("Tìm (bar 🔍)");});
document.querySelector('[data-action="clear-filter"]').addEventListener("click",()=>{
  ...
  document.getElementById("fMissOnly").checked=false;
...
document.getElementById("btnLeaveStay").addEventListener("click",()=>{closeModal("modal-leave");if(leaveResolve)leaveResolve(false);leaveResolve=null;});
document.getElementById("btnLeaveGo").addEventListener("click",()=>{closeModal("modal-leave");if(leaveResolve)leaveResolve(true);leaveResolve=null;});
...
document.getElementById("fImageFile").addEventListener("change",(e)=>{
  ...
  const guid="c"+Math.random().toString(16).slice(2,10)+"-1111-4111-8111-"+Math.random().toString(16).slice(2,14);
  document.getElementById("fImageFileId").value=guid;
  document.getElementById("frameLabel").textContent="Uploaded · resign preview · "+f.name;
  document.getElementById("frameMock").classList.remove("empty");
  formDirty=true;document.getElementById("dirtyBadge").hidden=false;
});
document.getElementById("formFooter").addEventListener("click",(e)=>{
  ...
  if(a==="save"){
    ...
    selected=payload.id;formDirty=false;closeForm(true);render();toast("Đã lưu Draft · "+payload.id+(payload.imageFileId?" · fileId":""));
  }
});
...
document.getElementById("btnMissOk").addEventListener("click",()=>{
  const exp=document.getElementById("missExpected").value.trim();
  const win=Number(document.getElementById("missWindow").value);
  if(!exp){toast("expectedAssetId *");return;}
  if(!win||win<1){toast("missWindowMin *");return;}
  const incidentId="INC-DRAFT-"+String(Math.floor(Math.random()*9000)+1000);
  let r=pendingMissId?rowById(pendingMissId):null;
  if(!r){
    r={id:"AC-MISS-"+(100+rows.length),assetClass:"Hộ lan",score:0,status:"Draft",lat:20.12,lng:105.85,routeId:"QL1",routeLabel:"QL.1 · miss",sectionId:"",patrolTripId:"",bbox:"",modelVersion:"gpt-4o-vision",engine:"P1",nearbyRisk:false,nearbyOf:null,note:document.getElementById("missNote").value||"Miss reconcile",assetCode:null,detectedAt:new Date().toISOString().slice(0,16),imageFileId:null,missFlag:true,expectedAssetId:exp,missWindowMin:win,incidentDraftId:incidentId,x:20,y:26};
    rows.unshift(r);filterChecked.add(r.id);
  } else {
    r.missFlag=true;r.expectedAssetId=exp;r.missWindowMin=win;r.incidentDraftId=incidentId;r.bbox="";r.score=0;
    if(document.getElementById("missNote").value) r.note=(r.note||"")+" · "+document.getElementById("missNote").value;
  }
  selected=r.id;closeModal("modal-miss");closeForm(true);render();toast("Gim Incident draft "+incidentId+" · POST /api/v1/incident/incidents");
});
