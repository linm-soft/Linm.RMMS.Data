# Team lead — tasks — so-ts-convex-mirror
...
| packKind | **`list`** (Kind **B** catalog A–D+F+H + **full-page** form 5 cột · S-ATTR **9 attr** dump editable) |
| formType | `list` |
| typeCode | `CONVEX_MIRROR` |
| cluster | `atgt_point` · tile `t31` |
| dump | `road_sphere_mirror` |
| gis | `guong-cau` |
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-MIRROR-TYPE/NAME/QTY/SCOPE/POINT/LEAVE/LABEL/ROUTE · flatten DEFER · Auth DEFER |
| solution_confirm | **approve** (`task_41734ed0`) |
| design_confirm | **approve** (`task_98fbbc05`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=CONVEX_MIRROR`** · alias board `/so-ts-convex-mirror` optional redirect · alt B primary-alias-only rejected |
...
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:36242a5e7648360ecaa70554a44a6f2b782712d0d924a0f7121ab77e14ad558f` |
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=CONVEX_MIRROR` | **giữ** · alias `/so-ts-convex-mirror` optional | **route_confirm=route_a** · **GAP-MIRROR-ROUTE-01** |
| Grid profile | generic / thiếu CONVEX_MIRROR | Hide `type`/`kmTo` · show name · qty · location/MST/shape/mat + ĐK/cao/nhịp/số biển dumpSpecs | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
| Form S-ATTR | `<dl>` readonly dumpSpecs | Editable **9 attr** dump · merge dumpSpecs · dumpSpecLabels FE | **T-UI-FORM-01** · **GAP-SOTS-FORM-01** · **GAP-MIRROR-LABEL-01** |
| Point `kmTo` | form hiện / bắt buộc risk | **Ẩn** + không required · **cấm** ép km `"0"` | **T-UI-FORM-01** · **GAP-MIRROR-POINT-01** |
| Name | IsWeak / đoạn tuyến risk | `name` ← loại+km else code/vidagis · **cấm** đoạn tuyến | **T-BE-CRUD-01** + form · **GAP-MIRROR-NAME-01** |
| Quantity | generic qty risk | `quantity` ← `total_number_post` | **T-BE-CRUD-01** · **GAP-MIRROR-QTY-01** |
| LOOKUP MST/shape/mat/loc | text dumpSpecs | LOOKUP_STATIC init `assetTypeMsts[]` · `shapeCutPosts[]` · `materialPosts[]` · `locationPosts[]` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-MIRROR-TYPE-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** · **GAP-MIRROR-LEAVE-01** |
| Filter context | thiếu file | `so-ts-convex-mirror-filter-bar.md` | **T-UI-FILTER-01** · **T-CTX-01** |
| Form surface | full-page 5 cột | **giữ** `data-form-cols="5"` + header chrome · **9 attr S-ATTR** | **T-UI-FORM-01** · **cấm** Slideout/footer Lưu · **cấm** tab legacy |
| Title / scope | tile gộp long môn | title «Sổ TS — Gương cầu / long môn» · data **chỉ** CONVEX_MIRROR · **cấm** field gantry | **GAP-MIRROR-SCOPE-01** |
| Flatten DB | — | **DEFER P2** | dumpSpecs P1 · migration **none** · **GAP-MIRROR-SCOPE-01** |
...
| `source.routes` | **`/so-ts?type=CONVEX_MIRROR`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=CONVEX_MIRROR` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=CONVEX_MIRROR` (Dev điền/verify) · alias `http://localhost:9301/so-ts-convex-mirror` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=CONVEX_MIRROR` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `CONVEX_MIRROR`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-convex-mirror-filter-bar.md` (create T-CTX-01) |
...
|--------|------|----------|
| A | `/so-ts?type=CONVEX_MIRROR` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-convex-mirror` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Gương cầu / long môn» khi `type=CONVEX_MIRROR` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-convex-mirror-filter-bar.md` |
| DES-GRID-C0 | listTitle «Danh sách gương cầu» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · CONVEX_MIRROR column profile |
...
| DES-FORM-Z2 | S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR(**9 attr**) · S-GPS · `data-form-cols="5"` |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=CONVEX_MIRROR` + filters |
...
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · units · **`assetTypeMsts[]`** · **`shapeCutPosts[]`** · **`materialPosts[]`** · **`locationPosts[]`** delta |
| API-07 | GET | `/api/v1/asset/road-assets/summary-by-type` | peer tile t31 · out of write pack |
...
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent parent line JSON / MirrorJson |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-MIRROR-NAME/QTY/LABEL** import + rebuild |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-MIRROR-TYPE-01** assetTypeMsts · shapeCutPosts · materialPosts · locationPosts |
...
| T-UI-FORM-01 | Dev | pending (delta) | T-UI-LIST-01 · T-BE-INIT-01 | `/agent-dev` | full-page 5 cột · S-ATTR **9 attr** · **GAP-SOTS-FORM/POINT/REUSE** · **GAP-MIRROR-*** |
| T-UI-LEAVE-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | LeaveConfirmModal · **GAP-MIRROR-LEAVE-01** |
...
| T-UI-FIELD-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | controlHint 1:1 · dumpSpecs keys 9 attr |
...
| T-QA-FORM-01 | QA | pending | T-UI-FORM-01 | `/agent-qa` | field e2e · body = UI · 9 attr |
...
**new_page.ssot_rereview:** pending → pass|fail
  checklist: tl-grid-ssot · list_parity · filter-bar V1–V5 · form full 5col · S-ATTR 9 attr · LAYOUT-06 · leave Modal
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-MIRROR-TYPE/NAME/QTY/SCOPE/POINT/LEAVE/LABEL/ROUTE
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-convex-mirror.md` + parent `so-ts-type-grid.md` · **create** `so-ts-convex-mirror-filter-bar.md` · lane web → `dev` · **cấm** re-scan demo
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B A–D+F+H · `shared_grid_example=v1` · type profile `CONVEX_MIRROR`  
...
**ssot.reuse:**
  ...
  ui_filter: LinErpListFilterBar · **T-UI-FILTER-01** · `so-ts-convex-mirror-filter-bar.md` — cấm ErpListHeaderFilters / LinListFilterField / nút Tìm
  ui_toolbar: catalogToolbar FULL (refresh·history·editConfig·view·edit·delete·create) · fa-cog
  ui_grid: LinCatalogDataGrid · kéo cột default ON · **CONVEX_MIRROR profile** hide type/kmTo · show name · quantity · location_post_id · asset_type_mst_id · shape_cut_post_id · material_post_id · diameter_post · height_post · span_length · number_sign · total_number_post dumpSpecs (**GAP-SOTS-COL-01**)
...
**implement.page_shell:**
  ...
  header: title «Sổ TS — Gương cầu / long môn» khi type=CONVEX_MIRROR · titleIconClass + pageId
  filterBarLayout: erp-filter-bar · **cấm** filterMaxWidthPx hack
  listTitle: «Danh sách gương cầu»
...
**implement.toolbar:** (tl-grid-full-flow §1)
  ...
  form_pair: onAdd→`/so-ts/tao-moi` · onEdit/onView→`/so-ts/sua?id=` · type lock CONVEX_MIRROR
  cấm: thiếu Create/Refresh/Config · History invent API · icon lệch erp-control-icon-map

**implement.grid:**
  component: LinCatalogDataGrid
  columns: buildDynamicGridColumns(schema) + type-profile CONVEX_MIRROR
  visible ON: name · route · routeNamed · routeSegment · kmFrom · quantity · location_post_id · asset_type_mst_id · shape_cut_post_id · material_post_id · diameter_post · height_post · span_length · number_sign · total_number_post · (status/gps optional)
  visible OFF default: type · kmTo · unitCode
  dumpSpecs: parse `road_sphere_mirror` keys · labels via dumpSpecLabels (**GAP-MIRROR-LABEL-01**)
...
**implement.wire:**
  ...
  alias: optional `/so-ts-convex-mirror` → `/so-ts?type=CONVEX_MIRROR` (**GAP-MIRROR-ROUTE-01**)

**implement.test:**
  listFilters · type lock · pageSize · selection → toolbar · column profile CONVEX_MIRROR

**DoD:**
- [ ] Kind B 1 shell · grid visible khi API có data · profile CONVEX_MIRROR (**GAP-SOTS-COL-01**)
...
**skills (REQUIRED load trước Write):**
  ...
  - context: `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-convex-mirror-filter-bar.md`
...
**implement.filter:**
  ...
  type lock: luôn `CONVEX_MIRROR` trên deep-link
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B · `formSurface: full` · `data-form-cols="5"` · header chrome · S-ATTR **9 attr** dump · **cấm** Slideout/Modal hồ sơ · **cấm** tab legacy  
...
**ssot.reuse:**
  ...
  controls: SearchInput type/route · Dropdown locationPosts / assetTypeMsts / shapeCutPosts / materialPosts · Number diameter_post / height_post / span_length / number_sign / total_number_post · Number quantity · Number lat/lng · Text name · Text code ro
  http: API-02/03/04 · IdCode readonly · init-data API-06

**implement.form:**
  modes: create · edit · view(readOnly **không** disabled xám) · copy(clear id/code → POST · keep CONVEX_MIRROR)
  sections: S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR(**9 attr**) · S-GPS
  required: type · status · route — name per GAP-MIRROR-NAME-01 · **không** required `kmTo` (**GAP-MIRROR-POINT-01**)
  S-NAME: `name` ← loại+km else code/vidagis · **cấm** đoạn tuyến (**GAP-MIRROR-NAME-01**)
  S-ATTR (dump `road_sphere_mirror` · editable · dumpSpecLabels FE):
    - `location_post_id` Dropdown LOOKUP · S-LOC
    - `asset_type_mst_id` Dropdown LOOKUP · ≠ shell type
    - `shape_cut_post_id` Dropdown LOOKUP
    - `diameter_post` Number
    - `height_post` Number
    - `span_length` Number
    - `material_post_id` Dropdown LOOKUP
    - `number_sign` Number
    - `total_number_post` Number → scalar `quantity` (**GAP-MIRROR-QTY-01**)
  **Không mount:** kmTo · S-LOC-RANGE · field gantry/long môn invent · tab Chi tiết/Bảo trì/Tệp/Lịch sử legacy
  cấm: invent History path · flatten Schema_* · ERP.* · Modal form · hardcode LOOKUP (**GAP-DEV-DROPDOWN-HARDCODE-01**) · invent long môn field (**GAP-MIRROR-SCOPE-01**)

**implement.init_data:**
  options: GET `/asset/road-assets/init-data` only — statuses · sources · **assetTypeMsts** · **shapeCutPosts** · **materialPosts** · **locationPosts** · **cấm** KIND_LABEL hardcode

**review.form:** dev-form-review-checklist PASS · full-page 5 cột + header chrome · 9 attr S-ATTR visible

**DoD:**
- [ ] C/E/V/Copy work · View readOnly
- [ ] 5 cột · header chrome · **cấm** footer Lưu
- [ ] S-ATTR editable **9 attr** · dumpSpecs merge · dumpSpecLabels
- [ ] kmTo ẩn · cấm ép `"0"` · LOOKUP Dropdown từ init-data · name ≠ đoạn
- [ ] reuse AssetFormPage S-* — **cấm** fork · **cấm** tab legacy · **cấm** field gantry

**Fail:** GAP-SOTS-FORM-01 · GAP-SOTS-REUSE-01 · GAP-MIRROR-POINT-01 · GAP-MIRROR-TYPE-01 · GAP-MIRROR-NAME-01 · GAP-MIRROR-LABEL-01 · GAP-MIRROR-QTY-01 · GAP-MIRROR-SCOPE-01 · GAP-P2-FORM-GRID-05 · GAP-LIST-FIELD-01
...
**DoD:** dirty → `LeaveConfirmModal` · full `useFormLeaveGuard` · **cấm** `window.confirm`/`alert` · **GAP-MIRROR-LEAVE-01** / **GAP-P2-36** / **GAP-DEV-LEAVE-01** / **GAP-TL-LEAVE-01**
...
| +Tạo mới | toolbar | navigate `/so-ts/tao-moi` type=CONVEX_MIRROR | POST API-03 |
...
| Alias board | `/so-ts-convex-mirror` | optional redirect → `?type=CONVEX_MIRROR` | — |
...
**status:** pending / verify  
**devSlash:** `/agent-dev`  
**DoD:** filter+form type/route/org = SearchInput Integration · location_post_id / asset_type_mst_id / shape_cut_post_id / material_post_id = LOOKUP_STATIC init-data (**không** SearchInput master P1) · **GAP-LIST-LKP-01** / **GAP-MIRROR-TYPE-01**
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** controlHint Design/DA 1:1 · SA field map · dumpSpecs keys `location_post_id` · `asset_type_mst_id` · `shape_cut_post_id` · `diameter_post` · `height_post` · `span_length` · `material_post_id` · `number_sign` · `total_number_post` · `quantity` · **GAP-LIST-FIELD-01** · **GAP-MIRROR-LABEL-01**
...
**ssot.reuse:**
  ...
  import: RoadAssetCatalogHandler · RebuildGovVn name ≠ đoạn · GIS `guong-cau`
  gates: tz_na · xco_get_only · share_tenant

**implement.wire:**
  API-01…05: verify live · **cấm** invent `api/v1/so-ts/*` · ERP.*
  GAP-MIRROR-NAME-01: rebuild CSV `name`←loại+km else code/vidagis · **cấm** IsWeak reject khi name hợp lệ · **cấm** fallback đoạn tuyến
  GAP-MIRROR-QTY-01: `quantity`←`total_number_post`
  GAP-MIRROR-LABEL-01: giữ road_sphere_mirror dumpSpecs attrs · FE dumpSpecLabels đủ 9 key
  GAP-MIRROR-SCOPE-01: data chỉ CONVEX_MIRROR · **cấm** invent gantry field · flatten DEFER
  XCO: GET by id AllowedCompanyIds
  POST/PUT: accept dumpSpecs merge · **không** required kmTo for CONVEX_MIRROR

**DoD:**
- [ ] API-01…05 verify
- [ ] name/qty/import fix (**GAP-MIRROR-NAME-01** · **GAP-MIRROR-QTY-01** · **GAP-MIRROR-LABEL-01**)
- [ ] no-parent-json · migration **none**

---

### T-BE-INIT-01 — init-data CONVEX_MIRROR LOOKUP

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**from_solution:** API-06  
**DoD:** `RoadAssetInitDataDto.assetTypeMsts` + `shapeCutPosts` + `materialPosts` + `locationPosts: RoadAssetLookupOption[]` · options = dump LOOKUP_STATIC / seed · **cấm** invent master SearchInput P1 · **cấm** FE hardcode · **GAP-MIRROR-TYPE-01**
...
| T-QA-FORM-01 | từng field required · UI value = body · dumpSpecs 9 attr · kmTo ẩn · LOOKUP Dropdown · qty rule |
| T-QA-FILTER-01 | V1–V5 + fields 1:1 `so-ts-convex-mirror-filter-bar.md` · 🔍 work · fail ErpListHeaderFilters / export trên bar |
...
| ID | Note |
|----|------|
| Flatten Schema_* | P2 · dumpSpecs P1 · **GAP-MIRROR-SCOPE-01** |
| Auth NuGet | DEFER |
...
| invent gantry/long môn fields | **cấm** · title tile only |
| tab legacy | **cấm** |
...
| focus | COL profile · S-ATTR editable **9 attr** · kmTo ẩn · assetTypeMsts/shapeCutPosts/materialPosts/locationPosts LOOKUP · name≠đoạn · qty←total_number_post · LeaveConfirm · filter-bar · dumpSpecs P1 · dumpSpecLabels |
| mfeStdRoute | `/so-ts?type=CONVEX_MIRROR` |
...
| T-UI-FORM-01 full-page 5 cột · S-ATTR 9 attr (không Slideout) | ✅ |
| T-UI-LEAVE-01 | ✅ |
| tl-list-shell-height DoD | ✅ |
| init-data assetTypeMsts/shapeCutPosts/materialPosts/locationPosts · cấm hardcode dropdown | ✅ |
| Source assignment · API cite SA · **cấm** invent | ✅ |
| GAP-MIRROR-NAME/QTY/TYPE/LABEL/POINT/LEAVE/ROUTE/SCOPE | ✅ |
| handoff compact ≤5KB | ✅ |
