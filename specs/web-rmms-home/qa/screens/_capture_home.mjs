...
async function loginViaSheet() {
  ...
  // staff Home after LoginSheet markStaff + refreshChrome
  await page
    .waitForSelector(
...
async function dumpZones() {
  return page.evaluate(() => {
    const root = document.querySelector('[data-feature="web-rmms-home"], [data-feature="web-rmms-shell"]');
...
try {
  // S0 — guest Home HM-00/01/02
  await clearSession();
  await page.goto(base + "/web-rmms-home", { waitUntil: "domcontentloaded", timeout: 60000 });
  await captureCurrent(
    "S0",
    '[data-feature="web-rmms-home"][data-zone="HM-00"][data-mode="guest"], #guestLogin, #guestFaq',
  );

  // S1 — staff Home HM-03..06 via LoginSheet (markStaff) — MFE /login soft-fails session-window
  await loginViaSheet();
  await captureCurrent(
    "S1",
    '[data-feature="web-rmms-home"][data-mode="staff"] #gridSupervise, #walletAsset, #profileName',
  );

  // QA-20 — login overlay from guest CTA (shell SH-02)
  await clearSession();
  await page.goto(base + "/web-rmms-home", { waitUntil: "domcontentloaded", timeout: 60000 });
  await page.waitForSelector('#guestLogin, [data-mode="guest"]', { timeout: 15000 });
  await page.locator("#guestLogin").click();
...
writeFileSync(
  ...
  JSON.stringify(
    {
      url: base + "/web-rmms-home",
      capturedAt: new Date().toISOString(),
      method:
        "capture_home · MFE /login · phone 430 · S0 guest HM-* · S1 staff HM-* · QA-20 SH-02 · playwright junction",
      steps: results,
      ok,
    },
    null,
