...
const featureUrl = base + "/web-rmms-gis";
...
async function dumpZones() {
  return page.evaluate(() => {
    const root = document.querySelector(
      '[data-feature="web-rmms-gis"], [data-feature="web-rmms-asset-hub"]',
    );
    const zones = [...document.querySelectorAll("[data-zone]")].map((el) =>
      el.getAttribute("data-zone"),
    );
    const ids = ["rowGis", "navBack", "mapHost", "search"].filter(
...
async function captureCurrent(id, waitSel) {
  ...
  try {
    ...
    if (id === "S0" || id === "QA-20") {
      if (!dump.zones?.includes("GIS-00") || !dump.zones?.includes("GIS-06")) {
        throw new Error("GAP-QA-E2E-MAP-01 GIS zones missing: " + JSON.stringify(dump.zones));
      }
      if (dump.feature && dump.feature !== "web-rmms-gis") {
        throw new Error("GAP-QA-E2E-MAP-01 wrong feature " + dump.feature);
      }
    }
    if (id === "S1") {
      if (!dump.ids?.includes("rowGis") && !/Xem bản đồ|bản đồ/i.test(dump.text || "")) {
        throw new Error("GAP-QA-E2E-PEER-01 Hub rowGis missing");
      }
    }
...
try {
  ...
  // S0 — GIS Map GIS-00…09
  {
    ...
    await page
      .waitForResponse(
        (r) => /gis\/(geojson|tiles|layers)/i.test(r.url()) && r.status() < 500,
        { timeout: 25000 },
      )
      .catch(() => null);
    await captureCurrent(
      "S0",
      '[data-feature="web-rmms-gis"] [data-zone="GIS-06"], [data-zone="GIS-00"]',
    );
  }

  // S1 — peer Hub entry (#rowGis)
  {
    ...
    await captureCurrent(
      "S1",
      '[data-feature="web-rmms-asset-hub"] #rowGis, [data-feature="web-rmms-asset-hub"] #walletTitle',
    );
  }

  // QA-20 — Hub rowGis → GIS (JWT kept)
  {
    const entry = page.locator('#rowGis, [data-route="/web-rmms-gis"]').first();
    if (await entry.count()) {
      ...
      await captureCurrent("QA-20", '[data-feature="web-rmms-gis"] [data-zone="GIS-06"]');
    }
  }
} catch (err) {
  results.push({
  ...
  JSON.stringify(
    {
      ...
      method:
        "capture_gis · MFE /login · phone 430 · S0 GIS map · S1 Hub #rowGis · QA-20 click→gis · geo grant · dismiss overlay · no kill worker",
      steps: results,
      ok,
    },
    null,
