/**
 * Live DOM assert + DTM filter-bar overflow for so-ts-weigh-station.
...
const TEST_ID = "rmms-so-ts-weigh-station-list";
...
const list = await page.evaluate((tid) => {
  ...
  const headers = Array.from(document.querySelectorAll("th, [role='columnheader']"))
    ...
    .filter(Boolean);
  return {
    titleWeigh: /Danh sách trạm kiểm soát tải/i.test(body),
    headerWeigh: /Sổ TS — Trạm kiểm soát tải/i.test(body),
...
await page.waitForSelector('[data-testid="asset-weigh-station-attr"]', {
  timeout: 15000,
});
await new Promise((r) => setTimeout(r, 1200));
...
const form = await page.evaluate(() => {
  ...
  return {
    cols5: cols,
    weighAttr: !!document.querySelector('[data-testid="asset-weigh-station-attr"]'),
    approaching: !!document.querySelector('[data-testid="asset-weigh-approaching-road"]'),
    stationNameLabel: /Tên trạm/i.test(body),
    equipment: /Loại TB cân|thiết bị cân|type_weighting_equipment/i.test(body),
    axleLoad: /Tải trục|max_axle_load/i.test(body),
    mgmtUnit: /Đơn vị quản lý|ĐVQL|management_unit/i.test(body),
    buildingArea: /DT nhà|Diện tích nhà|building_area/i.test(body),
    kmToVisible,
