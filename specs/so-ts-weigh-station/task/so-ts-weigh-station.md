# Team lead — tasks — so-ts-weigh-station
...
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-WEIGH-NAME/SPEC/POINT/LOOKUP/LEAVE/ROUTE/PREFIX/TILE · flatten DEFER P2 |
| solution_confirm | **approve** (`task_241ffb82`) |
| design_confirm | **approve** (`task_d6606268`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=WEIGH_STATION`** · alias board `/so-ts-weigh-station` optional redirect · alt B primary-alias-only rejected |
...
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:ce3b6142d8e9debae05124121bcf3856a8c4a06d186a2728a6d59eb55d58233a` |
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=WEIGH_STATION` | **giữ** · alias `/so-ts-weigh-station` optional | **route_confirm=route_a** · **GAP-WEIGH-ROUTE-01** |
| Grid profile | generic / thiếu WEIGH | Hide `type`/`kmTo`/qty/unit · **ON+hide-empty**: TB cân · tải max · ĐVQL · DT nhà · optional DT khu lắp/camera/đèn · length_approaching **OFF** | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
| Form S-ATTR | `<dl>` readonly dumpSpecs | Editable Dropdown/Number đủ dump §4 · merge dumpSpecs · gộp Đường vào length+width | **T-UI-FORM-01** · **GAP-SOTS-FORM-01** |
| Point `kmTo` | form hiện / bắt buộc | **Ẩn** + không required khi `WEIGH_STATION` · kmFrom **không** required · **cấm** ép `"0"` | **T-UI-FORM-01** · **GAP-WEIGH-POINT-01** |
| Name | IsWeak / đoạn tuyến risk | `name` ← `station_name` · trống OK · **cấm** IsWeak→đoạn | **T-BE-CRUD-01** + form · **GAP-WEIGH-NAME-01** |
| LOOKUP | text dumpSpecs | LOOKUP_STATIC init `weighManagementUnits[]` · `weighEquipmentTypes[]` · `weighPavementTypes[]` · `weighBoolOptions[]` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-WEIGH-LOOKUP-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** · **GAP-WEIGH-LEAVE-01** |
| Filter context | thiếu file | `so-ts-weigh-station-filter-bar.md` | **T-UI-FILTER-01** · **T-CTX-01** |
| Prefix IdCode | `TFP-` live (shared TOLL) | **giữ `TFP-`** · **cấm** invent FE | **T-UI-FORM-01** · **GAP-WEIGH-PREFIX-01** |
| Form surface | full-page 5 cột | **giữ** `data-form-cols="5"` + header chrome | **T-UI-FORM-01** · **cấm** Slideout/footer Lưu |
| Flatten DB | — | **DEFER P2** | dumpSpecs P1 · migration **none** (**GAP-WEIGH-FLAT-01**) |
| dumpSpecLabels | thiếu attr keys | đủ dump §4 WEIGH keys | **T-UI-FORM-01** · **GAP-WEIGH-SPEC-01** |
| Tile t27 | count cite 24 | list profile sync · summary-by-type | **T-UI-LIST-01** · **GAP-WEIGH-TILE-01** |
...
| `source.routes` | **`/so-ts?type=WEIGH_STATION`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=WEIGH_STATION` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=WEIGH_STATION` (Dev điền/verify) · alias `http://localhost:9301/so-ts-weigh-station` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=WEIGH_STATION` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `WEIGH_STATION`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-weigh-station-filter-bar.md` (create T-CTX-01) |
...
|--------|------|----------|
| A | `/so-ts?type=WEIGH_STATION` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-weigh-station` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Trạm kiểm soát tải» khi `type=WEIGH_STATION` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-weigh-station-filter-bar.md` |
| DES-GRID-C0 | listTitle «Danh sách trạm kiểm soát tải» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · WEIGH_STATION column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=WEIGH_STATION` + filters |
...
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · **`weighManagementUnits[]`** · **`weighEquipmentTypes[]`** · **`weighPavementTypes[]`** · **`weighBoolOptions[]`** (Có/Không) delta |
| API-07 | GET | `/api/v1/asset/road-assets/summary-by-type` | peer tile t27 · out of write pack |
...
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent parent line JSON / WeighJson |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-WEIGH-NAME/SPEC** import + rebuild |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-WEIGH-LOOKUP-01** weighManagementUnits + EquipmentTypes + PavementTypes + BoolOptions |
...
| T-UI-FORM-01 | Dev | pending (delta) | T-UI-LIST-01 · T-BE-INIT-01 | `/agent-dev` | full-page 5 cột · S-ATTR · approaching_road gộp · **GAP-SOTS-FORM/POINT/REUSE** |
| T-UI-LEAVE-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | LeaveConfirmModal · **GAP-WEIGH-LEAVE-01** |
...
**new_page.ssot_rereview:** pending → pass|fail
  checklist: tl-grid-ssot · list_parity · filter-bar V1–V5 · form full 5col · LAYOUT-06 · leave Modal · S-ATTR approaching_road
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-WEIGH-NAME/SPEC/POINT/LOOKUP/LEAVE/PREFIX
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-weigh-station.md` + parent `so-ts-type-grid.md` · tạo `so-ts-weigh-station-filter-bar.md` · lane web → `dev` · **cấm** re-scan demo
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B A–D+F+H · `shared_grid_example=v1` · type profile `WEIGH_STATION`  
...
**ssot.reuse:**
  ...
  ui_filter: LinErpListFilterBar · **T-UI-FILTER-01** · `so-ts-weigh-station-filter-bar.md` — cấm ErpListHeaderFilters / LinListFilterField / nút Tìm
  ui_toolbar: catalogToolbar FULL (refresh·history·editConfig·view·edit·delete·create) · fa-cog
  ui_grid: LinCatalogDataGrid · kéo cột default ON · **WEIGH_STATION profile** hide type/kmTo/qty/unit · show type_weighting_equipment_id · max_axle_load_limit · management_unit_id · building_area · optional site_area_installed_equipment · camera_observation · light (**GAP-SOTS-COL-01**)
...
**implement.page_shell:**
  ...
  header: title «Sổ TS — Trạm kiểm soát tải» khi type=WEIGH_STATION · titleIconClass + pageId
  filterBarLayout: erp-filter-bar · **cấm** filterMaxWidthPx hack
  listTitle: «Danh sách trạm kiểm soát tải»
...
**implement.toolbar:** (tl-grid-full-flow §1)
  ...
  form_pair: onAdd→`/so-ts/tao-moi` · onEdit/onView→`/so-ts/sua?id=` · type lock WEIGH_STATION
  cấm: thiếu Create/Refresh/Config · History invent API · icon lệch erp-control-icon-map

**implement.grid:**
  component: LinCatalogDataGrid
  columns: buildDynamicGridColumns(schema) + type-profile WEIGH_STATION
  visible ON: name · route · routeNamed · routeSegment · kmFrom · type_weighting_equipment_id · max_axle_load_limit · management_unit_id · building_area · (status/gps optional)
  visible OFF default: type · kmTo · quantity · unitCode · length_approaching_road
  hide-empty ON: type_weighting_equipment_id · max_axle_load_limit · management_unit_id · building_area · site_area_installed_equipment · camera_observation · light
  dumpSpecs: parse weight_station §4 keys · labels via dumpSpecLabels (**GAP-WEIGH-SPEC-01**)
...
**implement.wire:**
  list: GET `?type=WEIGH_STATION&search=&route=&kmFrom=&kmTo=&orgUnit=&page=&pageSize=`
  alias: optional `/so-ts-weigh-station` → `/so-ts?type=WEIGH_STATION` (**GAP-WEIGH-ROUTE-01**)

**implement.test:**
  listFilters · type lock WEIGH_STATION · pageSize · selection → toolbar · column profile WEIGH_STATION

**DoD:**
- [ ] Kind B 1 shell · grid visible khi API có data · profile WEIGH_STATION (**GAP-SOTS-COL-01**)
- [ ] Toolbar FULL · Grid menu · Config full cột · Grid flow
- [ ] Footer LinCatalogListPagination 50/100/200/500
- [ ] LAYOUT-06 live PASS
- [ ] tile t27 / count cite sync (**GAP-WEIGH-TILE-01**)
- [ ] `rg` anti-clone PASS · **yarn build chỉ Dev**

**Fail:** GAP-TL-GRID-TASK-01 · GAP-TL-GRID-FLOW-01 · GAP-P2-LAYOUT-06 · GAP-SOTS-COL-01 · GAP-DEV-DUP-GRID · GAP-WEIGH-TILE-01
...
**skills (REQUIRED load trước Write):**
  ...
  - context: `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-weigh-station-filter-bar.md`
...
**implement.filter:**
  ...
  type lock: luôn `WEIGH_STATION` trên deep-link
...
**ssot.reuse:**
  ...
  controls: SearchInput type/route · Dropdown status/source/weighManagementUnits/weighEquipmentTypes/weighPavementTypes/weighBoolOptions · Number/Text dump §4 · Text station_name
  http: API-02/03/04 · IdCode readonly prefix `TFP-` · init-data API-06

**implement.form:**
  modes: create · edit · view(readOnly **không** disabled xám) · copy(clear id/code → POST · keep WEIGH_STATION)
  sections: S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR · S-GPS
  required: type · status · route — name trống OK · kmFrom **không** required · **không** required `kmTo` (**GAP-WEIGH-POINT-01**)
  S-ATTR editable dump §4: station_name · site_area_installed_equipment · management_unit_id Dropdown LOOKUP · building_area · includes_load_reduction_area bool · light bool · camera_observation bool · equipment_measurement_vehicle_size bool · type_weighting_equipment_id Dropdown · origin_manufacturing · year_manufacturing · max_axle_load_limit · approval_code_number · inspection_date_weight_station · length_approaching_road + width_approaching_road (**gộp Đường vào**) · pavement_type_id Dropdown · location Text · merge dumpSpecs (**GAP-SOTS-FORM-01** · **GAP-WEIGH-LOOKUP-01** · **GAP-WEIGH-SPEC-01**)
  S-NAME: `name` ← `station_name` · trống OK · mirror dumpSpecs (**GAP-WEIGH-NAME-01**)
  kmFrom: **cấm** ép `"0"` khi dump null (**GAP-WEIGH-SPEC-01** · **GAP-WEIGH-POINT-01**)
  IdCode: prefix **`TFP-`** keep · **cấm** invent FE (**GAP-WEIGH-PREFIX-01**)
...
**implement.init_data:**
  options: GET `/asset/road-assets/init-data` only — statuses · sources · **weighManagementUnits** · **weighEquipmentTypes** · **weighPavementTypes** · **weighBoolOptions** · **cấm** KIND_LABEL hardcode (**GAP-DEV-DROPDOWN-HARDCODE-01**)
...
- [ ] S-ATTR editable · approaching_road gộp · dumpSpecs merge
- [ ] kmTo ẩn · WEIGH LOOKUP Dropdown từ init-data
- [ ] reuse AssetFormPage S-* — **cấm** fork

**Fail:** GAP-SOTS-FORM-01 · GAP-SOTS-REUSE-01 · GAP-WEIGH-POINT-01 · GAP-WEIGH-LOOKUP-01 · GAP-WEIGH-PREFIX-01 · GAP-P2-FORM-GRID-05 · GAP-LIST-FIELD-01
...
**DoD:** dirty → `LeaveConfirmModal` · full `useFormLeaveGuard` · **cấm** `window.confirm`/`alert` · **GAP-WEIGH-LEAVE-01** / **GAP-P2-36** / **GAP-DEV-LEAVE-01** / **GAP-TL-LEAVE-01**
...
| +Tạo mới | toolbar | navigate `/so-ts/tao-moi` type=WEIGH_STATION | POST API-03 |
...
| Alias board | `/so-ts-weigh-station` | optional redirect → `?type=WEIGH_STATION` | — |
...
**status:** pending / verify  
**devSlash:** `/agent-dev`  
**DoD:** filter+form type/route/org = SearchInput Integration · management_unit/equipment/pavement/bool = LOOKUP_STATIC init-data (**không** SearchInput master P1) · **GAP-LIST-LKP-01** / **GAP-WEIGH-LOOKUP-01**
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** controlHint Design/DA 1:1 · SA field map · dumpSpecs keys `station_name` · `site_area_installed_equipment` · `management_unit_id` · `building_area` · `includes_load_reduction_area` · `light` · `camera_observation` · `equipment_measurement_vehicle_size` · `type_weighting_equipment_id` · `origin_manufacturing` · `year_manufacturing` · `max_axle_load_limit` · `approval_code_number` · `inspection_date_weight_station` · `length_approaching_road` · `width_approaching_road` · `pavement_type_id` · `location` · **GAP-LIST-FIELD-01**
...
**implement.wire:**
  ...
  GAP-WEIGH-NAME-01: rebuild CSV `name`←`station_name` · **cấm** IsWeak reject «Trạm kiểm soát tải» · **cấm** fallback `name_of_route_asset` khi station_name hợp lệ
  GAP-WEIGH-SPEC-01: giữ weight_station §4 attrs trong dumpSpecs · km trống khi dump null · FE dumpSpecLabels đủ key
  XCO: GET by id AllowedCompanyIds
  POST/PUT: accept dumpSpecs merge · **không** required kmTo for WEIGH_STATION

**DoD:**
- [ ] API-01…05 verify
- [ ] name/import fix (**GAP-WEIGH-NAME-01** · **GAP-WEIGH-SPEC-01**)
- [ ] no-parent-json · migration **none**

---

### T-BE-INIT-01 — init-data WEIGH LOOKUP

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**from_solution:** API-06  
**DoD:** `RoadAssetInitDataDto.weighManagementUnits` + `weighEquipmentTypes` + `weighPavementTypes` + `weighBoolOptions: RoadAssetLookupOption[]` · options = dump LOOKUP_STATIC / seed · **cấm** invent weigh master SearchInput P1 · **cấm** FE hardcode · **GAP-WEIGH-LOOKUP-01**
...
| T-QA-FORM-01 | từng field required · UI value = body · dumpSpecs weigh attrs · kmTo ẩn · approaching_road gộp |
| T-QA-FILTER-01 | V1–V5 + fields 1:1 `so-ts-weigh-station-filter-bar.md` · 🔍 work · fail ErpListHeaderFilters / export trên bar |
...
| ID | Note |
|----|------|
| Flatten Schema_* | P2 · dumpSpecs P1 (**GAP-WEIGH-FLAT-01**) |
| Auth NuGet | DEFER |
| WEIGH LOOKUP master SearchInput | P2 |
| Excel import wizard UI | OUT |
| Kind F map canvas | **cấm** |
| invent History API | **cấm** |
| invent `api/v1/so-ts/*` | **cấm** |
| invent prefix FE | **cấm** · keep `TFP-` (**GAP-WEIGH-PREFIX-01**) |
...
| focus | COL profile WEIGH · S-ATTR editable · approaching_road gộp · kmTo ẩn · weigh* init LOOKUP · name←station_name · LeaveConfirm · filter-bar · dumpSpecs P1 · prefix TFP- |
| mfeStdRoute | `/so-ts?type=WEIGH_STATION` |
...
| T-UI-FORM-01 full-page 5 cột + S-ATTR approaching_road (không Slideout) | ✅ |
| T-UI-LEAVE-01 | ✅ |
| tl-list-shell-height DoD | ✅ |
| init-data weigh* · cấm hardcode dropdown | ✅ |
...
---
<!-- Version meta: skillId=agent-team-lead skillVersion=2026.08.19.04 schemaVersion=2 workflowVersion=2026.08.30.01 rulesVersion=2026.08.31.2 versionGate=rechecked taskId=task_75b037a8 contentHashPriorDataAnaly=sha256:ce3b6142d8e9debae05124121bcf3856a8c4a06d186a2728a6d59eb55d58233a route_confirm=route_a -->
