# Team lead — tasks — so-ts-spillway
...
| dump | `tbl_spill_way` |
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-SPW-NAME/SPEC/POINT/LOOKUP/LEAVE/ROUTE · GAP-SPW-AUTH-01 DEFER · GAP-SPW-FLAT-01 DEFER |
| solution_confirm | **approve** (`task_5f5265d5`) |
| design_confirm | **approve** (`task_b8f243b8`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=SPILLWAY`** · alias board `/so-ts-spillway` optional redirect · alt B primary-alias-only rejected |
| productRoot | `D:/AI-QLBD/Linm.RMMS.Data` |
| reviewUrl | `file:///D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-spillway/ui/prototype/so-ts-spillway-list-prototype.html` |
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:508eb2426b263e800949a533e300220f75a65b128771f77baf52d3e0d567517f` |
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=SPILLWAY` | **giữ** · alias `/so-ts-spillway` optional | **route_confirm=route_a** · **GAP-SPW-ROUTE-01** |
| Grid profile | 1 schema (+ profile chỉ KM_POST) | Hide `type`/`kmTo`/qty/unit/KM_POST-only · show spillway dump attrs (type/river/width/length/span/structure/load) | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
| Form S-ATTR | `<dl>` readonly dumpSpecs | Editable Dropdown/Number/Text đủ §4 · merge dumpSpecs | **T-UI-FORM-01** · **GAP-SOTS-FORM-01** |
| Point `kmTo` | form hiện / bắt buộc | **Ẩn** + không required khi `SPILLWAY` | **T-UI-FORM-01** · **GAP-SPW-POINT-01** |
| Name | IsWeak / đoạn tuyến risk | `name` ← `name_work` · `name_river` field/cột riêng | **T-BE-CRUD-01** + form · **GAP-SPW-NAME-01** |
| LOOKUP spillway/structure | text dumpSpecs | LOOKUP_STATIC init `spillwayTypes[]` · `structureTypeSpillways[]` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-SPW-LOOKUP-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** · **GAP-SPW-LEAVE-01** |
| Filter context | thiếu file | `so-ts-spillway-filter-bar.md` | **T-UI-FILTER-01** |
| Form surface | full-page 5 cột | **giữ** `data-form-cols="5"` + header chrome | **T-UI-FORM-01** · **cấm** Slideout/footer Lưu |
| Flatten DB | — | **DEFER P2** | **GAP-SPW-FLAT-01** · migration **none** |
...
| `source.routes` | **`/so-ts?type=SPILLWAY`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=SPILLWAY` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=SPILLWAY` (Dev điền/verify) · alias `http://localhost:9301/so-ts-spillway` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=SPILLWAY` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `SPILLWAY`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-spillway-filter-bar.md` |
...
|--------|------|----------|
| A | `/so-ts?type=SPILLWAY` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-spillway` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Đường tràn» khi `type=SPILLWAY` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-spillway-filter-bar.md` |
| DES-GRID-C0 | listTitle «Danh sách đường tràn» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · SPILLWAY column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=SPILLWAY` + filters |
...
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · units · **`spillwayTypes[]`** · **`structureTypeSpillways[]`** delta |
...
Permissions: `asset.road-assets.read|create|update|delete` · FE `rmms-asset:road-assets:read|write` — Auth align **DEFER** GAP-SPW-AUTH-01.
...
| SD-AUTH | **gap** · GAP-SPW-AUTH-01 DEFER |
| SD-TOKEN | required (BFF) |
| SD-HEADER | required · X-Company-Id |
| SD-SPLIT | Asset ownership |
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent parent line JSON / SpillwayJson |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-SPW-NAME/SPEC** import + rebuild |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-SPW-LOOKUP-01** spillwayTypes + structureTypeSpillways |
...
| T-UI-LEAVE-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | LeaveConfirmModal · **GAP-SPW-LEAVE-01** |
...
**new_page.ssot_rereview:** pending → pass|fail
  ...
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-SPW-NAME/SPEC/POINT/LOOKUP/LEAVE
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-spillway.md` + parent `so-ts-type-grid.md` · filter-bar context tồn tại · **cấm** re-scan demo
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B A–D+F+H · `shared_grid_example=v1` · type profile `SPILLWAY`  
...
**ssot.reuse:**
  ...
  ui_filter: LinErpListFilterBar · **T-UI-FILTER-01** · `so-ts-spillway-filter-bar.md` — cấm ErpListHeaderFilters / LinListFilterField / nút Tìm
  ui_toolbar: catalogToolbar FULL (refresh·history·editConfig·view·edit·delete·create) · fa-cog
  ui_grid: LinCatalogDataGrid · kéo cột default ON · **SPILLWAY profile** hide type/kmTo/qty/unit · show distance/spillwayTypes (**GAP-SOTS-COL-01**)
...
**implement.page_shell:**
  ...
  header: title «Sổ TS — Đường tràn» khi type=SPILLWAY · titleIconClass + pageId
  filterBarLayout: erp-filter-bar · **cấm** filterMaxWidthPx hack
  listTitle: «Danh sách đường tràn»
...
**implement.toolbar:** (tl-grid-full-flow §1)
  ...
  form_pair: onAdd→`/so-ts/tao-moi` · onEdit/onView→`/so-ts/sua?id=` · type lock SPILLWAY
  cấm: thiếu Create/Refresh/Config · History invent API · icon lệch erp-control-icon-map

**implement.grid:**
  component: LinCatalogDataGrid
  columns: buildDynamicGridColumns(schema) + type-profile SPILLWAY
  visible ON: name · route · routeNamed · routeSegment · kmFrom · spillway_type_id · name_river · width_spillway · length_spillway · no_span · span_length · structure_type_spillway_id · operational_load · (status/gps optional)
  visible OFF: type · kmTo · quantity · unitCode · distance_next_post · materials_id
  dumpSpecs: parse spillway §4 keys · labels via dumpSpecLabels (**GAP-SPW-SPEC-01**)
...
**implement.wire:**
  list: GET `?type=SPILLWAY&search=&route=&kmFrom=&kmTo=&orgUnit=&page=&pageSize=`
  alias: optional `/so-ts-spillway` → `/so-ts?type=SPILLWAY` (**GAP-SPW-ROUTE-01**)

**implement.state:**
  listFilters · type lock · pageSize · selection → toolbar · column profile SPILLWAY

**DoD:**
- [ ] Kind B 1 shell · grid visible khi API có data · profile SPILLWAY (**GAP-SOTS-COL-01**)
...
**skills (REQUIRED load trước Write):**
  ...
  - context: `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-spillway-filter-bar.md`
...
**implement.filter:**
  ...
  type lock: luôn `SPILLWAY` trên deep-link
...
**ssot.reuse:**
  ...
  controls: SearchInput type/route · Dropdown status/source/spillwayTypes/structureTypeSpillways · Number width/length/spans · Text name_work/name_river
  http: API-02/03/04 · IdCode readonly prefix `TR-` · init-data API-06

**implement.form:**
  modes: create · edit · view(readOnly **không** disabled xám) · copy(clear id/code → POST · keep SPILLWAY)
  sections: S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR · S-GPS
  required: name · type · status · route · kmFrom · spillway_type_id — **không** required `kmTo` (**GAP-SPW-POINT-01**)
  S-ATTR: spillway_type_id Dropdown · width/length Number · no_span/span_length Number · structure_type_spillway_id Dropdown · water_pole boolean · flood fields · operational_load · merge dumpSpecs (**GAP-SOTS-FORM-01** · **GAP-SPW-LOOKUP-01** · **GAP-SPW-SPEC-01**)
  S-NAME: `name` = `name_work` · `name_river` field riêng · mirror dumpSpecs (**GAP-SPW-NAME-01**)
  kmFrom: **cấm** ép `"0"` khi dump null (**GAP-SPW-SPEC-01**)
...
**implement.init_data:**
  options: GET `/asset/road-assets/init-data` only — statuses · sources · **spillwayTypes** · **structureTypeSpillways** · **cấm** KIND_LABEL hardcode (**GAP-DEV-DROPDOWN-HARDCODE-01**)
...
- [ ] kmTo ẩn · spillway/structure Dropdown từ init-data
- [ ] reuse AssetFormPage S-* — **cấm** fork

**Fail:** GAP-SOTS-FORM-01 · GAP-SOTS-REUSE-01 · GAP-SPW-POINT-01 · GAP-SPW-LOOKUP-01 · GAP-P2-FORM-GRID-05 · GAP-LIST-FIELD-01
...
**DoD:** dirty → `LeaveConfirmModal` · full `useFormLeaveGuard` · **cấm** `window.confirm`/`alert` · **GAP-SPW-LEAVE-01** / **GAP-P2-36** / **GAP-DEV-LEAVE-01** / **GAP-TL-LEAVE-01**
...
| +Tạo mới | toolbar | navigate `/so-ts/tao-moi` type=SPILLWAY | POST API-03 |
...
| Alias board | `/so-ts-spillway` | optional redirect → `?type=SPILLWAY` | — |
...
**status:** pending / verify  
**devSlash:** `/agent-dev`  
**DoD:** filter+form type/route/org = SearchInput Integration · spillway_type/structure_type = LOOKUP_STATIC init-data (**không** SearchInput master P1) · **GAP-LIST-LKP-01** / **GAP-SPW-LOOKUP-01**
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** controlHint Design/DA 1:1 · SA field map · dumpSpecs keys `name_work` · `name_river` · spillway §4 attrs · **GAP-LIST-FIELD-01**
...
**implement.wire:**
  ...
  GAP-SPW-NAME-01: rebuild CSV `name`←`name_work` · **cấm** IsWeak reject «Đường tràn»/«Cầu tràn» · **cấm** fallback `name_of_route_asset` khi name_work hợp lệ
  GAP-SPW-SPEC-01: giữ spillway attrs trong dumpSpecs · km trống khi dump null · FE dumpSpecLabels đủ key
  XCO: GET by id AllowedCompanyIds
  POST/PUT: accept dumpSpecs merge · **không** required kmTo for SPILLWAY

**DoD:**
- [ ] API-01…05 verify
- [ ] name/import fix (**GAP-SPW-NAME-01** · **GAP-SPW-SPEC-01**)
- [ ] no-parent-json · migration **none**

---

### T-BE-INIT-01 — init-data spillway LOOKUP

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**from_solution:** API-06  
**DoD:** `RoadAssetInitDataDto.spillwayTypes` + `structureTypeSpillways: RoadAssetLookupOption[]` · options = dump distinct / seed (cite `Cầu tràn` · `Bê tông`) · **cấm** invent spillway/structure master SearchInput · **cấm** FE hardcode · **GAP-SPW-LOOKUP-01**
...
**status:** pending / verify  
**DoD:** FE gate + BE codes `asset.road-assets.*` · Auth NuGet mount **DEFER** GAP-SPW-AUTH-01
...
| T-QA-FORM-01 | từng field required · UI value = body · dumpSpecs spillwayTypes/distance · kmTo ẩn |
| T-QA-FILTER-01 | V1–V5 + fields 1:1 `so-ts-spillway-filter-bar.md` · 🔍 work · fail ErpListHeaderFilters / export trên bar |
...
| focus | COL profile · S-ATTR editable · kmTo ẩn · spillwayTypes/structureTypeSpillways init · name_work/name_river · LeaveConfirm · filter-bar · dumpSpecs P1 |
| mfeStdRoute | `/so-ts?type=SPILLWAY` |
...
| init-data spillwayTypes/structureTypeSpillways · cấm hardcode dropdown | ✅ |
...
---
<!-- Version meta: skillId=agent-team-lead skillVersion=2026.08.19.04 schemaVersion=2 workflowVersion=2026.08.30.01 rulesVersion=2026.08.31.2 versionGate=rechecked taskId=task_d7836041 contentHashPriorDataAnaly=sha256:508eb2426b263e800949a533e300220f75a65b128771f77baf52d3e0d567517f route_confirm=route_a -->
