...
const listUrl = "http://localhost:9301/so-ts?type=SPILLWAY";
const aliasUrl = "http://localhost:9301/so-ts-spillway";
const formUrl = "http://localhost:9301/so-ts/tao-moi?type=SPILLWAY";
const listSel = '[data-testid="rmms-so-ts-spillway-list-page"]';
const formSel = '[data-testid="rmms-asset-form-shell"]';
const attrSel = '[data-testid="asset-spillway-attr"]';
...
const steps = [
  {
    ...
    url: listUrl,
    selector: listSel,
    note: "list SPILLWAY profile + filter bar",
  },
  {
    id: "S1",
    url: aliasUrl,
    selector: listSel,
    note: "alias /so-ts-spillway → type=SPILLWAY",
  },
  {
    id: "QA-20",
    url: formUrl,
    selector: formSel,
    also: attrSel,
    note: "Create form · S-ATTR spillway",
  },
];
...
const manifest = {
  ...
  note: "GAP-QA-E2E-PW-01: yarn e2e-qa hung on playwright install chromium — capture used channel=chrome; API host port 5111 (compose map) · --skip-start",
  testid: "rmms-so-ts-spillway-list-page",
