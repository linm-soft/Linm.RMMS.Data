...
const featureUrl = base + "/web-rmms-asset-kcht";
...
async function dumpZones() {
  return page.evaluate(() => {
    const root = document.querySelector(
      '[data-feature="web-rmms-asset-kcht"], [data-feature="web-rmms-asset-hub"]',
    );
    ...
    const tiles = [...document.querySelectorAll('[data-zone="AK-06"]')].length;
...
try {
  ...
  // S0 — KCHT type-grid Live
  {
    ...
    await captureCurrent(
      "S0",
      '[data-feature="web-rmms-asset-kcht"] #pageTitle, [data-feature="web-rmms-asset-kcht"] #akGuestGate',
    );
  }

  // S1 — peer Hub entry (#tileKcht)
  {
    ...
    await captureCurrent(
      "S1",
      '[data-feature="web-rmms-asset-hub"] #tileKcht, [data-feature="web-rmms-asset-hub"] #walletTitle',
    );
  }

  // QA-20 — click Hub tileKcht → KCHT (JWT kept)
  {
    const entry = page.locator('#tileKcht, [data-route="/web-rmms-asset-kcht"]').first();
    if (await entry.count()) {
      await entry.click();
      await captureCurrent(
        "QA-20",
        '[data-feature="web-rmms-asset-kcht"] #pageTitle, [data-feature="web-rmms-asset-kcht"] #AK-04',
      );
    } else {
      ...
      await captureCurrent(
        "QA-20",
        '[data-feature="web-rmms-asset-kcht"] #pageTitle',
      );
    }
  }
} catch (err) {
  ...
  JSON.stringify(
    {
      ...
      method:
        "capture_kcht · MFE /login · phone 430 · S0 KCHT · S1 Hub peer #tileKcht · QA-20 click→KCHT · no kill worker",
      steps: results,
      ok,
    },
    null,
