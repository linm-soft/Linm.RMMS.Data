...
| updatedAt | 2026-08-10T16:55:00.000Z |

## Source assignment

| Layer | Path | Confirm |
|-------|------|---------|
| UI | `D:/AI-QLBD/MFE-Source/Linm.Web.RMMS.Notification` | autopilot packet default |
| BE | `D:/AI-QLBD/Linm.RMMS.WebService` · domain **Notification** | autopilot packet default |
| Routes | `mfeStdRoute=/ops` | `mfeStdUrl=http://localhost:9308/ops` |
...
## FormType pack (canonical — `form-type-task-pack` · task_02070244)
...
| T-BE-CRUD-01 | Dev | **pending→done** | list/search + C/U/D + getById + mark-read/mark-all (= prior T-BE-01 verify) |
...
| +Tạo | toolbar | `openCreate` → Slideout create | POST `/inbox` |
| Edit (toolbar) | toolbar | `openRow(edit)` | GET `/{id}` · PUT |
| View (toolbar) | toolbar | `openRow(view)` | GET `/{id}` (+ mark-read if unread) |
...
| Form Send/Draft/Clear/Cancel/View actions | Slideout footer + Z1 | `handleSave` | POST/PUT |
...
| Anti-dup | reuse `notificationService.delete` · BE controller sẵn |
| UI SSOT | `MFE-Source/Linm.Web.RMMS.Notification` · `pages/NotificationListPage` |
...
| generatedAt | 2026-08-10T16:55:00.000Z |
