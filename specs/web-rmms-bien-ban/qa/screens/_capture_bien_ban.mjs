...
async function fatalUi(p) {
  ...
  if (await overlay.count()) {
    ...
    const featureOk = await p
      .locator(
        `[data-feature='${feature}'], [data-zone='BB-00'], #loginUser, #BB-ROOT`,
      )
      .count();
...
async function dumpZones() {
  return page.evaluate(() => {
    const root = document.querySelector(
      '[data-feature="web-rmms-bien-ban"], [data-feature="web-rmms-home"], [data-feature="web-rmms-shell"]',
    );
    ...
    const hasSearch = !!document.querySelector('[data-field="list.search"]');
    const hasCreateTd = !!document.querySelector('[data-field="btnCreateTd"]');
    const hasCreateTk = !!document.querySelector('[data-field="btnCreateTk"]');
    const hasEmpty = !!document.querySelector('[data-field="list.empty"]');
    return {
...
writeFileSync(join(outDir, "_capture_bien_ban.result.json"), JSON.stringify(summary, null, 2));
