# Team lead — tasks — so-ts-station-house
...
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-SH-NAME/SPEC/POINT/LOOKUP/LEAVE/ROUTE · flatten DEFER P2 |
| solution_confirm | **approve** (`task_56208969`) |
| design_confirm | **approve** (`task_93e161af`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=STATION_HOUSE`** · alias board `/so-ts-station-house` optional redirect · alt B primary-alias-only rejected |
...
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:3d78ed6a2ee20b192926ba4ab625d1af20e67dcb78d7035e543c976b364e7a45` |
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=STATION_HOUSE` | **giữ** · alias `/so-ts-station-house` optional | **route_confirm=route_a** · **GAP-SH-ROUTE-01** |
| Grid profile | generic / thiếu STATION_HOUSE | Hide `type`/`kmTo`/qty/unit · show type_work_id · hide-low-fill OFF: DT nhà · DT CT phụ · cấp nhà · cấp CT phụ · vật tư · khuôn viên | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
...
| Point `kmTo` | form hiện / bắt buộc | **Ẩn** + không required khi `STATION_HOUSE` | **T-UI-FORM-01** · **GAP-SH-POINT-01** |
| Name | IsWeak / đoạn tuyến risk | `name` ← `name_building` · trống OK · **cấm** IsWeak→đoạn | **T-BE-CRUD-01** + form · **GAP-SH-NAME-01** |
| LOOKUP loại/cấp/mặt cắt | text dumpSpecs | LOOKUP_STATIC init `stationWorkTypes[]` · `stationBuildLocations[]` · `officeBuildingGrades[]` · `auxiliaryWorksGrades[]` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-SH-LOOKUP-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** · **GAP-SH-LEAVE-01** |
| Filter context | thiếu file | `so-ts-station-house-filter-bar.md` | **T-UI-FILTER-01** · **T-CTX-01** |
...
| dumpSpecLabels | thiếu attr keys | đủ dump §4 + type_work label | **T-UI-FORM-01** · **GAP-SH-SPEC-01** |
...
| `source.routes` | **`/so-ts?type=STATION_HOUSE`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=STATION_HOUSE` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=STATION_HOUSE` (Dev điền/verify) · alias `http://localhost:9301/so-ts-station-house` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=STATION_HOUSE` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `STATION_HOUSE`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-station-house-filter-bar.md` (create T-CTX-01) |
...
|--------|------|----------|
| A | `/so-ts?type=STATION_HOUSE` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-station-house` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Nhà hạt QLĐB» khi `type=STATION_HOUSE` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-station-house-filter-bar.md` |
| DES-GRID-C0 | listTitle «Danh sách nhà hạt QLĐB» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · STATION_HOUSE column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=STATION_HOUSE` + filters |
...
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · units · **`stationWorkTypes[]`** · **`stationBuildLocations[]`** · **`officeBuildingGrades[]`** · **`auxiliaryWorksGrades[]`** delta |
| API-07 | GET | `/api/v1/asset/road-assets/summary-by-type` | peer tile t22 · out of write pack |
...
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent parent line JSON / StationHouseJson |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-SH-NAME/SPEC** import + rebuild |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-SH-LOOKUP-01** stationWorkTypes + stationBuildLocations + officeBuildingGrades + auxiliaryWorksGrades |
...
| T-UI-LEAVE-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | LeaveConfirmModal · **GAP-SH-LEAVE-01** |
...
**new_page.ssot_rereview:** pending → pass|fail
  ...
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-SH-NAME/SPEC/POINT/LOOKUP/LEAVE
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-station-house.md` + parent `so-ts-type-grid.md` · tạo `so-ts-station-house-filter-bar.md` · lane web → `dev` · **cấm** re-scan demo
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B A–D+F+H · `shared_grid_example=v1` · type profile `STATION_HOUSE`  
...
**ssot.reuse:**
  ...
  ui_filter: LinErpListFilterBar · **T-UI-FILTER-01** · `so-ts-station-house-filter-bar.md` — cấm ErpListHeaderFilters / LinListFilterField / nút Tìm
  ui_toolbar: catalogToolbar FULL (refresh·history·editConfig·view·edit·delete·create) · fa-cog
  ui_grid: LinCatalogDataGrid · kéo cột default ON · **STATION_HOUSE profile** hide type/kmTo/qty/unit · show type_work_id · name · route · kmFrom (**GAP-SOTS-COL-01**)
...
**implement.page_shell:**
  ...
  header: title «Sổ TS — Nhà hạt QLĐB» khi type=STATION_HOUSE · titleIconClass + pageId
  filterBarLayout: erp-filter-bar · **cấm** filterMaxWidthPx hack
  listTitle: «Danh sách nhà hạt QLĐB»
...
**implement.toolbar:** (tl-grid-full-flow §1)
  ...
  form_pair: onAdd→`/so-ts/tao-moi` · onEdit/onView→`/so-ts/sua?id=` · type lock STATION_HOUSE
  cấm: thiếu Create/Refresh/Config · History invent API · icon lệch erp-control-icon-map

**implement.grid:**
  component: LinCatalogDataGrid
  columns: buildDynamicGridColumns(schema) + type-profile STATION_HOUSE
  visible ON: name · route · routeNamed · routeSegment · kmFrom · type_work_id · (status/gps optional)
  visible OFF default: type · kmTo · quantity · unitCode · total_area_office_building · total_area_auxiliary_works · office_building_grade_id · auxiliary_works_grade_id · materials_in_office · site_area_using_land · build_location
  dumpSpecs: parse station dump §4 keys · labels via dumpSpecLabels (**GAP-SH-SPEC-01**)
...
**implement.wire:**
  ...
  alias: optional `/so-ts-station-house` → `/so-ts?type=STATION_HOUSE` (**GAP-SH-ROUTE-01**)

**implement.test:**
  listFilters · type lock · pageSize · selection → toolbar · column profile STATION_HOUSE

**DoD:**
- [ ] Kind B 1 shell · grid visible khi API có data · profile STATION_HOUSE (**GAP-SOTS-COL-01**)
...
**skills (REQUIRED load trước Write):**
  ...
  - context: `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-station-house-filter-bar.md`
...
**implement.filter:**
  ...
  type lock: luôn `STATION_HOUSE` trên deep-link
...
**ssot.reuse:**
  ...
  controls: SearchInput type/route · Dropdown status/source/stationWorkTypes/stationBuildLocations/officeBuildingGrades/auxiliaryWorksGrades · Text/Number/TextArea dump §4 · Text name_building
  http: API-02/03/04 · IdCode readonly prefix `NH-` · init-data API-06

**implement.form:**
  modes: create · edit · view(readOnly **không** disabled xám) · copy(clear id/code → POST · keep STATION_HOUSE)
  sections: S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR · S-GPS
  required: type · status · route · type_work_id — name trống OK · kmFrom **không** required · **không** required `kmTo` (**GAP-SH-POINT-01**)
  S-ATTR: type_work_id Dropdown · build_location Dropdown · office_building_grade_id Dropdown · total_area_office_building Number · site_area_using_land Number · auxiliary_works_grade_id Dropdown · total_area_auxiliary_works Number · materials_in_office TextArea · merge dumpSpecs (**GAP-SOTS-FORM-01** · **GAP-SH-LOOKUP-01** · **GAP-SH-SPEC-01**)
  S-NAME: `name` ← `name_building` · trống OK · mirror dumpSpecs (**GAP-SH-NAME-01**)
  kmFrom: **cấm** ép `"0"` khi dump null (**GAP-SH-SPEC-01** · **GAP-SH-POINT-01**)
...
**implement.init_data:**
  options: GET `/asset/road-assets/init-data` only — statuses · sources · **stationWorkTypes** · **stationBuildLocations** · **officeBuildingGrades** · **auxiliaryWorksGrades** · **cấm** KIND_LABEL hardcode (**GAP-DEV-DROPDOWN-HARDCODE-01**)
...
- [ ] kmTo ẩn · station LOOKUP Dropdown từ init-data
- [ ] reuse AssetFormPage S-* — **cấm** fork

**Fail:** GAP-SOTS-FORM-01 · GAP-SOTS-REUSE-01 · GAP-SH-POINT-01 · GAP-SH-LOOKUP-01 · GAP-P2-FORM-GRID-05 · GAP-LIST-FIELD-01
...
**DoD:** dirty → `LeaveConfirmModal` · full `useFormLeaveGuard` · **cấm** `window.confirm`/`alert` · **GAP-SH-LEAVE-01** / **GAP-P2-36** / **GAP-DEV-LEAVE-01** / **GAP-TL-LEAVE-01**
...
| +Tạo mới | toolbar | navigate `/so-ts/tao-moi` type=STATION_HOUSE | POST API-03 |
...
| Alias board | `/so-ts-station-house` | optional redirect → `?type=STATION_HOUSE` | — |
...
**status:** pending / verify  
**devSlash:** `/agent-dev`  
**DoD:** filter+form type/route/org = SearchInput Integration · type_work_id/build_location/office_building_grade_id/auxiliary_works_grade_id = LOOKUP_STATIC init-data (**không** SearchInput master P1) · **GAP-LIST-LKP-01** / **GAP-SH-LOOKUP-01**
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** controlHint Design/DA 1:1 · SA field map · dumpSpecs keys `name_building` · `type_work_id` · `build_location` · `office_building_grade_id` · `total_area_office_building` · `site_area_using_land` · `auxiliary_works_grade_id` · `total_area_auxiliary_works` · `materials_in_office` · **GAP-LIST-FIELD-01** · **GAP-SH-SPEC-01**
...
**implement.wire:**
  ...
  GAP-SH-NAME-01: rebuild CSV `name`←`name_building` · **cấm** IsWeak reject «Nhà hạt»/«Loại CT» · **cấm** fallback `name_of_route_asset` khi name_building hợp lệ
  GAP-SH-SPEC-01: giữ station dump §4 attrs trong dumpSpecs · km trống khi dump null · FE dumpSpecLabels đủ key
  XCO: GET by id AllowedCompanyIds
  POST/PUT: accept dumpSpecs merge · **không** required kmTo for STATION_HOUSE

**DoD:**
- [ ] API-01…05 verify
- [ ] name/import fix (**GAP-SH-NAME-01** · **GAP-SH-SPEC-01**)
- [ ] no-parent-json · migration **none**

---

### T-BE-INIT-01 — init-data STATION_HOUSE LOOKUP

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**from_solution:** API-06  
**DoD:** `RoadAssetInitDataDto.stationWorkTypes` + `stationBuildLocations` + `officeBuildingGrades` + `auxiliaryWorksGrades: RoadAssetLookupOption[]` · options = dump LOOKUP_STATIC / seed · **cấm** invent station master SearchInput P1 · **cấm** FE hardcode · **GAP-SH-LOOKUP-01**
...
| T-QA-FORM-01 | từng field required · UI value = body · dumpSpecs station attrs · kmTo ẩn |
| T-QA-FILTER-01 | V1–V5 + fields 1:1 `so-ts-station-house-filter-bar.md` · 🔍 work · fail ErpListHeaderFilters / export trên bar |
...
| Auth NuGet | DEFER |
| Station LOOKUP master SearchInput | P2 |
...
| focus | COL profile · S-ATTR editable · kmTo ẩn · stationWorkTypes/stationBuildLocations/officeBuildingGrades/auxiliaryWorksGrades init · name←name_building · LeaveConfirm · filter-bar · dumpSpecs P1 |
| mfeStdRoute | `/so-ts?type=STATION_HOUSE` |
...
| init-data stationWorkTypes/stationBuildLocations/officeBuildingGrades/auxiliaryWorksGrades · cấm hardcode dropdown | ✅ |
...
---
<!-- Version meta: skillId=agent-team-lead skillVersion=2026.08.19.04 schemaVersion=2 workflowVersion=2026.08.30.01 rulesVersion=2026.08.31.2 versionGate=rechecked taskId=task_c105b325 contentHashPriorDataAnaly=sha256:3d78ed6a2ee20b192926ba4ab625d1af20e67dcb78d7035e543c976b364e7a45 route_confirm=route_a -->
