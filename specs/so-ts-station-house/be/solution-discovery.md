# SA — Solution discovery — so-ts-station-house (Sổ TS — Nhà hạt QLĐB)

> Status: **confirmed** (`solution_confirm=approve` · autoApprove=ON · `task_56208969`)  
...
| solution_confirm | **approve** (autoApprove=ON · `task_56208969`) |
| domain_map | **Asset** (`so-ts-station-house` → inherit parent `asset` · prefix `api/v1/asset`) |
| sa_tz_gate | **`tz_na`** |
...
| domain | **Asset** |
| controlHint | `specs/_data-analy/features/so-ts-station-house-control-hint.md` |
| realData | `specs/_data-analy/features/so-ts-station-house-real-data.md` |
| design | `specs/so-ts-station-house/ui/design.md` (confirmed) |
...
| priorTask | `task_93e161af` (design completed) |
...
| DOMAIN-MAP | slug `asset` → Asset · `so-ts-station-house` chưa liệt kê | Cite **Asset** · docs delta optional row `so-ts-station-house`→Asset | cite + optional docs |
| Entity | `RoadAssetEntity` · `rmms_road_assets` · `DumpSpecs` text | **giữ** dumpSpecs P1 cho toàn bộ attr STATION_HOUSE §4 · mirror `name_building` | **no Schema_*** flatten |
| Import name | rebuild `FindOfficialName` gồm `name_building` · sample «hạt 1 QL1» | `name` ← `name_building` · trống OK · **cấm** IsWeak → đoạn tuyến — **GAP-SH-NAME-01** | guard rebuild + form |
| dumpSpecs attrs | FE `dumpSpecLabels` thiếu attr keys · `type_work` label | Label VN đủ header dump · form Input/Select merge keys — **GAP-SH-SPEC-01** | FE labels + form write |
| LOOKUP type_work / mặt cắt / cấp | text trong dumpSpecs | Dropdown LOOKUP_STATIC · init-data `stationWorkTypes[]` · `stationBuildLocations[]` · `officeBuildingGrades[]` · `auxiliaryWorksGrades[]` (dump distinct / seed) — **GAP-SH-LOOKUP-01** | delta init-data |
...
| Grid profile | 1 schema (+ profile peer types) | Hide `type`/`kmTo`/qty/unit · show `type_work_id` · hide-low-fill OFF default DT/cấp/vật tư/khuôn viên | FE type-profile |
| Point `kmTo` | form hiện / bắt buộc với type ≠ KM_POST | **Ẩn** + không required khi `type=STATION_HOUSE` · `kmFrom` **không** required · S-LOC-POINT only | FE validation |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal — **GAP-SH-LEAVE-01** | FE only |
...
| Import | `RoadAssetCatalogHandler` · catalogKey `road_assets` · type seed `STATION_HOUSE` · dump `tbl_road_admin_office` |
| Rebuild CSV | `Linm.RMMS.Data/local-script/RebuildGovVn.cs` · `FindOfficialName` incl. `name_building` · Prefix `NH` · `IsWeakAssetName` |
...
| Docs | `docs/context/features/so-ts-station-house.md` · parent `so-ts-type-grid.md` · `import-gov-asset-fields.md` |
...
| Surface | Path |
|---------|------|
| UI list | `/so-ts?type=STATION_HOUSE` · alias board `/so-ts-station-house` (optional redirect) |
...
| Out of pack | flatten DB columns · nhà-hạt master SearchInput · Excel wizard · Kind F map · invent History API |
...
AskQuestion (autoApprove=ON · agent confirm): `sa_tz_gate=tz_na` · `sa_xco_gate=xco_get_only` · `sa_shared_table=share_tenant` · `2026-09-01T08:10:00.000Z`
...
|---------|---------|----------|-----|
| S-LIST | Kind B A–D+F+H | list | API-01 list `?type=STATION_HOUSE` |
| S-FORM-CREATE | Kind B full-page 5 cột | create | API-03 POST (`type=STATION_HOUSE`) |
...
| S-ALIAS | navigate optional | — | `/so-ts-station-house` → `/so-ts?type=STATION_HOUSE` |
...
| type_work_id | Dropdown LOOKUP_STATIC | form S-ATTR · grid ON | API-06 init-data `stationWorkTypes` |
| build_location | Dropdown LOOKUP_STATIC | form S-ATTR | API-06 `stationBuildLocations` |
| office_building_grade_id | Dropdown LOOKUP_STATIC | form S-ATTR · grid OFF default | API-06 `officeBuildingGrades` |
| auxiliary_works_grade_id | Dropdown LOOKUP_STATIC | form S-ATTR · grid OFF default | API-06 `auxiliaryWorksGrades` |
...
|----------|------|---------------|-------|
| list | GET list `?type=STATION_HOUSE` + filters | — | page=1 on filter change · profile cột STATION_HOUSE |
| create | empty · type lock `STATION_HOUSE` · init-data | POST body + dumpSpecs merge | IdCode BE prefix `NH-` · **không** required `kmTo`/`kmFrom` |
| edit | GET `/{id}` | PUT `/{id}` · merge dumpSpecs | leave-confirm dirty |
| view | GET `/{id}` | — | readOnly · **không** disabled xám |
| copy | GET `/{id}` → clear id/code | POST | keep `STATION_HOUSE` · new IdCode |
...
|-----------|------------|--------|
| `type` | SearchInput asset-type (prefill/ẩn) | **required** `STATION_HOUSE` deep-link |
| `search` | SearchTextInput | mã · tên CT · tuyến · QR |
...
| List grid | name · route · routeNamed · routeSegment · kmFrom · type_work_id · status/gps optional | transaction + dumpSpecs parse | `RoadAssetEntity` | hide type/kmTo/qty/unit · hide-low-fill OFF: total_area_office_building · total_area_auxiliary_works · office_building_grade_id · auxiliary_works_grade_id · materials_in_office · site_area_using_land |
...
| type | SearchInput `asset-type` | `?type=` / body `type` · lock `STATION_HOUSE` |
...
| kmTo | Text filter only | `?kmTo=` filter · **ẩn** form/grid STATION_HOUSE |
| orgUnit | SearchInput tree | `?orgUnit=` |
| code | Text readonly | BE IdCode · prefix `NH-` |
| name / name_building | Text | body `name` = `name_building` · mirror dumpSpecs.key · trống OK |
| status | Dropdown | init-data `statuses` · body `status` |
| source | Dropdown | init-data `sources` · body `source` |
| type_work_id | Dropdown LOOKUP_STATIC | **dumpSpecs** · options init-data `stationWorkTypes` · **required** form |
| build_location | Dropdown LOOKUP_STATIC | **dumpSpecs** · options `stationBuildLocations` |
| office_building_grade_id | Dropdown LOOKUP_STATIC | **dumpSpecs** · `officeBuildingGrades` · grid OFF default |
| total_area_office_building | Number | **dumpSpecs** · grid OFF default |
| site_area_using_land | Number | **dumpSpecs** · grid OFF default |
| auxiliary_works_grade_id | Dropdown LOOKUP_STATIC | **dumpSpecs** · `auxiliaryWorksGrades` · grid OFF default |
| total_area_auxiliary_works | Number | **dumpSpecs** · grid OFF default |
| materials_in_office | TextArea | **dumpSpecs** · grid OFF default |
| lat / lng | Number | body `lat`/`lng` · dump `from_coordinatey/x` |
| qr / valueVnd / note | Text / Money / TextArea | scalars |
| quantity / unitCode | — | **ẩn** STATION_HOUSE · giữ DTO parent |
...
| materials_in_office | via DumpSpecs | `dumpSpecs.materials_in_office` |
...
**Cấm** parent `AttrsJson` ngoài cột `DumpSpecs` đã có · **cấm** invent `StationHouseJson` / child table P1.
...
| Flatten nhà-hạt attrs | **DEFER P2** — **không** Schema_* this turn (Design/PO OUT · dumpSpecs P1 · **GAP-SH-FLAT-01**) |
| dumpSpecs write | Form S-ATTR (+ name_building mirror) merge keys vào JSON string trên POST/PUT · list grid parse cùng keys |
| New Schema_* | **none** |
| Seed / init | delta `stationWorkTypes[]` · `stationBuildLocations[]` · `officeBuildingGrades[]` · `auxiliaryWorksGrades[]` trên init-data · UI schema `road-assets` type-profile hide-empty |
| data-import | `RoadAssetCatalogHandler` · rebuild CSV **name**=`name_building` · giữ toàn bộ attr STATION_HOUSE trong dumpSpecs · km trống khi null |
| Default đơn vị | II.1/QL.1 khi reopen import (parent) |

### Import IsWeak / name (GAP-SH-NAME-01)

| Cite | Decision |
|------|----------|
| `RebuildGovVn.cs` `FindOfficialName` | gồm `name_building` — **MUST** ưu tiên khi non-junk cho STATION_HOUSE |
| `ResolveTypeAssetName` / `ResolveAssetName` | Dev verify CSV `name` = `name_building` (cite sample `NH-road_admin_office_525966` → «hạt 1 QL1») |
| `IsWeakAssetName` | chỉ QL./CT. — **cấm** classify tên nhà hạt weak · **cấm** fallback `name_of_route_asset` khi `name_building` hợp lệ (trống OK — **không** ép đoạn) |
| `RoadAssetCatalogHandler` | đọc CSV `name` đã đúng · dumpSpecs giữ `name_building` + attrs §4 |
| Form | S-NAME bind `name` ← `name_building` · sync mirror dumpSpecs |
...
| | |
|--|--|
| Purpose | Paged list · filter type STATION_HOUSE |
...
| Context | `docs/context/features/so-ts-station-house.md` |
...
BFF: `GET web-bff/api/v1/asset/road-assets?type=STATION_HOUSE&…`
...
| | |
|--|--|
| Purpose | Create · type `STATION_HOUSE` · IdCode BE `NH-` |
| Permission | `asset.road-assets.create` |
| Tenant | current company |
| Request | `CreateRoadAssetRequest` · `DumpSpecs` JSON string incl. name_building · station attrs §4 |
| Response | `RoadAssetDto` |
| Errors | 422 required type · status · route · type_work_id (name trống OK · kmFrom/kmTo không required) |
| Form surfaces | create · copy |
| Notes | **không** required `kmTo`/`kmFrom` · `name` = tên công trình |
| Migration | none |
...
| | |
|--|--|
| Purpose | LOOKUP_STATIC statuses · sources · units · **stationWorkTypes** · **stationBuildLocations** · **officeBuildingGrades** · **auxiliaryWorksGrades** (delta) |
| Permission | read |
| Request | — |
| Response | `RoadAssetInitDataDto` + 4 lookup arrays `RoadAssetLookupOption[]` |
| Form surfaces | S-META · S-ATTR |
| Decision | **GAP-SH-LOOKUP-01** P1 — options = dump distinct / seed labels (value=label dump string · cite dump `tbl_road_admin_office`) · **cấm** invent nhà-hạt master SearchInput · **cấm** hardcode FE không cite dump |
...
|-------------|-----|-------------|
| asset-type | LKP | SearchInput · lock `STATION_HOUSE` |
| road-route | LKP | SearchInput |
| org-unit | LKP | SearchInput tree |
| LOOKUP_STATIC status/source/units | API-06 | keep init-data |
| LOOKUP_STATIC type_work_id | API-06 `stationWorkTypes` | **delta** · dump distinct |
| LOOKUP_STATIC build_location | API-06 `stationBuildLocations` | **delta** · dump distinct |
| LOOKUP_STATIC office_building_grade_id | API-06 `officeBuildingGrades` | **delta** · dump distinct |
| LOOKUP_STATIC auxiliary_works_grade_id | API-06 `auxiliaryWorksGrades` | **delta** · dump distinct |
...
|----|-------------|-------|
| GAP-SOTS-COL-01 | Type-profile STATION_HOUSE hide type/kmTo/qty/unit · show `type_work_id` · hide-low-fill OFF default DT/cấp/vật tư/khuôn viên | Dev UI |
...
| GAP-SH-NAME-01 | `name`←`name_building` · trống OK · guard IsWeak + rebuild | Dev BE import + rebuild + form |
| GAP-SH-SPEC-01 | Label VN đủ attr keys · type_work label · giữ attrs trong dumpSpecs · km trống khi null | Dev FE labels + form + import |
| GAP-SH-POINT-01 | Ẩn + không required `kmTo` · `kmFrom` không required · không ép `"0"` · không mount S-LOC-RANGE | Dev UI |
| GAP-SH-LOOKUP-01 | Dropdown LOOKUP_STATIC · init-data 4 arrays · dumpSpecs write | Dev BE init + UI |
| GAP-SH-LEAVE-01 | LeaveConfirmModal + useAlert/Modal | Dev UI |
| GAP-SH-ROUTE-01 | Live URL `?type=STATION_HOUSE` · alias board optional redirect | Dev UI optional |
| GAP-SOTS-API-DOC | Cite `api/v1/asset/road-assets` only · DOMAIN-MAP Asset | docs |
| GAP-SOTS-TAB-01 | **Cấm** tab legacy DRVN | Dev UI |
| GAP-SH-FLAT-01 | Flatten DB columns **DEFER P2** | — |
| GAP-SH-AUTH-01 | RequirePermission attr TODO align Auth codes | BE later |

---

## Out of pack

Flatten Schema_* · nhà-hạt master SearchInput · Excel import wizard UI · Kind F map canvas · invent History path · invent `api/v1/so-ts/*` · ERP fork · yarn build/e2e/start:std ở SA · Step 4b/migration · GIS deep-link `nha-hat`.
...
| confirmedAt | `2026-09-01T08:10:00.000Z` |
| taskId | `task_56208969` |
...
| Role | Artifact | Need |
|------|----------|------|
| **team-lead** | `task/so-ts-station-house.md` | T-* từ Gaps · profile · S-ATTR · init LOOKUP · LeaveConfirm · dumpSpecLabels |
| Dev | implement | `/agent-dev` · **cấm** start trong task SA |
| QA | scenarios + e2e | queued `/agent-qa*` only |

Compact: `specs/so-ts-station-house/handoff/sa-compact.md`
