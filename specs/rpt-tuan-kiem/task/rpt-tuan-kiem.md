# Team-lead — task pack · rpt-tuan-kiem (Kind E)
...
| packKind | **`report`** (Kind **E** AnalyticsReportShell) — packet board `list` **stale** (GAP-PO-BTK-02) |
| Feature Kind | **E** · leaf `/bao-cao/tuan-kiem` · **không** CRUD form |
| mfeStdRoute | `/bao-cao/tuan-kiem` |
| route_confirm | **route_a** `/bao-cao/tuan-kiem` (live + STATUS + Design · Autopilot ON **không** AskQuestion `route_confirm` · **keep_current**) |
...
**Cấm ERP.*** · **cấm** `Domains/Master` · **cấm** `api/v1/rmms/*` · **cấm** `api/v1/reports` · **cấm** reuse `GET api/v1/report/patrol-log-inspect` · **cấm** gộp `PatrolType=road` / tuần đường · **cấm** parent JSON · **cấm** copy CRUD `patrol` · **cấm** folder domain mới · **cấm** POST/PUT/DELETE `PatrolSession` trên slug này · **cấm** Kind B `LinCatalogUiSchemaEditorModal`.
...
|--------|------|-----------|
| Design | `specs/rpt-tuan-kiem/ui/design.md` + reviewUrl | T-UI-LIST A–D · T-UI-ACT · T-UI-LKP · T-UI-FIELD · T-UI-PROD · T-UI-UX · GAP-DS-BTK-01..12 |
| Solution | `specs/rpt-tuan-kiem/be/solution-discovery.md` | T-BE · T-BFF · T-PERM · GAP-SA-BTK-* |
| Prototype | `ui/prototype/rpt-tuan-kiem-prototype.html` | UI DoD parity |
| controlHint | `specs/_data-analy/features/rpt-tuan-kiem-control-hint.md` | SearchInput tuyến/trạng thái/NV · Date · Input |
| PO | `specs/rpt-tuan-kiem/po/requirement.md` | GAP-PO-BTK-01..12 |
...
| Concern | Reuse | Cấm |
|---------|-------|-----|
| UI | `Lin*` / `PatrolInspectFilterBar` | nested CatalogListShell · raw `<table>` · local pager · native `<select>` · `LinListTableConfigModal` · `configHint` · Kind B `LinCatalogUiSchemaEditorModal` |
| HTTP | `apiClient` · `reportEndpoint` re-export · query **`q`** | fork ApiClient · reuse `patrol-log-inspect` / tuần đường |
...
| Persist | in-memory 14 `inspect` + 1 `road` excluded P1 | warehouse tables · parent JSON · EF join P1 |
...
| `source.routes` | `/bao-cao/tuan-kiem` only P1 · **không** `/new` · `/:id` |
| `source.backend` | `D:/AI-QLBD/Linm.RMMS.WebService` |
| `source.domain` | **Report** (DOMAIN-MAP slug `rpt-tuan-kiem` → kebab `report`) |
...
| `source.persistence` | **không** P1 — in-memory `PatrolInspect` seed |
...
**Cấm** invent domain folder. MFE live = `Linm.Web.RMMS.Report` (DOMAIN-MAP `BaoCao` = tên cũ — **không** đổi repo). **Cấm** `Linm.Web.ERP.WebService`. **Cấm** gộp `rpt-nhat-ky-tuan-kiem` / tuần đường.
...
| API-01 | GET | `/patrol-inspect` | DONE in-memory · query `staffId` `routeId` `status` `from` `to` `q` `page` `pageSize` · alias `search` · `type` **ignored** · hard-filter `PatrolType=inspect` | **keep** · FilterRoute **exact** · FE gửi **`q`** |
| API-02 | GET | `/patrol-inspect/export` | DONE CSV UTF-8 BOM · filename **`patrol-inspect.csv`** · **không** page | **keep** BE · FE `canExport: viewed` · subset cột đang hiện |
...
**Row DTO** `ReportPatrolRoadRowDto` (reuse scalar — **không** class mới P1): `id` `sessionId` `code` `day` `startedAt` `route` `staffId` `userName` `patrolType` `patrolTypeLabel` `checkInCount` `coveragePercent` `status` `statusLabel` `offlineQueued` `note`.
...
| SD-AUTH | stub | `report.tuan-kiem.read` (list + export) |
...
|------|--------|-----------|
| A | Header | `LinPageLayout` kind=`report` · icon `fas fa-clipboard-check` · title **Báo cáo tuần kiểm** · **cấm** Thêm mới |
...
## retry.ssot_rereview (TL live MFE 2026-08-16 · `PatrolInspectReportPage` + `PatrolInspectFilterBar` + BE)

Audit `Linm.Web.RMMS.Report` `src/pages/PatrolInspectReportPage/PatrolInspectReportPage.tsx` + `PatrolInspectFilterBar.tsx` + `reportEndpoint.getPatrolInspect` / `exportPatrolInspect` + `ReportQueryController` `patrol-inspect` + `ReportService.FilterPatrolInspect` + BFF `patrol-inspect` — **cấm** Dev chỉ patch 1 chỗ nếu còn GAP cùng surface.
...
| 11 | Xem mới load · empty hint | **PASS** `viewed` · «Chưa xem — nhấn «Xem» để tải báo cáo tuần kiểm.» | — |
| 12 | Làm mới khi `!viewed` | live `reloadAll`: toast + **return** · **không** fetch | **PASS** (GAP-DS-BTK-10) |
| 13 | Excel sau Xem + applied + toolbar | `canExport: viewed` · `onExport` toolbar · filename `patrol-inspect.csv` · `CSV_COL_BY_GRID` | **PASS** |
...
| 22 | Seed 14 inspect + 1 road excluded · CUC2 · không QL.22 | BE `pi1`–`pi13`+`pi15` inspect · `pi14` `TD-*` road · Filter `PatrolType==inspect` | **PASS** |
...
| 28 | Chỉ `PatrolType=inspect` trên lưới | FilterPatrolInspect hard-filter | **PASS** |
| 29 | Prefix | `reportEndpoint` `/patrol-inspect` | **PASS** — **cấm** `api/v1/reports` |
...
|----|---------|---------|-------|----------|---------|----------|
| S-BTK | Report A–D + SoCai | Full page Kind E | `/bao-cao/tuan-kiem` | report | filter `LinErpListFilterBar` → Xem · refresh gated · Excel toolbar · chart SoCai · config FULL · drill phiên | `/agent-dev` + `/erp-report-context` |
...
|----|------|-------|------|-------------|-----|----------|
| **T-CTX-01** | rpt-tuan-kiem | docs | — | **done** | Context + controlHint + Design + SA · Kind E · prefix `api/v1/report` · **cấm** ERP.* · **cấm** `reports/patrol-inspect` · **cấm** `patrol-log-inspect` | — |
| **T-PERM-01** | rpt-tuan-kiem | ui+api | T-CTX-01 | stub | Code `report.tuan-kiem.read` (API-00…02). Gắn `[RequirePermission]` khi CommonLib ≥1.4.0 · **không** block P1 | `/agent-dev` |
| **T-UI-LIST-01** | /bao-cao/tuan-kiem | ui-list | T-PERM-01 | **PASS** | **Giữ** 1× LinPageLayout · `resizable: true` · LinCatalogListPagination · Zone A title Báo cáo tuần kiểm · Xem mới load · cột PO §5 · **cấm** nested CatalogListShell / footerPagination / pageSizeBar / raw table / Thêm mới A / Company-QL-Km · FE gửi **`q`** | `/agent-dev` |
...
| **T-UI-RPT-01** | /bao-cao/tuan-kiem | ui | T-PERM-01 | **PASS** | **`LinErpListFilterBar`** + **filter-bar-layout-hard V1–V5** · Xem = `onSearch` · Enter trên Input = Xem · **cấm** `LinListFilterField` / wrapper / Excel trên bar | `/agent-dev` |
| **T-UI-RPT-TB-01** | /bao-cao/tuan-kiem | ui | T-UI-RPT-01 | **PASS** | Common actions **chỉ** `reportToolbar` (Làm mới · Chart · In · Config · Xuất Excel) · **cấm** filter button extra · GAP-FILTER-BAR-08 | `/agent-dev` |
| **T-UI-RPT-CONFIG-01** | /bao-cao/tuan-kiem | ui | T-UI-RPT-01 | **done analog** | Config FULL `load/saveErpReportDisplayConfig` — **cấm** `configHint` / `LinListTableConfigModal` / Kind B schema editor | `/agent-dev` |
| **T-UI-RPT-EXPORT-01** | /bao-cao/tuan-kiem | ui | T-UI-RPT-TB-01 | **PASS** | `report_export=export_yes` · toolbar `onExport` · viewed only · `patrol-inspect.csv` UTF-8 BOM subset | `/agent-dev` |
| **T-UI-RPT-CHART-01** | /bao-cao/tuan-kiem | ui | T-UI-RPT-CONFIG-01 | **PASS** | SoCai client từ `items` — số phiên theo ngày · trạng thái · tuyến · **cấm** stub toast · **không** API chart · **cấm** KPI 4 card / map | `/agent-dev` |
| **T-UI-ACT-01** | rpt-tuan-kiem | ui | T-UI-LIST-01 | **PASS gated** | Inventory dưới · Làm mới `!viewed` toast · Excel **chỉ** khi viewed | `/agent-dev` |
| **T-UI-LKP-01** | rpt-tuan-kiem | ui | T-UI-LIST-01 | **PASS** | tuyến SearchInput road-route CUC2 + empty=Tất cả · trạng thái CI enum · NV nva/ttb/lvc/pmd · **cấm** native select · **cấm** invent QL.22 | `/agent-dev` |
| **T-UI-FIELD-01** | rpt-tuan-kiem | ui | T-UI-LIST-01 | **PASS** | from/to Date SSOT · ISO date-only query · grid `day` `formatDayVi` · cột DTO scalars readonly | `/agent-dev` |
| **T-UI-PROD-01** | rpt-tuan-kiem | ui | T-UI-FORM-01 | **PASS** | Kind E product · form OUT | `/agent-dev` |
| **T-UI-UX-01** | rpt-tuan-kiem | ui | T-UI-LIST-01 | **PASS** | gap 4/8/16 · **cấm** `filterMaxWidth` cap · toast SSOT · title không clip · empty/skeleton không blank body (`tl-list-shell-height` analog report) | `/agent-dev` |
| **T-BE-01** / **T-BE-RPT-01** | report | api | T-CTX-01 | **PASS keep** | `GET api/v1/report/patrol-inspect` · FilterRoute **exact** · FilterDay · pageSize allow-list · seed 14 inspect · **không** migration · **không** path mới | `/agent-dev` |
| **T-BE-02** | export | api | T-BE-01 | **PASS keep** | CSV UTF-8 BOM filename `patrol-inspect.csv` · header union SA · **không** page | `/agent-dev` |
...
| **T-QA-01** / **T-QA-RPT-01** | rpt-tuan-kiem | qa | T-UI-LIST-01 | **done** | scenarios Kind E · mfeStdUrl · Xem · filter V1+V5 · Excel toolbar · chỉ `inspect` · form OUT · typecheck+build PASS (`task_043b63fc`) | `/agent-qa` |
| **T-RV-01** | rpt-tuan-kiem | review | T-QA-01 | pending | findings.md · review_confirm khi tới lượt (autoApprove ON) | `/agent-review` |

## T-UI-LIST (A–D · Kind E)

| Zone | Task | DoD |
|------|------|-----|
| A | Header icon `fas fa-clipboard-check` + title **Báo cáo tuần kiểm** 22px không clip | **Cấm** Thêm mới / Tạo mới trên A |
| B | `LinErpListFilterBar` title trái · input + tìm cụm phải · 1 hàng wrap | GAP-FILTER-BAR / GAP-PO-BTK-09 |
| C | `listTitle` **Kết quả tuần kiểm** · `LinCatalogDataGrid` | skeleton 8 · **cấm** cột CRUD ⋯ · **cấm** Company/QL/Km |
| D | **Luôn** `LinCatalogListPagination` 50/100/200/500 · `totalCount=0` khi chưa Xem | **Cấm** ẩn D |

Empty: chưa Xem — «Chưa xem — nhấn «Xem» để tải báo cáo tuần kiểm.» · viewed 0 dòng — «Không có dòng phù hợp bộ lọc.»
...
| patrolTypeLabel | Loại tuần | Text (`inspect` = Tuần kiểm) |
...
|----|-------|-----------------|------------|
| T-BE-01 | API | `Domains/Report/Controllers/ReportQueryController.cs` · `GET api/v1/report/patrol-inspect` | **Giữ** envelope · FilterPatrolInspect `PatrolType=inspect` · **cấm** gộp road |
| T-BE-02 | API | `GET api/v1/report/patrol-inspect/export` | CSV UTF-8 BOM `patrol-inspect.csv` · **không** page |
...
| T-BE-04 | Seed | 14 phiên `inspect` CUC2 + 1 road excluded (`pi14`) | **Giữ** · **cấm** invent `QL.22` · EF **P2** |
...
| T-BFF-01 | BFF | `ReportBffController` `patrol-inspect` + `patrol-inspect/export` | **không** business · raw envelope |
| T-LKP-01 | Integration | `GET api/v1/integration/road-routes/search` | Report consume |
| T-PERM-01 | stub | `report.tuan-kiem.read` | document only P1 |

## Dev backlog (IN P1)

| ID | Layer | Task |
|----|-------|------|
| GAP-TL-BTK-VERIFY | FE/BE | `yarn build` + `yarn typecheck` MFE · `dotnet build` **chỉ nếu** đụng API · ghi implement § Build + `retry.ssot_rereview` |

**OUT P1:** warehouse · EF PatrolSession join · CRUD · `ERP.*` · `api/v1/reports` · nhật ký sổ · tuần đường lưới · Kind B schema editor · invent tuyến ngoài CUC2 38.
...
- This task = `roleOnly=team_lead` · **không** chạy Dev trong `task_3c6ff75a`.
