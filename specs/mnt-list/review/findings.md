...
| status | **done** |
| review_confirm | **done** (autopilot · `task_6eafecab` · autoApprove=ON) |
| packKind | **`list`** |
...
| autoApprove | **ON** |
| e2eQa | **ON** · queued prior QA · **cấm** re-run e2e ở role review |
| updatedAt | `2026-08-28T19:47:18.000Z` |
...
| Hash input | Notes |
|------------|-------|
| iOS | `MntListView` · `MntListViewModel` · `MaintenanceRepositoryImpl` · `FetchWorkOrdersUseCase` · `MntListDto` · Keychain |
| Android | `MntListScreen` · `MntListViewModel` · `ApiService.workOrders` · `MntListDtoMapper` · EncryptedSharedPreferences |
| BFF | `MobileApiProxyController` catch-all → `api/v1/maintenance/work-orders` · forward Bearer + `X-Company-Id` |
| API | GET list `page=1` · `pageSize=50` · empty/fail → demo **2** SSOT · **cấm** AssignerName |
| QA store | `qa/store/mnt-list/` A11/A9/A3/P6/P6-2 live PNG · `ok:true` |
...
| Location / camera Info.plist · Manifest | **N/A** feature — list không gọi GPS/camera · app-level location copy exists for patrol |
...
| Demo fallback 2 cards SSOT | **OK** dual |
...
| Zone | Result |
|------|--------|
| A3-CORE vs demo `#sc-mnt-list` | **PASS** — title · Lọc · hub · 2 cards · status · actions · tab Công việc on |
| P6-CORE / P6-CORE-2 vs demo | **PASS** — same copy dual · filter icon Material OK · fold status **Đã hoàn thành** |
| Hub `.row-icon` green leading | **PASS** — green + glyph (kit `LinmRowIcon` plus / Compose equiv) · **cấm** reopen COMP-03 |
| Card actions chat · sync/list · plus | **PASS** dual |
...
| Landing / BffBase store listing HTTPS | **Accept** — Release xcconfig HTTPS · Debug localhost OK · **cấm** LAN IP in store |
...
| R-03 | DTO | — | Dual parity · no AssignerName · demo SSOT | **OK** |
| R-04 | UX | P2 | Sibling CTA / Lọc toast-only P1 | **Accept** |
| R-05 | Align | — | A3 + P6 vs demo · Must **0** · Aligned | **OK** |
| R-06 | Copy | Should | Kit search **Tìm** vs demo **Tìm kiếm công việc…** (`GAP-MOB-COPY-SEARCH-01`) | **Defer** non-block |
| R-07 | QA | — | e2eQa ON · Maestro · store live · prior PASS | **OK** |
| R-08 | Store | P2 | PrivacyInfo / Data safety | **Accept** |
| R-09 | Scope | — | sibling `estimate` · `mnt-chat` · `mnt-progress` · `mnt-log` | **Defer** pending_confirm |
...
| Task | Result |
|------|--------|
| T-IOS-MNT-LIST | PASS (prior Dev) |
| T-AND-MNT-LIST | PASS (prior Dev) |
| T-BE-* | **n/a** |
| T-QA | PASS (`ok:true` · Must align 0) |
| T-REVIEW-SEC / DTO / ALIGN | PASS · Must align = **0** |

## VERIFY GATE (`task_6eafecab` · roleOnly=`review`)
...
List Công việc dual-native: security + DTO + UI align Must **0** · prior QA/Dev VERIFY PASS · Step 4b N/A · Should search copy non-block · sibling pending_confirm. **review_confirm=done** (autopilot). Pipeline **complete**.
...
| generatedAt | 2026-08-28T19:47:18.000Z |
