/**
 * Live DOM assert + DTM filter-bar overflow for so-ts-retaining.
 * RANGE: kmTo visible on form · list kmTo ON · type filter hidden · S-ATTR retaining.
 */
...
const outDir = "D:\\AI-QLBD\\Linm.RMMS.Data\\specs\\so-ts-retaining\\qa\\screens";
const listUrl = "http://localhost:9301/so-ts?type=RETAINING";
const formUrl = "http://localhost:9301/so-ts/tao-moi?type=RETAINING";
const TEST_ID = "rmms-so-ts-retaining-list";
...
const list = await page.evaluate((tid) => {
  ...
  const headers = Array.from(
    ...
    .filter(Boolean);
  return {
    titleList: /Danh sách kè|tường chắn/i.test(body),
    headerRetaining: /Sổ TS — Kè \/ tường chắn/i.test(body),
    ...
    colLoaiTuong: headers.some((h) => /Loại tường chắn|tường chắn/i.test(h)),
    colVatLieu: headers.some((h) => /vật liệu|Loại vật liệu/i.test(h)),
    colChieuDai: headers.some((h) => /Chiều dài|dài/i.test(h)),
    colChieuCao: headers.some((h) => /Chiều cao|cao/i.test(h)),
...
await page.waitForSelector('[data-testid="asset-retaining-attr"]', {
  timeout: 15000,
});
await new Promise((r) => setTimeout(r, 1200));
...
const form = await page.evaluate(() => {
  ...
  const codePrefixKE = !!(
    codeInput && /^KE-/i.test(String(codeInput.value || ""))
  );
  return {
    cols5: cols,
    retainingAttr: !!document.querySelector(
      '[data-testid="asset-retaining-attr"]',
    ),
    loaiTuong: /Loại tường chắn/i.test(body),
    vatLieu: /Loại vật liệu/i.test(body),
    chieuDai: /Chiều dài/i.test(body),
    chieuCao: /Chiều cao/i.test(body),
    loaiMong: /Loại móng/i.test(body),
    kmToVisible,
    codePrefixKE,
