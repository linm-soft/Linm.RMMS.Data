# Team lead — tasks — so-ts-retaining
...
| formType | `list` |
| typeCode | `RETAINING` |
| cluster | `linear_protect` · tile `t20` |
| dump | `tbl_retaining_wall` |
| prefix | **`KE-`** (GIS short **`KE`**) |
| unit | `KET_CAU` · CSV `9660` |
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-RETAINING-LOOKUP/NAME/ASSETTYPE/PREFIX/RANGE/ROUTE/SPEC/PEER · flatten DEFER P1 |
| solution_confirm | **approve** (`task_d63d88eb`) |
| design_confirm | **approve** (`task_476f6ddd`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=RETAINING`** · alias board `/so-ts-retaining` optional redirect · alt B primary-alias-only rejected |
| productRoot | `D:/AI-QLBD/Linm.RMMS.Data` |
| reviewUrl | `file:///D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-retaining/ui/prototype/so-ts-retaining-list-prototype.html` |
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:81662f66f48ea982b12b06d93e0716f7449b1356d169541e62a40b377178c061` |
...
**Cấm:** implement product code (trừ template task) · e2e · `yarn build` / `start:std` · Step 4b/migration · ERP.* · invent `api/v1/so-ts/*` · fork `AssetFormPage` · tab legacy · start role khác (**GAP-PKT-ROLE-01**) · gộp `SLOPE_PROTECT`.
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=RETAINING` | **giữ** · alias `/so-ts-retaining` optional | **route_confirm=route_a** · **GAP-RETAINING-ROUTE-01** |
| Grid profile | generic / thiếu RETAINING | **ON**: loại tường · 3 tầng · kmFrom · kmTo · VL · dài · cao TB · số đoạn · móng · **hide-empty** vị trí/asset_type/địa danh · **ẩn** type/ảnh | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
| Form S-ATTR | `<dl>` readonly dumpSpecs | Editable Dropdown/Number đủ dump · merge dumpSpecs | **T-UI-FORM-01** · **GAP-SOTS-FORM-01** |
| Range | risk POINT / ép `"0"` | **S-LOC-RANGE** km* + **4 XY dumpSpecs** · **cấm** ép `"0"` · **không** S-LOC-POINT | **T-UI-FORM-01** · **GAP-RETAINING-RANGE-01** |
| Name | IsWeak / đoạn risk | `name` **optional** · list primary = `retaining_wall_type_id` · **cấm** IsWeak→đoạn | **T-BE-CRUD-01** + form · **GAP-RETAINING-NAME-01** |
| Prefix | lệch / thiếu | IdCode create/import **`KE-`** · GIS short **`KE`** | **T-BE-CRUD-01** · **GAP-RETAINING-PREFIX-01** |
| LOOKUP | text dumpSpecs | LOOKUP_STATIC init `retainingWallTypes[]` · `materialTypes[]` · `foundationTypes[]` · `locationOptions[]` · `dumpAssetTypes[]` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-RETAINING-LOOKUP-01** |
| dump asset_type | confuse entity type | dump `asset_type` **hide-empty** · **≠** entity type | **T-UI-FORM-01** · **GAP-RETAINING-ASSETTYPE-01** |
| dumpSpecLabels | thiếu 7/8 key | đủ **8 key** RETAINING + labels VN | **T-UI-FORM-01** · **GAP-RETAINING-SPEC-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** |
| Filter context | thiếu file | `so-ts-retaining-filter-bar.md` | **T-UI-FILTER-01** · **T-CTX-01** |
| Form surface | full-page 5 cột | **giữ** `data-form-cols="5"` + header chrome | **T-UI-FORM-01** · **cấm** Slideout/footer Lưu |
| Flatten DB | — | **DEFER** (GAP-RETAINING-FLAT-01) | dumpSpecs P1 · migration **none** |
| Peer scope | risk gộp linear_protect / GIS ta-luy | page **RETAINING only** · `SLOPE_PROTECT` riêng | **GAP-RETAINING-PEER-01** |
...
| `source.routes` | **`/so-ts?type=RETAINING`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=RETAINING` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=RETAINING` (Dev điền/verify) · alias `http://localhost:9301/so-ts-retaining` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=RETAINING` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `RETAINING`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-retaining-filter-bar.md` (create T-CTX-01) |
...
|--------|------|----------|
| A | `/so-ts?type=RETAINING` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-retaining` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Kè / tường chắn» khi `type=RETAINING` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-retaining-filter-bar.md` · **cấm** nút Tìm riêng |
| DES-GRID-C0 | listTitle «Danh sách kè / tường chắn» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · RETAINING column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=RETAINING` + filters |
| API-02 | GET | `/api/v1/asset/road-assets/{id}` | detail · XCO get_only |
| API-03 | POST | `/api/v1/asset/road-assets` | create · dumpSpecs merge · prefix **`KE-`** |
| API-04 | PUT | `/api/v1/asset/road-assets/{id}` | update · dumpSpecs merge |
| API-05 | DELETE | `/api/v1/asset/road-assets/{id}` | soft |
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · **`retainingWallTypes[]`** · **`materialTypes[]`** · **`foundationTypes[]`** · **`locationOptions[]`** · **`dumpAssetTypes[]`** |
| API-07 | GET | `/api/v1/asset/road-assets/summary-by-type` | peer tile t20 · out of write pack |
| API-LKP-01 | GET | Integration `asset-types` | filter type · form lock RETAINING |
...
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent RetainingJson / Schema_* P1 |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-RETAINING-NAME/PREFIX** · import + rebuild · dumpSpecs P1 |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-RETAINING-LOOKUP-01** retainingWallTypes/materialTypes/foundationTypes/locationOptions/dumpAssetTypes |
...
| T-UI-FORM-01 | Dev | pending (delta) | T-UI-LIST-01 · T-BE-INIT-01 | `/agent-dev` | full-page 5 cột · S-ATTR · **S-LOC-RANGE** · **GAP-SOTS-FORM/REUSE** · **GAP-RETAINING-RANGE/SPEC/ASSETTYPE** |
...
|-------|-----------|-------|
| T-KE-01 | T-UI-LIST-01 | RETAINING grid profile · hide-empty · primary `retaining_wall_type_id` |
| T-KE-02 | T-UI-FORM-01 | S-ATTR editable · dumpSpecLabels 8 key · LOOKUP Dropdown |
| T-KE-03 | T-UI-FORM-01 | S-LOC-RANGE · 4 XY dumpSpecs · cấm ép `"0"` |
| T-KE-04 | T-BE-CRUD-01 + form | name optional · list primary loại tường · cấm IsWeak |
| T-KE-05 | T-BE-CRUD-01 | DefaultCodePrefix **`KE-`** · GIS KE |
| T-KE-06 | T-BE-INIT-01 | LOOKUP_STATIC retainingWallTypes/materialTypes/foundationTypes/locationOptions/dumpAssetTypes |
| T-KE-07 | T-UI-LEAVE-01 | LeaveConfirmModal |
| T-KE-08 | T-CTX-01 / route | alias board-only optional · **GAP-RETAINING-ROUTE-01** |
| T-KE-09 | T-BE-CRUD-01 | DOMAIN-MAP Asset · cấm ERP.* |
| T-KE-10 | this file | pack + gates · tile t20 · unit KET_CAU · peer RETAINING only |
...
**new_page.ssot_rereview:** pending → pass|fail
  ...
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-RETAINING-LOOKUP/NAME/ASSETTYPE/PREFIX/RANGE/SPEC/PEER
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-retaining.md` + parent `so-ts-type-grid.md` · tạo `so-ts-retaining-filter-bar.md` · lane web → `dev` · **cấm** re-scan demo

---

### T-BE-CRUD-01 — Road-assets CRUD delta (RETAINING)

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**DoD:**
- API-01…05 verify · type filter `RETAINING`
- Persist scalars + **dumpSpecs P1** · **cấm** Schema_* / flatten P1
- `name` optional · **cấm** IsWeak→đoạn (**GAP-RETAINING-NAME-01**)
- IdCode create/import prefix **`KE-`** · GIS short **`KE`** (**GAP-RETAINING-PREFIX-01**)
- import `RoadAssetCatalogHandler` / GIS tuong-chan align
...
**deps:** T-BE-CRUD-01  
**devSlash:** `/agent-dev`  
**DoD:** init-data trả `retainingWallTypes[]` · `materialTypes[]` · `foundationTypes[]` · `locationOptions[]` · `dumpAssetTypes[]` · FE Dropdown LOOKUP_STATIC bind `retaining_wall_type_id` / `material_type_id` / `foundation_type_id` / `location_id` / `asset_type` (**GAP-RETAINING-LOOKUP-01**)
...
- Column profile RETAINING: **ON** loại tường (`retaining_wall_type_id` primary) · 3 tầng tuyến · kmFrom · kmTo · `material_type_id` · `actual_protected` · `average_height` · `number` · `foundation_type_id`
- **Ẩn** type · ảnh
- **hide-empty** `location_id` · dump `asset_type` · địa danh province*/commune*
- Header title khi `type=RETAINING`
...
**devSlash:** `/agent-dev`  
**DoD:** load `so-ts-retaining-filter-bar.md` trước Write · `LinErpListFilterBar` · fields: search · type (prefill/ẩn RETAINING) · route 3 tầng · kmFrom/kmTo filter · orgTree · **cấm** nút Tìm riêng · V1–V5
...
- **S-LOC-RANGE:** kmFrom/kmTo + **4 XY** (`lat*`/`lng*` đầu/cuối) dumpSpecs · **cấm** ép `"0"` · **không** S-LOC-POINT (**GAP-RETAINING-RANGE-01**)
- S-NAME: `name` optional · trống OK · **cấm** IsWeak
- S-ATTR editable: `retaining_wall_type_id` · `material_type_id` · `actual_protected` · `average_height` · `number` · `foundation_type_id` · `location_id` · dump `asset_type` (hide-empty · ≠ entity) · dumpSpecLabels VN đủ **8 key** (**GAP-RETAINING-SPEC-01** · **GAP-RETAINING-ASSETTYPE-01**)
- Dropdown LOOKUP_STATIC retainingWallTypes/materialTypes/foundationTypes/locationOptions/dumpAssetTypes · Number/Text đúng controlHint
...
| type | Loại TS | SearchInput asset-type | scalar lock RETAINING |
...
| material_type_id | Loại vật liệu | Dropdown | dumpSpecs · LOOKUP_STATIC · grid ON |
...
| foundation_type_id | Loại móng | Dropdown | dumpSpecs · LOOKUP_STATIC · grid ON |
...
| handoff compact | `specs/so-ts-retaining/handoff/team_lead-compact.md` |
| mfe cwd | `D:/AI-QLBD/MFE-Source/Linm.Web.RMMS.Asset` |
| be | `D:/AI-QLBD/Linm.RMMS.WebService` |
| primary URL | `http://localhost:9301/so-ts?type=RETAINING` |
...
- task: `D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-retaining/task/so-ts-retaining.md`
- compact: `D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-retaining/handoff/team_lead-compact.md`
