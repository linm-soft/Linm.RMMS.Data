...
| formType | `list` |
| typeCode | `DITCH` |
| cluster | `linear_protect` · tile `t10` |
| dump | `tbl_longitudinal` |
| prefix | **`CD-`** (GIS short **`CD`**) |
| unit | `THOAT_NUOC` |
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-DITCH-NAME/LOOKUP/PREFIX/RANGE/ROUTE/PEER · flatten DEFER P2 |
| solution_confirm | **approve** (`task_8f67882d`) |
| design_confirm | **approve** (`task_5b77b576`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=DITCH`** · alias board `/so-ts-ditch` optional redirect · alt B primary-alias-only rejected |
| productRoot | `D:/AI-QLBD/Linm.RMMS.Data` |
| reviewUrl | `file:///D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-ditch/ui/prototype/so-ts-ditch-list-prototype.html` |
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:8f37e4455aded2ca3a045f47a50916be0563e859af9b18bdcc59000ce4305854` |
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=DITCH` | **giữ** · alias `/so-ts-ditch` optional | **route_confirm=route_a** · **GAP-DITCH-ROUTE-01** |
| Grid profile | generic / thiếu DITCH | **ON**: loại rãnh (`ditch_type_id` primary) · 3 tầng · kmFrom · kmTo · hình dạng · dài · cao · **hide-empty** rộng/KT/hố ga/địa danh · **ẩn** type/ảnh | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
| Form S-ATTR | `<dl>` readonly dumpSpecs | Editable Dropdown/Number/Text đủ dump · merge dumpSpecs | **T-UI-FORM-01** · **GAP-SOTS-FORM-01** |
| Range | risk POINT / ép `"0"` | **S-LOC-RANGE** km* + **4 XY dumpSpecs** · **cấm** ép `"0"` · **không** S-LOC-POINT | **T-UI-FORM-01** · **GAP-DITCH-RANGE-01** |
| Name | IsWeak / đoạn risk | `name` **optional** · list primary = `ditch_type_id` · **cấm** IsWeak→đoạn | **T-BE-CRUD-01** + form · **GAP-DITCH-NAME-01** |
| Prefix | lệch / thiếu | IdCode create/import **`CD-`** · GIS short **`CD`** · unit `THOAT_NUOC` | **T-BE-CRUD-01** · **GAP-DITCH-PREFIX-01** |
| LOOKUP | text dumpSpecs | LOOKUP_STATIC init `ditchTypes` · `culvertShapes` · structure/work/materials · vitri | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-DITCH-LOOKUP-01** |
| Peer filter | risk CULVERT_L mix | Page filter **DITCH only** · CULVERT_L DEFER | **T-UI-LIST-01** · **GAP-DITCH-PEER-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** |
| Filter context | thiếu file | `so-ts-ditch-filter-bar.md` | **T-UI-FILTER-01** · **T-CTX-01** |
| Form surface | full-page 5 cột | **giữ** `data-form-cols="5"` + header chrome | **T-UI-FORM-01** · **cấm** Slideout/footer Lưu |
| Flatten DB | — | **DEFER P2** (GAP-DITCH-FLAT-01) | dumpSpecs P1 · migration **none** |
| dumpSpecLabels | thiếu attr keys | đủ dump keys + labels VN | **T-UI-FORM-01** |
...
| `source.routes` | **`/so-ts?type=DITCH`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=DITCH` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=DITCH` (Dev điền/verify) · alias `http://localhost:9301/so-ts-ditch` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=DITCH` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `DITCH`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-ditch-filter-bar.md` (create T-CTX-01) |
...
|--------|------|----------|
| A | `/so-ts?type=DITCH` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-ditch` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Cống / rãnh dọc» khi `type=DITCH` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-ditch-filter-bar.md` · **cấm** nút Tìm riêng |
| DES-GRID-C0 | listTitle «Danh sách cống / rãnh dọc» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · DITCH column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=DITCH` + filters · **DITCH only** |
| API-02 | GET | `/api/v1/asset/road-assets/{id}` | detail · XCO get_only |
| API-03 | POST | `/api/v1/asset/road-assets` | create · dumpSpecs merge · prefix **`CD-`** |
| API-04 | PUT | `/api/v1/asset/road-assets/{id}` | update · dumpSpecs merge |
| API-05 | DELETE | `/api/v1/asset/road-assets/{id}` | soft |
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · **`ditchTypes[]`** · **`culvertShapes[]`** · structure/work/materials · vitri |
| API-07 | GET | `/api/v1/asset/road-assets/summary-by-type` | peer tile t10 · out of write pack |
| API-LKP-01 | GET | Integration `asset-types` | filter type · form lock DITCH |
...
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent DitchJson / Schema_* P1 |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-DITCH-NAME/PREFIX** · import + rebuild · dumpSpecs P1 |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-DITCH-LOOKUP-01** `ditchTypes` · `culvertShapes` · structure/work/materials |
...
| T-UI-LIST-01 | Dev | pending | T-BFF-01 | `/agent-dev` | full `tl-grid-task-template` · **GAP-SOTS-COL-01** · **GAP-DITCH-PEER-01** · LAYOUT-06 |
...
| T-UI-FORM-01 | Dev | pending (delta) | T-UI-LIST-01 · T-BE-INIT-01 | `/agent-dev` | full-page 5 cột · S-ATTR · **S-LOC-RANGE** · **GAP-SOTS-FORM/REUSE** · **GAP-DITCH-RANGE** |
...
|-------|-----------|-------|
| T-DITCH-01 | T-UI-LIST-01 | DITCH grid profile · hide-empty · DITCH-only filter |
| T-DITCH-02 | T-UI-FORM-01 | S-ATTR editable · dumpSpecLabels |
| T-DITCH-03 | T-UI-FORM-01 | S-LOC-RANGE · 4 XY dumpSpecs · cấm ép `"0"` |
| T-DITCH-04 | T-BE-CRUD-01 + form | name optional · list primary `ditch_type_id` · cấm IsWeak |
| T-DITCH-05 | T-BE-CRUD-01 | DefaultCodePrefix **`CD-`** · GIS CD · unit THOAT_NUOC |
| T-DITCH-06 | T-BE-INIT-01 | LOOKUP_STATIC ditchTypes · culvertShapes · structure/work/materials |
| T-DITCH-07 | T-UI-LEAVE-01 | LeaveConfirmModal |
| T-DITCH-08 | T-CTX-01 / route | alias board-only optional |
| T-DITCH-09 | T-BE-CRUD-01 | DOMAIN-MAP Asset · cấm ERP.* |
| T-DITCH-10 | this file | pack + gates |
...
**new_page.ssot_rereview:** pending → pass|fail
  ...
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-DITCH-NAME/LOOKUP/PREFIX/RANGE/PEER
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-ditch.md` + parent `so-ts-type-grid.md` · tạo `so-ts-ditch-filter-bar.md` · lane web → `dev` · **cấm** re-scan demo

---

### T-BE-CRUD-01 — Road-assets CRUD delta (DITCH)

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**DoD:**
- API-01…05 verify · type filter `DITCH` only (**GAP-DITCH-PEER-01**)
- Persist scalars + **dumpSpecs P1** · **cấm** Schema_* / flatten P1
- `name` optional · **cấm** IsWeak→đoạn (**GAP-DITCH-NAME-01**)
- IdCode create/import prefix **`CD-`** · GIS short **`CD`** · unit `THOAT_NUOC` (**GAP-DITCH-PREFIX-01**)
- import `RoadAssetCatalogHandler` / GIS cong-doc align
...
**deps:** T-BE-CRUD-01  
**devSlash:** `/agent-dev`  
**DoD:** init-data trả `ditchTypes[]` · `culvertShapes[]` · structure/work/materials · vitri · FE Dropdown LOOKUP_STATIC bind `ditch_type_id` / `culvert_shape_id` / `structural_type_id` / `work_type_id` / `materials_work_id` / `location_id` (**GAP-DITCH-LOOKUP-01**)
...
- Column profile DITCH: **ON** loại rãnh (`ditch_type_id` primary) · 3 tầng tuyến · kmFrom · kmTo · `culvert_shape_id` · `actual_length` · `height_culvert`
- **Ẩn** type · ảnh
- **hide-empty** rộng đáy/miệng · KT/CT/VL · hố ga · địa danh (province*/commune*)
- Header title khi `type=DITCH`
- Page filter **DITCH only** · CULVERT_L DEFER
...
**devSlash:** `/agent-dev`  
**DoD:** load `so-ts-ditch-filter-bar.md` trước Write · `LinErpListFilterBar` · fields: search · type (prefill/ẩn DITCH) · route 3 tầng · kmFrom/kmTo filter · orgTree · **cấm** nút Tìm riêng · V1–V5
...
- **S-LOC-RANGE:** kmFrom/kmTo + **4 XY** (`lat*`/`lng*` đầu/cuối) dumpSpecs · **cấm** ép `"0"` · **không** S-LOC-POINT (**GAP-DITCH-RANGE-01**)
- S-NAME: `name` optional · trống OK · **cấm** IsWeak
- S-ATTR editable: `ditch_type_id` · `culvert_shape_id` · `actual_length` · `height_culvert` · `width_bottom`/`width_top` · structural/work/materials · `*_manhole` · `location_id` · `province*`/`commune*` · dumpSpecLabels VN
- Dropdown LOOKUP_STATIC `ditchTypes` / `culvertShapes` / structure/work/materials · Number/Text đúng controlHint
...
| type | Loại TS | SearchInput asset-type | scalar lock DITCH |
...
| ditch_type_id | Loại rãnh/cống | Dropdown | dumpSpecs · LOOKUP_STATIC * · grid primary |
| culvert_shape_id | Hình dạng | Dropdown | dumpSpecs · LOOKUP_STATIC · grid ON |
| actual_length / height_culvert | Dài/cao | Number | dumpSpecs · grid ON |
| width_bottom / width_top | Rộng đáy/miệng | Number | dumpSpecs · hide-empty |
| structural_type_id / work_type_id / materials_work_id | KT/CT/VL | Dropdown | dumpSpecs · LOOKUP_STATIC |
| *_manhole | KT hố ga | Number | dumpSpecs · hide-empty |
...
| handoff compact | `specs/so-ts-ditch/handoff/team_lead-compact.md` |
| mfe cwd | `D:/AI-QLBD/MFE-Source/Linm.Web.RMMS.Asset` |
| be | `D:/AI-QLBD/Linm.RMMS.WebService` |
| primary URL | `http://localhost:9301/so-ts?type=DITCH` |
...
- task: `D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-ditch/task/so-ts-ditch.md`
- compact: `D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-ditch/handoff/team_lead-compact.md`
- STATUS: `D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-ditch/STATUS.md`
- prior: `handoff/sa-compact.md` · `design-compact.md` · `po-compact.md` · `handoff/data_analy-compact.md`
