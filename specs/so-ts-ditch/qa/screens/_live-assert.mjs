/**
 * Live DOM assert + DTM filter-bar overflow for so-ts-ditch.
 * RANGE: kmTo visible on form · list kmTo ON · type filter hidden · CD- · ditch attrs.
 */
...
const outDir =
  "D:\\AI-QLBD\\Linm.RMMS.Data\\specs\\so-ts-ditch\\qa\\screens";
const listUrl = "http://localhost:9301/so-ts?type=DITCH";
const formUrl = "http://localhost:9301/so-ts/tao-moi?type=DITCH";
const TEST_ID = "rmms-so-ts-ditch-list";
...
const list = await page.evaluate((tid) => {
  ...
  const headers = Array.from(
    ...
    .filter(Boolean);
  return {
    titleList: /Danh sách cống \/ rãnh dọc|Cống \/ rãnh dọc/i.test(body),
    headerDitch: /Sổ TS — Cống \/ rãnh dọc/i.test(body),
    ...
    colLoaiRanh: headers.some((h) =>
      /Loại rãnh|loại rãnh|cống dọc/i.test(h),
    ),
...
await page.waitForSelector('[data-testid="asset-ditch-attr"]', {
  timeout: 15000,
});
await new Promise((r) => setTimeout(r, 1200));
...
const form = await page.evaluate(() => {
  ...
  const codeHint = /CD-/i.test(body) || /prefix|Mã/i.test(body);
  return {
    cols5: cols,
    ditchAttr: !!document.querySelector('[data-testid="asset-ditch-attr"]'),
    loaiRanh: /Loại rãnh|Loại cống|ditch_type/i.test(body),
