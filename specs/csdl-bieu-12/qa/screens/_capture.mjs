/**
 * QA E2E capture — edit_page T-XLS-S12 + CRUD KEEP.
 ...
 * Import DEFER P1 · export_only_p0 · cấm invent so-ts-green.
 */
...
const hubListSel = '[data-testid="rmms-csdl-bieu-12-list-page"]';
const exportSel =
  '[data-testid="rmms-csdl-bieu-12-list-export-excel-btn"]';
...
const steps = [
  {
    ...
    selector: listSel,
    also: exportSel,
    note: "list Biểu 12 · toolbar Xuất · Import ẩn · filter-bar · no peer",
  },
  {
    id: "S1",
    url: hubUrl,
  ...
  },
  {
    ...
    note: "Create Slideout · form=create KEEP · 2 section khóm+thảm cỏ · CX-",
  },
];
      ...
      if (step.id === "S0") {
        const live = await page.evaluate(() => {
          ...
          const filterRoot = document.querySelector(
            '[data-testid="rmms-csdl-bieu-12-list-filters"]',
          );
          ...
          return {
            ...
            hasExport: testids.includes(
              "rmms-csdl-bieu-12-list-export-excel-btn",
            ),
            hasImport: testids.includes(
              "rmms-csdl-bieu-12-list-import-excel-btn",
            ),
        ...
        /** S-XLS-EXPORT: click → Bieu12_CayXanh_{yyyyMMdd}.xls */
        let exportCheck = {
          attempted: false,
          ok: false,
          fileName: "",
        ...
        try {
          ...
          const okName = /^Bieu12_CayXanh_\d{8}\.xls$/i.test(fileName);
          const dlPath = await download.path().catch(() => null);
          if (dlPath) {
            writeFileSync(
              join(outDir, fileName || "Bieu12_CayXanh_download.xls"),
              readFileSync(dlPath),
            );
          }
          exportCheck = {
...
const manifest = {
  url: listUrl,
  method:
    "playwright channel=chrome · yarn e2e-qa playwright resolve fail → AutoCode createRequire fallback · skip-start · T-XLS-S12",
