...
| Shot | Observed |
|------|----------|
| A3-CORE (iOS) | Title **Cập nhật / bớt** · back **Tài sản** · search `Tìm` · **3** rows live `BB-QL1-NA-478` · `HL-QL1-NA-468` · `KM-QL1-NA-461` · leading cube · **Sửa** / **Bớt** · tab shell · **không** watermark |
| P6-CORE (Android) | Cùng zone title/search · **empty** «Không có tài sản» · toast **Không tải được danh sách tài sản.** · **không** rows · **không** `TS-20260810-*` |
| Demo HTML | Search `#i-search` · placeholder dài · rows sample · Sửa/Bớt inline |

## Must

| ID | Result |
|----|--------|
| GAP-QA-REAL-01 | **OPEN** — Android LoadFailed CORE khi BFF+iOS live |
| GAP-MOB-UX-COMP-03 | **PASS** — demo không `.row-icon` row; live iOS có leading cube (richer) |
