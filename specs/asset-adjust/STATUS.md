...
| e2eQa | ON · yarn e2e-qa-mobile CLI PASS · visual **FAIL** · **cấm** start:std / mfeStdUrl |
| updatedAt | `2026-08-31T00:20:49.569Z` |
## Lock

| agent | scope | id | at |
|-------|-------|-----|-----|
| — | — | — | **released** (qa fail · `task_74581051`) |
...
| task_9e935efd | asset-adjust | dev | team_lead | **completed** | implement/ios.md + android.md · SoftDelete client + dual screen · hub wire · VERIFY iOS xcodegen+xcodebuild iPhone 17 Pro · Android assembleDebug · BFF dotnet build **PASS** · Step 4b N/A · **cấm** e2e / mfeStdUrl · next `/agent-qa*` |
...
- data-analy → po → ui → be → task → implement → **qa blocked** → review pending
- native: e2eQa ON → `yarn e2e-qa-mobile` CLI PASS · visual FAIL — **cấm** mfeStdUrl · **cấm** completed
- bugs: `specs/asset-adjust/qa/bugs/asset-adjust.md` · align: `ui/review/align-ux.md`
- Next: board **`qa_fail_rollback`** → Dev `asset-adjust-qa-fix-plan.md` · **cấm** chain review
