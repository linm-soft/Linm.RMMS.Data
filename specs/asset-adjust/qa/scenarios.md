...
| Maestro Android → `#sc-asset-adjust` | **PASS** (screen) |
| `yarn e2e-qa-mobile` CLI | **PASS** · `ok: true` |
| Real BFF list bind CORE | **FAIL** · iOS live `KM-QL1-NA-*` · Android demo `TS-20260810-014` + toast loadFail · **GAP-QA-REAL-01** |
...
| QA-04 | Title **Cập nhật / bớt** · Search · row · **Sửa** · **Bớt** | **PASS** chrome | A3 / P6 |
| QA-05 | Live GET list (cấm demo-only CORE) | **FAIL** Android | ![A3-CORE](screens/A3-CORE.png) live · ![P6-CORE](screens/P6-CORE.png) demo+toast · **GAP-QA-REAL-01** |
| QA-06 | Soft-delete modal open/cancel (không mutate) | **PASS** smoke | Maestro optional remove→cancel |
| QA-07 | Watermark / «Phiên bản Gói» | **PASS** | A3 / P6 không watermark |
| QA-08 | Search placeholder SSOT dài | **Should** | Kit hardcode `Tìm` · MapFile muốn dài · **DEFER kit** |
...
| P6-CORE-2 | P6 | **PASS** CLI | ![P6-CORE-2](screens/P6-CORE-2.png) |
...
| ID | Severity | Summary |
|----|----------|---------|
| GAP-QA-REAL-01 | **Must** | Android CORE demo `TS-20260810-014` + toast «Không tải được danh sách…» trong khi BFF live + iOS bind `KM-QL1-NA-*` |
| GAP-MOB-SEARCH-PLACEHOLDER | Should · DEFER kit | `LinmSearchField` hardcode placeholder `Tìm` · MapFile/PO SSOT dài |
...
| Android | `qa/e2e/android.yaml` | **PASS** Maestro · P6 demo fallback |

## Notes

- `--skip-start`: API listen **:5111** (compose không bind :5101 · Win64 camera) · BFF :5202.
- `--skip-build` lần 2 (sau build PASS lần 1).
- Demo HTML `#sc-asset-adjust` **không** `.row-icon` trên row — live có leading cube · **không** GAP-MOB-UX-COMP-03.
...
|-------|-------|
| Verdict | **fail** |
| Next | board **`qa_fail_rollback`** → Dev `implement/asset-adjust-qa-fix-plan.md` + `qa_fix_plan` |
...
---
<!-- Version meta: skillId=agent-qa-mobile dorGate=FAIL contentHash=sha256:asset-adjust-qa-20260831 -->
