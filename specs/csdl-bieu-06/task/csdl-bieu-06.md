...
| peerSoTs | `so-ts-underpass` · deep-link only · **cấm** merge form |
| solution_confirm | **approve** (`task_789a57e3`) |
| design_confirm | **approve** (`task_2224e771`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — **`/csdl-bieu-06`** + hub entry |
| productRoot | `D:/AI-QLBD/Linm.RMMS.Data` |
| reviewUrl | `file:///D:/AI-QLBD/Linm.RMMS.Data/specs/csdl-bieu-06/ui/prototype/csdl-bieu-06-list-prototype.html` |
| prior · data_analy | **confirmed** · contentHash `sha256:ffc5c8381b04dcec3f0376df87187012f34b16bf211a3e3fb7311a8375a386f0` |
...
| Area | Current (hub generic) | New (Biểu 06 typed) | Action |
|------|------------------------|---------------------|--------|
| Route | hub-only `?resource=underpasses` | **alias** `/csdl-bieu-06` + hub | **route_confirm=route_a** · **T-FE-01** / **T-UI-LIST-01** |
| Form | 3 ô `detail*` | Typed **19 cột** Kind D Slideout | **T-FE-02** / **T-UI-FORM-01** · **GAP-BIEU06-TYPED-01** |
| underpassKind | missing / free | Dropdown hc_ds / hop_kt | **T-BE-03** · **GAP-BIEU06-KIND-01** |
| apertureM | — | Number **number_m** * | **T-UI-FIELD-01** · Q-APERTURE |
| pipeCount | — | Number optional ≥1 | **T-BE-03** · **GAP-BIEU06-PIPE-01** |
| body/portal | — | Dropdown STRUCT | **T-BE-03** · **GAP-BIEU06-STRUCT-01** |
| designLoad | — | Dropdown lookup_hl | **T-BE-03** · **GAP-BIEU06-LOAD-01** |
| pavementInside | — | BTXM/BTN | **T-BE-03** · **GAP-BIEU06-PAVE-01** |
| lighting/drainage | — | yes_no | **T-BE-03** · **GAP-BIEU06-LIGHT/DRAIN-01** |
| Point | — / range | **kmPoint** filter+form · **không** ép kmTo | **T-BE-05** · **GAP-BIEU06-POINT-01** |
| Persist | shell detail* only | shell + **`Schema_CsdlBieu6`** 1:1 | **T-BE-01** · **T-BE-02** (Dev/4b) |
...
| Peer Sổ TS | `so-ts-underpass` | deep-link only | **T-FE-05** · **GAP-BIEU06-PEER-01** |
| DOMAIN-MAP | thiếu slug | add `csdl-bieu-06`→Asset | **T-DM-01** |
...
| `source.routes` | **`/csdl-bieu-06`** · `route_confirm=route_a` (autoApprove · PO/Design/SA) |
...
| entity | shell `CsdlCatalogRecordEntity` + typed `CsdlBieu6Entity` · `rmms_csdl_bieu6` |
| migration | `Schema_CsdlBieu6` · **Dev / Step 4b only** |
...
|--------|------|----------|
| A | `/csdl-bieu-06` (+ hub entry) | **SELECTED** — Q-ROUTE `alias_now` · Design/SA locked |
| B | hub-only `?resource=underpasses` | rejected — thiếu alias P1 |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header · title Biểu 06 |
...
| DES-GRID-F | `LinCatalogUiSchemaEditorModal` · catalogKind `underpasses` |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/csdl-records?resource=underpasses&…` | list paged · filters |
| API-02 | GET | `/api/v1/asset/csdl-records/{id}` | shell+typed join · XCO |
| API-03 | POST | `/api/v1/asset/csdl-records` | create · typed · IdCode `HC-` |
...
| copy | GET → clear id/code | POST (new `HC-`) |
| delete | — | DELETE soft |

### List filter query keys

`resource` · `search` · `province` · `status` · `underpassKind` · `roadCode` · `kmPoint` · `page`/`pageSize`
...
|---------|------|--------|------|------------|-------|
| T-DM-01 | Dev | **pending** | — | `/agent-dev` | DOMAIN-MAP `csdl-bieu-06`→Asset |
| T-CTX-01 | Dev | **pending** | — | `/agent-dev` | context sync feature page |
| T-BE-01 | Dev | **pending** | T-DM-01 | `/agent-dev` | `CsdlBieu6Entity` + EF config |
| T-BE-02 | Dev | **pending** | T-BE-01 | `/agent-dev` | Migration `Schema_CsdlBieu6` · shell KmPoint nếu thiếu · **Step 4b only** |
| T-BE-03 | Dev | **pending** | T-BE-01 | `/agent-dev` | DTO typed + service join · **stop detail* write** |
| T-BE-04 | Dev | **pending** | T-BE-03 | `/agent-dev` | IdCode `HC-` generator · **cấm** Guid |
| T-BE-05 | Dev | **pending** | T-BE-03 | `/agent-dev` | list filter `roadCode` + `kmPoint` + `underpassKind` |
...
| T-BE-INIT-01 | — | **n/a P1** | — | — | province/status/underpassKind LOOKUP_STATIC · **cấm** invent init-data |
...
| T-UI-FORM-01 | Dev | **pending** | T-UI-LIST-01 | `/agent-dev` | = **T-FE-02** Slideout 19 cột typed |
...
| T-UI-FIELD-01 | Dev | **pending** | T-UI-FORM-01 | `/agent-dev` | controlHint 1:1 · aperture number_m · pipe optional · light/drain yes_no |
...
| T-QA-FORM-01 | QA | **pending** | T-UI-FORM-01 | `/agent-qa` | 19 cột typed · **cấm** detail* only |
| T-QA-FILTER-01 | QA | **pending** | T-UI-FILTER-01 | `/agent-qa` | V1–V5 + roadCode/kmPoint/underpassKind |
| T-QA-TYP-01 | QA | **pending** | T-UI-UX-01 | `/agent-qa` | typography |
| T-QA-TAB-01 | QA | **pending** | T-UI-UX-01 | `/agent-qa` | tab index |
| T-QA-ROUTE-01 | QA | **pending** | T-UI-LIST-01 | `/agent-qa` | alias `/csdl-bieu-06` + hub |
...
- Add row `csdl-bieu-06` → **Asset** in `DOMAIN-MAP.md`.
- DoD: slug resolve Asset · **cấm** ERP.* / invent domain.

### T-BE-01 / T-BE-02 — Entity + migration

- `CsdlBieu6Entity` · table `rmms_csdl_bieu6` · FK `CatalogRecordId` unique 1:1.
- Columns: UnderpassKind, ApertureM, PipeCount, BodyStructure, PortalStructure, LengthM, DesignLoad, PavementInside, Lighting, Drainage, BuiltYear (+ SA types).
- Shell: KmPoint (ensure nếu thiếu) · **không** require KmTo.
- Migration name **`Schema_CsdlBieu6`** — **chỉ Dev / Step 4b** · **cấm** TL chạy.
- DoD: EF config · unique FK · **cấm** parent `*Json`.

### T-BE-03 / T-BE-04 / T-BE-05 — Service + IdCode + filters

- Widen DTO `CsdlBieu6Dtos` · join shell↔typed on `resource=underpasses`.
- **Stop** writing detail* for this resource at runtime.
- IdCode `HC-yyyyMMdd-nnnn` BE generate.
- List filters: `roadCode`, `kmPoint`, `underpassKind`, search, province, status.
- Gates: TZ **n/a** · XCO on GET detail · tenant `CompanyCode`.
- DoD: FormMode↔API green · aperture number_m · pipe optional · light/drain yes_no · kind hc_ds/hop_kt.
...
- Controls: search · province · status · underpassKind · roadCode SearchInput · kmPoint.
...
- Typed **19 cột** · controlHint 1:1 · **cấm** 3 ô detail* only · gồm hộp KT.
- underpassKind Dropdown · apertureM Number * · pipeCount optional · body/portal STRUCT · designLoad lookup_hl · pavementInside · lighting/drainage yes_no · lengthM Number *.
...
- Hub card entry `?resource=underpasses` still works.
- Peer Sổ TS `so-ts-underpass` deep-link only · **cấm** merge form · **≠** road-assets.
...
- Cover CRUD · 19-col form · filter V1–V5 · route alias+hub · leave · copy · soft delete · **cấm** detail*-only regression.
...
| province/status/underpassKind | Dropdown | LOOKUP_STATIC |
| roadCode | SearchInput | road-route P1 |
| kmPoint | Number | Point · no kmTo |
| code | Text ro | HC- |
| apertureM | Number | * m · number_m |
| pipeCount | Number | optional ≥1 |
| bodyStructure/portalStructure | Dropdown | STRUCT |
| lengthM | Number | typed * |
| designLoad | Dropdown | lookup_hl |
| pavementInside | Dropdown | BTXM/BTN |
| lighting/drainage | Dropdown | yes_no |
...
- reviewUrl prototype · mfeStdUrl `/csdl-bieu-06`
...
| Lane | Slash | Scope |
|------|-------|-------|
| BE | `/agent-dev` | T-DM · T-BE-* · T-BFF · T-PERM · Schema_CsdlBieu6 @ 4b |
...
| Form | Slideout 2col · typed 19 · **cấm** Full-page / detail* only |
| Route | alias + hub |
...
| Role | Need |
|------|------|
| **Dev** | implement/csdl-bieu-06.md · T-* theo matrix · migration @ 4b |
