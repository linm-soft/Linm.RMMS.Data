# Design — csdl-so-03 (Sổ 03 — Trực BĐGT + chốt + sự cố · new_page)

| Field | Value |
|-------|-------|
| feature | `csdl-so-03` |
| this role | `design` · `/agent-design` |
| Feature Kind | **B** catalog A–D+F+H + **D** Slideout Z1–Z3 · entries `pattern_inline_grid` |
| status | `confirmed` |
| design_confirm | **approve** (`autoApprove=ON` · `task_0fc07693`) |
| changeScope | `new_page` |
| packKind | `list` |
| gap | `new_page` · GAP-SO03-MERGE-01 · TYPED-01 · ROUTE-01 · FORMNO-01 · GAP-CSDL-ROAD-01 · PROV-01 · CUC-03 · CUC-06 · RPT-SRC |
| mfe | `D:/AI-QLBD/MFE-Source/Linm.Web.RMMS.Asset` · `ui_repo_confirm` **locked** (PO) |
| mfeStdRoute | `/csdl-so-03` |
| mfeStdUrl | `http://localhost:9301/csdl-so-03` |
| peerStdUrl | `http://localhost:9301/so-ts/csdl-so-sach?resource=duty-incident-logs` |
| reviewUrl | `file:///D:/AI-QLBD/Linm.RMMS.Data/specs/csdl-so-03/ui/prototype/csdl-so-03-list-prototype.html` |
| prototype | `specs/csdl-so-03/ui/prototype/csdl-so-03-list-prototype.html` |
| real_view_parity | `v1` |
| shared_grid_example | `v1` |
| backend | `D:/AI-QLBD/Linm.RMMS.WebService` · domain **Asset** · **`api/v1/asset/csdl-records?resource=duty-incident-logs`** · **cấm ERP.*** |
| resource | `duty-incident-logs` (**merge** · retire `duty-logs` + `checkpoint-duties`) |
| formNo | `03` · title VN **Sổ 03 — Trực BĐGT + chốt + sự cố** |
| IdCode | `SO-yyyyMMdd-nnnn` · **cấm** Guid |
| prior · data_analy | `confirmed`/`done` · hash skip · contentHash `sha256:1e8b4b6d6149c1ff2f27010cbf0d6649af9408b05738f416cd58d8c7361fdd9d` · headerFingerprint `sha256:b5b6baa32c1a5ebbf3d8eb2ecaad922d90a291958347aa22ec8fa27096d93997` · **cấm** re-scan demo (**GAP-DES-DEMO-RESCAN-01**) |
| prior · po | `confirmed` · `po/requirement.md` · `task_44997354` · Q-* resolved |
| autoApprove | **ON** |
| e2eQa | **ON** — queued `/agent-qa*` · **cấm** e2e/start:std ở Design |
| updatedAt | `2026-09-06T02:30:00.000Z` |
| taskId | `task_0fc07693` · analy `task_bbeb376c` · po `task_44997354` |
| skillVersion | `2026.08.25.01` |
| workflowVersion | `2026.09.01.02` |
| rulesVersion | `2026.08.31.2` |

**Cấm:** Dev/BE · re-scan DEM · invent `api/v1/so-ts/*` / `api/v1/infra/*` · ERP.* · Guid IdCode · form chỉ `detail*`/`col1–3` · invent map canvas · giữ 2 resource song song P1 · cột `dutyKind` P1 · yarn build/e2e/start:std · start role khác (**GAP-PKT-ROLE-01**).

## 0. Context & inventory (hash skip — **cấm** re-scan demo)

| ID | Path | Notes |
|----|------|-------|
| CTX-01 | `docs/context/features/csdl-so-03.md` | feature CTX |
| DA-01 | `specs/_data-analy/features/csdl-so-03-control-hint.md` | controlHint SSOT |
| DA-02 | `specs/_data-analy/features/csdl-so-03-real-data.md` | real-data §A–§F |
| PO-01 | `specs/csdl-so-03/po/requirement.md` | Grid AC · Screens · Leave · Q chốt |
| CLUSTER | `specs/_data-analy/csdl-cuc-2026/ANALYSIS-AND-TASKS.md` § T-SO-03 · Q-SO3 | typed + merge |
| DEM-01 | `…/csdl-so-sach-demo.html` → `asset/csdl-so-sach.html` | zone chrome only · **cấm** SSOT data |
| DOMAIN | `Linm.RMMS.WebService/docs/DOMAIN-MAP.md` | Asset · csdl-so-sach |
| MFE hub | `CsdlSoSachPage` · form `CsdlFormSlideout` | store keys `duty-logs`/`checkpoint-duties` → retire |
| MFE svc | `services/csdlSoSach/endpoint.ts` | `BASE=/asset/csdl-records` · **reuse** |

Persona: Khu QLĐB · Hạt trưởng · NV trực BĐGT / chốt · NV sự cố / hotline · Nhà thầu BDTX.

**≠** Sổ TS `so-ts-*` · ≠ hub Kind G shell `csdl-so-sach` (giữ entry; alias mfeStd riêng).

### § Delta Current vs New (`new_page`)

| Area | Current (live) | New (this Design) | GAP |
|------|----------------|-------------------|-----|
| Resources | `duty-logs` + `checkpoint-duties` (2 card) | **1** `duty-incident-logs` · retire 2 key | GAP-SO03-MERGE-01 · CUC-06 |
| Entry | Hub `?resource=duty-logs` / `checkpoint-duties` | Alias **`/csdl-so-03`** + hub `?resource=duty-incident-logs` · legacy QS **redirect** | GAP-SO03-ROUTE-01 |
| Hub label | 2 card Sổ 2 + Sổ 3 | **1** card **«Sổ 03 — Trực BĐGT + chốt + sự cố»** | GAP-SO03-FORMNO-01 |
| Form | 3 ô `detail*` + Col1–3 | Typed T-SO-03 header + entries | GAP-SO03-TYPED-01 · CUC-03 |
| `dutyKind` | Tách 2 resource | **Không** cột · gộp trong `content` | Q-DUTYKIND |
| `roadCode` | Text free | **SearchInput** `road-route` filter+form | GAP-CSDL-ROAD-01 |
| `province` | Select 5 tỉnh | LOOKUP_STATIC P1 keep | GAP-CSDL-PROV-01 |
| `status` | generic | `draft\|active\|closed` | Q-STATUS |
| `contractor` | Text | **Text P1** · SearchInput **DEFER P2** | GAP-CSDL-ORG-01 |
| `shift` | — | Text free P1 | Q-SHIFT |
| Import/Export | stub | **OUT pack** stub only | GAP-CSDL-XLS-01 |
| Report source | flat Col1–3 | Typed entries READY sau form | GAP-RPT-SRC-CSDL-01 |
| Map | — | **none** | — |

**Không đổi:** API prefix · Kind B A–D · Kind D Slideout · IdCode `SO-` · pagination 50/100/200/500 · **cấm** ERP.*.

## 1. Kind + UI pattern (HARD)

| | |
|--|--|
| Feature Kind | **B+D** |
| List pattern | **1×** `LinPageLayout` kind=`catalog` — **cấm** nested `CatalogListShell` |
| Grid | `LinCatalogDataGrid` · kéo cột **default ON** |
| Filter | **`LinErpListFilterBar`** **1 hàng** · SearchText + 🔍 cụm phải — **cấm** nút Tìm riêng · **cấm** wrap 2 hàng default desktop (**filter-bar-layout-hard**) |
| Footer | `LinCatalogListPagination` **50 / 100 / 200 / 500** |
| Form pattern | **Slideout** · `formSurface: slideout` · **`data-form-cols="2"`** · `slideout_layout: footer_actions_only` |
| Entries | `pattern_inline_grid` add/remove · typed cols — **cấm** chỉ Col1–3 |
| Zone F | `LinCatalogUiSchemaEditorModal` catalogKind `duty-incident-logs` |
| Zone H | `LinCatalogHistoryModal` — **cấm** invent History API |
| Leave | Dirty → **`LeaveConfirmModal`** — **cấm** native alert/confirm |
| Typography | label **13** · input D14/M16 (**GAP-TYP-01**) |
| View mode | `readOnly` — **cấm** Input disabled xám toàn form |
| Map | `map: none` — deep-link GIS only |
| Skip chrome | GOVOne · demo sidebar/note |
| DES-RPT | **N/A** (list pack · report riêng · GAP-RPT-SRC-CSDL-01 form READY) |

### Routes (Design chốt)

| Surface | Path |
|---------|------|
| List (alias) | `/csdl-so-03` |
| Hub entry | `/so-ts/csdl-so-sach?resource=duty-incident-logs` |
| Legacy redirect | `?resource=duty-logs` / `checkpoint-duties` → `duty-incident-logs` + toast |
| Form | overlay Slideout · QS `?form=` optional |
| Map | deep-link GIS only · **cấm** canvas |

## 2. Screens / zones

| Screen | FormMode | Zones | Controls / notes |
|--------|----------|-------|------------------|
| S-LIST | list | **DES-GRID-A…D · F · H** | Typed cols · filter HARD |
| S-HUB-ENTRY | hub card | Kind G card | **1** card title VN · open-resource · **cấm** slug · **cấm** 2 card cũ |
| S-FORM-CREATE | create | **DES-GRID-Z** · Z1–Z3 | footer Hủy/Lưu · leave |
| S-FORM-EDIT | edit | DES-GRID-Z | footer Hủy/Lưu |
| S-FORM-VIEW | view | DES-GRID-Z | readOnly · footer Đóng/Sửa/Copy |
| S-FORM-COPY | create | DES-GRID-Z | clear id · new code |
| S-ACT-DELETE | — | Confirm modal | soft delete |
| S-HIST | — | DES-GRID-H | `LinCatalogHistoryModal` |
| S-ENTRIES | — | inline_grid trong Z2 | typed · **cấm** dutyKind col |
| S-SKIP-MAP | — | — | **Cấm** map canvas |

### Zone A — Header (DES-GRID-A)

- Back hub · title **«Sổ 03 — Trực BĐGT + chốt + sự cố»** · meta `resource=duty-incident-logs`
- **Cấm** Thêm mới trên A

### Zone B — Toolbar + filter (DES-GRID-B · DES-GRID-B-FILTER)

**Toolbar (trái/phải):** Refresh · History · SchemaConfig · Import stub · Export stub · Delete · **Tạo mới** (primary phải).

**Filter 1 hàng (HARD):**

| key | Label | Control | catalogKind |
|-----|-------|---------|-------------|
| search | Tìm kiếm | `SearchTextInput` (+ 🔍 cụm phải) | text |
| province | Tỉnh/TP | `Dropdown` | LOOKUP_STATIC (5 tỉnh) |
| status | Tình trạng | `Dropdown` | LOOKUP_STATIC `draft\|active\|closed` |
| roadCode | Tên đường | `SearchInput` | **road-route** |
| fromDate | Từ ngày | `Date` | — |
| toDate | Đến ngày | `Date` | — |

Rules: search must work (mã · sổ · đường · người trực · nội dung) · **cấm** nút Tìm riêng · filter đổi → `page=1`.

### Zone C — Grid (DES-GRID-C)

| Col | Field | Notes |
|-----|-------|-------|
| STT | — | ổn định khi sort/kéo |
| Mã | `code` | link → View |
| Số quyển | `bookNo` | |
| Nhà thầu | `contractor` | |
| Đường | `roadName` / `roadCode` | |
| Km | `kmFrom`–`kmTo` | |
| Kỳ | `periodStart`–`periodEnd` | |
| Tỉnh | `province` | |
| TT | `status` | Nháp / Hiệu lực / Đóng |
| ⋮ | row menu | Xem / Sửa / Copy / Xóa / Lịch sử |

Empty copy VN: **«Chưa có nhật ký trực BĐGT / chốt / sự cố»**.

### Zone D — Footer (DES-GRID-D)

`LinCatalogListPagination` **50 / 100 / 200 / 500** · đổi size → page=1.

### Form Slideout Z1–Z3 (DES-GRID-Z · DES-FORM-Z1–Z3)

| Zone | Content |
|------|---------|
| Z1 | Title mode Create/Edit/View/Copy · X đóng |
| Z2 | Header fields 2-col + entries inline grid |
| Z3 | Footer Lưu/Hủy (View: Đóng/Sửa/Copy) · leave-confirm dirty |

#### Control-map — form header (Design chốt = controlHint)

| key | Label | Control | req |
|-----|-------|---------|-----|
| code | Mã | Text **readonly** | auto `SO-` |
| bookNo | Số quyển / sổ | Text | * |
| contractor | Nhà thầu | Text (P1) | * |
| roadCode | Mã đường | SearchInput `road-route` | * |
| roadName | Tên đường | Text display (bind) | * |
| kmFrom | Lý trình từ | Number | * |
| kmTo | Lý trình đến | Number | |
| periodStart | Kỳ từ | Date | * |
| periodEnd | Kỳ đến | Date | |
| province | Tỉnh | Dropdown LOOKUP_STATIC | * |
| status | Tình trạng | Dropdown `draft\|active\|closed` | |
| notes | Ghi chú sổ | Textarea | |

#### Control-map — entries[] inline grid

| key | Label | Control | req |
|-----|-------|---------|-----|
| lineNo | STT | Integer ro | auto |
| dutyDate | Ngày | Date | * |
| shift | Ca | Text free | * |
| personName | Tên | Text | * |
| content | Nội dung | Textarea | * | BĐGT · chốt · SC gộp 1 cột |
| handling | Xử lý | Textarea | |
| signRemark | Ký / nhận xét | Text | |

Required form DoD: bookNo · contractor · roadCode · kmFrom · periodStart · province · ≥1 entry với dutyDate · shift · personName · content.

**Cấm:** cột `dutyKind` · chỉ Col1–3 · Guid IdCode.

## 3. Grid AC (Design mirror PO)

| AC | Assert |
|----|--------|
| G-01 | `LinCatalogDataGrid` · empty không crash |
| G-02 | SearchTextInput lọc mã/sổ/đường/người/nội dung · **không** nút Tìm |
| G-03 | Filter province/status/road/from/to → refetch · page=1 |
| G-04 | Sort/kéo cột ON · STT ổn định |
| G-05 | Row menu Xem/Sửa/Copy/Xóa/Lịch sử |
| G-06 | Pagination 50/100/200/500 · size → page=1 |
| G-07 | Tạo mới → Slideout Create · Lưu → row mới |
| G-08 | Soft-delete → row khỏi list · toast OK |
| G-09 | 422 thiếu resource → toast · không blank |
| G-10 | Empty «Chưa có nhật ký trực BĐGT / chốt / sự cố» |
| G-11 | Hub **1** card Sổ 03 · không 2 card Sổ 2+3 |
| G-12 | Legacy QS duty-logs/checkpoint-duties → redirect |
| G-13 | Alias `/csdl-so-03` = hub resource mới |

## 4. Leave / dirty / error

| Case | UX |
|------|-----|
| Dirty + Hủy/X/Esc/route | `LeaveConfirmModal` |
| View mode | Không leave-confirm |
| Save OK | Đóng · refresh · toast |
| Validation fail | Ở lại · toast field |
| 404 detail | Đóng slideout · toast |
| Legacy resource QS | redirect + toast |

## 5. API / bind (cite real-data · SA widen)

| Op | Path |
|----|------|
| List | `GET /web-bff/api/v1/asset/csdl-records?resource=duty-incident-logs` |
| Detail | `GET /web-bff/api/v1/asset/csdl-records/{id}` |
| Create | `POST /web-bff/api/v1/asset/csdl-records` + typed body |
| Update | `PUT /web-bff/api/v1/asset/csdl-records/{id}` |
| Delete | `DELETE …/{id}` soft |
| road-route | `GET /integration/road-routes/search` |

FE: reuse `BASE=/asset/csdl-records`. SA: `Schema_CsdlSo03` · typed DTO/UiSchema · migrate/retire 2 keys.

## 6. Prototype + reviewUrl

| | |
|--|--|
| HTML | `specs/csdl-so-03/ui/prototype/csdl-so-03-list-prototype.html` |
| reviewUrl | `file:///D:/AI-QLBD/Linm.RMMS.Data/specs/csdl-so-03/ui/prototype/csdl-so-03-list-prototype.html` |
| Zones tagged | `data-des-id` DES-GRID-A/B/B-FILTER/C/D/Z · DES-FORM-Z1–Z3 · entries · leave |
| Sample rows | **synthetic UI chrome only** · **cấm** demo-json SSOT |

## 7. design_confirm

| | |
|--|--|
| Gate | `design_confirm` |
| Result | **approve** |
| Reason | autoApprove=ON · DoR PASS · control-map = controlHint · prototype + reviewUrl · filter-bar HARD · merge 1 card · Q-* PO resolved · hash skip |
| Board | skipped (autoApprove) |

## 8. Out of scope / Cấm

- ERP.* / Domains/Master / invent infra / so-ts API
- Report pack / Kind F map canvas / Excel full wizard
- Guid IdCode · merge Sổ TS · parent JSON-only DoD
- Demo/localStorage SSOT · re-scan demo (**GAP-DES-DEMO-RESCAN-01**)
- yarn build / e2e / start:std ở Design
- contractor SearchInput P1 (DEFER P2) · master province P1
- cột `dutyKind` · giữ 2 resource song song P1

## 9. Handoff

| Next | Need |
|------|------|
| **SA** | Schema_CsdlSo03 · typed DTO/UiSchema · migrate+retire `duty-logs`/`checkpoint-duties` · legacy QS redirect |
| TL/Dev | alias page `/csdl-so-03` + typed form · 1 hub card · reuse BASE |
| QA | Grid+form AC G-01…G-13 · e2e queued `/agent-qa*` only |

## DoR design

- [x] Kind B+D · zones A–D+F+H · Slideout Z1–Z3
- [x] Control-map chốt = controlHint (filters · header · entries)
- [x] Filter-bar HARD · Grid AC G-01…G-13
- [x] Prototype HTML + browser-openable reviewUrl
- [x] design_confirm **approve** (autoApprove)
- [x] hash skip · **cấm** re-scan demo
- [x] handoff compact ≤5KB
- [x] **cấm** Dev/BE · e2e · start:std · yarn build
