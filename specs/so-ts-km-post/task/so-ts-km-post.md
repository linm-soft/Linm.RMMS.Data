...
| gap | `edit_page` · GAP-SOTS-COL/FORM/REUSE · GAP-KM-NAME/SPEC/POINT/MAT/LEAVE/ROUTE · GAP-KM-AUTH-01 DEFER · GAP-KM-FLAT-01 DEFER |
| solution_confirm | **approve** (`task_29150871`) |
| design_confirm | **approve** (`task_0bf8f848`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=KM_POST`** · alias board `/so-ts-km-post` optional redirect · alt B primary-alias-only rejected |
| productRoot | `D:/AI-QLBD/Linm.RMMS.Data` |
| reviewUrl | `file:///D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-km-post/ui/prototype/so-ts-km-post-list-prototype.html` |
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:3a11d776482d57eebc6be1ed1a101e42525ea576986a8e8f7a49ef00b542e9fc` |
...
| taskId | `task_a2f290f2` |
| saTaskId | `task_29150871` |
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=KM_POST` | **giữ** · alias `/so-ts-km-post` optional | **route_confirm=route_a** · **GAP-KM-ROUTE-01** |
| Grid profile | 1 schema mọi type | Hide `type`/`kmTo`/qty/unit · show `distance_next_post`/`materials_id` từ dumpSpecs | **T-UI-LIST-01** · **GAP-SOTS-COL-01** |
| Form S-ATTR | `<dl>` readonly dumpSpecs | Editable Number + Dropdown · merge dumpSpecs | **T-UI-FORM-01** · **GAP-SOTS-FORM-01** |
| Point `kmTo` | form hiện / bắt buộc | **Ẩn** + không required khi `KM_POST` | **T-UI-FORM-01** · **GAP-KM-POINT-01** |
| Name | IsWeak / đoạn tuyến risk | `name` ← `name_km_post` · guard IsWeak | **T-BE-CRUD-01** import + rebuild · **GAP-KM-NAME-01** |
| materials | text dumpSpecs | LOOKUP_STATIC init-data `materials[]` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-KM-MAT-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** · **GAP-KM-LEAVE-01** |
| Filter context | thiếu file | `so-ts-km-post-filter-bar.md` | **T-UI-FILTER-01** |
| Form surface | full-page 5 cột | **giữ** `data-form-cols="5"` + header chrome | **T-UI-FORM-01** · **cấm** Slideout/footer Lưu |
| Flatten DB | — | **DEFER P2** | **GAP-KM-FLAT-01** · migration **none** |
...
| `source.routes` | **`/so-ts?type=KM_POST`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=KM_POST` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=KM_POST` (Dev điền/verify) · alias `http://localhost:9301/so-ts-km-post` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=KM_POST` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `KM_POST`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-km-post-filter-bar.md` |
...
|--------|------|----------|
| A | `/so-ts?type=KM_POST` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-km-post` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Cột Km» khi `type=KM_POST` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-km-post-filter-bar.md` |
| DES-GRID-C0 | listTitle «Danh sách cột Km» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · KM_POST column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=KM_POST` + filters |
...
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · units · **`materials[]` delta** |
...
Permissions: `asset.road-assets.read|create|update|delete` · FE `rmms-asset:road-assets:read|write` — Auth align **DEFER** GAP-KM-AUTH-01.
...
| SD-AUTH | **gap** · GAP-KM-AUTH-01 DEFER |
| SD-TOKEN | required (BFF) |
| SD-HEADER | required · X-Company-Id |
| SD-SPLIT | Asset ownership |
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent parent line JSON / MaterialsJson |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-KM-NAME/SPEC** import + rebuild |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-KM-MAT-01** `materials[]` on init-data |
...
| T-UI-LEAVE-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | LeaveConfirmModal · **GAP-KM-LEAVE-01** |
...
### edit_page SSOT re-audit (`tl-retry-ssot-rereview`)

Board retry data_analy **closed**. Dev **vẫn** audit delta surfaces trước Write:

```markdown
**edit.ssot_rereview:** pending → pass|fail
  checklist: tl-grid-ssot · list_parity · filter-bar V1–V5 · form full 5col · LAYOUT-06 · leave Modal
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-KM-NAME/SPEC/POINT/MAT/LEAVE
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-km-post.md` + parent `so-ts-type-grid.md` · filter-bar context tồn tại · **cấm** re-scan demo
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B A–D+F+H · `shared_grid_example=v1` · type profile `KM_POST`  
...
**ssot.reuse:**
  ...
  ui_filter: LinErpListFilterBar · **T-UI-FILTER-01** · `so-ts-km-post-filter-bar.md` — cấm ErpListHeaderFilters / LinListFilterField / nút Tìm
  ui_toolbar: catalogToolbar FULL (refresh·history·editConfig·view·edit·delete·create) · fa-cog
  ui_grid: LinCatalogDataGrid · kéo cột default ON · **KM_POST profile** hide type/kmTo/qty/unit · show distance/materials (**GAP-SOTS-COL-01**)
...
**implement.page_shell:**
  ...
  header: title «Sổ TS — Cột Km» khi type=KM_POST · titleIconClass + pageId
  filterBarLayout: erp-filter-bar · **cấm** filterMaxWidthPx hack
  listTitle: «Danh sách cột Km»
...
**implement.toolbar:** (tl-grid-full-flow §1)
  ...
  form_pair: onAdd→`/so-ts/tao-moi` · onEdit/onView→`/so-ts/sua?id=` · type lock KM_POST
  cấm: thiếu Create/Refresh/Config · History invent API · icon lệch erp-control-icon-map

**implement.grid:**
  component: LinCatalogDataGrid
  columns: buildDynamicGridColumns(schema) + type-profile KM_POST
  visible ON: name · route · routeNamed · routeSegment · kmFrom · distance_next_post · materials_id · (status/gps optional)
  visible OFF: type · kmTo · quantity · unitCode
  dumpSpecs: parse `distance_next_post` · `materials_id` labels
...
**implement.wire:**
  ...
  alias: optional `/so-ts-km-post` → `/so-ts?type=KM_POST` (**GAP-KM-ROUTE-01**)

**implement.state:**
  listFilters · type lock · pageSize · selection → toolbar · column profile KM_POST

**DoD:**
- [ ] Kind B 1 shell · grid visible khi API có data · profile KM_POST (**GAP-SOTS-COL-01**)
...
**skills (REQUIRED load trước Write):**
  ...
  - context: `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-km-post-filter-bar.md`
...
**implement.filter:**
  ...
  type lock: luôn `KM_POST` trên deep-link
...
**ssot.reuse:**
  ...
  controls: SearchInput type/route · Dropdown status/source/materials · Number distance · Text name_km_post
  http: API-02/03/04 · IdCode readonly · init-data API-06

**implement.form:**
  modes: create · edit · view(readOnly **không** disabled xám) · copy(clear id/code → POST · keep KM_POST)
  sections: S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR · S-GPS
  required: name · type · status · route · kmFrom — **không** required `kmTo` (**GAP-KM-POINT-01**)
  S-ATTR: `distance_next_post` Number · `materials_id` Dropdown LOOKUP_STATIC · merge dumpSpecs on save (**GAP-SOTS-FORM-01** · **GAP-KM-MAT-01**)
  S-NAME: `name` = `name_km_post` · mirror dumpSpecs (**GAP-KM-NAME-01**)
  kmFrom: **cấm** ép `"0"` khi dump null (**GAP-KM-SPEC-01**)
  **Không mount:** kmTo · quantity/unit · tab Chi tiết/Bảo trì/Tệp/Lịch sử legacy
  cấm: invent History path · flatten Schema_* · ERP.* · Modal form

**implement.init_data:**
  options: GET `/asset/road-assets/init-data` only — statuses · sources · **materials** · **cấm** KIND_LABEL hardcode (**GAP-DEV-DROPDOWN-HARDCODE-01**)
...
- [ ] kmTo ẩn · materials Dropdown từ init-data
- [ ] reuse AssetFormPage S-* — **cấm** fork

**Fail:** GAP-SOTS-FORM-01 · GAP-SOTS-REUSE-01 · GAP-KM-POINT-01 · GAP-KM-MAT-01 · GAP-P2-FORM-GRID-05 · GAP-LIST-FIELD-01
...
**DoD:** dirty → `LeaveConfirmModal` · full `useFormLeaveGuard` · **cấm** `window.confirm`/`alert` · **GAP-KM-LEAVE-01** / **GAP-P2-36** / **GAP-DEV-LEAVE-01** / **GAP-TL-LEAVE-01**
...
| +Tạo mới | toolbar | navigate `/so-ts/tao-moi` type=KM_POST | POST API-03 |
...
| Alias board | `/so-ts-km-post` | optional redirect → `?type=KM_POST` | — |
...
**status:** pending / verify  
**devSlash:** `/agent-dev`  
**DoD:** filter+form type/route/org = SearchInput Integration · materials = LOOKUP_STATIC init-data (**không** SearchInput master P1) · **GAP-LIST-LKP-01** / **GAP-KM-MAT-01**
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** controlHint Design/DA 1:1 · SA field map · dumpSpecs keys `name_km_post` · `distance_next_post` · `materials_id` · **GAP-LIST-FIELD-01**
...
**ssot.reuse:**
  ...
  import: RoadAssetCatalogHandler · RebuildGovVn `ResolveTypeAssetName(KM_POST)` / `IsWeakAssetName`
  gates: tz_na · xco_get_only · share_tenant

**implement.wire:**
  API-01…05: verify live · **cấm** invent `api/v1/so-ts/*` · ERP.*
  GAP-KM-NAME-01: rebuild CSV `name`←`name_km_post` · **cấm** IsWeak reject `Km*` · **cấm** fallback `name_of_route_asset` for KM_POST
  GAP-KM-SPEC-01: giữ materials/distance trong dumpSpecs · km trống khi dump null
  XCO: GET by id AllowedCompanyIds
  POST/PUT: accept dumpSpecs merge · **không** required kmTo for KM_POST

**DoD:**
- [ ] API-01…05 verify
- [ ] name/import fix (**GAP-KM-NAME-01** · **GAP-KM-SPEC-01**)
- [ ] no-parent-json · migration **none**

---

### T-BE-INIT-01 — init-data materials

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**from_solution:** API-06  
**DoD:** `RoadAssetInitDataDto.materials: RoadAssetLookupOption[]` · options = dump distinct / seed labels · **cấm** invent materials master SearchInput · **cấm** FE hardcode · **GAP-KM-MAT-01**
...
**status:** pending / verify  
**DoD:** FE gate + BE codes `asset.road-assets.*` · Auth NuGet mount **DEFER** GAP-KM-AUTH-01
...
| T-QA-FORM-01 | từng field required · UI value = body · dumpSpecs materials/distance · kmTo ẩn |
| T-QA-FILTER-01 | V1–V5 + fields 1:1 `so-ts-km-post-filter-bar.md` · 🔍 work · fail ErpListHeaderFilters / export trên bar |
...
| write | `specs/so-ts-km-post/implement/so-ts-km-post.md` |
| focus | COL profile · S-ATTR editable · kmTo ẩn · materials init · name IsWeak · LeaveConfirm · filter-bar · dumpSpecs P1 |
| mfeStdRoute | `/so-ts?type=KM_POST` |
...
| init-data materials · cấm hardcode dropdown | ✅ |
...
| generatedAt | 2026-09-01T02:55:00.000Z |
...
---
<!-- Version meta: skillId=agent-team-lead skillVersion=2026.08.19.04 schemaVersion=2 workflowVersion=2026.08.30.01 rulesVersion=2026.08.31.2 versionGate=rechecked taskId=task_a2f290f2 contentHashPriorDataAnaly=sha256:3a11d776482d57eebc6be1ed1a101e42525ea576986a8e8f7a49ef00b542e9fc route_confirm=route_a -->
