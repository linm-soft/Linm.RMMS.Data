...
(auth) Login → Tab 5
  ...
  Tap log row → push `#sc-attendance-log` + live Guid `Id` (`attendanceId`) · iOS `navigationDestination(item:)` · **cấm** toast-only · **cấm** fake UUID GET · **cấm** push supervise-detail
...
| toastLogTap | Chi tiết lần chấm | `LinmListRow` | — | **push** `#sc-attendance-log` + live `Id` · **cấm** toast-only · **cấm** fake UUID GET |
