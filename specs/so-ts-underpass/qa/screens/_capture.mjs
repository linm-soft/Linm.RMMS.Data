 ...
 * `yarn e2e-qa` headed login hang · Docker API :5111 · BFF :5201.
...
const aliasUrl = "http://localhost:9301/so-ts-underpass";
const formUrl = "http://localhost:9301/so-ts/tao-moi?type=UNDERPASS";
const listSel = '[data-testid="rmms-so-ts-underpass-list-page"]';
const formSel = '[data-testid="rmms-asset-form-shell"]';
const attrSel = '[data-testid="asset-underpass-attr"]';
...
const steps = [
  {
    ...
    url: listUrl,
    selector: listSel,
    note: "list UNDERPASS profile + filter · kmTo OFF grid · CC-",
  },
  {
    id: "S1",
    url: aliasUrl,
    selector: listSel,
    note: "alias /so-ts-underpass → type=UNDERPASS",
  },
  {
    id: "QA-20",
    url: formUrl,
    selector: formSel,
    also: attrSel,
    note: "Create form · S-ATTR underpass · S-LOC-POINT · CC-",
  },
];
...
const manifest = {
  ...
  testid: "rmms-so-ts-underpass-list-page",
};
