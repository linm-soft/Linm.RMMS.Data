...
const feature = "web-rmms-incident-chat";
/** Live in_progress incident — Mobile.Bff seed */
const INC_ID = "65b118cc-3038-43b8-8e7f-13821d89d09b";
...
async function fatalUi(p) {
  ...
  if (await overlay.count()) {
    ...
    const featureOk = await p
      .locator(
        `[data-feature='${feature}'], [data-zone='CH-00'], #loginUser, #sc-incident-chat, [data-des-id='sc-incident-chat']`,
      )
      .count();
...
async function dumpZones() {
  return page.evaluate(() => {
    const root = document.querySelector(
      '[data-feature="web-rmms-incident-chat"], [data-feature="web-rmms-home"], [data-feature="web-rmms-shell"], [data-feature="web-rmms-incident"]',
    );
    ...
    const entryChat = document.querySelector(
      '[data-des-id="i-chat"], [data-field="action.chat"], a[href*="chat"], [id^="btn-inc-chat-"]',
    );
    return {
...
try {
  // S0 — guest INC-CHAT Live GET{id} · topBar + thread/empty + composer · 0 GPS · 0 crash
  pageErrors.length = 0;
  await clearSession();
  await page.goto(`${base}/web-rmms-incident-chat?id=${encodeURIComponent(INC_ID)}`, {
    waitUntil: "domcontentloaded",
    timeout: 60000,
  });
  await page.waitForSelector(
    "[data-feature='web-rmms-incident-chat'] [data-field='topBarTitle'], [data-des-id='sc-incident-chat']",
    { timeout: 25000 },
  );
  await page.waitForTimeout(1500);
  await captureCurrent(
    "S0",
    "[data-feature='web-rmms-incident-chat'] [data-field='composer'], [data-field='composerInput'], [data-field='threadItems'], [data-field='emptyThread']",
  );
  ...
  pageErrors.length = 0;
  await clearSession();
  await page.goto(`${base}/web-rmms-incident-chat`, {
    waitUntil: "domcontentloaded",
    timeout: 60000,
  });
  await page.waitForSelector(
    "[data-feature='web-rmms-incident-chat'] [data-field='missingId'], [data-des-id='sc-incident-chat']",
    { timeout: 25000 },
  );
  await page.waitForTimeout(800);
  await captureCurrent(
    "S1",
    "[data-feature='web-rmms-incident-chat'] [data-field='missingId']",
  );
} finally {
  await browser.close();
}
...
writeFileSync(join(outDir, "_capture_incident_chat.result.json"), JSON.stringify(summary, null, 2));
