/**
 * Live DOM assert + DTM filter-bar overflow for so-ts-parking.
 */
...
const { chromium } = pw;
const outDir =
  "D:\\AI-QLBD\\Linm.RMMS.Data\\specs\\so-ts-parking\\qa\\screens";
const listUrl = "http://localhost:9301/so-ts?type=PARKING";
const formUrl = "http://localhost:9301/so-ts/tao-moi?type=PARKING";
const TEST_ID = "rmms-so-ts-parking-list";
...
const list = await page.evaluate((tid) => {
  ...
  return {
    titleParking: /Danh sách bãi đỗ xe/i.test(body),
    headerParking: /Sổ TS — Bãi đỗ xe/i.test(body),
...
await page.waitForSelector('[data-testid="asset-parking-attr"]', {
  timeout: 15000,
});
await new Promise((r) => setTimeout(r, 1200));
...
const form = await page.evaluate(() => {
  ...
  return {
    cols5: cols,
    parkingAttr: !!document.querySelector('[data-testid="asset-parking-attr"]'),
    typeWork: /Loại công trình|type_work/i.test(body),
    categorized: /Xếp loại|categorized/i.test(body),
    owner: /Chủ sở hữu|owner/i.test(body),
    actualLength: /Chiều dài|actual_length/i.test(body),
    siteArea: /DT khuôn viên|khuôn viên/i.test(body),
    parkingLot: /Bãi đỗ xe|parking_lot/i.test(body),
    totalParking: /Tổng DT bãi đỗ|total_parking/i.test(body),
    emergency: /Cứu hộ|traffic_emergency/i.test(body),
    firstAid: /Cấp cứu|first_aid/i.test(body),
    buildLocation: /Mặt cắt/i.test(body),
    kmToVisible,
    nameParking: /Tên bãi/i.test(body),
