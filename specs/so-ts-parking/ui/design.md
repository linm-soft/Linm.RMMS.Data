# Design — so-ts-parking (Sổ TS — Bãi đỗ xe)
...
| status | `confirmed` (`design_confirm=approve` · autoApprove ON · `task_db842a4d`) |
...
| dump | `tbl_rest_stops` |
| prefix | `BD-` |
| prior · po | `confirmed` · `po/requirement.md` · task `task_8fff69b2` |
| prior · data_analy | `confirmed` · `_data-analy/features/so-ts-parking-control-hint.md` · `so-ts-parking-real-data.md` · contentHash `sha256:da3d142d8478870e4474f88b0ba02aeac7d84ef2766ea90fc65e7102c079d1ba` |
...
| ID | Path | Notes |
|----|------|-------|
| CTX-01 | `docs/context/features/so-ts-parking.md` | feature |
| CTX-02 | `docs/context/features/so-ts-type-grid.md` | parent shell · S-* reuse · `station` · ô `t37` |
...
| DA-HINT | `specs/_data-analy/features/so-ts-parking-control-hint.md` | controlHint SSOT |
| DA-REAL | `specs/_data-analy/features/so-ts-parking-real-data.md` | §A+§B bind |

**Delta this Design (`new_page`):** type-profile cột `PARKING` · S-ATTR editable đủ dump §4 · `name` ← `name_work` · ẩn `kmTo` form · Dropdown LOOKUP_STATIC type_work/categorized/owner/grade/location · grid ON+hide-empty chiều dài/DT/**bãi đỗ**/cứu hộ/cấp cứu · **parking_lot/total_parking_lot ON** profile PARKING · `LeaveConfirmModal` · reuse S-* — **không** invent field ngoài PO/analy · **cấm** lẫn REST_AREA trong list.
...
| Leave | **`LeaveConfirmModal`** dirty — **cấm** native confirm (**GAP-DES-LEAVE-01** / **GAP-PK-LEAVE-01**) |
| Typography | label **13** · input D14/M16 (**GAP-TYP-01**) |
| Alias | `/so-ts-parking` = board deep-link · **optional** redirect → `/so-ts?type=PARKING` (**GAP-PK-ROUTE-01**) |

## 2. Screens / zones

| Screen | FormMode | Zones | Controls / notes |
|--------|----------|-------|------------------|
| S-LIST | list | **A Header · B Toolbar+filter · C Grid · D Pagination · F Schema · H History** | `LinErpListFilterBar` · profile cột PARKING · filter `?type=PARKING` |
| S-FORM-CREATE | create | full-page Z1 toolbar · Z2 S-* · (no footer Lưu) | type lock `PARKING` · leave-confirm |
| S-FORM-EDIT | edit | same | leave-confirm dirty |
| S-FORM-VIEW | view | same · **`<dl>` / readOnly display** | **cấm** Input disabled xám |
| S-FORM-COPY | create (copy) | same | clear id · keep `PARKING` |
| S-ACT-DELETE | confirm Modal | — | `useAlert` / Modal · soft delete |
| S-HIST | `LinCatalogHistoryModal` | DES-GRID-H | stub OK |
| S-ALIAS | navigate | — | `/so-ts-parking` → `/so-ts?type=PARKING` (optional redirect) |
...
|---------|----------|---------|-------------|-------|
| search | Tìm kiếm | `SearchTextInput` | text | mã · tên bãi · loại · chủ SH · tuyến · QR · apply → page=1 · **🔍 cụm phải** |
...
GET list: `?type=PARKING&search=&route=&kmFrom=&kmTo=&orgUnit=&page=&pageSize=`

### 3.2 Grid columns (`type=PARKING` profile)

| uiField | Label VN | Control / col | Visible | Notes |
|---------|----------|---------------|---------|-------|
| name | Tên bãi | link Text | **ON** | bind `name` = `name_work` |
...
| type_work_id | Loại công trình | Dropdown label | **ON** | dumpSpecs · «Bãi đỗ xe» |
| categorized_id | Xếp loại | Dropdown label | **ON · hide-empty** | dumpSpecs |
| owner_id | Chủ sở hữu | Dropdown label | **ON · hide-empty** | dumpSpecs |
...
| service_area / office_building_grade_id / total_area_floors | — | — | optional form | SchemaConfig ON nếu cần · hide-empty list |
...
| type | Loại tài sản | `SearchInput` | * | lock `PARKING` từ tile `t37` |
...
| build_location_id | Vị trí mặt cắt | `Dropdown` LOOKUP_STATIC | | Bên trái / Bên phải / Giữa · **GAP-PK-LOOKUP-01** |

**Không mount:** `S-LOC-RANGE` / `kmTo` trên form `PARKING` (**GAP-PK-POINT-01**).

#### S-NAME

| uiField | Label VN | Control | Required | Notes |
|---------|----------|---------|----------|-------|
| name / name_work | Tên bãi đỗ | `Text` | * | SSOT dump `name_work` · trống OK · **cấm** IsWeak đoạn tuyến (**GAP-PK-NAME-01**) |

#### S-ATTR (mẫu Thông tin chung — editable · **cấm** chỉ `<dl>`)

| uiField | Label VN | Control | Required | Notes |
|---------|----------|---------|----------|-------|
| type_work_id | Loại công trình | `Dropdown` LOOKUP_STATIC | * | «Bãi đỗ xe» … · **GAP-PK-LOOKUP-01** |
...
| office_building_grade_id | Cấp nhà làm việc | `Dropdown` LOOKUP_STATIC | | dump · optional · **không** ẩn cứng khi fill 0 |
...
| auxiliary_works_grade_id | Cấp công trình phụ | `Dropdown` LOOKUP_STATIC | | dump · optional |
| total_area_auxiliary_works | Tổng DT CT phụ (m²) | `Number` | | dump · optional |
| parking_lot | Bãi đỗ xe | `Select` boolean | * | dump · **GAP-PK-PARK-01** · profile PARKING **ON** |
...
**Không mount:** quantity/unit · tab Chi tiết / Bảo trì / Tệp / Lịch sử legacy · **cấm** lẫn `REST_AREA` trong PARKING list (**GAP-PK-SPLIT-01**).

### 3.4 DES-GRID / DES-FORM zone ids

| Zone | Spec |
|------|------|
| DES-GRID-A | Header title «Sổ TS — Bãi đỗ xe» khi `type=PARKING` — **cấm** Thêm mới trên A |
...
| DES-GRID-C0 | Card title «Danh sách bãi đỗ xe» + help row-menu |
| DES-GRID-C2 | `LinCatalogDataGrid` · profile PARKING · resize ON |
...
| | |
|--|--|
| Artifact | `ui/prototype/so-ts-parking-list-prototype.html` |
...
```
[A] icon + «Sổ TS — Bãi đỗ xe» (**no** Thêm mới)
...
[C] card: title · row-menu help · grid profile PARKING (ẩn type/kmTo/SL/ĐVT · ON+hide-empty chiều dài/DT/bãi đỗ/cứu hộ/cấp cứu)
...
```
Full page: [Quay lại | Hủy Lưu] Title · 5 cột data-form-cols=5 · S-META…S-GPS · cấm footer Lưu · LeaveConfirmModal · cấm kmTo/S-LOC-RANGE · kmFrom không required · parking ON default
```

Cite mẫu proto row: `BD-rest_stops_222171` · `name=Bãi đậu xe` · `type_work_id=Bãi đỗ xe` · `route=QL.1` · `routeNamed=QL.1-DANANG(BOT)` · `routeSegment=Km 933 + 082 - Km 942 + 000` · `kmFrom=` (nullable) · lat/lng `15.99` / `108.19` · peer fill `BD-rest_stops_115849` · `name=bãi đậu xe` · `route=QL.9` · lat/lng `16.82` / `107.01`.
...
| AC-G-05 | Profile cột PARKING · ẩn type / kmTo / quantity / unitCode · ON+hide-empty chiều dài/DT/**bãi đỗ**/cứu hộ/cấp cứu |
...
| AC-G-09 | Filter/import **tách** PARKING vs REST_AREA (**GAP-PK-SPLIT-01**) |
...
| AC-F-03 | Ẩn + không required `kmTo` khi `PARKING` · `kmFrom` không required · không mount S-LOC-RANGE |
...
| AC-F-08 | parking_lot / total_parking_lot **ON default** profile PARKING (**GAP-PK-PARK-01**) |
...
| ID | Decision |
|----|----------|
| GAP-PK-LOOKUP-01 | `type_work_id` / `categorized_id` / `owner_id` / `office_building_grade_id` / `auxiliary_works_grade_id` / `build_location_id` = **Dropdown LOOKUP_STATIC** dump P1 — **không** SearchInput master |
| GAP-PK-NAME-01 | `name` ← `name_work` · trống OK · **cấm** IsWeak đoạn tuyến |
| GAP-PK-ROUTE-01 | Live = `/so-ts?type=PARKING` · alias board-only · optional redirect `/so-ts-parking` |
| GAP-PK-SPEC-01 | Attr trong **`dumpSpecs` P1** · flatten → SA · FE dumpSpecLabels đủ key PARKING |
| GAP-PK-POINT-01 | Ẩn + không required `kmTo` · `kmFrom` không required · S-LOC-POINT only |
| GAP-SOTS-COL-01 | ON+hide-empty: chiều dài · DT khuôn viên · **bãi đỗ** · cứu hộ GT · cấp cứu |
| GAP-SOTS-REUSE-01 | Reuse S-* — **cấm** fork form |
| GAP-SOTS-TAB-01 | **Cấm** tab legacy DRVN |
| GAP-PK-SPLIT-01 | Filter/import **tách** PARKING vs REST_AREA · **cấm** gộp list |
| GAP-PK-LEAVE-01 | `LeaveConfirmModal` · **cấm** native confirm |
| GAP-PK-PARK-01 | `parking_lot` / `total_parking_lot` **ON default** profile PARKING |
| GAP-PK-OFFICE-01 | office/service_area/CT phụ: optional form S-ATTR · hide-empty list · **không** ẩn cứng form khi fill 0 |

## 9. Out of scope (this pack)

- Fork `AssetFormPage` / tab legacy DRVN
- Kind F map canvas / GIS `bai-do` deep-link (optional out)
...
| packKind | `list` |
| Kind / surfaces | B A–D+F + Full page 5 cột · type `PARKING` · cluster `station` · dump `tbl_rest_stops` |
| reviewUrl | `file:///D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-parking/ui/prototype/so-ts-parking-list-prototype.html` |
| prototype.artifact | `specs/so-ts-parking/ui/prototype/so-ts-parking-list-prototype.html` |
| zone ids | DES-GRID-A…D · B-FILTER · F · H · DES-FORM-Z1/Z2 · DES-LEAVE |
| control-map | §3 · Dropdown LOOKUP_STATIC P1 · dumpSpecs P1 · hide-empty · PARKING/REST split · parking ON |
| Screens | §2 · FormMode C/E/V/Copy · devSlash=`/agent-dev` |
| BE cite | `api/v1/asset/road-assets` — **cấm** invent so-ts path |
| Open SA | dumpSpecs vs flatten · LOOKUP seed · dumpSpecLabels FE · DOMAIN-MAP Asset · PARKING/REST split |
...
---
<!-- Version meta: skillId=agent-design skillVersion=2026.08.29.03 schemaVersion=1 workflowVersion=2026.08.30.01 rulesVersion=2026.08.31.2 versionGate=rechecked contentHashPriorDataAnaly=sha256:da3d142d8478870e4474f88b0ba02aeac7d84ef2766ea90fc65e7102c079d1ba taskId=task_db842a4d -->
