...
| formType | `list` |
| typeCode | `PARKING` |
| cluster | `station` · tile `t37` |
| dump | `tbl_rest_stops` |
| prefix | `BD-` |
| gap | `new_page` · GAP-SOTS-COL/FORM/REUSE · GAP-PK-NAME/SPEC/POINT/LOOKUP/LEAVE/ROUTE/SPLIT/PARK · flatten DEFER P2 |
| solution_confirm | **approve** (`task_a1f6e42a`) |
| design_confirm | **approve** (`task_db842a4d`) |
| autoApprove | **ON** |
| e2eQa | **ON** (queued — chỉ `/agent-qa*`) |
| route_confirm | **`route_a`** (autoApprove) — keep live **`/so-ts?type=PARKING`** · alias board `/so-ts-parking` optional redirect · alt B primary-alias-only rejected |
| productRoot | `D:/AI-QLBD/Linm.RMMS.Data` |
| reviewUrl | `file:///D:/AI-QLBD/Linm.RMMS.Data/specs/so-ts-parking/ui/prototype/so-ts-parking-list-prototype.html` |
| prior · data_analy | **confirmed** · control-hint + real-data · contentHash `sha256:da3d142d8478870e4474f88b0ba02aeac7d84ef2766ea90fc65e7102c079d1ba` |
...
| saTaskId | `task_a1f6e42a` |
| updatedAt | `2026-09-01T04:55:00.000Z` |
...
**Cấm:** implement product code (trừ template task) · e2e · `yarn build` / `start:std` · Step 4b/migration · ERP.* · invent `api/v1/so-ts/*` · lẫn REST_AREA trong PARKING list · start role khác (**GAP-PKT-ROLE-01**).
...
|------|----------------|----------------------|--------|
| Route | `/so-ts?type=PARKING` | **giữ** · alias `/so-ts-parking` optional | **route_confirm=route_a** · **GAP-PK-ROUTE-01** |
| Grid profile | generic / thiếu PARKING | Hide `type`/`kmTo`/qty/unit · **ON+hide-empty**: chiều dài · DT khuôn viên · **bãi đỗ** · cứu hộ GT · cấp cứu | **T-UI-LIST-01** · **GAP-SOTS-COL-01** · **GAP-PK-PARK-01** |
...
| Point `kmTo` | form hiện / bắt buộc | **Ẩn** + không required khi `PARKING` · kmFrom **không** required · **cấm** ép `"0"` | **T-UI-FORM-01** · **GAP-PK-POINT-01** |
| Name | IsWeak / đoạn tuyến risk | `name` ← `name_work` · trống OK · **cấm** IsWeak→đoạn | **T-BE-CRUD-01** + form · **GAP-PK-NAME-01** |
| LOOKUP | text dumpSpecs | LOOKUP_STATIC init `parkingWorkTypes[]` · `parkingCategories[]` · `parkingOwners[]` · `officeBuildingGrades[]` · `auxiliaryWorksGrades[]` · `buildLocations[]` | **T-BE-INIT-01** + **T-UI-FORM-01** · **GAP-PK-LOOKUP-01** |
| Leave/alert | `window.confirm` risk | `LeaveConfirmModal` + `useAlert`/Modal | **T-UI-LEAVE-01** · **T-UI-HIST-01** · **GAP-PK-LEAVE-01** |
| Filter context | thiếu file | `so-ts-parking-filter-bar.md` | **T-UI-FILTER-01** · **T-CTX-01** |
| PARKING vs REST | lẫn type trong list/filter/import | **tách** PARKING vs REST_AREA | **T-UI-LIST-01** · **T-BE-CRUD-01** · **GAP-PK-SPLIT-01** |
| parking_lot | OFF / thiếu | **ON** default profile PARKING · total_parking_lot ON hide-empty | **T-UI-LIST-01** · **T-UI-FORM-01** · **GAP-PK-PARK-01** |
| office/service_area | — | optional form S-ATTR · hide-empty list · **không** ẩn cứng form khi fill 0 | **T-UI-FORM-01** |
| Form surface | full-page 5 cột | **giữ** `data-form-cols="5"` + header chrome | **T-UI-FORM-01** · **cấm** Slideout/footer Lưu |
| Flatten DB | — | **DEFER P2** | dumpSpecs P1 · migration **none** (**GAP-PK-FLAT-01**) |
| dumpSpecLabels | thiếu attr keys | đủ dump §4 + type_work/categorized/owner/grade/location/parking | **T-UI-FORM-01** · **GAP-PK-SPEC-01** |
...
| `source.routes` | **`/so-ts?type=PARKING`** · confirmed `route_confirm=route_a` (autoApprove · live Design/SA) |
| `mfeStdRoute` | `/so-ts?type=PARKING` |
| `mfeStdUrl` | `http://localhost:9301/so-ts?type=PARKING` (Dev điền/verify) · alias `http://localhost:9301/so-ts-parking` |
| `peerStdUrl` | `http://localhost:9301/so-ts?type=PARKING` |
| form routes | `/so-ts/tao-moi` · `/so-ts/sua?id=` (type lock `PARKING`) |
...
| filter context | `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-parking-filter-bar.md` (create T-CTX-01) |
...
|--------|------|----------|
| A | `/so-ts?type=PARKING` | **SELECTED** — live MFE + Design/SA |
| B | `/so-ts-parking` as primary list URL | rejected — board alias only · optional redirect → A |
| C | custom | n/a |
...
| Zone | Component |
|------|-----------|
| DES-GRID-A | `LinPageLayout` header «Sổ TS — Bãi đỗ xe» khi `type=PARKING` — **cấm** Thêm mới trên A |
| DES-GRID-B | `catalogToolbar` / `buildCatalogListToolbarActions` · `fa-cog` |
| DES-GRID-B-FILTER | `LinErpListFilterBar` · **T-UI-FILTER-01** · `so-ts-parking-filter-bar.md` |
| DES-GRID-C0 | listTitle «Danh sách bãi đỗ xe» · `listRowMenuHelp` |
| DES-GRID-C2 | `LinCatalogDataGrid` · PARKING column profile |
...
|----|--------|------|-------|
| API-01 | GET | `/api/v1/asset/road-assets` | list · `?type=PARKING` + filters · **cấm** lẫn REST_AREA |
...
| API-06 | GET | `/api/v1/asset/road-assets/init-data` | statuses · sources · **`parkingWorkTypes[]`** · **`parkingCategories[]`** · **`parkingOwners[]`** · **`officeBuildingGrades[]`** · **`auxiliaryWorksGrades[]`** · **`buildLocations[]`** delta |
| API-07 | GET | `/api/v1/asset/road-assets/summary-by-type` | peer tile t37 · out of write pack |
...
| SD-NO-JSON | **required** · dumpSpecs attr bag only · **cấm** invent parent line JSON / ParkingJson |
...
| T-BE-CRUD-01 | Dev | pending (delta) | — | `/agent-dev` | verify API-01…05 · **GAP-PK-NAME/SPEC/SPLIT** import + rebuild |
| T-BE-INIT-01 | Dev | pending (delta) | T-BE-CRUD-01 | `/agent-dev` | **GAP-PK-LOOKUP-01** parkingWorkTypes + Categories + Owners + office/aux grades + buildLocations |
...
| T-UI-LIST-01 | Dev | pending | T-BFF-01 | `/agent-dev` | full `tl-grid-task-template` · **GAP-SOTS-COL-01** · **GAP-PK-SPLIT-01** · **GAP-PK-PARK-01** · LAYOUT-06 |
...
| T-UI-FORM-01 | Dev | pending (delta) | T-UI-LIST-01 · T-BE-INIT-01 | `/agent-dev` | full-page 5 cột · S-ATTR · **GAP-SOTS-FORM/POINT/REUSE** · parking ON |
| T-UI-LEAVE-01 | Dev | pending | T-UI-FORM-01 | `/agent-dev` | LeaveConfirmModal · **GAP-PK-LEAVE-01** |
...
**new_page.ssot_rereview:** pending → pass|fail
  ...
  gaps: GAP-SOTS-COL/FORM/REUSE · GAP-PK-NAME/SPEC/POINT/LOOKUP/LEAVE/SPLIT/PARK
  then: fix_all
```
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** `docs/context/features/so-ts-parking.md` + parent `so-ts-type-grid.md` · tạo `so-ts-parking-filter-bar.md` · lane web → `dev` · **cấm** re-scan demo
...
**devSlash:** `/agent-dev`  
**layer:** ui  
**from_design:** Kind B A–D+F+H · `shared_grid_example=v1` · type profile `PARKING`  
...
**ssot.reuse:**
  ...
  ui_filter: LinErpListFilterBar · **T-UI-FILTER-01** · `so-ts-parking-filter-bar.md` — cấm ErpListHeaderFilters / LinListFilterField / nút Tìm
  ui_toolbar: catalogToolbar FULL (refresh·history·editConfig·view·edit·delete·create) · fa-cog
  ui_grid: LinCatalogDataGrid · kéo cột default ON · **PARKING profile** hide type/kmTo/qty/unit · show type_work_id · categorized_id · owner_id · actual_length · site_area_using_land · **parking_lot** · **total_parking_lot** · traffic_emergency_service · first_aid_service (**GAP-SOTS-COL-01** · **GAP-PK-PARK-01**)
...
**implement.page_shell:**
  ...
  header: title «Sổ TS — Bãi đỗ xe» khi type=PARKING · titleIconClass + pageId
  filterBarLayout: erp-filter-bar · **cấm** filterMaxWidthPx hack
  listTitle: «Danh sách bãi đỗ xe»
...
**implement.toolbar:** (tl-grid-full-flow §1)
  ...
  form_pair: onAdd→`/so-ts/tao-moi` · onEdit/onView→`/so-ts/sua?id=` · type lock PARKING
  cấm: thiếu Create/Refresh/Config · History invent API · icon lệch erp-control-icon-map

**implement.grid:**
  component: LinCatalogDataGrid
  columns: buildDynamicGridColumns(schema) + type-profile PARKING
  visible ON: name · route · routeNamed · routeSegment · kmFrom · type_work_id · categorized_id · owner_id · actual_length · site_area_using_land · **parking_lot** · **total_parking_lot** · traffic_emergency_service · first_aid_service · (status/gps optional)
  visible OFF default: type · kmTo · quantity · unitCode
  hide-empty ON: actual_length · site_area_using_land · **parking_lot** · **total_parking_lot** · traffic_emergency_service · first_aid_service
  dumpSpecs: parse rest_stops §4 keys · labels via dumpSpecLabels (**GAP-PK-SPEC-01**)
  resizable: DEFAULT true
  selection: useErpListSelection
  empty: empty state VN · **cấm** fake row / demo-json
  split: list/filter **chỉ** PARKING · **cấm** lẫn REST_AREA (**GAP-PK-SPLIT-01**)
...
**implement.wire:**
  list: GET `?type=PARKING&search=&route=&kmFrom=&kmTo=&orgUnit=&page=&pageSize=`
  alias: optional `/so-ts-parking` → `/so-ts?type=PARKING` (**GAP-PK-ROUTE-01**)

**implement.test:**
  listFilters · type lock PARKING · pageSize · selection → toolbar · column profile PARKING · **cấm** REST_AREA rows

**DoD:**
- [ ] Kind B 1 shell · grid visible khi API có data · profile PARKING (**GAP-SOTS-COL-01** · **GAP-PK-PARK-01**)
- [ ] Toolbar FULL · Grid menu · Config full cột · Grid flow
- [ ] Footer LinCatalogListPagination 50/100/200/500
- [ ] LAYOUT-06 live PASS
- [ ] PARKING/REST split PASS (**GAP-PK-SPLIT-01**)
- [ ] `rg` anti-clone PASS · **yarn build chỉ Dev**

**Fail:** GAP-TL-GRID-TASK-01 · GAP-TL-GRID-FLOW-01 · GAP-P2-LAYOUT-06 · GAP-SOTS-COL-01 · GAP-PK-SPLIT-01 · GAP-PK-PARK-01 · GAP-DEV-DUP-GRID
...
**skills (REQUIRED load trước Write):**
  ...
  - context: `D:/AI-QLBD/Linm.RMMS.Data/docs/context/features/so-ts-parking-filter-bar.md`
...
**implement.filter:**
  ...
  type lock: luôn `PARKING` trên deep-link · **cấm** REST_AREA (**GAP-PK-SPLIT-01**)
...
**ssot.reuse:**
  ...
  controls: SearchInput type/route · Dropdown status/source/parkingWorkTypes/parkingCategories/parkingOwners/officeBuildingGrades/auxiliaryWorksGrades/buildLocations · Number/Text dump §4 · Text name_work · Select boolean parking_lot/traffic_emergency_service/first_aid_service
  http: API-02/03/04 · IdCode readonly prefix `BD-` · init-data API-06

**implement.form:**
  modes: create · edit · view(readOnly **không** disabled xám) · copy(clear id/code → POST · keep PARKING)
  sections: S-META · S-ROUTE · S-LOC-POINT · S-NAME · S-ATTR · S-GPS
  required: type · status · route · type_work_id — name trống OK · kmFrom **không** required · **không** required `kmTo` (**GAP-PK-POINT-01**)
  S-ATTR: type_work_id Dropdown * · categorized_id Dropdown · owner_id Dropdown · actual_length Number · site_area_using_land Number · **parking_lot Select boolean ON** · **total_parking_lot Number ON hide-empty** · traffic_emergency_service Select boolean · first_aid_service Select boolean · office_building_grade_id Dropdown · auxiliary_works_grade_id Dropdown · build_location_id Dropdown L/R/C · service_area Number optional · merge dumpSpecs (**GAP-SOTS-FORM-01** · **GAP-PK-LOOKUP-01** · **GAP-PK-SPEC-01** · **GAP-PK-PARK-01**)
  S-NAME: `name` ← `name_work` · trống OK · mirror dumpSpecs (**GAP-PK-NAME-01**)
  kmFrom: **cấm** ép `"0"` khi dump null (**GAP-PK-SPEC-01** · **GAP-PK-POINT-01**)
  **Không mount:** kmTo · quantity/unit · S-LOC-RANGE · tab Chi tiết/Bảo trì/Tệp/Lịch sử legacy
  office/service_area: optional form · **không** ẩn cứng khi fill 0
  cấm: invent History path · flatten Schema_* · ERP.* · Modal form

**implement.init_data:**
  options: GET `/asset/road-assets/init-data` only — statuses · sources · **parkingWorkTypes** · **parkingCategories** · **parkingOwners** · **officeBuildingGrades** · **auxiliaryWorksGrades** · **buildLocations** · **cấm** KIND_LABEL hardcode (**GAP-DEV-DROPDOWN-HARDCODE-01**)
...
- [ ] kmTo ẩn · parking LOOKUP Dropdown từ init-data
- [ ] parking_lot/total_parking_lot **ON** · reuse AssetFormPage S-* — **cấm** fork

**Fail:** GAP-SOTS-FORM-01 · GAP-SOTS-REUSE-01 · GAP-PK-POINT-01 · GAP-PK-LOOKUP-01 · GAP-PK-PARK-01 · GAP-P2-FORM-GRID-05 · GAP-LIST-FIELD-01
...
**DoD:** dirty → `LeaveConfirmModal` · full `useFormLeaveGuard` · **cấm** `window.confirm`/`alert` · **GAP-PK-LEAVE-01** / **GAP-P2-36** / **GAP-DEV-LEAVE-01** / **GAP-TL-LEAVE-01**
...
| +Tạo mới | toolbar | navigate `/so-ts/tao-moi` type=PARKING | POST API-03 |
...
| Alias board | `/so-ts-parking` | optional redirect → `?type=PARKING` | — |
...
**status:** pending / verify  
**devSlash:** `/agent-dev`  
**DoD:** filter+form type/route/org = SearchInput Integration · type_work_id/categorized_id/owner_id/office_building_grade_id/auxiliary_works_grade_id/build_location_id = LOOKUP_STATIC init-data (**không** SearchInput master P1) · **GAP-LIST-LKP-01** / **GAP-PK-LOOKUP-01**
...
**status:** pending  
**devSlash:** `/agent-dev`  
**DoD:** controlHint Design/DA 1:1 · SA field map · dumpSpecs keys `name_work` · `type_work_id` · `categorized_id` · `owner_id` · `actual_length` · `site_area_using_land` · **parking_lot** · **total_parking_lot** · `traffic_emergency_service` · `first_aid_service` · `office_building_grade_id` · `auxiliary_works_grade_id` · `build_location_id` · `service_area` · **GAP-LIST-FIELD-01**
...
**implement.wire:**
  ...
  GAP-PK-NAME-01: rebuild CSV `name`←`name_work` · **cấm** IsWeak reject «Bãi đỗ»/«Tên bãi» · **cấm** fallback `name_of_route_asset` khi name_work hợp lệ
  GAP-PK-SPEC-01: giữ rest_stops §4 attrs trong dumpSpecs · km trống khi dump null · FE dumpSpecLabels đủ key
  GAP-PK-SPLIT-01: list/filter/import **chỉ** PARKING · **cấm** lẫn REST_AREA
  XCO: GET by id AllowedCompanyIds
  POST/PUT: accept dumpSpecs merge · **không** required kmTo for PARKING

**DoD:**
- [ ] API-01…05 verify
- [ ] name/import fix (**GAP-PK-NAME-01** · **GAP-PK-SPEC-01**)
- [ ] PARKING/REST split (**GAP-PK-SPLIT-01**)
- [ ] no-parent-json · migration **none**

---

### T-BE-INIT-01 — init-data PARKING LOOKUP

**status:** pending (delta)  
**devSlash:** `/agent-dev`  
**from_solution:** API-06  
**DoD:** `RoadAssetInitDataDto.parkingWorkTypes` + `parkingCategories` + `parkingOwners` + `officeBuildingGrades` + `auxiliaryWorksGrades` + `buildLocations: RoadAssetLookupOption[]` · options = dump LOOKUP_STATIC / seed · **cấm** invent parking master SearchInput P1 · **cấm** FE hardcode · **GAP-PK-LOOKUP-01**
...
| T-QA-FORM-01 | từng field required · UI value = body · dumpSpecs parking attrs · kmTo ẩn · parking ON |
| T-QA-FILTER-01 | V1–V5 + fields 1:1 `so-ts-parking-filter-bar.md` · 🔍 work · fail ErpListHeaderFilters / export trên bar |
...
| ID | Note |
|----|------|
| Flatten Schema_* | P2 · dumpSpecs P1 (**GAP-PK-FLAT-01**) |
| Auth NuGet | DEFER |
| Parking LOOKUP master SearchInput | P2 |
...
| focus | COL profile PARKING · S-ATTR editable · kmTo ẩn · parking* init LOOKUP · name←name_work · LeaveConfirm · filter-bar · dumpSpecs P1 · PARKING/REST split · parking_lot ON |
| mfeStdRoute | `/so-ts?type=PARKING` |
...
| init-data parking* · cấm hardcode dropdown | ✅ |
| PARKING/REST split GAP-PK-SPLIT-01 | ✅ |
| parking_lot ON GAP-PK-PARK-01 | ✅ |
...
---
<!-- Version meta: skillId=agent-team-lead skillVersion=2026.08.19.04 schemaVersion=2 workflowVersion=2026.08.30.01 rulesVersion=2026.08.31.2 versionGate=rechecked taskId=task_6c86ab37 contentHashPriorDataAnaly=sha256:da3d142d8478870e4474f88b0ba02aeac7d84ef2766ea90fc65e7102c079d1ba route_confirm=route_a -->
