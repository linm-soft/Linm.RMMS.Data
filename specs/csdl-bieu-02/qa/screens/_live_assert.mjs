...
await p.goto("http://localhost:9301/csdl-bieu-02", {
  waitUntil: "domcontentloaded",
  timeout: 60000,
});
await p.waitForSelector('[data-testid="rmms-csdl-bieu-02-list-page"]', {
  timeout: 25000,
});
await p.waitForSelector(
  '[data-testid="rmms-csdl-bieu-02-list-export-excel-btn"]',
  { timeout: 15000 },
);
...
const hasExportBtn =
  (await p
    .locator('[data-testid="rmms-csdl-bieu-02-list-export-excel-btn"]')
    .count()) > 0;
const hasImportBtn =
  (await p
    .locator('[data-testid="rmms-csdl-bieu-02-list-import-excel-btn"]')
    .count()) > 0;

/** filter bar must NOT host Xuất (GAP-FILTER-BAR-08) */
const filterBarExport = await p.evaluate(() => {
  const filter = document.querySelector(
    '[data-testid="rmms-csdl-bieu-02-list-filters"]',
  );
...
try {
  ...
  const [download] = await Promise.all([
    ...
    p.locator('[data-testid="rmms-csdl-bieu-02-list-export-excel-btn"]').click(),
  ]);
  const fileName = download.suggestedFilename() || "";
  const okName = /^Bieu02_ThongKeCau_\d{8}\.xls$/i.test(fileName);
...
const assert = {
  url: "http://localhost:9301/csdl-bieu-02",
  changeScope: "edit_page",
  hasTitle: /Thống kê cầu|Biểu 02|cầu/i.test(body),
...
writeFileSync(
  "D:/AI-QLBD/Linm.RMMS.Data/specs/csdl-bieu-02/qa/screens/live-assert.json",
