...
async function fatalUi(p) {
  ...
  if (await overlay.count()) {
    ...
    const featureOk = await p
      .locator(
        "[data-feature='web-rmms-photo-geo'], #PGC, #sheet-pgc, #loginUser",
      )
      .count();
...
async function dumpZones() {
  return page.evaluate(() => {
    const root = document.querySelector(
      '[data-feature="web-rmms-photo-geo"], [data-feature="web-rmms-home"], [data-feature="web-rmms-shell"]',
    );
    ...
    const guestGate = /Đăng nhập để dùng chụp ảnh kèm tọa độ/i.test(text);
    const titleOk = /Chụp ảnh kèm tọa độ/i.test(text);
    const shutterDisabled = (() => {
      const el = document.getElementById("btn-shutter");
      return el ? el.disabled === true || el.hasAttribute("disabled") : null;
    })();
    const useDisabled = (() => {
      const el = document.getElementById("btn-use");
      return el ? el.disabled === true || el.hasAttribute("disabled") : null;
    })();
...
writeFileSync(join(outDir, "_capture_pgc.result.json"), JSON.stringify(summary, null, 2));
