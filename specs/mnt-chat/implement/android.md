...
- **GAP-MOB-CHAT-IME-01:** `linmChatImeFlush()` — extra = `max(0, ime − Scaffold.bottom)`. Composer **flush** IME. **Cấm** raw `imePadding` (đếm trùng tab).
...
Không bỏ `linmChatImeFlush`. Worker **cấm** raw `imePadding` trên `#sc-mnt-chat`.
