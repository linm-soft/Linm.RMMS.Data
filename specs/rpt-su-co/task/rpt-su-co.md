# Team-lead — task pack · rpt-su-co (Kind E)
...
| packKind | **`report`** (Kind **E** AnalyticsReportShell) — packet board `list` **stale** (GAP-PO-SC-02) |
| Feature Kind | **E** · leaf `/bao-cao/su-co` · **không** CRUD form |
| mfeStdRoute | `/bao-cao/su-co` |
| route_confirm | **route_a** `/bao-cao/su-co` |
| autoApprove | **ON** |
| design_confirm | **approve** (`task_6b0f9f87`) |
| solution_confirm | **approve** (`task_e98abc5b`) |
| be_repo_confirm | **approve** (run packet BE `Linm.RMMS.WebService`) |
| ui_repo_confirm | **approve** (run packet MFE `Linm.Web.RMMS.Report`) |
| taskId | `task_89fd3207` |
...
**Supersedes** TL stub `task_4906443c` (ngắn). Pack này re-audit live MFE/BE + Design/SA confirmed `task_6b0f9f87` / `task_e98abc5b`.

**Cấm ERP.*** · **cấm** `Domains/Master` · **cấm** `api/v1/rmms/*` · **cấm** `api/v1/reports` · **cấm** parent JSON · **cấm** copy CRUD `incident` · **cấm** path API mới P1 · **cấm** folder domain mới.

## from design / solution (scope gate)

| Source | Path | Task dùng |
|--------|------|-----------|
| Design | `specs/rpt-su-co/ui/design.md` + reviewUrl | T-UI-LIST A–D · T-UI-ACT · T-UI-LKP · T-UI-FIELD · T-UI-PROD · T-UI-UX · DES-SC-01..06 |
| Solution | `specs/rpt-su-co/be/solution-discovery.md` | T-BE · T-BFF · T-PERM · GAP-SA-SC-* |
| Prototype | `ui/prototype/rpt-su-co-prototype.html` | UI DoD parity |
| controlHint | `specs/_data-analy/features/rpt-su-co-control-hint.md` | SearchInput loại/tuyến/mức/TT · Date · Input |
| PO | `specs/rpt-su-co/po/requirement.md` | GAP-PO-SC-01..11 |
...
| Concern | Reuse | Cấm |
|---------|-------|-----|
| UI | analog `DefectReportPage` / `IncidentReportPage` leaf | nested CatalogListShell · raw `<table>` · local pager · native `<select>` · `LinListTableConfigModal` · `configHint` · Kind B `LinCatalogUiSchemaEditorModal` · hub `ReportListPage` family `incidents` làm trang này |
| HTTP | `apiClient` · `reportEndpoint` re-export | fork ApiClient |
| Lookup | `ROAD_ROUTE_LOOKUP_CONFIG` + `GET /integration/road-routes/search` fallback · enum loại/mức/TT | copy catalog road-route vào Report DTO · invent QL.22 · `INCIDENT_KIND_LOOKUP` trên leaf |
...
| `source.routes` | `/bao-cao/su-co` only P1 · **không** `/new` · `/:id` |
| `source.backend` | `D:/AI-QLBD/Linm.RMMS.WebService` |
| `source.domain` | **Report** (DOMAIN-MAP slug `rpt-su-co` → kebab `report`) |
...
| `source.persistence` | **không** P1 — in-memory 12 mapped `IncidentEntity` |
...
| API-01 | GET | `/incidents` | DONE in-memory 12 · query `type` `routeId` `severity` `status` `from` `to` `search` `period` `page` `pageSize` | **keep** · **cấm** alias `q` P1 · FilterRoute **exact** |
| API-02 | GET | `/incidents/export` | DONE CSV UTF-8 BOM `incidents.csv` full filtered set | **keep** BE · **FE** subset cột hiện + **applied** filters |
| API-LKP-01 | GET | `/integration/road-routes/search` | FE consume + seed fallback | keep · **không** copy catalog Report |
| init-data | — | — | **OUT P1** | enum loại/mức/TT tĩnh FE |

Query: pageSize allow `{50,100,200,500}` · invalid → 50. Empty/`all` = all trên `routeId` `type` `severity` `status`. `routeId` **exact** `Route` case-insensitive.
...
| SD-AUTH | stub | `report.su-co.read` (list + export) |
...
|------|--------|-----------|
| A | Header | `LinPageLayout` kind=`report` · icon `fas fa-exclamation-triangle` · title **BC Sự cố** · **cấm** Thêm mới |
| B | Filter + reportToolbar | **`LinErpListFilterBar`** (SearchInput loại/tuyến/mức/TT · Date · Input qSearch · **Xem** = `onSearch`) · Excel · Làm mới · Biểu đồ SoCai · In · Config FULL — **cấm** stub config |
...
## retry.ssot_rereview (TL live MFE 2026-08-16 · `IncidentReportPage` + `IncidentFilterBar` + BE)

Audit `Linm.Web.RMMS.Report` `IncidentReportPage.tsx` + `IncidentFilterBar.tsx` + `reportEndpoint` + DOMAIN-MAP `rpt-su-co` → Report — **cấm** Dev chỉ patch 1 chỗ nếu còn GAP cùng surface.
...
| 12 | Excel sau Xem + applied filters | `canExport: viewed` · `handleExport` dùng applied (`type` `routeId` …) · subset `columnPrefs` | **PASS** DES-SC-02 |
| 13 | Lookup SearchInput + empty Tất cả · **cấm QL.22** | `ROAD_ROUTE_LOOKUP_CONFIG` + enum type/severity/status · seed filter `QL.22` | **T-UI-LKP** giữ |
...
| 20 | Chart SoCai client | **PASS** by-severity / by-route khi viewed + có dòng · KPI Dòng · Tuyến · Mở | — |
...
| 25 | Hub `INCIDENT_KIND_LOOKUP` | chỉ hub family `incidents` trong `lookups.ts` | **cấm** gắn lên leaf `IncidentFilterBar` |
...
|----|---------|---------|-------|----------|---------|
| S-SC | Report A–D | Full page Kind E | `/bao-cao/su-co` | report | filter `LinErpListFilterBar` → Xem · refresh (applied) · Excel applied · chart SoCai · config FULL · drill Field |
...
|----|------|-------|------|-------------|-----|
| **T-CTX-01** | rpt-su-co | docs | — | **done** | Context + controlHint + Design + SA linked · Kind E · prefix `api/v1/report` · **cấm** ERP.* |
| **T-PERM-01** | rpt-su-co | ui+api | T-CTX-01 | stub | Code `report.su-co.read` (API-00…02). Gắn `[RequirePermission]` khi CommonLib ≥1.4.0 · **không** block P1 |
| **T-UI-LIST-01** | /bao-cao/su-co | ui-list | T-PERM-01 | shell **PASS** | **Giữ** 1× LinPageLayout · `resizable: true` · LinCatalogListPagination · Zone A title · listTitle BC Sự cố · Xem mới load · cột Design (code/route/type/severity/status/at/drill) · **cấm** nested CatalogListShell / footerPagination / pageSizeBar / raw table / Thêm mới A · FE gửi **`search`** (không `q`) |
| **T-UI-FORM-01** | — | ui-form | T-UI-LIST-01 | **OUT PASS** | **không** mount CRUD · **cấm** Resource/Slideout/View=`readOnly` |
| **T-UI-RPT-01** | /bao-cao/su-co | ui | T-PERM-01 | **done** | **`LinErpListFilterBar`** · Xem = `onSearch` · Enter trên Input = Xem |
| **T-UI-RPT-CONFIG-01** | /bao-cao/su-co | ui | T-UI-RPT-01 | **done analog** | Config FULL — **cấm** `configHint` / `LinListTableConfigModal` / Kind B schema editor |
| **T-UI-RPT-CHART-01** | /bao-cao/su-co | ui | T-UI-RPT-CONFIG-01 | **done** | SoCai client từ `items` — KPI Dòng · Tuyến · Mở · **cấm** stub toast · **không** API chart |
| **T-UI-ACT-01** | rpt-su-co | ui | T-UI-LIST-01 | **PASS** | Inventory dưới · Làm mới: chưa Xem → toast «Chưa xem» **không** fetch · Excel **chỉ** khi `viewed` (`canExport: viewed`) · params **applied** (`type` `routeId` `severity` `status` `from` `to` `search`) · subset CSV theo `columnPrefs` · drill `/incident?id={id}` top window |
| **T-UI-LKP-01** | rpt-su-co | ui | T-UI-LIST-01 | **PASS** | tuyến SearchInput road-route CUC2 + empty=Tất cả · loại/mức/TT enum FE + empty · **cấm** native select · **cấm** invent QL.22 · **cấm** `INCIDENT_KIND_LOOKUP` trên leaf |
| **T-UI-FIELD-01** | rpt-su-co | ui | T-UI-LIST-01 | **PASS** | from/to Date SSOT · ISO date-only query · grid `at` `formatAtVi` `vi-VN` · cột Nguồn = drill (không editor) |
| **T-UI-PROD-01** | rpt-su-co | ui | T-UI-FORM-01 | **PASS** | Kind E product · form OUT · testid `rmms-incident-report` · route `/bao-cao/su-co` · pageId `rpt-su-co` |
| **T-UI-UX-01** | rpt-su-co | ui | T-UI-LIST-01 | **PASS** | gap 4/8/16 · **cấm** `filterMaxWidth` cap · toast SSOT · empty/skeleton không blank body |
| **T-BE-01** | report | api | T-CTX-01 | **PASS keep** | `GET api/v1/report/incidents` · FilterRoute exact · FilterDate `At` · pageSize allow-list · seed 12 · **không** migration · **không** path mới · **không** folder domain mới |
| **T-BE-02** | export | api | T-BE-01 | **PASS keep** | CSV UTF-8 BOM `incidents.csv` full default header SA · **không** page |
| **T-BFF-01** | report | bff | T-BE-01 | **PASS** | Proxy GET list + export bytes · QS passthrough raw · **không** business |
| **T-QA-01** | rpt-su-co | qa | T-UI-ACT-01 | pending QA | scenarios · mfeStdUrl · Xem · Làm mới gated · Excel applied · form OUT · build PASS |
| **T-RV-01** | rpt-su-co | review | T-QA-01 | pending Review | findings.md |

## T-UI-ACT inventory (P1)

| Action | Zone | Handler | API |
|--------|------|---------|-----|
| Chọn loại / tuyến / mức / TT / kỳ / tìm | B | draft only | — |
...
CRUD sự cố trên slug này · GOVOne chrome · warehouse schema · EF `rmms_incidents` P1 · parent JSON · Kind B · `LinCatalogUiSchemaEditorModal` · `ERP.*` · `api/v1/rmms/*` · `api/v1/reports` · invent QL.22 · KPI hub `reports` · `[RequirePermission]` block P1 · API chart riêng · path API mới P1.
