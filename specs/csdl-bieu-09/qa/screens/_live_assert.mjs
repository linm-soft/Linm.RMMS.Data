...
await p.goto("http://localhost:9301/csdl-bieu-09", {
  waitUntil: "domcontentloaded",
  timeout: 60000,
});
await p.waitForSelector('[data-testid="rmms-csdl-bieu-09-list-page"]', {
  timeout: 45000,
});
await p.waitForSelector(
  '[data-testid="rmms-csdl-bieu-09-list-export-excel-btn"]',
  { timeout: 20000 },
);
...
const hasExportBtn =
  (await p
    .locator('[data-testid="rmms-csdl-bieu-09-list-export-excel-btn"]')
    .count()) > 0;
const hasImportBtn =
  (await p
    .locator('[data-testid="rmms-csdl-bieu-09-list-import-excel-btn"]')
    .count()) > 0;

/** filter bar must NOT host Xuất (GAP-FILTER-BAR-08) */
const filterBarExport = await p.evaluate(() => {
  const filter = document.querySelector(
    '[data-testid="rmms-csdl-bieu-09-list-filters"]',
  );
...
try {
  ...
  const [download] = await Promise.all([
    ...
    p.locator('[data-testid="rmms-csdl-bieu-09-list-export-excel-btn"]').click(),
  ]);
  const fileName = download.suggestedFilename() || "";
  const okName = /^Bieu09_MocLoGioiGPMB_\d{8}\.xls$/i.test(fileName);
...
const assert = {
  url: "http://localhost:9301/csdl-bieu-09",
  changeScope: "edit_page",
  hasTitle: /Biểu\s*09|Mốc lộ giới|GPMB/i.test(body),
  hasFilter: body.includes("Tìm") || /Tỉnh|Loại mốc|markerKind/i.test(body),
...
writeFileSync(
  "D:/AI-QLBD/Linm.RMMS.Data/specs/csdl-bieu-09/qa/screens/live-assert.json",
