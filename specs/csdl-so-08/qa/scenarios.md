...
| docker | API `:5111` healthy · BFF `:5201` healthy · postgres healthy · maintenance-work-logs HTTP 200 |
| MFE | `D:/AI-QLBD/MFE-Source/Linm.Web.RMMS.Asset` |
| BE | `D:/AI-QLBD/Linm.RMMS.WebService` · `api/v1/asset/csdl-records` · **cấm ERP.*** |
| packKind | **`list`** · Kind B A–D+F+H · Kind D Slideout 2col · entries `inline_grid` |
| changeScope | `new_page` |
| resource | `maintenance-work-logs` · formNo `08` · IdCode `SO-` |
| autoApprove | ON |
...
| updatedAt | `2026-09-06T02:17:30.000Z` |
| prior · dev | **confirmed** · `implement/csdl-so-08.md` · `task_85207485` |
...
| Check | Result |
|-------|--------|
| `docker compose up -d` | **PASS** · api `:5111` · bff `:5201` · postgres healthy · `maintenance-work-logs` HTTP 200 |
...
| Form assert QA-20 | **PASS** · `form-assert.json` · Z2 + entries + Z3 · 2col · Lưu · entry-0 workItem/solution/mainResult |
| API GET `?resource=maintenance-work-logs` | **PASS** · HTTP 200 (`:5111` + BFF `:5201/web-bff`) |
...
|----|-------|----------|--------|----------|
| S0 | Mở `mfeStdUrl` | List `rmms-csdl-so-08-list-page` · title Sổ 08 · filter-bar · empty/grid | **PASS** | ![S0](screens/S0.png) |
| S1 | Hub `?resource=maintenance-work-logs` | Redirect → `/csdl-so-08` · list page (route_a) | **PASS** | ![S1](screens/S1.png) |
| QA-20 | `?form=create` | Slideout create · Z2 header · entries inline_grid 5 cột · 2col · footer Lưu | **PASS** | ![QA-20](screens/QA-20.png) |

`screens/manifest.json` · capturedAt `2026-09-05T19:17:22.453Z` · SHA256_16 S0=`b28a37d433523394` · S1=`b28a37d433523394` · QA-20=`68dc6cccc57cc008`.
...
|----|-------|----------|--------|
| QA-20 | Create `?form=create` / toolbar Tạo mới | Slideout · POST `csdl-records` · resource=maintenance-work-logs · entries[] | **PASS** (runtime + code) |
...
| QA-F-02 | Required bookNo/contractor/officeUnit/zoneUnit/road/kmFrom/periodStart + entry workItem/kmFrom/solution/mainResult | **PASS** (validate code) |
| QA-F-03 | road = `SearchInput` road-route · **cấm** Text free | **PASS** (code · SearchInput; testid may not surface — GAP-QA-ROAD-TESTID) |
| QA-F-04 | entries `inline_grid` · 5 cột workItem·kmFrom/To·solution·mainResult·note · add/remove · media N/A | **PASS** (live entry-grid + code) |
| QA-F-05 | Dirty leave = `LeaveConfirmModal` · **0** native dialog | **PASS** (code) |
| QA-F-06 | Typed So08 · **cấm** detail*-only · **cấm** kmAt · **cấm** flatten-only | **PASS** (live Z2/entries/Z3) |
...
| QA-ROUTE-01 | alias `/csdl-so-08` + hub redirect maintenance-work-logs | **PASS** (S0+S1) |
...
| GAP-QA-ROAD-TESTID | P3 | form `csdl-so-08-field-road` may not appear in DOM query (SearchInput wrapper) |
| Auth / org / XLS | DEFER\|OUT | per PO |
| UiSchema seed default | DEFER | Dev debt |
| Migration apply runtime | P1 | Schema_CsdlSo08 ready · apply on deploy |
