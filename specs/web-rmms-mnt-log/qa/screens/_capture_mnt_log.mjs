...
const feature = "web-rmms-mnt-log";
...
async function fatalUi(p) {
  ...
  if (await overlay.count()) {
    ...
    const featureOk = await p
      .locator(
        `[data-feature='${feature}'], [data-zone='WORK-G'], #loginUser, #sc-mnt-log`,
      )
      .count();
...
async function dumpZones() {
  return page.evaluate(() => {
    const root = document.querySelector(
      '[data-feature="web-rmms-mnt-log"], [data-feature="web-rmms-home"], [data-feature="web-rmms-shell"]',
    );
    ...
    const writeCta = document.querySelector(
      '[data-field="primaryWrite"], button[type="submit"], [data-field="submitProgress"], [data-field="submitComplete"]',
    );
    const gpsEl = document.querySelector('[data-field="gps"], [data-zone="WORK-G-GPS"]');
    const tlItems = document.querySelectorAll('[data-field="timeline"] [role="listitem"]');
    const tlAts = [...tlItems].map((el) =>
      el.querySelector(".tlAt, [class*='tlAt']")?.textContent?.trim() || "",
    );
    return {
      ...
      text,
      href: location.href,
      hasWriteCta: !!writeCta,
      hasGps: !!gpsEl,
      timelineCount: tlItems.length,
      timelineAts: tlAts.slice(0, 8),
      woCode:
        document.querySelector('[data-field="woCode"]')?.textContent?.trim() || null,
      emptyState: !!document.querySelector('[data-field="emptyState"]'),
    };
  });
}
...
try {
  // S0 — guest WORK-G Live GET{id} · header + timeline RO · 0 write · 0 GPS
  pageErrors.length = 0;
  await clearSession();
  await page.goto(`${base}/web-rmms-mnt-log?id=${encodeURIComponent(WO_ID)}`, {
    waitUntil: "domcontentloaded",
    timeout: 60000,
  });
  await page.waitForSelector(
    "[data-feature='web-rmms-mnt-log'] [data-field='woHeader'], [data-field='woCode'], #sc-mnt-log",
    { timeout: 25000 },
  );
  await page.waitForTimeout(1200);
  await captureCurrent(
    "S0",
    "[data-feature='web-rmms-mnt-log'] [data-field='woHeader'], [data-field='timeline'], [data-field='emptyTimeline']",
  );
  ...
  // S1 — guest missing id → emptyState · RO · 0 crash
  pageErrors.length = 0;
  await clearSession();
  await page.goto(`${base}/web-rmms-mnt-log`, {
    waitUntil: "domcontentloaded",
    timeout: 60000,
  });
  await page.waitForSelector(
    "[data-feature='web-rmms-mnt-log'] [data-field='emptyState'], #sc-mnt-log",
    { timeout: 25000 },
  );
  await page.waitForTimeout(800);
  await captureCurrent(
    "S1",
    "[data-feature='web-rmms-mnt-log'] [data-field='emptyState']",
  );
} finally {
  await browser.close();
}
...
writeFileSync(join(outDir, "_capture_mnt_log.result.json"), JSON.stringify(summary, null, 2));
