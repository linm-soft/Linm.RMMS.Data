...
await p.goto("http://localhost:9301/csdl-bieu-04", {
  waitUntil: "domcontentloaded",
  timeout: 60000,
});
await p.waitForSelector('[data-testid="rmms-csdl-bieu-04-list-page"]', {
  timeout: 25000,
});
await p.waitForSelector(
  '[data-testid="rmms-csdl-bieu-04-list-export-excel-btn"]',
  { timeout: 15000 },
);
...
const hasExportBtn =
  (await p
    .locator('[data-testid="rmms-csdl-bieu-04-list-export-excel-btn"]')
    .count()) > 0;
const hasImportBtn =
  (await p
    .locator('[data-testid="rmms-csdl-bieu-04-list-import-excel-btn"]')
    .count()) > 0;

/** filter bar must NOT host Xuất (GAP-FILTER-BAR-08) */
const filterBarExport = await p.evaluate(() => {
  const filter = document.querySelector(
    '[data-testid="rmms-csdl-bieu-04-list-filters"]',
  );
...
try {
  ...
  const [download] = await Promise.all([
    ...
    p.locator('[data-testid="rmms-csdl-bieu-04-list-export-excel-btn"]').click(),
  ]);
  const fileName = download.suggestedFilename() || "";
  const okName = /^Bieu04_CongCacLoai_\d{8}\.xls$/i.test(fileName);
...
const assert = {
  url: "http://localhost:9301/csdl-bieu-04",
  changeScope: "edit_page",
  hasTitle: /Cống|Biểu 04|cống/i.test(body),
...
writeFileSync(
  "D:/AI-QLBD/Linm.RMMS.Data/specs/csdl-bieu-04/qa/screens/live-assert.json",
